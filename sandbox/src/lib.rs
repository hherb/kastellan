...
// The sole production caller is the cfg(target_os = "linux") bwrap backend;
// keep the pure fn (and its tests) compiled on macOS rather than cfg-gating
// it away, so the cross-platform test suite still pins the validation logic.
