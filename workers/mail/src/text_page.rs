...
/// might have the first 12,000 characters of two million.
pub fn page_result(picked: &Picked, body: &[u8]) -> Result<Value, String> {
    let fault = |what: &str| {
        format!(
...
mod tests {
    ...
    fn a_middle_page_carries_the_servers_next_offset_and_says_how_to_continue() {
        let body = br#"{"text":"page two","offset":12000,"limit":12000,"total":30000,"next_offset":24000}"#;
        let out = page_result(&sha_picked(), body).unwrap();
        assert_eq!(out["text"], "page two");
        assert_eq!(out["offset"], 12000);
        assert_eq!(out["total"], 30000);
        assert_eq!(out["next_offset"], 24000);
        let more = out["more"].as_str().expect("a continuing page says so");
        assert!(more.contains("offset: 24000") && more.contains("12000 of 30000"), "{more}");
    }
    ...
    fn next_offset_is_copied_not_computed_from_the_text() {
        ...
        assert_eq!(page_result(&sha_picked(), body).unwrap()["next_offset"], 2);
    }

    #[test]
    fn the_last_page_has_a_null_next_offset_and_no_continuation_note() {
        let body = br#"{"text":"end","offset":0,"limit":12000,"total":3,"next_offset":null}"#;
        let out = page_result(&sha_picked(), body).unwrap();
    ...
    fn a_body_without_paging_fields_is_a_fault_not_a_complete_document() {
        for body in [
            ...
            let e = page_result(&sha_picked(), body).unwrap_err();
    ...
    fn a_body_without_text_or_not_json_is_a_fault() {
        assert!(page_result(&sha_picked(), b"raw text").unwrap_err().contains("not JSON"));
        assert!(page_result(&sha_picked(), br#"{"other":"x"}"#).unwrap_err().contains("`text`"));
    }
    ...
    fn a_page_of_ordinary_text_fits_the_planners_step_view() {
        ...
        .unwrap();
        let out = serde_json::to_vec(&page_result(&sha_picked(), &body).unwrap()).unwrap();
