...
echo
echo "WARNING (issue #263): browser-driver is DEVELOPMENT-ONLY. It is not yet"
echo "egress-proxy-routable, so under a force-routed (production) deployment it"
echo "refuses to spawn unless KASTELLAN_BROWSER_DRIVER_INSECURE_DIRECT_NET=1 is"
echo "set — which runs the browser with egress confined only in-process, NOT at"
echo "the OS boundary. Do not use in production until egress slice #2 lands."
