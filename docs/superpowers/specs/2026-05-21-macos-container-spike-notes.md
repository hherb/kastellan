...
| `mem_max: u64` | `-m <bytes>` (suffix-aware) | **Floor: 200 MiB.** `container` rejects anything smaller with `invalidArgument: minimum memory amount allowed is 200 MiB`. We must `clamp(mem_max, 200 << 20, _)` when emitting this flag. |
...
| `cpu_quota_pct: Option<u32>` | `-c <fractional>` (e.g. `200% → -c 2.0`) | Number of vCPUs to allocate. |
| `tasks_max: Option<u32>` | `--ulimit nproc=<N>:<N>` | POSIX `RLIMIT_NPROC` as a reasonable approximation of cgroup `pids.max`. |
...
The warm 0.8 s per-spawn is the load-bearing number. For `SingleUseLifecycle` workers (today's `shell-exec` posture) this is real per-call latency tax. For `IdleTimeoutLifecycle` workers (the GLiNER-Relex shape, already in tree) it amortises to zero across the idle window.
...
- New `sandbox/src/macos_container.rs` implementing `SandboxBackend`.
  ...
  - Wire the `--progress none` flag globally so `[6/6]` lines don't pollute stderr.
- New `clamp_memory_to_minimum(u64) -> u64` pure helper to encode the 200 MiB floor. Pinned with a unit test (1 MiB → 200 MiB; 100 MiB → 200 MiB; 256 MiB → 256 MiB; 1 GiB → 1 GiB).
...
- One-line operator opt-in for `gliner-relex` on macOS to validate end-to-end with the real model.
...
- **No Linux Firecracker counterpart.** Linux already has cgroup memory enforcement; the micro-VM is a defense-in-depth strengthening on Linux, not a parity fix. Out of scope for this spike.
