...
**Goal:** Wire a producer-side `actor='cli' action='task.submitted'` audit row from `kastellan-cli ask`, mirroring the cancel slice (PR #43) so the audit-log lifecycle stream covers submit → claim → terminal in one query.

**Architecture:** New constant `ACTION_TASK_SUBMITTED` in `core::scheduler::audit`; new helper `cli_audit::submit_and_audit(pool, lane, payload) -> Result<i64, DbError>` mirroring `cancel_and_audit`'s chokepoint posture; one-line swap at `kastellan-cli ask`'s sole `insert_pending` call site.

**Tech Stack:** Rust + sqlx + tokio + per-test Postgres clusters via `kastellan-tests-common`.
...
Run: `source "$HOME/.cargo/env" && cargo test -p kastellan-core --test cli_submit_audit_e2e 2>&1 | tail -10`

Expected: compile error on `use kastellan_core::cli_audit::{submit_and_audit, CLI_AUDIT_ACTOR};` — `submit_and_audit` is not yet defined. **Do not proceed to Task 3 until this red is confirmed.**
...
use kastellan_db::tasks::{mark_cancelled, Task};
...
```rust
use kastellan_db::audit;
use kastellan_db::tasks::{insert_pending, mark_cancelled, Lane, Task};
use kastellan_db::DbError;
use sqlx::PgPool;
...
Run: `source "$HOME/.cargo/env" && cargo test -p kastellan-core --test cli_submit_audit_e2e -- --nocapture 2>&1 | tail -20`
...
Closes the audit gap where kastellan-cli ask emitted no audit row at
...
### Task 4: Rewire `kastellan-cli ask` to use `submit_and_audit`

**Files:**
- Modify: `core/src/bin/kastellan-cli.rs` (lines 242, 244, 267)

- [ ] **Step 1: Widen the `cli_audit` import to include `submit_and_audit`**

Edit `core/src/bin/kastellan-cli.rs` line 242. Current:
...
```rust
    use kastellan_db::tasks::{get, insert_pending};
```
...
Run: `source "$HOME/.cargo/env" && cargo build -p kastellan-core --bin kastellan-cli 2>&1 | tail -5`
...
Run: `source "$HOME/.cargo/env" && cargo test -p kastellan-core --test cli_submit_audit_e2e --test cli_cancel_audit_e2e -- --nocapture 2>&1 | tail -20`
...
Run: `source "$HOME/.cargo/env" && cargo test -p kastellan-core --test cli_ask_e2e -- --nocapture 2>&1 | tail -25`
...
```bash
git add core/src/bin/kastellan-cli.rs core/tests/cli_ask_e2e.rs
git commit -m "$(cat <<'EOF'
feat(cli): kastellan-cli ask emits producer-side task.submitted audit row
...
```markdown
- [x] **[follow-up] Producer-side `task.submitted` audit row from `kastellan-cli ask`** — landed 2026-05-13 on branch `feat/cli-task-submitted-audit`. Closes the gap symmetric to the cli-cancel-audit slice: `kastellan-cli ask` was calling `tasks::insert_pending` directly with no producer-side audit row, so the lifecycle stream visible in `audit_log` only started at the scheduler's `task.running` observation on claim — submit-to-claim latency queries had to join `audit_log` against `tasks.created_at` across two clocks. New `core::scheduler::audit::ACTION_TASK_SUBMITTED = "task.submitted"` const + new `core::cli_audit::submit_and_audit(pool, lane, payload) -> Result<i64, DbError>` helper (mirror of `cancel_and_audit`: chokepoint posture, audit insert best-effort, id propagates even on audit failure). `tasks::insert_pending` left unchanged (no `RETURNING *` widening needed since all payload fields are known up-front at submit). `kastellan-cli ask` rewired in one line. **NOT in scope (filed as a follow-up):** producer row from future channel adapters (none exist today); producer `task.failed` row from `tasks fail` subcommand. Test count 353 → **354** (+1 integration test in `cli_submit_audit_e2e.rs`; `cli_ask_e2e` multiset bumps don't add `#[test]` functions).
```
...
the gap where kastellan-cli ask emitted no audit row at submit time and
...
Expected: `354 passed; 0 failed; 0 ignored` (or higher `ignored` for the two pre-existing doctests in `kastellan-sandbox` and `kastellan-worker-prelude` that are explicitly marked).
...
./target/debug/kastellan-cli audit tail --from-start --no-follow 2>&1 | grep '"task.submitted"' | head -3
```
...
- `Lane::Fast` / `Lane::Long` (in tests) = exact enum variants from `kastellan_db::tasks::Lane`.
