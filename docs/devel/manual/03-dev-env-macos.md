...
hhagent uses `sandbox-exec` (macOS Seatbelt) to isolate worker processes.
...
export HHAGENT_LLM_LOCAL_MODEL=gemma2:9b
```
...
| Seatbelt `[SKIP]` in sandbox tests | Sandbox probe failed | Run `cargo test -p hhagent-sandbox -- --nocapture` to see why |
