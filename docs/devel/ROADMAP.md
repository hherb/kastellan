...
- [x] Egress proxy (per-worker host allowlist, TLS pinning, audit logging) — **all 4 slices COMPLETE** (#1 boundary/SSRF, #2 force-routing, #3a MITM, #3b leak scanner, #4 TLS pinning).
  ...
  - [x] **Slice #4 — TLS pinning — COMPLETE 2026-06-13** (PR [#272](https://github.com/hherb/kastellan/pull/272), branch `feat/egress-slice4-tls-pinning`). SPKI-SHA256 pin set (RFC 7469) overlaid on the proxy's upstream webpki validation (`workers/egress-proxy/src/pins.rs`: `spki_sha256`/`PinSet`/`chain_has_pin`/`PinningVerifier` + `build_upstream_client_config`), so a compromised public CA can't silently MITM kastellan's own high-value frontier/LLM egress. Operator pins arrive via `KASTELLAN_EGRESS_PROXY_PINS` (unset/blank/`{}` ⇒ **byte-identical plain webpki**); a configured-pin mismatch **fails closed** as `Verdict::BlockedTlsPin` → host `egress.blocked.tls_pin`; a malformed pin set **aborts proxy startup** (fail loud). **Additive** defense — never weakens webpki and leaves the netns/allowlist/SSRF boundary untouched (`.dangerous()` installs a verifier strictly stronger than webpki). Host `proxy_policy`/`spawn_sidecar` thread `cert_pins_json`; spawn args bundled into `NetWorkerSpawn` (dropped both `#[allow(too_many_arguments)]`). **Forward-looking** (mirrors #3b): no frontier worker yet ⇒ all callers pass `None` today; operator pin-config wiring + routing frontier egress through a pinned sidecar deferred until the first frontier worker. New deps `x509-cert`/`sha2`/`base64` (all Apache-2.0/MIT). Mac `1668/0/8`, clippy `-D warnings` clean. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-13-egress-proxy-slice4-tls-pinning*`.
- [x] **Credential-leak scanner co-located in the egress proxy** (egress-proxy slice **#3b**) — **SHIPPED 2026-06-12** (see the `Slice #3b` line above). Outbound/inbound plaintext is scanned at the trust boundary for the verbatim bytes of every secret materialized for the calling worker (host provisions one-way value hashes into `<scratch>/secret_hashes.json`; the proxy holds no plaintext); hits kill the connection + audit hash+offset only. Mechanism + spawn-wiring complete; dispatch-time live-append deferred ([#268](https://github.com/hherb/kastellan/issues/268)) until the first secret-bearing egress worker. (Pattern: IronClaw `safety::leak_detector`, ZeroClaw `security/leak_detector.rs`.)
...
- [x] `browser-driver` worker (Playwright headless, dedicated profile, scratch FS) — ROADMAP:147 — **DONE (egress-proxy-routed, 2026-06-14)**
  ...
  - [x] **#281 (browser-driver portion) RESOLVED — 2026-06-15** (branch `feat/281-python-worker-seccomp-shim`, PR open). Pure-Python venv workers are spawned by bwrap directly and never ran the Rust prelude, so on Linux the built-and-tested `browser_client` seccomp filter was never applied. New **`kastellan-worker-lockdown-exec`** bin in the `prelude` crate (`rlimit` → `lock_down()` → `execve(target)`; the venv script inherits the seccomp filter under `NO_NEW_PRIVS`). browser-driver manifest discovers it (fail-closed on Linux ⇒ `Misconfigured`; binds it into `fs_read`) + sets `ToolEntry.lockdown_shim`; pure `tool_host::build_program_and_args` wraps the spawn at both cold-spawn sites (default `None` = byte-identical). **seccomp-only** (Landlock deferred behind the new additive `KASTELLAN_LANDLOCK_PROFILE=none`; bwrap mounts remain the FS layer). `capget`+`capset` added to `browser_client` (DGX-confirmed load-bearing: Playwright-Node startup + Chromium-zygote page-spawn). **DGX acceptance:** `browser_driver_e2e --ignored` 4/4 under the real filter + `lockdown_exec_smoke` (seccomp crosses `execve`); workspace `1829/0`. Mac `1837/0/13` + clippy clean. **Still open:** Landlock-for-browser (the deferred half) + gliner-relex (reuse the shim). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-15-python-worker-linux-seccomp*`.
