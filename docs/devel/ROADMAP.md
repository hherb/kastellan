...
- [ ] Extensions: `pgvector` available; `pg_search` (ParadeDB BM25) and `Apache AGE` graph deferred — `postgresql-18-pgvector 0.8.2` is installed by the install script, but the `CREATE EXTENSION vector` step lands with the migrations runner (next session). AGE deferred to [#9](https://github.com/hherb/hhagent/issues/9): PGDG ships `postgresql-NN-age` only up to PG 16 today. pg_search deferred to [#10](https://github.com/hherb/hhagent/issues/10): ParadeDB primarily distributes via Docker, `apt.paradedb.com` doesn't currently resolve for noble-arm64. Phase 1 memory-recall is open to FTS=`tsvector`+GIN and graph=plain join tables in the meantime.
