...
- [ ] Extensions: `pgvector` + native `tsvector`/GIN + `pg_trgm`. `postgresql-18-pgvector 0.8.2` is installed by `scripts/linux/install-postgres.sh`; the `CREATE EXTENSION vector` step lands with the migrations runner (next session). **Full-text search uses native Postgres `tsvector`+GIN with `ts_rank`** — committed-to after a 2026-05-09 review (closed [#10](https://github.com/hherb/hhagent/issues/10) won't-fix): for our corpus size (≤ ~1M memories) and write rate (a few hundred/day), `ts_rank` is comparable to BM25 in recall quality, and the embedding (pgvector) dominates the lexical re-ranker anyway. Hybrid lex + vector via RRF is ~5 lines of SQL, not a dependency. AGE graph extension deferred to [#9](https://github.com/hherb/hhagent/issues/9): PGDG ships `postgresql-NN-age` only up to PG 16 today; we'll use plain join tables on `entities`/`relations` and revisit when AGE for PG 18 ships.
