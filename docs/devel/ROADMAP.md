...
- [ ] Skill catalog (named/persisted Python skills) with optional human-approve gate
  - [x] **Slice 1 — crystallise + approval + operator CLI — 2026-06-13** (branch `feat/python-exec-skill-catalog`,
    ...
    `<skills>` reserved-tag guard on the surfaced description) + `l3py_approval.rs` (`evaluate_python_approval` —
    ...
    `evaluate_python_approval`, no registry). Workspace 1723/0/8, clippy `-D warnings` clean; live-PG verified
    (writer dedup, scheduler crystallise capture, CLI approve-without-registry). Spec/plan:
