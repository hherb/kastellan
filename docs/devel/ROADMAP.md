...
- [ ] Extensions: `pgvector` only. `postgresql-18-pgvector 0.8.2` is installed by `scripts/linux/install-postgres.sh`; the `CREATE EXTENSION vector` step lands with the migrations runner (next session). **Full-text search uses native Postgres `tsvector`+GIN with `ts_rank`** — committed-to after a 2026-05-09 review (closed [#10](https://github.com/hherb/hhagent/issues/10) won't-fix): for our corpus size (≤ ~1M memories) and write rate (a few hundred/day), `ts_rank` is comparable to BM25 in recall quality, and the embedding (pgvector) dominates the lexical re-ranker anyway. Hybrid lex + vector via RRF is ~5 lines of SQL, not a dependency. **Graph storage uses plain relational `entities`/`relations` tables behind a `Graph` trait in `db/src/graph.rs`** — committed-to in the same review (closed [#9](https://github.com/hherb/hhagent/issues/9) won't-fix): for a personal-agent graph (low thousands of nodes, occasional 2-hop, almost never 5-hop), recursive CTEs handle variable-length paths fine; the abstraction layer means we can swap to a real graph DB (AGE, Neo4j, Memgraph) without touching call sites if a real bottleneck ever appears.
- [ ] `db/migrations/` skeleton: `memories`, `tasks`, `entities`, `relations`, `audit_log`, `secrets` — next session (Option C2.2 in HANDOVER).
- [ ] `sqlx-cli` migration runner integration in core startup — next session (Option C2.2).
