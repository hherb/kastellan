...
- [ ] Auto-restart with backoff on worker crash (partial: `keep_alive=true` in ServiceSpec → `Restart=on-failure RestartSec=5` in the systemd unit, no exponential backoff yet; flip `core_service_spec` to `keep_alive=true` once the daemon body becomes a real long-running event loop — see HANDOVER Option H)
