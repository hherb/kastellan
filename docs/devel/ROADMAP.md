...
- [x] **One REQUIRE-knob contract, and a positive control that fails when zero tests ran** — [#714](https://github.com/hherb/kastellan/issues/714), [#622](https://github.com/hherb/kastellan/issues/622), [#664](https://github.com/hherb/kastellan/issues/664), PR [#720](https://github.com/hherb/kastellan/pull/720) — 2026-09-17. `tests_common::require::RequireKnob` is the one vocabulary: the flag dialect, the skip/fail split, the out-of-dialect warning and the new `[E2E]` success marker, with the knob named as **data** so a tier is one `const`. Three hand-rolled copies folded into it (gliner #653, micro-VM #667, and the new Postgres/sandbox/guard knobs). ⚠️ **A knob alone was never enough:** it fires only from inside a test body, so a filtered-out or renamed-out run emits no `[SKIP]` and exits 0 — `scripts/run-e2e-gate.sh` therefore asserts floors on `[E2E]` count **and** tests passed. Measured on `guard_tier_e2e` against a host with no Postgres: **without the knob `21 passed`, exit 0, 11 silent skips; with it, exit 1 naming the knob** — which is exactly what #622 said it could not see. ⚠️ Deliberately **no umbrella variable**: the Mac has no KVM, so a "demand everything" flag would make honest skips fail and get exported `=0`. Filed: [#718](https://github.com/hherb/kastellan/issues/718) — **92 hand-written `[SKIP]` lines in 47 files** bypass `skip_line` and every knob, so `HANDOVER.md`'s claim that they all render through it was false.
