...
- [ ] `browser-driver` worker (Playwright headless, dedicated profile, scratch FS) — ROADMAP:147
  ...
  - [ ] **Slice #2 (production egress)** — make browser-driver egress-proxy-routable: loopback-TCP↔UDS shim + in-browser per-instance-CA trust → `proxy_uds: Some(...)`, drop the dev exemption + the `KASTELLAN_BROWSER_DRIVER_INSECURE_DIRECT_NET` escape hatch, broaden the subresource allowlist policy, and close #263. [#280](https://github.com/hherb/kastellan/issues/280).
