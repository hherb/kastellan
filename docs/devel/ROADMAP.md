...
- [ ] Egress proxy (per-worker host allowlist, TLS pinning, audit logging) — **decomposed into 4 slices; slice #1 shipped.**
  ...
  - [ ] **Slice #2 — unbypassable force-routing** (private netns whose only route out is the proxy UDS on Linux; pf/Seatbelt story on macOS) + the `web-common` CONNECT-over-UDS connector (hyper+tokio-rustls) + the `tool_host` auto-spawn hookup. This is what makes real workers actually route through the proxy.
