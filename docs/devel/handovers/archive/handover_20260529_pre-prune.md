...
**Last updated:** 2026-05-29 (★ **Issue [#147](https://github.com/hherb/kastellan/issues/147) — redact secret plaintext in the tool audit row** — shipped on branch `fix/issue-147-redact-tool-req-plaintext`, PR pending. Slice-1 (Item 31) scoped the secret-plaintext privacy invariant to `actor='policy'` rows only; the `tool:<name>` row's `payload.req` was snapshotted AFTER substitution, so it carried the redeemed plaintext — anyone with `audit_log` read access could recover every materialized secret from the tool row. Fix: snapshot `req_for_audit` in `tool_host::dispatch` BEFORE `substitute_refs_in_params` (one moved `params.clone()`), so the tool row records the opaque `secret://<8-hex>` refs exactly as the planner issued them. Chosen over the issue's "substitute plaintext back via `redemption_events`" proposal because snapshot-before is more robust (no risk of redacting a coincidental plaintext match) and faithfully records the request-as-issued; the worker still receives the real plaintext via the mutated `params`. The worker's *output* may still echo a secret it was legitimately given — that lands in `payload.result`, scoped out by design. TDD: widened `policy_rows_contain_no_substring_of_redeemed_plaintext` (test 7) with a tool-row `req`-scoped negative assertion + added focused test 9 `tool_row_req_shows_opaque_ref_not_plaintext` (negative: no plaintext; positive: ref present). Both confirmed RED on pre-fix code (tool row `req` carried the marker), GREEN after. **Workspace 1144 → 1145 (+1 integration test) on macOS without PG**; `secret_vault_e2e` 9/9 verified live against Postgres.app v18. No new clippy warnings (`kastellan-core` stays at 10 pre-existing). **Filed [#150](https://github.com/hherb/kastellan/issues/150):** pre-existing `mem_burner` `set_len()`-after-`with_capacity` is now a clippy *deny*-level error (toolchain promoted `clippy::uninit_vec`), blocking `cargo clippy --all-targets`; unrelated to this change (sandbox crate, untouched), `cargo build`/`cargo test` unaffected. — earlier this session: ★ **Opaque secret references — Slice 1** — **merged to `main` via PR [#146](https://github.com/hherb/kastellan/pull/146) at `bc36e4c`** + post-review polish `5885e26` + `9348e9c`, closes HANDOVER Item 31. New `core::secrets` module with `Vault` (TTL'd `std::sync::RwLock<HashMap>`, default 1h), `SecretRef` opaque type, `RedeemResult` (Hit/Expired/NotFound), `substitute_refs_in_params` recursive walker (exact-match only). Chokepoint wiring: `tool_host::dispatch` substitutes BEFORE `worker.call`; on miss/expired/non-UTF-8 fails closed with `ToolHostError::SecretRedemptionFailed` + one `policy / secret.redemption_failed` row, **worker not called, no tool row**. On Hit, substitute in place and emit one `policy / secret.redeemed` row per ref (best-effort). Materialize writes one `policy / secret.materialized` row carrying `{name, ref_hash, ttl_secs, actor}` — **hard-fail on audit error** (asymmetric vs redeem: no materialized-but-unaudited ref can exist). `KASTELLAN_BOOTSTRAP_SECRETS=name1,name2` env var drives bootstrap-time materialization in `main.rs`. `ToolHostError` widened to `#[non_exhaustive]` (first variant addition since Option M / 2026-05-10). 4 substantive commits TDD-ordered: Task 1 module skeleton at `b58cd55`, Task 2 Vault impl at `3bc66a3`, Task 3 walker + FakeVault at `7b1e3c5`, Task 4 dispatch wiring + 8 integration tests + classifier test at `19eebd6`. **Workspace 1096 → 1137 (+41) on macOS without PG** (7 unit + 9 vault + 16 walker + 8 integration + 1 classifier); with PG live 636 passed (vs 603 on main = +33 exercised by PG). Pre-existing `gliner_relex_e2e` and `embedding_recall_e2e` flakes (PG initdb/pg_notify race, exists on `main` too) noted but unrelated. No new clippy warnings after fixing 4 `explicit_auto_deref` instances in `secret_vault_e2e.rs`. `tool_host.rs` 767 → 867 LOC (367 over the 500-LOC cap; pre-existing tech-debt extended; defer sibling-lift to a separate slice bundling Items 30 + 31). Item 31 closed; next pickup TBD from operator-picks bucket (Slice 2: CLI ↔ daemon IPC + `kastellan-cli secrets materialize`; `tool_host.rs` sibling-lift bundling Items 30 + 31). — earlier this session 2026-05-28 (★ **Worker-output prompt-injection guard — Slice 1** — **merged to `main` via PR [#141](https://github.com/hherb/kastellan/pull/141) at `62905ae`**, closes HANDOVER Item 30. New module `core::cassandra::injection_guard` with pure-function 22-entry English-substring catalogue spanning 4 attack classes (instruction_override, role_hijack, secret_exfiltration, unsafe_tool_coercion); per-rule weights summed (cap 1.0); ≥ `BLOCK_THRESHOLD` (0.70) → `Block`. Wired into `tool_host::dispatch` chokepoint: on Block, replace worker result with redacted placeholder JSON + write `actor='policy' action='injection.blocked'` audit row carrying SHA-256 + length + score + class codes; raw scanned text **never** persisted (pinned end-to-end by `policy_audit_row_contains_no_substring_of_blocked_body` integration test). 4 commits TDD-ordered + 2 post-claim fixes: Task 1 module skeleton at `536d23a`, Task 2 `extract_scannable_text` at `11a0a8e`, Task 3 catalogue + `screen` + `normalize` at `eae2a6f`, Task 4 dispatch wiring + 6 integration tests at `4e32988`; post-claim fix `45627fd` scopes the privacy-invariant test to `WHERE actor='policy' AND action='injection.blocked'` (the tool row's `req` field legitimately carries input argv; spec §4.2 privacy promise is scoped to the policy row) + adds a positive-presence assertion (`rows.is_empty()` now fails) so a future regression where the screen stops firing trips the same test; post-claim fix `e81b079` widens the zero-width strip set (adds Word Joiner U+2060, Soft Hyphen U+00AD, Mongolian Vowel Separator U+180E to the existing ZWSP/ZWNJ/ZWJ/BOM) + documents two evasion limits as filed issues [#142](https://github.com/hherb/kastellan/issues/142) (chat-template tokens may false-positive on technical docs once web-fetch/MCP workers ship) and [#143](https://github.com/hherb/kastellan/issues/143) (`walk()` has no explicit recursion-depth guard; reachable only if upstream protocol/serde_json limits are removed). **Workspace 1065 → 1096 (+31) on macOS** (2 const pins + 7 extract + 13 screen + 6 integration + 3 normalize-strip), all green; no new clippy warnings. `InjectionDecision` is `#[non_exhaustive]` so a future Review tier (HANDOVER's 0.45–0.70 band) slots in without breaking callers. PG-required integration tests skip-as-pass on this Mac (PG not configured); empirical validation against real PG deferred to operator. Closes Item 30; **Item 31 (opaque secret references) remains the next pickup**. — earlier this session 2026-05-28: test pin: Issue [#89](https://github.com/hherb/kastellan/issues/89) — `/tmp` per-spawn ephemeral tmpfs invariant — **merged to `main` via PR [#139](https://github.com/hherb/kastellan/pull/139) at `504094e`**; PR [#140](https://github.com/hherb/kastellan/pull/140) also merged at `6fd82bf` (docs-only — import 7 openhuman design patterns + promote 2 to immediate pickups as Item 30 + 31 in the bucket). Adds one Linux integration test (`tmp_is_per_spawn_ephemeral_tmpfs_under_worker_strict` in `sandbox/tests/linux_smoke.rs`) that runs two real-bwrap spawns under `Profile::WorkerStrict` and pins three properties of the bwrap `--tmpfs /tmp` mount (see `linux_bwrap::build_argv`): (1) a marker written in spawn A is **not** visible in spawn B (ephemerality / per-spawn isolation), (2) spawn B exits 0 (writability — guards against an over-correction to read-only `/tmp`), (3) spawn B's own freshly-written marker IS visible in its own `/tmp` listing (sanity check the listing isn't silently empty). Each marker is uniquified with PID+nanos so parallel CI lanes can't collide. **`[SKIP]` discipline preserved** (gated by `skip_if_no_userns()` like the rest of `linux_smoke.rs`). **Why this pin matters now:** the GLiNER-Relex worker (PR #88 onwards) sets `TORCHINDUCTOR_CACHE_DIR=/tmp/torchinductor` in its sandbox `policy.env` with an inline comment "*/tmp is tmpfs inside the sandbox so this is ephemeral per-spawn*" — that comment is the only thing protecting against drift today. If a future refactor switches `--tmpfs /tmp` → `--bind /tmp /tmp` to expose host caches for performance, two concurrent workers would race on `/tmp/torchinductor` → intermittent cache corruption, hard to attribute. **Verification done on macOS:** workspace `cargo test --workspace`: **1065 / 0 / 3** unchanged (the new test is cfg-gated to `target_os = "linux"` so it doesn't execute here); cross-target syntax check via `cargo check --tests -p kastellan-sandbox --target aarch64-unknown-linux-gnu`: clean. **Verification deferred to DGX (operator-driven):** (a) test should PASS against current code; (b) temporarily replacing `--tmpfs /tmp` with `--bind-try /tmp /tmp` in `linux_bwrap::build_argv` should make the ephemerality assertion trip (proves the pin catches real drift). Closes Issue #89. — earlier this session 2026-05-28: refactor: `idle_timeout/release.rs` sibling-lift — **shipped on branch `refactor/idle-timeout-release-sibling-lift`, merged to `main` via PR [#138](https://github.com/hherb/kastellan/pull/138) at `5fc5fee`** + in-PR polish `78305cf` (two self-review cleanups: (a) banner-rot in `idle_timeout.rs` — the "Release path (slice-2 tasks 5 + 6 + 7)" comment block had rotted past the file move and now sat above only `mod release;` + the re-export; replaced with a tighter 5-line doc-comment directly above `mod release`; (b) visibility on `replace_idle_teardown_handle` narrowed from `pub(crate)` → `pub(super)` — the only out-of-module caller is the parent's `tests.rs` sibling submodule of `release.rs`, so `pub(super)` is the strictly smaller scope that compiles; rationale recorded inline so a future contributor doesn't widen it back without a real out-of-crate caller. Third review finding (defensive `None`-slot branch in `release_idle_timeout_worker`) is pre-existing and was flagged out-of-scope for the PR; deferred). Workspace **1065 → 1065 (+0)** post-polish, byte-equivalent to pre-fix PR baseline. The natural follow-up flagged at the end of the Issue #136 slice: that slice grew `core/src/worker_lifecycle/idle_timeout.rs` from 576 → 647 LOC (+71), pushing the file from 76 over the 500-LOC soft cap to 147 over. This slice lifts the release path (`release_idle_timeout_worker` + `abort_idle_teardown_handle` + `replace_idle_teardown_handle`) into a new sibling submodule `core/src/worker_lifecycle/idle_timeout/release.rs`. **Pure mechanical refactor** — three functions moved verbatim; no behaviour change. File-size delta: `idle_timeout.rs` 647 → **490 LOC** (now 10 under cap, closes the 147-LOC overage); new `release.rs` **195 LOC** (well under cap); `idle_timeout/tests.rs` 467 → 472 LOC (+5 — the new explicit `use super::release::replace_idle_teardown_handle;` import banner with a 4-line doc comment explaining why tests reach into the sibling submodule directly). Visibility: `release_idle_timeout_worker` stays `pub(crate)` in the new file and is re-exported from `idle_timeout.rs` via `pub(crate) use release::release_idle_timeout_worker;` so external callers (notably `manager::WorkerHandle::Drop` via `super::idle_timeout::release_idle_timeout_worker`) resolve byte-equivalently. `abort_idle_teardown_handle` stays module-private (only called from within `release.rs`); `replace_idle_teardown_handle` widens from `fn` to `pub(crate) fn` so the parent `tests.rs` can import it explicitly. Chose explicit-import over `#[cfg(test)] pub(crate) use release::replace_idle_teardown_handle;` re-export so the structural home of the helper is visible at the import site — future contributors reading `tests.rs` see directly that `replace_idle_teardown_handle` now lives in the sibling submodule. **Workspace 1065 → 1065 (+0) on macOS**, all green: pure refactor, existing tests are the regression pin — `replace_idle_teardown_handle_aborts_prior_and_stores_new` + `replace_idle_teardown_handle_steady_state_holds_at_most_one_alive_per_slot` (the two issue #85 pins) pass byte-equivalently. The 6 existing `worker_lifecycle_idle_timeout_e2e` integration tests are the load-bearing regression pin for the release-path semantics (especially `idle_seconds_teardown_clears_warm_slot` which exercises the rewritten replace-and-spawn helper end-to-end). Linux DGX expected at +0 by the same argument. No new clippy warnings introduced — the pre-existing 4 `MutexGuard`-across-await warnings on `manager.rs` test inspectors, pre-existing doc-list-indent in `db/src/probe.rs`, pre-existing `io_other_error` in `kastellan-protocol`, pre-existing `mem_burner` `set_len()`-after-`reserve` are all unchanged. **2 unused-import warnings** that appeared after the move (`OwnedMutexGuard` and `IdleTimeoutCaps` — both moved to `release.rs` with their sole callers) **were cleaned up inline**, leaving the `idle_timeout.rs` import block tighter than pre-refactor. Cumulative file-size watch for `worker_lifecycle/`: `idle_timeout.rs` 490 LOC (under cap), `release.rs` 195 LOC, `manager.rs` 339 LOC, `composite.rs` 256 LOC, `types.rs` 246 LOC — all four production-code parents now well under the 500-LOC cap. Closes the Item 29 operator-picks sub-bullet "idle_timeout/release.rs sibling-lift" filed at end of Issue #136. — earlier this multi-day session 2026-05-27: Issue [#136](https://github.com/hherb/kastellan/issues/136) — debounce pending-acquires queue-depth warn — **shipped on branch `fix/issue-136-pending-acquires-warn-debounce`, merged to `main` via PR [#137](https://github.com/hherb/kastellan/pull/137) at `fed0f21`**. Natural close-out follow-up to PR #135's #84 work — filed in #135 review when it became clear the new `tracing::warn!` fires at request rate under sustained queue depth. 2 commits on branch: 1 doc-sync to main = `7f98ee4` (`8204a3c`, PR #135 / trio merged + polish `1fc7eca`) + 1 feature `9fae2f9` (`feat(worker_lifecycle/idle_timeout): debounce pending-acquires queue-depth warn (closes #136)`). Implementation: new `last_warn_unix_nanos: AtomicI64` field on `ToolSlot` (peer to `pending_acquires`); new `PENDING_ACQUIRES_WARN_COOLDOWN: Duration = Duration::from_secs(30)` const colocated with `PENDING_ACQUIRES_WARN_THRESHOLD`; new pure predicate `debounce_warn(last_warn_nanos: i64, now_nanos: i64, cooldown: Duration) -> bool` using `saturating_sub` over i64 so clock-skew-backward suppresses (under-warn during NTP drift); `acquire_impl` warn site rewritten with outer depth gate → debounce predicate → `compare_exchange(last, now, AcqRel, Relaxed)` to claim the warn slot so only the CAS winner emits the warn under concurrent acquirers. **Workspace 1058 → 1065 (+7) on macOS**, all green: +1 cooldown const pin + 5 `debounce_warn` predicate tests (cold-path / suppress-within / boundary inclusive / past-cooldown / clock-skew-backward) + 1 `slot_for` zero-init pin. Linux DGX expected at +7 by the same argument (every new test is platform-independent; `SystemTime`/`UNIX_EPOCH`/`AtomicI64` are stdlib). No new clippy warnings introduced. **File-size growth:** `core/src/worker_lifecycle/idle_timeout.rs` 576 → 647 LOC (+71); was 76 over the 500-LOC cap, now 147 over. **The natural sibling-lift target is now a fresh `idle_timeout/release.rs` carrying `release_idle_timeout_worker` (~85 LOC) + `abort_idle_teardown_handle` (~5 LOC) + `replace_idle_teardown_handle` (~35 LOC)** — a clean ~125-LOC seam between "acquire path" and "release path" that would land at ~522 LOC each. Deliberately deferred to a separate refactor slice; this slice ships the surface. — earlier this session 2026-05-27: worker_lifecycle hardening trio — issues [#84](https://github.com/hherb/kastellan/issues/84) / [#85](https://github.com/hherb/kastellan/issues/85) / [#86](https://github.com/hherb/kastellan/issues/86) — **merged to `main` via PR [#135](https://github.com/hherb/kastellan/pull/135) at `7f98ee4`** + in-PR polish `1fc7eca` (5-issue review pass: success-path `slot: None` abort symmetry; `PendingAcquireGuard::depth()` doc tightening for the concurrent case; `_test_slot_pending_acquires` doc reframing dropping the `kastellan-cli supervisor status` overclaim; test-isolation fix on the two `replace_idle_teardown_handle_*` tests so they lock the genuine slot's `state`; filed [#136](https://github.com/hherb/kastellan/issues/136) for the deferred warn-debounce cooldown — which is exactly the slice this session picked up next). Workspace 1058 → 1058 (+0) after the polish — same test count, tighter doc + correctness. Bundles three small follow-ups filed during PR #83 (worker lifecycle slices 1 + 2) post-review into one slice per HANDOVER's "Item 29 operator-picks" bucket. 4 commits: 1 doc-sync to main = `8655319` (`d0f356d`) + 1 feature `0983586` for #86 (`#[non_exhaustive]` on `Lifecycle::IdleTimeout` variant blocks out-of-crate struct-literal construction so future manifest plumbing is forced through the validated `Lifecycle::idle_timeout` constructor; pinned by a `compile_fail,E0639` doctest on the enum's documentation since doctests compile as separate crates) + 1 feature `94381f3` for #85 (new `ToolState.idle_teardown_handle: Option<JoinHandle<()>>` field + `abort_idle_teardown_handle` + `replace_idle_teardown_handle` helpers — abort the prior teardown task before spawning a new one, so the slot holds exactly one pending teardown at steady state instead of accumulating ~`idle_seconds` stale sleepers per tool under high request rate) + 1 feature `dc545cb` for #84 (new `ToolSlot.pending_acquires: AtomicU32` + `PendingAcquireGuard` RAII bracket on the `lock_owned().await` queue wait + `pending_acquires_should_warn(depth)` pure predicate + new `PENDING_ACQUIRES_WARN_THRESHOLD: u32 = 5` const driving a `tracing::warn!` when queue depth crosses the operator-visible boundary + sync `_test_slot_pending_acquires(tool_name)` inspector that the future `kastellan-cli supervisor status` surface will use). **Workspace 1050 → 1058 (+8) on macOS**, all green: +5 for #84 (4 in `idle_timeout::tests` + 1 in `manager::tests`) + +2 for #85 + +1 doctest for #86. Linux DGX expected at +8 by the same argument (everything new is platform-independent). The 6 existing `worker_lifecycle_idle_timeout_e2e` integration tests pass unchanged — they're the regression pin for the load-bearing teardown semantics (especially `idle_seconds_teardown_clears_warm_slot`, which exercises the rewritten replace-and-spawn helper end-to-end). No new clippy warnings introduced: the 4 pre-existing `MutexGuard`-across-await warnings on `manager.rs:278/284/293/299` (test inspectors `_test_slot_has_warm` + `_test_slot_consecutive_restarts`) are unchanged; the new `_test_slot_pending_acquires` is sync (atomic load, no tokio mutex involved on the read path) so it doesn't add to the count. — earlier this session: Issue [#131](https://github.com/hherb/kastellan/issues/131) — `bring_up_pg_cluster_with_timeout` sibling for per-call timeout override **merged to `main` via PR [#133](https://github.com/hherb/kastellan/pull/133) at `8655319`** + post-merge code-review nit `876fb42` (panic messages emit `{timeout:?}` via `Duration`'s Debug formatter rather than `.as_secs()`, so a caller passing `Duration::from_millis(500)` sees "within 500ms" instead of the misleading "within 0s"; `Duration::from_secs(30)` still renders as "30s" — bit-identical visual output for all 50+ existing default-cap callers). 3 commits: 1 doc-sync to main = `162f71f` (`8f965ac`) + 1 feature `2d4084c` (new `pub fn bring_up_pg_cluster_with_timeout(bin_dir, data_label, log_label, service_name, timeout: Duration) -> PgCluster` next to the existing 4-arg `bring_up_pg_cluster` which becomes a 1-line wrapper forwarding `Duration::from_secs(PG_BRING_UP_TIMEOUT_SECS)`; +2 signature-pin unit tests) + 1 docs claim `4edc250` + post-review `876fb42`. Sibling-function shape chosen over `Option<Duration>` parameter on the existing function to avoid `, None` noise at all 50+ existing call sites; matches the `pg_bin_dir_candidates_with_env_override` precedent from PR #126. The two panic messages inside `_with_timeout` emit `{timeout:?}` (Duration Debug formatter — accurate at every order of magnitude including sub-second), so a caller passing `Duration::from_secs(10)` sees "within 10s" on timeout and `Duration::from_millis(500)` sees "within 500ms". Closes Issue #131; advances Item 29's operator-picks bucket. **Workspace 1048 → 1050 (+2) on macOS**, all green (the +2 are the new compile-time signature pins). The 50+ existing `bring_up_pg_cluster` callers continue to pass through the refactored 1-line wrapper byte-equivalently — integration suite is the regression pin for the default 30 s path. No new clippy warnings; pre-existing `mem_burner`, `io_other_error`, doc-list-indent warnings unchanged. — earlier this session: test-module lift quadruple **shipped on branch `refactor/item-29-test-module-lift-quadruple`, merged to `main` via PR [#132](https://github.com/hherb/kastellan/pull/132) at `162f71f`**. 5 commits: 1 doc-sync to main = `4e94e42` (`91b9dcc`) + 4 pure-mechanical test-module sibling lifts: `core/src/entity_extraction/batch_upsert.rs` 589 → 428 LOC (`4e39cf9`), `core/src/worker_lifecycle/idle_timeout.rs` 581 → 415 LOC (`17e3540`), `core/src/worker_lifecycle/manager.rs` 565 → 339 LOC (`d891d41`), `db/src/lib.rs` 912 → 525 LOC (`91e2e6e`). Net **-940 LOC** off the production parent files (945 LOC moved to 4 new sibling `tests.rs` modules with module-doc-comment headers; 58 tests moved verbatim). All 4 use Rust 2018 sibling-directory module resolution (`mod tests;` resolves to `<parent>/tests.rs` for the 3 module files and to `db/src/tests.rs` for the crate-root `lib.rs`). Three files now well under the 500-LOC soft cap; `db/src/lib.rs` is 25 LOC over (production-code residual: error type, options structs, pg-discovery helpers, `require_absolute` — a further split would require production-code surgery). Closes the `batch_upsert.rs` 547-LOC sibling-lift sub-bullet plus the bundled `idle_timeout.rs`/`manager.rs`/`db/src/lib.rs` lifts in Item 29's operator-picks bucket. **`crate::env_lock` (defined in `db/src/lib.rs`, `pub(crate) #[cfg(test)]`, shared with `conn::tests`) deliberately stays at the crate root** — moving it would have rippled to every other `#[cfg(test)]` module in the crate. Workspace **1048 / 0 / 3 macOS unchanged** from pre-lift baseline (pure refactor; existing tests are the regression pin — same names, same behaviour, only the storage location changed). No new clippy warnings introduced: pre-existing `MutexGuard`-across-await in `manager.rs::_test_slot_*` test inspectors (lines 278 + 293, untouched by this lift), pre-existing doc-list-indent in `db/src/probe.rs`, pre-existing `io_other_error` in `kastellan-protocol`, pre-existing `mem_burner` `set_len()`-after-`reserve` error all unchanged. PR merged at `162f71f` with zero review comments. — earlier this multi-day session: `bring_up_pg_cluster` polling-cap lift + 5-site consolidation **merged to `main` via PR [#129](https://github.com/hherb/kastellan/pull/129) at `4e94e42`** + same-PR post-review polish `f906d18` (doc-only: inline `// IMPORTANT: do not reorder fields` comment above `PgCluster` makes the RAII teardown invariant explicit at the struct definition + clarifies in the `bring_up_pg_cluster` docstring that the `service_name.len() <= 200` cap is the launchd label limit / systemd unit-file basename limit with headroom for the `.service` suffix, *not* the 108-byte `sun_path` cap which governs the socket directory). Two new follow-up issues filed during PR #129 review: [#130](https://github.com/hherb/kastellan/issues/130) (parallel-launchd bring-up contention under `KASTELLAN_PG_BIN_DIR` override) and [#131](https://github.com/hherb/kastellan/issues/131) (optional per-call timeout override for `bring_up_pg_cluster` for heterogeneous launchd vs Homebrew). Review item #4 (two supervisor handles in `PgCluster.sup` + `ServiceGuard`) deferred to existing issue #39. Branch carried 2 substantive commits + 1 docs-claim commit + 1 post-review polish: `02d6691` widens canonical `tests_common::bring_up_pg_cluster`'s polling cap from 15s → 30s via new `PG_BRING_UP_TIMEOUT_SECS` named const; `0c7b644` migrates 5 byte-duplicate local-copy sites (`core/tests/{agent_prompts,scheduler_lanes,scheduler_step_dispatch,scheduler_inner_loop,scheduler_crash_recovery}_e2e.rs`) onto the shared helper, **net -1062 LOC**; `7b71003` doc claim of work; `f906d18` post-review doc polish. Closes the polling-cap lift sub-bullet in Item 29's operator-picks bucket. Workspace **1048 / 0 / 3 macOS unchanged** (no behaviour change on hosts where env unset; verified post-fix). Empirically under `KASTELLAN_PG_BIN_DIR=/Applications/Postgres 2.app/Contents/Versions/18/bin/`: 13/13 previously-failing tests now pass individually (was: 12-13 timing out at `"pg active: timeout 15s; last=Inactive"`). Three deliberate scope splits remain tracked: `pg_bin_dir_or_skip`/`unique_suffix`/`skip_if_no_supervisor` local-copy consolidation in the 5 files (~30 LOC, separate code-dedup slice); parallel-launchd-bring-up contention under full `cargo test --workspace` with override (now tracked as issue #130 — requires serial-mutex or `--test-threads=1`); `observation_capture.rs`'s `Duration::from_secs(15)` line waits for the *core daemon's* log, not PG. — earlier this multi-day session: Issues [#114](https://github.com/hherb/kastellan/issues/114) + [#115](https://github.com/hherb/kastellan/issues/115) — `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL **shipped on branch `fix/issues-114-115-walk-edges-dedupe-and-combine`, **merged to `main` via PR [#128](https://github.com/hherb/kastellan/pull/128) at `bb32cab`** + post-merge polish `978b5be` (named const `WALK_EDGES_AROUND_DIRECTION_COL` for the trailing partition column index + SQL comment on the outer `ORDER BY direction ASC` lex-order incidental + test header pin documenting that `per_direction_limit` shares semantics with each per-direction method's `limit`; no behaviour change). Four commits TDD-ordered: 1 failing diamond-topology test → dedupe via DISTINCT ON in both per-direction walks (`668a36f`, closes #114) + 1 new `walk_edges_around` method + `EdgesAround` struct with smoke integration test pinning byte-equivalence vs separate walks + per-direction LIMIT (`c25bf96`) + 1 CLI migration in `relations_show.rs` from two round-trips to one (`c12774a`) + 1 doc-comment hint on per-direction methods recommending `walk_edges_around` when both directions are needed (`0bb65b1`). Per-direction methods retained as a stable surface for callers that genuinely need only one direction (e.g. a future recall lane expanding seed entities outbound only). **Workspace 1046 → 1048 (+2) on macOS**, all green: +1 `walk_edges_dedupes_diamond_topology_to_shortest_depth` + +1 `walk_edges_around_matches_separate_walks_and_clips_per_direction`. The existing `cli_relations_show_e2e` integration test verifies byte-equivalent CLI output through the new query path. **Diamond-dedupe semantics:** an edge_id reachable via multiple paths from the seed appears exactly once, at its shortest-path depth. Implemented via `DISTINCT ON (edge_id) ORDER BY edge_id, depth ASC` in an intermediate CTE between the recursive walk and the entity-join. The `LIMIT` applies *after* dedupe so it bounds the final row count rather than the intermediate traversal-row count. **`walk_edges_around` SQL shape:** WITH RECURSIVE outbound_walk + inbound_walk + outbound_deduped + inbound_deduped + outbound_rendered + inbound_rendered (each rendered CTE applies its own ORDER BY + LIMIT $3) UNION ALL final ORDER BY (direction, depth, edge_id); Rust decoder reads trailing `direction` text column (col 11) to partition into `EdgesAround::{outbound, inbound}`. No clippy regression. Earlier in this multi-day cycle: Next-TODO Item 29 — `KASTELLAN_PG_BIN_DIR` env-var override on the test-fixture Postgres-discovery path **merged to `main` via PR [#126](https://github.com/hherb/kastellan/pull/126) at `7adc582` on 2026-05-26** + post-merge refactor `463553d` (capacity from `defaults.len()` instead of magic number `17`; +1 issue [#127](https://github.com/hherb/kastellan/issues/127) filed for an env-var save/restore RAII helper around the 4 unit tests). Three branch commits: 1 session-start doc-sync `c3dd3e4` (sync to main = a1dbef6 for PR #125 / Item 28 merge) + 1 feature `557ca3b` (new public `pg_bin_dir_candidates_with_env_override()` + `PG_BIN_DIR_ENV` const + 4 unit tests under `crate::env_lock()` covering unset / valid-prepend / empty-string / whitespace-only) + 1 mechanical refactor `954088c` (6 call sites switched: `tests-common::skip::pg_bin_dir_or_skip` plus the 5 byte-identical local copies in `core/tests/scheduler_{crash_recovery,lanes,inner_loop,step_dispatch}_e2e.rs` + `core/tests/agent_prompts_e2e.rs`). Closes the Postgres.app session-local-override papercut that PR #118 / Slice 2.5 and PR #125 / Issue #95 Layer B both re-encountered (operator memory `postgres-app-bin-paths.md` documents the workflow). Workspace **1042 → 1046 (+4 unit) on macOS**, all green; ignored unchanged at 3. Behaviour byte-equivalent when env var is unset (every developer's default state) — pinned by `pg_bin_dir_candidates_with_env_override_returns_defaults_when_unset` unit test. Production runtime discovery (`kastellan-db-init` binary) deliberately NOT switched — override helper is documented test-infra-only. Out of scope per scope-discipline: consolidating the 5 local-copy fixtures onto `tests_common::pg_bin_dir_or_skip` (separate code-dedup slice; flagged in tech-debt) and closing the 15-second `wait_for_status_active` polling cap in the 6 sites with local `bring_up_pg_cluster` copies that still overshoot on Postgres.app even with the env override (memory file's "override cost" section documents the 12 expected test-failures the override unmasks; separate test-infra slice to lift those polling caps or consolidate onto the shared `tests-common::bring_up_pg_cluster` which already has appropriate timing). Earlier in this multi-day cycle: Next-TODO Item 28 — Issue [#95](https://github.com/hherb/kastellan/issues/95) entity-upsert Layer B (full-batch unnest + per-row attribution fallback) **merged to `main` via PR [#125](https://github.com/hherb/kastellan/pull/125) at `dac0dcd`** + same-PR post-review fix `a1dbef6` (six review follow-ups, all scoped to `batch_upsert.rs` + matching e2e; +1 unit test pinning `is_constraint_violation_code` 5-char SQLSTATE guard against `"23"` / `"230"` / `"2300"` / `"230000"` / `"23X05X"`; +1 e2e `upsert_batch_empty_input_returns_zero_outcome` pinning both entity and relation empty-input early-returns; `dedup_entity_inputs` now also returns parallel `Vec<String>` of name_norms so the dispatcher avoids double-normalize; phase-2 partial-commit semantics documented on both `per_row_*` helpers; stray double blank line removed). Branch carried 12 implementation commits + the post-merge fix: 2 docs (spec at `c70ae5d`, plan at `654a17d`) + 4 pure-helper TDD slices (dedup_entity_inputs at `222e06c`, build_entity_unnest_arrays at `7dd0e6a`, is_constraint_violation at `42f74e2`, error formatters at `08a7ea7`) + 6 integration slices (entity batch+delegate `5aed725` amended, order+dedup `6cb02ab`, quarantine `277cf29`, entity FK fallback `88992a4`, relations batch+fallback `3395bf5`, relations FK fallback `37ef656`) + post-merge fix `a1dbef6`. New module `core/src/entity_extraction/batch_upsert.rs` (~550 LOC, ~50 over cap from inline tests — sibling-`tests.rs` lift is the natural follow-up) holds both phases + 5 pure helpers. `gliner_relex.rs` 625 → 571 LOC (down 54; public function becomes 1-line delegate; legacy per-row relation loop deleted). **Workspace 1023 → 1042 (+19, +11 unit / +8 integration) on macOS**, all green; ignored unchanged at 3. Empirically verified against real PostgreSQL (Postgres.app v18 at `/Applications/Postgres 2.app/Contents/Versions/18/bin/`) via session-local `default_pg_bin_dir_candidates` override (both for the original branch and for the post-merge fix); override reverted before each commit per PR #118 / Slice 2.5 pattern. Both fallback tests fire the per-row attribution chain correctly — error messages contain `kind='drug', name_norm='aspirin'` (entity FK) and `src=N, dst=M, kind='eats'` (relation FK), proving the attribution improvement over Layer A's `format!("upsert entity: {e}")` which doesn't identify the failing row. Public API (`UpsertOutcome`, `EntityExtractionError`, 8-key audit payload) byte-frozen. All 5 existing Layer A integration tests continue to pass byte-equivalently through the new delegate path (regression pin). Two review items deferred per the PR description: (a) `batch_upsert.rs` 547 LOC over the 500-LOC soft cap by inline tests (sibling-lift candidate); (b) single-quote ambiguity in error formatters when an entity name itself contains `'` (not load-bearing for attribution, defer until a real PII-safe quote shape emerges). Worktree `/Users/hherb/src/kastellan-issue-95` and merged branch `feat/issue-95-upsert-layer-b` ready for cleanup. Earlier in this multi-day cycle: doc-sync commit `f6fa5b0` (sync to main = e93997e for PR #124 merge).

(prior session) Next-TODO Item 27 — Slice 2.5 follow-up triple (issues [#120](https://github.com/hherb/kastellan/issues/120) + [#121](https://github.com/hherb/kastellan/issues/121) + [#122](https://github.com/hherb/kastellan/issues/122)) **merged to `main` via PR [#124](https://github.com/hherb/kastellan/pull/124) at `e93997e`** with same-PR post-review fixup `55bb8f1` (empty-tag guard on `MacosContainer::probe_image` + short-circuit on `MacosContainer::probe()` failure in `probe_registered_container_images` + `tool_names` tracing field switched from Debug to comma-joined Display + `_probe_results` discard-rationale comment in `main.rs`). Branch carried 3 commits + 1 post-review fixup (one per issue plus fixup): `07de743` warm-cache key invariant test (#121); `9cb375e` `MacosContainer::probe_image(tag)` + pure `build_image_inspect_argv` + new `sandbox_health` module + daemon-startup image-tag walk (#120); `a569885` `skip_if_image_missing` delegates to `probe_image` (#122); `55bb8f1` post-review fixup. Workspace **1011 → 1023 (+12, post-review fixup)** on macOS, all green; ignored unchanged at 3. Worktree `../kastellan-item-27` and merged local branch `feat/item-27-slice-2.5-followups` cleaned up post-merge. Earlier this multi-day session: Next-TODO Item 26 — `gliner_relex.rs` test-module sibling lift **merged to `main` via PR [#123](https://github.com/hherb/kastellan/pull/123) at `920e0c9`**; pure refactor, 1547 → 811 LOC parent + new 745-LOC sibling `tests.rs`; workspace re-verified post-merge at macOS **1011 / 0 / 3**. Earlier still: Next-TODO Item 25 — GLiNER-Relex Slice 2.5 (Containerfile + macOS image build) **merged to `main` via PR [#118](https://github.com/hherb/kastellan/pull/118) at `a9e3385`** + post-merge `e11e3f3` (macOS test-infra hardening + container e2e empirically verified — `Dr Smith --[treats]--> asthma` extraction through Apple `container` PASSED in 12.58s when Postgres.app was added to the local discovery candidates; that override was reverted before commit to avoid regressing every macOS dev who uses Postgres.app — see HANDOVER tech-debt re: Postgres.app launchd-status-timing) + post-merge `1c7eced` (3 in-scope review-comment cleanups + 3 deferred follow-up issues filed: **[#120](https://github.com/hherb/kastellan/issues/120)** surface image-not-found earlier via per-tag `probe_image`, **[#121](https://github.com/hherb/kastellan/issues/121)** invariant test pinning that `IdleTimeoutLifecycle` warm-cache key omits image-tag, **[#122](https://github.com/hherb/kastellan/issues/122)** tighten `skip_if_image_missing` substring match against `dev-old`-style false positives). Separate doc-only PR **[#119](https://github.com/hherb/kastellan/pull/119)** also landed (`99bbfab`, `b6e78b2`) — 11-chapter `docs/devel/manual/` developer onboarding manual targeting contributors unfamiliar with Rust or OS-level security; each chapter capped at ≤ 2 A4 printed pages; index file provides three reading paths by experience level. **Closes [#107](https://github.com/hherb/kastellan/issues/107)** (PID-1 signal handling) inline by adding `--init` unconditionally to `build_container_argv`. Branch carries 18 commits: 2 docs (spec + plan) + 1 `--init` always-on (closes #107) + 1 `SandboxBackends::resolve` widening + 1 `ToolEntry.container_image` field + 1 lifecycle-manager rerouting + 1 `GlinerRelexEnv`/`resolve_env` env-var support + 1 `gliner_relex_entry` host/container split + 1 Containerfile + build-image.sh + 1 container `--init` smoke + 1 e2e through container + 8 review-fixup commits sprinkled throughout (Tasks 1, 2, 4, 5, 6, 7, 8, 10 each had a small followup addressing spec/quality reviewer findings — every Important issue closed inline, only minors deferred). Three follow-up TODOs surfaced during reviews and **deferred to HANDOVER tech-debt**: (1) `core/src/worker_lifecycle/manager.rs` 559 LOC over cap (pre-existing Slice 2 routing pin tests pushed it over; not load-bearing for Slice 2.5); (2) `core/src/workers/gliner_relex.rs` ~1494 LOC over cap (Item 26 already tracks); (3) `KASTELLAN_GLINER_RELEX_REQUIRE_E2E=1` CI env knob to turn the container e2e's skip-as-pass into a hard fail — useful for any future CI runner with PG + container + image + weights all staged. Operator built `kastellan/gliner-relex:dev` image on this Mac via the new `scripts/workers/gliner-relex/build-image.sh` (Python 3.12-slim + `uv pip install --system` + `USER nobody` + `ENTRYPOINT`); image present in local `container image list`. **macOS workspace 998 → 1011 (+13), all green**; Python workers/gliner-relex 36 passed unchanged (no Python changes shipped). **Empirical verification done at session-time** (post-PR-open follow-up on 2026-05-25): the new `happy_path_container_extract_returns_entities_and_triples` e2e was run against a session-local override (Postgres.app v18 at `/Applications/Postgres 2.app/Contents/Versions/18/bin/` added to the discovery candidates) and **PASSED in 12.58s** — `1 passed; 0 failed; 0 ignored`. Canonical extraction confirmed through Apple `container` on this Mac. The override itself was reverted before commit (would have regressed every macOS dev using Postgres.app — see HANDOVER tech-debt re: Postgres.app launchd-status-timing gap below). What DID ship: (a) macOS `unique_temp_root` now uses `/tmp` instead of `/var/folders/.../T/` (the latter's 49-char prefix leaves no headroom for the 103-byte `sockaddr_un.sun_path` macOS cap once you add `<label>-<pid>-<nanos>/data/sockets/.s.PGSQL.5432`); (b) test label `"happy-container"` → `"hc"` (matches sibling `"happy"`/`"warm"` convention; defensive against the same path-length cap). The container e2e remains a permanent regression pin — on hosts where PG is discoverable, it runs end-to-end automatically; on hosts where PG is not discoverable, it skip-as-passes via `pg_bin_dir_or_skip`.

(prior, earlier in this multi-day session) Next-TODO Item 23 option (a) — test-module sibling lifts **merged to `main` via PR [#117](https://github.com/hherb/kastellan/pull/117) at `919882d`**. Branch carried 4 commits: 2 pure-refactor (`db/src/graph.rs` 761 → 673 LOC + new `graph/tests.rs` 96 LOC at `72cd3e7`; `core/src/bin/kastellan-cli/relations_show.rs` 576 → 380 LOC + new `relations_show/tests.rs` 203 LOC at `40d9be2`, now well under cap, closing the 76-LOC inline-tests overage flagged at the end of Item 22) + 2 docs (HANDOVER updates at `718afaa` and `0a9c352` — the latter addressed PR review nits by adding the explicit 23(a)/(b) split in the Next-TODO list and a clean-build pin call-out). Both code commits use Rust 2018 sibling-directory module resolution (`mod tests;` in parent resolves to `<parent>/tests.rs`); `use super::*` continues to import from the parent module so test names + behaviour are byte-equivalent. macOS workspace **998 / 0 / 3 unchanged**; existing tests are the regression pin (verified from a clean `cargo clean && cargo test --workspace` to rule out the sibling-directory stale-target-dir footgun). Option (b) — walk-impl split into `graph/walk.rs` — deliberately deferred per the design note ("preferred if a second `WalkedEdge` consumer materialises… until then, (a) is the cheaper one"). The `graph.rs` 673 LOC residual stays over cap; called out for follow-up. Earlier this session: docs sync on `main` (`9a2ea91`) backfilling PR #116 (Item 22) merge.

(prior, earlier this session) Next-TODO Item 22 — kinds-CLI shared lift + over-cap CLI file splits **merged to `main` via PR [#116](https://github.com/hherb/kastellan/pull/116) at `1abb061`** — closes [#111](https://github.com/hherb/kastellan/issues/111) (consolidated kinds-CLI tech-debt: list pool / fixed-width truncation / description size cap) and [#112](https://github.com/hherb/kastellan/issues/112) (split `entities.rs` 723 LOC). Six logical commits after the post-PR-#113 doc sync: **Phase A1** splits `entities.rs` 723 → 488 LOC + new `entities_kinds.rs` 293 LOC (`crate::entities_kinds::run` arm in `run_entities`); **Phase A2** splits `relations.rs` 851 → 47 LOC + new `relations_kinds.rs` 291 LOC + new `relations_show.rs` 576 LOC (slight 76-LOC inline-tests overage; production code ~400 LOC under cap); **Phase B1** swaps `kinds list` from `connect_admin_pool` → `connect_runtime_pool` in both consumers (runtime role has SELECT per migrations 0015 + 0017; `add`/`remove` continue admin-pool because 0016/0017 REVOKE writes); **Phase B2** introduces shared `common::format_kinds_table(&[KindRow<'_>])` helper with dynamic column widths anchored to `max(header_len, longest_value_len)`, replacing the `{:<24}` truncation footgun; **Phase B3** adds new `MAX_{ENTITY,RELATION}_KIND_DESCRIPTION_LEN: usize = 2048` const + `validate_*_kind_description` pure helper + `{Entity,Relation}KindError::DescriptionTooLong { len: usize }` variant, all wired into `add()` so oversize descriptions reject with typed error + CLI exit 2 (carries the offending byte length in stderr). Workspace **971 → 990 (+19)** on Linux DGX, all green: +5 `common::format_kinds_table_tests`, +6 each in `entity_kinds`/`relation_kinds` tests (1 const pin + 5 validate_description boundary cases), +2 `postgres_e2e` integration twins pinning the new runtime-pool list path. Earlier this session:

(prior) Next-TODO Item 21 — `kastellan-cli relations show <entity-id> [--depth N] [--format plain|json]` shipped on branch `feat/relations-show-cli`, **merged to `main` via PR [#113](https://github.com/hherb/kastellan/pull/113) at `9a46e18`**, with a same-session post-review fixup addressing 4 of 6 reviewer concerns inline + 2 deferred as follow-up issues [#114](https://github.com/hherb/kastellan/issues/114) (diamond-graph duplicate edges) and [#115](https://github.com/hherb/kastellan/issues/115) (single UNION ALL query). New operator-facing graph-edge introspection: walks outbound + inbound edges from the seed entity up to `--depth N` hops (default 1, hard-capped at new `MAX_WALK_DEPTH = 5`), prints one row per edge in canonical `(src_kind, "src_name") --[edge_kind]--> (dst_kind, "dst_name")` shape regardless of which walk direction surfaced it, tags quarantined endpoints with `[Q]`. Read-only path: uses `connect_runtime_pool` (runtime role has SELECT on `entities` + `relations`), emits no audit row. Two new `Graph` trait methods (`walk_outbound_edges` / `walk_inbound_edges`) carry the recursive-CTE machinery with visited-set cycle protection; new `WalkedEdge` struct bundles depth + both endpoints' kind/name/quarantine + edge kind so the caller renders without secondary lookups. Workspace **939 → 971 (+32)** on Linux DGX, all green (+30 from Item 21 + 2 from the post-review fixup's escape-quote unit tests). Earlier in session: HANDOVER header cleanup confirming PR #109 + #110 merged (was stale: said "PR pending"). Previous session (2026-05-22): Item 20 (`kastellan-cli entities kinds {add, remove, list}` shipped on branch `feat/entities-kinds-cli` (stacked on `feat/relations-kinds-cli`); **both PRs merged to `main`: #109 (`relations kinds`) at `f234d0c` and #110 (`entities kinds`) at `a65bb4a`**. Mechanical mirror of Item 16: new `db::entity_kinds::{validate_entity_kind, add, remove, list_all}` + `ENTITY_KIND_UNDEFINED` sentinel + `EntityKindError` (parallel to `relation_kinds` shape); new `ACTION_ENTITY_KINDS_{ADD,REMOVE}` audit constants + `entity_kinds_{add,remove}_and_audit` helpers in `core::cli_audit`; existing `entities.rs` CLI extended with a new `kinds` action that delegates to a `run_entities_kinds` substree (rather than a separate top-level namespace — preserves operator UX "everything about entities is under `entities`"). All vocab-management writes ride on `connect_admin_pool` introduced by Item 16 (migration 0016 REVOKEs writes on `entity_kinds` from the runtime role symmetric to 0017's `relation_kinds`). Workspace **921 → 939 (+18)** on Linux DGX, all green; cumulative session delta from `main` at `1b86f84`: **901 → 939 (+38)**. Earlier this session: Item 16 (`relations kinds` CLI, branch `feat/relations-kinds-cli`, merged via PR #109, +20) and Slice 2 (per-worker sandbox-backend selection, merged via PR #108 + post-merge fixup `2b271e8`). New `SandboxBackendKind` enum + `SandboxBackends` resolver in `kastellan_sandbox` (both cfg-gated per-OS so cross-OS mis-config is a compile-time error). New `sandbox_backend: Option<SandboxBackendKind>` field on `ToolEntry` defaulting to `None` everywhere (byte-equivalent to slice 1; only `gliner-relex` will flip to `Some(Container)` in Slice 2.5 to opt into macOS memory enforcement). All three lifecycle managers (`SingleUseLifecycle`, `IdleTimeoutLifecycle`, `CompositeLifecycle`) switch from holding a single `Arc<dyn SandboxBackend>` to holding an `Arc<SandboxBackends>` bundle; `acquire` calls `self.sandboxes.resolve(entry.sandbox_backend)` per call. Daemon `main.rs` swaps the now-dead local cfg-gated `sandbox_backend()` helper for `SandboxBackends::default_for_current_os()`. New `core/tests/lifecycle_container_routing_e2e.rs` integration smoke spawns alpine's `/sbin/apk` through the production `SingleUseLifecycle::acquire` path, proving end-to-end routing reaches the container backend (skip-as-pass when `container` / `alpine:3.20` missing). Post-merge fixup `2b271e8` addressed two PR-#108 review items: (1) symmetric `idle_timeout_lifecycle_acquire_routes_via_entry_sandbox_backend_kind` counter-backend pin (the resolve call is the same as `SingleUseLifecycle` but it was not independently asserted; a future refactor dropping the resolve from one manager but not the other would have slipped through); (2) `SandboxBackendKind` doc-comment expansion explaining why `Serialize + Deserialize` derives exist (forward-looking for operator-config plumbing) and pinning `Container` as deliberately bound to Apple `container` (a future Linux micro-VM backend would add its own variant rather than overloading). Slice 1 (PR #106 at `cc0b0de`) shipped earlier on 2026-05-21.

**Last commit on `main`:** `bc36e4c` (PR [#146](https://github.com/hherb/kastellan/pull/146) merge — Item 31 opaque secret references slice 1 + post-review polish `5885e26` + `9348e9c`). **Currently on `main`; no working branch — next item TBD (operator picks, see "Next TODO" below).** Substantive merges in order: `bc36e4c` (PR [#146](https://github.com/hherb/kastellan/pull/146), Item 31 — opaque secret references slice 1; the 4 TDD commits `b58cd55` / `3bc66a3` / `7b1e3c5` / `19eebd6` plus post-review polish), `62905ae` (PR [#141](https://github.com/hherb/kastellan/pull/141), Item 30 — injection guard slice 1), `504094e` (PR [#139](https://github.com/hherb/kastellan/pull/139), Issue #89 — `/tmp` per-spawn ephemeral tmpfs invariant test), `6fd82bf` (PR [#140](https://github.com/hherb/kastellan/pull/140), docs-only — 7 openhuman patterns imported), `5fc5fee` (PR [#138](https://github.com/hherb/kastellan/pull/138), refactor — `idle_timeout/release.rs` sibling-lift), `fed0f21` (PR [#137](https://github.com/hherb/kastellan/pull/137), Issue #136 — debounce pending-acquires queue-depth warn), `7f98ee4` (PR [#135](https://github.com/hherb/kastellan/pull/135), worker_lifecycle hardening trio — closes #84 + #85 + #86), `8655319` (PR [#133](https://github.com/hherb/kastellan/pull/133), Issue #131 — `bring_up_pg_cluster_with_timeout` sibling), `162f71f` (PR [#132](https://github.com/hherb/kastellan/pull/132), test-module lift quadruple), `4e94e42` (PR [#129](https://github.com/hherb/kastellan/pull/129), `bring_up_pg_cluster` polling-cap lift + 5-site consolidation), `bb32cab` (PR [#128](https://github.com/hherb/kastellan/pull/128), Issues #114 + #115 — `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL), `7adc582` (PR [#126](https://github.com/hherb/kastellan/pull/126), Item 29 — `KASTELLAN_PG_BIN_DIR` env-override), `dac0dcd` (PR [#125](https://github.com/hherb/kastellan/pull/125), Item 28 — Issue #95 entity-upsert Layer B).
...
**Branch `feat/worker-lifecycle-hardening-trio`:** merged into `main` via PR #135 at `7f98ee4` on 2026-05-27 + in-PR polish `1fc7eca`; closes issues [#84](https://github.com/hherb/kastellan/issues/84) + [#85](https://github.com/hherb/kastellan/issues/85) + [#86](https://github.com/hherb/kastellan/issues/86); follow-up [#136](https://github.com/hherb/kastellan/issues/136) filed for warn-debounce (now shipped on `fix/issue-136-pending-acquires-warn-debounce`, see below). Safe to delete locally (`git branch -d feat/worker-lifecycle-hardening-trio`).

**Branch `fix/issue-136-pending-acquires-warn-debounce`:** merged into `main` via PR #137 at `fed0f21` on 2026-05-27; closes [#136](https://github.com/hherb/kastellan/issues/136). Safe to delete locally (`git branch -d fix/issue-136-pending-acquires-warn-debounce`).

**Branch `refactor/idle-timeout-release-sibling-lift`:** merged into `main` via PR #138 at `5fc5fee` on 2026-05-28 + in-PR polish `78305cf`; closes the Item 29 operator-picks sub-bullet for the `idle_timeout/release.rs` sibling-lift (the natural follow-up flagged at end of Issue #136). Safe to delete locally (`git branch -d refactor/idle-timeout-release-sibling-lift`).

**Branch `fix/issue-89-tmpfs-per-spawn-invariant-test`:** merged into `main` via PR #139 at `504094e` on 2026-05-28; closes Issue [#89](https://github.com/hherb/kastellan/issues/89). Pure test addition; no production-code changes. Safe to delete locally (`git branch -d fix/issue-89-tmpfs-per-spawn-invariant-test`).

**Branch `feat/injection-guard-slice-1`:** merged into `main` via PR [#141](https://github.com/hherb/kastellan/pull/141) at `62905ae` on 2026-05-28 + post-claim fixes `45627fd` + `e81b079`; closes HANDOVER Item 30. Safe to delete locally (`git branch -d feat/injection-guard-slice-1`).

**Branch `feat/opaque-secret-refs-slice-1`:** merged into `main` via PR [#146](https://github.com/hherb/kastellan/pull/146) at `bc36e4c` on 2026-05-28 + post-review polish `5885e26` + `9348e9c`; closes HANDOVER Item 31 (opaque secret references slice 1). 4 substantive TDD-ordered commits (`b58cd55` Task 1 module skeleton, `3bc66a3` Task 2 Vault impl, `7b1e3c5` Task 3 walker + FakeVault, `19eebd6` Task 4 dispatch wiring + 8 integration tests + classifier test) + 2 polish commits. Workspace 1096 → 1137 (+41) on macOS without PG. Safe to delete locally (`git branch -d feat/opaque-secret-refs-slice-1`).

**Branch `claude/youthful-volta-I5asz`:** merged into `main` via PR #140 at `6fd82bf` on 2026-05-28; docs-only — imported 7 openhuman design patterns into ROADMAP Phase 1/2/3/5 and flagged 2 (worker-output prompt-injection guard, opaque secret references) as immediate-pickup items 30 + 31 in HANDOVER's Item 29 bucket. Safe to delete locally if not already gone.
...
**TDD.** Widened test 7 (`policy_rows_contain_no_substring_of_redeemed_plaintext`) in [`core/tests/secret_vault_e2e.rs`](../../../core/tests/secret_vault_e2e.rs) per the issue's acceptance: after the policy-row loop it now fetches the `tool:shell-exec` row and asserts `payload.req` (serialized) contains no plaintext marker. Added focused test 9 `tool_row_req_shows_opaque_ref_not_plaintext` — negative (no plaintext) **and** positive (the opaque `secret://<ref>` IS present in `req`, proving snapshot-before happened and stdout still carries the plaintext so the worker did receive it). Both confirmed RED on pre-fix code (the tool row `req` carried `SECRET_LEAK_MARKER_xyz789` / `SECRET_REQ_MARKER_qrs456`), GREEN after the fix.

**Verification (macOS M3 Max).** `cargo test --workspace` without PG: **1145 / 0 / 3** (1144 → 1145, +1 from new test 9; test 7's widening adds assertions, no new `#[test]`). `secret_vault_e2e` live against Postgres.app v18 (`KASTELLAN_PG_BIN_DIR` override): **9 / 9 passed**. `cargo clippy -p kastellan-core --all-targets`: **10 warnings, all pre-existing** (no new warnings; the 2 `tool_host.rs` doc-list warnings at 581/609 predate this change and are away from the edited regions). Updated the `dispatch` docstring's privacy note (was "tool row's `payload.req` carries plaintext after substitution… scoped to policy rows only" → now documents the pre-substitution snapshot + the result-vs-request scoping).

**Drive-by filed, not fixed (Issue [#150](https://github.com/hherb/kastellan/issues/150)).** `cargo clippy --workspace --all-targets` now hard-**errors** on `sandbox/tests/fixtures/mem_burner.rs:46` (`set_len()` immediately after `with_capacity` — `clippy::uninit_vec` was promoted to deny-by-default in the current toolchain). It was a *warning* in prior toolchains (HANDOVER documented it as pre-existing). Untouched by this change (different crate), and `cargo build`/`cargo test` are unaffected (the `SAFETY` is sound) — only the `--all-targets` clippy gate is blocked. Filed as #150 with a one-line fix (`vec![0u8; bytes]`); kept out of this PR to keep the diff focused on `core/`.

**File-size watch.** `core/src/tool_host.rs`: net **+0 production LOC** (a clone moved, a comment swapped for a comment). Still 867 LOC, 367 over the 500-LOC cap — the Items 30+31 sibling-lift bundle remains the tracked follow-up. `core/tests/secret_vault_e2e.rs`: 537 → ~620 LOC (test file; +1 test + widened test 7).

---

## Recently completed (this session, 2026-05-29 — ★ Opaque secret references slice 1, branch `feat/opaque-secret-refs-slice-1`, **merged to `main` via PR [#146](https://github.com/hherb/kastellan/pull/146) at `bc36e4c`** + post-review polish `5885e26` + `9348e9c` — closes HANDOVER Item 31)
...
1. **`b58cd55` — Task 1: module skeleton + types + const pins.** New `core/src/secrets/mod.rs` with `SecretRef` opaque newtype (wraps `String`, printable as the ref string itself, never the plaintext), `RedeemResult` enum (`Hit(String)` / `Expired` / `NotFound`), `SECRET_REF_PREFIX = "secret://"`, `SECRET_REF_HEX_LEN = 8`, `DEFAULT_TTL: Duration = Duration::from_secs(3600)`. Edit to `core/src/lib.rs` adds `pub mod secrets`. +7 unit tests (prefix const pin, hex-len const pin, TTL const pin, `SecretRef` round-trip, Display/Debug divergence, prefix-match helpers).

2. **`3bc66a3` — Task 2: Vault impl + materialize/redeem.** New `core/src/secrets/vault.rs` with `Vault` (TTL'd `std::sync::RwLock<HashMap<SecretRef, Entry>>`, lazy GC on read, `OsKeyringProvider` trait for the secret backend). `materialize(name, provider, audit_pool, actor)` mints a new `SecretRef` (8-hex random), writes the `policy / secret.materialized` audit row hard-fail (no materialized-but-unaudited ref can exist), stores the plaintext with a `Instant`-based expiry. `redeem(&SecretRef)` returns `RedeemResult` with lazy GC of expired entries. `drop_after(Duration)` replaces TTL on an existing ref for testing. +9 unit tests (TTL boundary, lifecycle, lazy GC, Drop smoke, concurrent readers, expired returns Expired, missing returns NotFound, materialize-audit-fail propagates).

3. **`7b1e3c5` — Task 3: substitution walker + FakeVault.** New `core/src/secrets/substitute.rs` with `substitute_refs_in_params(params: &mut Value, vault: &dyn VaultLike) -> Result<Vec<SecretRef>, SecretRedemptionError>` — recursive `serde_json::Value` walker; on `Value::String`, exact-match check for `SECRET_REF_PREFIX`; on hit, calls `vault.redeem()`; on `Hit`, replace in place; on `Expired` or `NotFound`, return `Err(SecretRedemptionError::Missing(ref_))` (fail-closed). Non-string nodes recurse without substitution. `FakeVault` test seam in the same file — simple `HashMap<SecretRef, Option<String>>` (`None` = expired, `Some(v)` = plaintext). +16 unit tests (14 walker scenarios: null/bool/integer pass-through, string no-ref pass-through, string exact-hit, expired-ref, missing-ref, nested-object hit, nested-array hit, multiple-refs-in-array, object-key bypass, partial-walk unspecified-state; 2 review-driven: object-key bypass pin, partial-walk acknowledged non-determinism).

4. **`19eebd6` — Task 4: dispatch wiring + scheduler plumbing + main.rs bootstrap + 8 integration tests + classifier test fix.** Wires `substitute_refs_in_params` into `tool_host::dispatch` before `worker.call`: on `Ok(refs)`, write one `policy / secret.redeemed` audit row per ref (best-effort — redeem-time audit is best-effort because plaintext is already in process memory; killing the tool call at that point would discard the substituted value with no benefit). On `Err(SecretRedemptionFailed)`, write `policy / secret.redemption_failed` row fail-closed, return `Err(ToolHostError::SecretRedemptionFailed)` — **worker not called, no tool row written**. `ToolHostError` widened to `#[non_exhaustive]` (first variant addition since Option M); `scheduler::idle_timeout::classify` maps the new variant to `POLICY_DENIED`. `main.rs` reads `KASTELLAN_BOOTSTRAP_SECRETS` env var (comma-separated names) and materializes each via `OsKeyringProvider` at daemon startup. New `core/tests/secret_vault_e2e.rs` with 8 PG-required integration tests pinning the end-to-end audit shape. +1 classifier unit test (TaskStopReason maps `SecretRedemptionFailed` to `POLICY_DENIED`).

**Verification on macOS (M3 Max):**

- `cargo test --workspace` with `KASTELLAN_PG_BIN_DIR` set: **636 passed / 2 failed** (the 2 failures are pre-existing `embedding_recall_e2e` + `gliner_relex_e2e` PG initdb/pg_notify race flakes, confirmed identical on `main`; not regressions). Baseline on `main` at `c505b36`: 603 passed / 2 pre-existing flakes.
- `cargo test --workspace` without `KASTELLAN_PG_BIN_DIR` (skip-as-pass posture): **1137 passed / 0 failed / 3 ignored**. Baseline 1096 / 0 / 3 on `main` at `62905ae`; delta = +41 (7 unit + 9 vault + 16 walker + 1 classifier + 8 integration).
- `cargo clippy --workspace --all-targets`: **no new warnings** after fixing 4 `clippy::explicit_auto_deref` instances in `core/tests/secret_vault_e2e.rs` (`&*vault` → `&vault`). Pre-existing set unchanged: 10 `kastellan-core` (constant-assertions, `manual_char_comparison`, `err_expect`, 4 `MutexGuard`-across-await), 3 `db::probe` doc-list-indent, 2 `kastellan-protocol` io_other_error, 1 `mem_burner` set_len-after-reserve.
- 8 integration tests in `core/tests/secret_vault_e2e.rs` verified live against Postgres.app v18.

**Architectural notes worth keeping:**

- **Substitution BEFORE injection guard.** The two dispatch guards are ordered: secret substitution runs on INPUT (resolves refs in `params` before `worker.call`), injection guard runs on OUTPUT (screens worker result after `worker.call`). Neither guard knows about the other; they compose at the chokepoint.
- **Asymmetric audit posture.** Materialize-time audit is hard-fail (no materialized-but-unaudited ref can exist — if the audit row fails to write, `materialize` returns `Err` and the ref is never stored). Redeem-time audit is best-effort (plaintext already in process memory; failing the audit at that point would discard a successfully-substituted value with no security benefit). Documented inline in `tool_host.rs`.
- **Tool row `payload.req` is allowed to carry the substituted plaintext.** The privacy promise is that the opaque ref is what the planner sees and what the LLM transcript records — not that the plaintext never reaches a sandboxed worker. The worker is the legitimate consumer. The `policy / secret.redeemed` row carries only `{ref_hash, tool, actor}`, never the value. Precedent from injection guard (spec §4.2 scoped to policy row).
- **Exact-match substitution only.** The walker replaces a JSON string value that IS `"secret://…"` entirely; it does NOT do embedded substitution (e.g. `"Bearer secret://…"` stays as-is). Deferred to Slice 2; exact-match covers every current use case.
- **`ToolHostError::SecretRedemptionFailed` is `#[non_exhaustive]`-safe.** The enum was widened in this slice; all existing match arms continue to compile; the new variant is mapped in `classify` and documented in `dispatch`'s error section.

**File-size watch:**

- `core/src/secrets/mod.rs`: new, ~130 LOC (types + re-exports). Under cap.
- `core/src/secrets/vault.rs`: new, ~260 LOC (Vault + OsKeyringProvider + unit tests inline). Under cap.
- `core/src/secrets/substitute.rs`: new, ~290 LOC (walker + FakeVault + unit tests inline). Under cap.
- `core/src/tool_host.rs`: 767 → 867 LOC (+100). Pre-existing tech-debt extended; now 367 over the 500-LOC cap. Defer sibling-lift; bundle with Item 30 (injection guard) in a future single refactor slice.
- `core/tests/secret_vault_e2e.rs`: new, ~550 LOC (8 integration tests). Test file; no production code.

**Open follow-ups for future slices:**

- **No CLI surface yet (Slice 2).** Operators can only stage secrets via `KASTELLAN_BOOTSTRAP_SECRETS`. Slice 2 builds `kastellan-cli secrets materialize` + CLI ↔ daemon IPC.
- **Per-task scoping (Slice 2).** Today the `Vault` is per-process (singleton in `main.rs`). Slice 2 should scope TTL and ref lifecycle to the task that called `materialize`, not the daemon.
- **Embedded substitution (Slice 2).** `"Bearer secret://…"` is not substituted today; only whole-string exact-match. Deferred per YAGNI until an actual use case materialises.
...
- **Issue [#147](https://github.com/hherb/kastellan/issues/147) — `tool:` audit row carries substituted plaintext.** The chokepoint snapshots `req_for_audit` AFTER substitution, so the tool row's `payload.req` contains the redeemed plaintext. Slice 2 should substitute plaintext back to `secret://<8-hex>` in the audit snapshot using the `redemption_events` vector we already collect.
- **Issue [#148](https://github.com/hherb/kastellan/issues/148) — audit-insert failure paths are untested.** Both best-effort paths (`secret.redeemed` loop, `secret.redemption_failed` early-return) log-and-swallow on audit insert failure. Forcing the failure requires an `AuditSink` trait seam in `kastellan_db::audit`; deferred.
- **Issue [#149](https://github.com/hherb/kastellan/issues/149) — `Vault::materialize` ref-collision branch untested.** The forward-compat polish replaced `HashMap::insert` with `Entry::Vacant`-or-`RefCollision`; the `Occupied` arm needs RNG injection (or a `_test_materialize_at_ref` seam) to exercise.
...
## Recently completed (this session, 2026-05-28 — ★ Worker-output prompt-injection guard slice 1, **merged to `main` via PR [#141](https://github.com/hherb/kastellan/pull/141) at `62905ae`** — closes HANDOVER Item 30)
...
This slice closes the loop end-to-end with the smallest meaningful surface.

**Why now:** flagged in the recent operator-driven import of openhuman design patterns (PR #140) as a real-not-theoretical attack surface that the production `cli_ask_e2e` chain exercises every test run. The dispatcher chokepoint is already a one-file invariant (Option M, issue #16), so the wiring is small. Picked over Item 31 (opaque secret references) because Item 30 closes a TODAY risk while Item 31 is a pre-req for tools that don't exist yet (`web-fetch` etc.).
...
- 22-entry English-substring catalogue across 4 attack classes (instruction_override, role_hijack, secret_exfiltration, unsafe_tool_coercion). Substring matching post-`normalize` (lowercase + zero-width strip — ZWSP/ZWNJ/ZWJ/BOM). No regex, no leetspeak fold, no multilingual coverage.
- Per-rule weights summed (cap 1.0); `BLOCK_THRESHOLD = 0.70`. Canonical phrases at 0.75 weight block on a single hit; two medium-confidence 0.40 phrases (same or different class) also block.
- Catalogue invariant pinned: every class has at least one entry with weight ≥ `BLOCK_THRESHOLD` (catches accidental class-dropouts in future edits).

**What shipped (4 substantive commits TDD-ordered + 3 docs commits, all on branch):**

1. **`fbc46dc` — `docs(handover,roadmap): sync to main = 504094e (PR #139 + #140 merged)`.** Session-start doc-sync. PR #139 (Issue #89) and PR #140 (openhuman docs import) both merged between the previous session's claim-of-work commit `643bbe8` and this session's start.
2. **`7f301e2` — `docs(spec): worker-output prompt-injection guard slice 1 design`.** Full design spec at `docs/superpowers/specs/2026-05-28-worker-output-prompt-injection-guard-design.md`. Brainstormed scope, public surface, threshold model, catalogue layout, privacy invariants.
3. **`465e03d` — `docs(plan): worker-output prompt-injection guard slice 1 plan`.** 5-task TDD-ordered implementation plan at `docs/superpowers/plans/2026-05-28-worker-output-prompt-injection-guard-slice-1.md`.
4. **`536d23a` — Task 1: module skeleton + types + const pins.** New `core/src/cassandra/injection_guard.rs` with `InjectionVerdict`, `InjectionDecision` (`#[non_exhaustive]`), `BLOCK_THRESHOLD = 0.70`, `SCAN_BYTE_CAP = 64 * 1024`, `screen` and `extract_scannable_text` stubs (panicking `unimplemented!()`), 2 const-pin tests. Edit to `core/src/cassandra/mod.rs` adds the new `pub mod injection_guard;` and re-exports all 6 public items to keep with the established single-surface convention (`extract_scannable_text`, `screen`, `InjectionDecision`, `InjectionVerdict`, `BLOCK_THRESHOLD`, `SCAN_BYTE_CAP`). +2 unit tests.
5. **`11a0a8e` — Task 2: `extract_scannable_text` helper.** Recursive walk of `serde_json::Value` concatenating `Value::String` content with `'\n'` separators; non-string nodes skipped (so the catalogue scan cannot fire on JSON shape itself); UTF-8-aware truncation at `byte_cap` (the `char_indices().take_while(...)` pattern finds the largest valid byte prefix); short-circuit return once cap is hit. Empty-string guard prevents spurious `'\n'` separators. +7 unit tests including UTF-8 multi-byte boundary and empty-string regression pins.
6. **`eae2a6f` — Task 3: `normalize` + 22-entry catalogue + `screen`.** Pure-function substring scan. `normalize` does lowercase + zero-width strip in one pass. `screen` iterates `CATALOGUE`, sums weights (cap 1.0), collects deduped class names via `BTreeSet<&'static str>`. Catalogue weights tuned per class so the "every class has a block-capable phrase" invariant holds (chat-template tokens `<|im_start|>` and `<|system|>` at 0.75; `rm -rf /` with literal slash at 0.75; one intermediate-tier 0.50 weight on `"leak the api key"` annotated inline with rationale). +13 unit tests (11 screen + 2 normalize).
7. **`4e32988` — Task 4: wire into `tool_host::dispatch` + 6 integration tests.** New `core/tests/injection_guard_e2e.rs` (PG-required, skip-as-pass when PG/sandbox absent). After `worker.call` returns Ok, `extract_scannable_text` + `screen` run; on Block, the worker result is replaced with a placeholder JSON (`{"injection_blocked": true, "score": ..., "reason_codes": [...]}`) and a SECOND audit row is written (`actor='policy'`, `action='injection.blocked'`) carrying SHA-256 (computed via `sha2::Sha256` over the scanned body bytes) + byte length + truncation flag + score + class codes. Errors are NOT screened (they don't reach the planner as text). The 6 integration tests cover: placeholder shape, exactly-one policy row, **privacy invariant** (a unique marker substring must NOT appear in the policy row's serialized payload — scoped to `WHERE actor='policy' AND action='injection.blocked'`; the tool row's `req` field is allowed to carry the input argv), SHA shape (64 lowercase hex chars), benign passthrough, error-path bypass. Updated `dispatch` docstring's "Audit-log shape" section from "one row per call" to "one or two rows per call" to reflect the new pattern. +6 integration tests.

**Verification on macOS (M3 Max):**

- `cargo test --workspace`: **1096 / 0 / 3** (+31 over the 1065 baseline, re-verified post-merge on `main` at `62905ae`). Breakdown: +2 const + +7 extract_scannable_text + +13 screen/normalize + +6 integration + +3 post-claim normalize-strip tests (from `e81b079`: `normalize_strips_word_joiner_and_mongolian_vowel_separator`, `normalize_strips_soft_hyphen`, `screen_blocks_word_joiner_obfuscated_phrase`). The 6 integration tests skip-as-pass on this Mac (Postgres not configured by default; see `postgres-app-bin-paths.md` for the session-local override workflow).
- `cargo clippy --workspace --all-targets`: **no new warnings**. The 10 pre-existing `kastellan-core` warnings (constant-assertions in `db::memories`/`memory::recall`, `manual_char_comparison` in `observation::capture`, `err_expect` in `scheduler::plan_parser`, 4 `MutexGuard`-across-await in `worker_lifecycle::manager::_test_slot_*` test inspectors), 3 doc-list-indent in `db::probe`, 2 `io_other_error` in `kastellan-protocol`, 1 `mem_burner` `set_len()`-after-`reserve` — all unchanged.

**Verification deferred to operator (PG-required integration suite):**

The 6 integration tests in `core/tests/injection_guard_e2e.rs` require a live Postgres + sandbox + supervisor + shell-exec worker binary. On this Mac without `KASTELLAN_PG_BIN_DIR` set they skip-as-pass cleanly via the standard `pg_bin_dir_or_skip` pattern. To force-run: set `KASTELLAN_PG_BIN_DIR=/Applications/Postgres\ 2.app/Contents/Versions/18/bin/` (per `postgres-app-bin-paths.md`) and run `cargo test --test injection_guard_e2e`; all 6 should pass. Linux DGX is expected to run the integration suite by default once PG is staged.

**Architectural notes worth keeping:**

- **The screen runs at the chokepoint, not at the call site.** Option M's sealed `WorkerCommand` constructor (issue #16) means every worker dispatch goes through `tool_host::dispatch`. Adding the screen there is one wiring point with no bypass path — exactly the invariant we want for a security-relevant filter.
- **The Err path is not screened.** Errors from `worker.call` are not text-channel content (the scheduler short-circuits on `Err` and writes `step.spawn_failed` / `step.unknown_tool` audit rows); they cannot carry injection. Skipping the screen on Err preserves the existing error-handling shape and avoids spurious "policy blocked" rows on legitimate worker failures.
- **Block returns `Ok(placeholder)`, not `Err`.** The planner sees a tool *result*, not a tool *failure*, so it can decide whether to retry with a different prompt, abandon, or finalise. Returning an error would burn the scheduler's retry budget on a deterministically blocked input.
- **Two audit rows on Block, tool row first.** The `tool:<name>` row carries the placeholder as `result` so an operator's existing tool-tail flow surfaces every dispatch event regardless. The `policy / injection.blocked` row sits next to it (same timestamp) carrying the forensic data. Operator correlates by timestamp, tool, method, or SHA-256.
- **Privacy invariant is end-to-end enforced (policy row scope).** The spec's privacy promise (§4.2) is scoped to the policy row: the raw scanned body never appears in the `policy / injection.blocked` row's payload — pinned at run-time by `policy_audit_row_contains_no_substring_of_blocked_body` which uses a unique `"AUDIT_LEAK_MARKER_xyz123"` substring and asserts it does NOT appear in the serialized payload of any `injection.blocked` audit row. The tool row's `payload.req` field is allowed to carry the input argv (in this test, `printf` echoes its argv as stdout, so the marker appears in both req and output — scanning the tool row would be a false positive the spec doesn't forbid). A positive-presence assertion (`rows.is_empty()` now fails) catches a future regression where the screen stops firing entirely.
- **`#[non_exhaustive]` on `InjectionDecision` is the forward-compat seam.** Adding the Review tier (HANDOVER's 0.45–0.70 band) later is a one-variant addition. Out-of-crate callers (none today, but `core::scheduler` and the future operator CLI are likely consumers) will have to update their `match` arms — which is the desired behaviour: each consumer must explicitly decide how to handle Review.

**File-size watch:**

- `core/src/cassandra/injection_guard.rs`: new, ~430 LOC (~70 production + ~360 tests). Under the 500-LOC cap.
- `core/src/cassandra/mod.rs`: +7 LOC (new module declaration + widened re-export block).
- `core/src/tool_host.rs`: 708 → 763 LOC (+55). Was over the 500-LOC cap before this slice (208 over); now 263 over. The over-cap status was already documented as continued tech-debt; a sibling-lift refactor is the natural follow-up but deliberately out of scope for this slice.
- `core/tests/injection_guard_e2e.rs`: new, ~362 LOC (test file, no production code).

**Open follow-ups for future slices (not addressed here):**

- **Review tier** (HANDOVER's 0.45–0.70 band): adds a new `InjectionDecision::Review` variant + a new operator-facing surface (audit-row review queue or CLI). Defer until the catalogue's false-positive profile is observable in production.
- **Heuristic / combinatorial scoring** beyond simple per-rule weight sums.
- **Multilingual coverage**: catalogue is English-only.
- **Per-tool policy**: today every tool's output is screened identically. Future slices may want to relax for known-safe-shape workers (`gliner-relex` returns only entity tuples) or tighten for known-risky workers.
- **`kastellan-cli policy review` operator surface** for blocked rows (the audit row is the only artifact today).
- **`tool_host.rs` sibling-lift**: 263 LOC over the 500-LOC cap; production-code surgery candidate, separate refactor slice.
- **Issue [#142](https://github.com/hherb/kastellan/issues/142)** (filed 2026-05-28 from `e81b079`) — chat-template tokens (`<|im_start|>`, `<|system|>`) may false-positive on technical docs once `web-fetch` or MCP workers land that legitimately surface model-template content. Catalogue tightening + per-tool exemption hook are the candidate shapes.
- **Issue [#143](https://github.com/hherb/kastellan/issues/143)** (filed 2026-05-28 from `e81b079`) — `extract_scannable_text` recursive `walk()` has no explicit depth guard. Reachable only if upstream protocol or `serde_json` recursion limits are removed; currently defence-in-depth via `serde_json`'s ~128-level default. Add an explicit `MAX_WALK_DEPTH` const if the upstream guard is ever loosened.

---

## Recently completed (this session, 2026-05-28 — test pin: Issue [#89](https://github.com/hherb/kastellan/issues/89) — `/tmp` per-spawn ephemeral tmpfs invariant, branch `fix/issue-89-tmpfs-per-spawn-invariant-test`, **merged to `main` via PR [#139](https://github.com/hherb/kastellan/pull/139) at `504094e`**)

Issue #89 was the smallest available item in the operator-picks bucket and the natural follow-up to the GLiNER-Relex slice's reliance on `/tmp` being a per-spawn tmpfs. The bwrap argv at [`linux_bwrap.rs:177`](../../../sandbox/src/linux_bwrap.rs#L177) sets `--tmpfs /tmp` unconditionally for every `LinuxBwrap::spawn_under_policy` call; until this slice that property was asserted only by an inline comment in `core/src/workers/gliner_relex.rs` ("/tmp is tmpfs inside the sandbox so this is ephemeral per-spawn; no leakage to the host") and by inspection of `build_argv`. No positive runtime pin existed.

**Why now:** smallest item in the operator-picks bucket; minimal context-switch from the worker-lifecycle work last session; the `TORCHINDUCTOR_CACHE_DIR=/tmp/torchinductor` fixed-path call site in the GLiNER-Relex worker (PR #88) is the most concrete near-term beneficiary — if `/tmp` ever became shared across spawns, two concurrent GLiNER-Relex workers would race on the same torchinductor cache path. This slice ships the regression pin so a future "let's expose host caches for performance" refactor can't quietly break the isolation.

**What shipped (1 commit on branch, plus 1 doc-sync to main):**

1. **`ad74916` — `docs(handover,roadmap): sync to main = 5fc5fee (PR #138 / release.rs sibling-lift merged)`.** Session-start doc-sync. The previous session's claim-of-work commit (`c2e01ef`) had flagged PR #138 as pending; the PR landed at `5fc5fee` with one self-review polish commit (`78305cf` — banner-rot cleanup in `idle_timeout.rs` + visibility narrowing `pub(crate)` → `pub(super)` on `replace_idle_teardown_handle`). HANDOVER header + branch-status + Item 29 bucket entry + ROADMAP entry all updated to reflect merged + safe-to-delete. "Last commit on main" pointer flipped `fed0f21` → `5fc5fee`.

2. **`784ab51` — `test(sandbox): pin '/tmp is per-spawn ephemeral tmpfs' invariant (closes #89)`.** Single new test function `tmp_is_per_spawn_ephemeral_tmpfs_under_worker_strict` appended to `sandbox/tests/linux_smoke.rs` (file goes 222 → 311 LOC). No production-code changes. Test design:
    - **Spawn A** under `Profile::WorkerStrict` (default `SandboxPolicy`) runs `/bin/sh -c "touch /tmp/probe-spawn-a-<PID>-<nanos>"` — exit 0 asserted.
    - **Spawn B** under the same policy runs `/bin/sh -c "touch /tmp/probe-spawn-b-<PID>-<nanos> && ls /tmp"` — three assertions on the result:
        - `status_b.success()` → writability under WorkerStrict (no EROFS regression).
        - `!stdout_b.contains(spawn_a_marker)` → ephemerality / per-spawn isolation.
        - `stdout_b.contains(spawn_b_marker)` → listing is real, not silently empty (would otherwise let a future bug where `/tmp` rejects reads masquerade as "ephemerality holds").
    - `/bin/sh` is used instead of `/usr/bin/sh` so the test works on hosts without usr-merge: bwrap's `--symlink usr/bin /bin` (already in `build_argv`) resolves the call.
    - Markers carry PID + UNIX-nanos so parallel CI lanes sharing the same host `/tmp` could not falsely succeed if isolation ever broke (this is the worst-case-defence; on a working invariant the host's `/tmp` never sees these names at all).
    - `skip_if_no_userns()` guard preserved — same `[SKIP]` discipline as the rest of `linux_smoke.rs`; hosts without bwrap user-ns silently skip with `[SKIP]` rather than fail opaquely.

**Verification done on macOS (M3 Max):**

- Workspace `cargo test --workspace`: **1065 / 0 / 3** unchanged from PR #138 baseline. The new test is `#![cfg(target_os = "linux")]` at the file level (every test in `linux_smoke.rs` shares the gate) so on macOS it doesn't enter the test binary at all — workspace count is correctly unaffected.
- Cross-target syntax check: `cargo check --tests -p kastellan-sandbox --target aarch64-unknown-linux-gnu` clean (after `rustup target add aarch64-unknown-linux-gnu`). Confirms the test code compiles against the Linux target — no `cfg` ladder errors, no missing imports, no lifetime issues with the format-temp argv slice.

**Verification deferred to DGX (operator-driven, two steps):**

1. **Pass-on-current-code (regression pin works):** `cargo test -p kastellan-sandbox --test linux_smoke tmp_is_per_spawn_ephemeral` against unmodified `linux_bwrap.rs` should PASS in ~1 s. Workspace test count Linux DGX: 1065 → 1066 (+1) expected.

2. **Catches-drift (the pin actually catches the failure mode):** temporarily replace `argv.extend(["--tmpfs".into(), "/tmp".into()]);` in `linux_bwrap::build_argv` (line 177) with `argv.extend(["--bind-try".into(), "/tmp".into(), "/tmp".into()]);` — this is the most plausible future-refactor failure mode. Re-run the test; it should FAIL with stderr `"/tmp leaked across spawns! spawn A wrote ..."`. Restore the original line and re-run to confirm green again. **This is not committed — purely an operator-side empirical proof that the pin works.**

**Architectural notes worth keeping:**

- **The test is at the sandbox layer, not the worker layer.** The bwrap argv is the canonical invariant; testing per-worker (e.g. GLiNER-Relex spawning twice + asserting torchinductor cache isolation) would conflate "is `/tmp` tmpfs?" with "does GLiNER-Relex use `/tmp` correctly?". Putting the test in `sandbox/tests/linux_smoke.rs` keeps the regression pin co-located with the contract.
- **Why `/bin/sh -c '...'` over a custom fixture binary.** The `mem_burner` precedent suggested a fixture-binary approach. But for this invariant the test needs only `touch` + `ls`, both already in `/usr/bin/` (bound by the existing `--ro-bind /usr /usr` line). A custom fixture would have added build complexity for no protection gain. `/bin/sh` resolves through the existing symlink mapping; no host-specific path assumption needed.
- **Three-assertion design over two-spawn-pair.** Could have shipped just the ephemerality assertion (spawn A writes, spawn B doesn't see). Added the writability + listing-is-real sanity checks because a future bug where `/tmp` becomes EROFS (or where `ls` silently fails) would also pass the ephemerality assertion vacuously. The three assertions together pin the invariant from three different failure modes; any single one tripping is a meaningful signal.
- **Macos counterpart deliberately deferred.** The Seatbelt backend doesn't bind `/tmp` to a tmpfs — it whitelists paths via a `.sb` profile and lets writes pass through to the host's `/tmp` (which is per-user but not per-spawn). The `macos_container` backend (Apple `container` micro-VM) would naturally have per-spawn `/tmp` isolation because each container starts with a fresh root filesystem, but that backend isn't the macOS default (it's opt-in per-worker via `ToolEntry.sandbox_backend = Some(Container)`). The issue body called this out as "macOS Seatbelt when the backend lands" — meaning a macOS Seatbelt-side `/tmp` isolation primitive does not exist today, so a parallel test cannot be written without backend changes first. Out of scope.

**File-size watch:** `sandbox/tests/linux_smoke.rs` 222 → 311 LOC (under cap; integration-test file, no production code).

---

## Recently completed (prior, this session 2026-05-28 — refactor: `idle_timeout/release.rs` sibling-lift, branch `refactor/idle-timeout-release-sibling-lift`, **merged to `main` via PR [#138](https://github.com/hherb/kastellan/pull/138) at `5fc5fee`** + in-PR polish `78305cf`)

Picked up the natural-follow-up slice that the Issue #136 ship explicitly flagged. After PR #137's merge, the `core/src/worker_lifecycle/idle_timeout.rs` production file stood at 647 LOC — 147 over the 500-LOC soft cap. The flagged target was a fresh sibling submodule carrying the three release-path functions. This slice ships exactly that, pure mechanical refactor.

**Why now:** smallest, best-defined item available; head context was still warm from #136 (every release-path function was just touched for the success-path `slot: None` abort-symmetry polish `1fc7eca`); the lift is purely structural so reviewability is high. The PR #137 merge ended the previous bucket of work; this slice closes the bucket-residual before picking the next substantive feature.

**What shipped (1 commit on branch `refactor/idle-timeout-release-sibling-lift`, plus 1 doc-sync to main):**

1. **`<doc-sync-hash>` — `docs(handover,roadmap): sync to main = fed0f21 (PR #137 / issue #136 merged)`.** Session-start doc-sync. PR #137 had been merged at `fed0f21` between the previous session's `75514cb` claim-of-work commit and this session's start. HANDOVER's header + branch-status + Item 29 bucket entry + Open follow-ups entry all still said "PR pending"; ROADMAP's #136 entry said the same. Updates: HANDOVER header (flip to "merged via PR #137 at `fed0f21`"), "Last commit on main" pointer (`7f98ee4` → `fed0f21`), branch-status entry (current working → merged + safe-to-delete), section heading (this session → prior session), Item 29 bucket entry, Open follow-ups entry, "Last updated" date 2026-05-27 → 2026-05-28. ROADMAP: #136 entry flipped from PR-pending to merged.
2. **`<feature-hash>` — `refactor(worker_lifecycle/idle_timeout): lift release path into sibling submodule`.** Pure mechanical refactor; no behaviour change. Three functions moved verbatim into a new sibling submodule:
    - `release_idle_timeout_worker` — the public release entrypoint called from `manager::WorkerHandle::Drop` via `super::idle_timeout::release_idle_timeout_worker(...)`. Stays `pub(crate)` in the new file; re-exported from `idle_timeout.rs` via `pub(crate) use release::release_idle_timeout_worker;` so the call site in `manager.rs` resolves byte-equivalently with no edit there.
    - `abort_idle_teardown_handle` — only ever called from within the release path. Stays module-private (`fn`) inside `release.rs`.
    - `replace_idle_teardown_handle` — called from `release_idle_timeout_worker`'s success branch AND from two pin tests in `idle_timeout/tests.rs` (the issue #85 regression guarantees). Widens from `fn` to `pub(crate) fn` so the tests can import it explicitly; tests gain a 6-line import banner (`use super::release::replace_idle_teardown_handle;` + a 4-line doc comment naming where the helper now lives + why the explicit import).
    
   Visibility design choice: explicit `use super::release::replace_idle_teardown_handle;` in `tests.rs` over a `#[cfg(test)] pub(crate) use release::replace_idle_teardown_handle;` re-export in `idle_timeout.rs`. The explicit import is more honest: any future contributor reading `tests.rs` sees directly that the helper lives in the sibling submodule, not in the parent. The re-export would have kept `use super::*;` working with no new lines but would have buried the structural fact that the helper's home is `release.rs`. Pure refactor scope is preserved either way; the explicit import edge is documentation, not load-bearing.

**File-size deltas:**

- `core/src/worker_lifecycle/idle_timeout.rs`: **647 → 490 LOC** (-157; **now 10 LOC under the 500-LOC cap**; closes the 147-LOC overage flagged at end of Issue #136). Also drops 2 lines from the import block (`OwnedMutexGuard` + `IdleTimeoutCaps` — both moved to `release.rs` along with their callers; the post-lift unused-import warnings were cleaned up inline before the commit).
- `core/src/worker_lifecycle/idle_timeout/release.rs`: **new 195 LOC** (well under cap; 9-line module-doc-comment header + 3 functions with full doc comments preserved verbatim from `idle_timeout.rs`).
- `core/src/worker_lifecycle/idle_timeout/tests.rs`: **467 → 472 LOC** (+5; 4-line doc-comment banner explaining the new `use super::release::replace_idle_teardown_handle;` import + the import itself).

**Test count delta: +0**. Workspace `cargo test --workspace` on macOS: **1065 → 1065**, all green. Pure mechanical refactor; existing tests are the regression pin — every test name, body, and behaviour is byte-equivalent. The two load-bearing issue #85 pin tests (`replace_idle_teardown_handle_aborts_prior_and_stores_new` + `replace_idle_teardown_handle_steady_state_holds_at_most_one_alive_per_slot`) pass unchanged through the import-rewired path. The 6 `worker_lifecycle_idle_timeout_e2e` integration tests are the end-to-end regression pin for the release-path semantics. Linux DGX expected at +0 by the same argument (pure refactor; no platform-specific code touched).

**Clippy:** no new warnings. The 4 pre-existing `MutexGuard`-across-await warnings on `manager.rs::_test_slot_*` test inspectors are unchanged; pre-existing doc-list-indent in `db/src/probe.rs`, `io_other_error` in `kastellan-protocol`, `mem_burner` `set_len()`-after-`reserve` are all unchanged. The 2 transient unused-import warnings that appeared between the move and the import-block cleanup were fixed inline before claiming the work.

**File-size watch (full `worker_lifecycle/` directory post-slice):**

- `core/src/worker_lifecycle/idle_timeout.rs` — **490 LOC** (under cap ✓)
- `core/src/worker_lifecycle/idle_timeout/release.rs` — **195 LOC** (under cap ✓, new)
- `core/src/worker_lifecycle/idle_timeout/tests.rs` — **472 LOC** (under cap ✓)
- `core/src/worker_lifecycle/manager.rs` — **339 LOC** (under cap ✓)
- `core/src/worker_lifecycle/manager/tests.rs` — verify in next file-size sweep
- `core/src/worker_lifecycle/composite.rs` — **256 LOC** (under cap ✓)
- `core/src/worker_lifecycle/types.rs` — **246 LOC** (under cap ✓)
- `core/src/worker_lifecycle/mod.rs` — **36 LOC** (under cap ✓)

Every production-code file in the `worker_lifecycle/` directory now well under the 500-LOC soft cap. The next sibling-lift candidate elsewhere in the workspace is `db/src/lib.rs` (525 LOC, 25 over — production-code residual: error type, options structs, pg-discovery helpers, `require_absolute`; a further split would require production-code surgery, deliberately deferred per Item 29's prior sub-bullet).

**Architectural notes worth keeping:**

- **The acquire-vs-release seam is the right module boundary.** Looking at `idle_timeout.rs` (490 LOC remaining) the file holds: imports, `RestartBackoff` + helpers, pure predicates (`is_request_capped`, `is_aged_out`, `pending_acquires_should_warn`, `debounce_warn`), the structural types (`ToolSlot`, `ToolState`, `WarmWorker`, `PendingAcquireGuard`), registry construction (`empty_registry`, `slot_for`), and `acquire_impl` (the warm-cache lookup + lock-wait + queue-depth observability + crash classification). All of that is concerned with **entering** the lifecycle. `release.rs` holds **exiting** the lifecycle: cap evaluation, teardown task lifecycle, restart-backoff bookkeeping. The seam is semantic, not just a line-count split — future readers can pick one file to load based on which half of the lifecycle they're tracing.
- **Re-export pattern keeps the `manager::Drop` call site untouched.** A common refactor trap is moving an item and forcing every caller to update its `use` statement. The `pub(crate) use release::release_idle_timeout_worker;` re-export in `idle_timeout.rs` preserves the public surface — `super::idle_timeout::release_idle_timeout_worker(...)` continues to resolve to the same fully-qualified path. Future submodule reshuffles inside `idle_timeout/` can stay invisible to consumers as long as the re-export stays. Same pattern is used elsewhere in the workspace (e.g. `db/src/lib.rs` re-exporting from submodules).
- **Explicit import in `tests.rs` over conditional re-export.** Both work. The explicit import wins on documentation value: a contributor reading `tests.rs` cold sees `use super::release::replace_idle_teardown_handle;` and immediately knows where the helper lives. A `#[cfg(test)]` re-export would have kept `use super::*;` unchanged but hidden the structural fact. The 5 added LOC is a fair price for that clarity.

**Tech-debt deliberately not addressed (separate follow-up slices):**

- The Issue #84 BUSY-rejection half — still waiting on empirical evidence of queue starvation per the trio's design choice.
- The `kastellan-cli supervisor status` operator-facing accessor for `pending_acquires` + `last_warn_unix_nanos` — atomics are `pub(crate)` on `ToolSlot` and would be reached via a future `pub fn slot_*` wrapper; flagged in both the trio's polish and Issue #136's shipped notes.
- `db/src/lib.rs` 525 LOC residual (production-code surgery required, not a clean test-module or sibling lift).
- Pre-existing `MutexGuard`-across-await in `manager.rs::_test_slot_*` test inspectors — still flagged in HANDOVER tech-debt.

---

## Recently completed (prior session, 2026-05-27 — Issue [#136](https://github.com/hherb/kastellan/issues/136): debounce pending-acquires queue-depth warn, branch `fix/issue-136-pending-acquires-warn-debounce`, **merged to `main` via PR [#137](https://github.com/hherb/kastellan/pull/137) at `fed0f21`**)

Picked up Issue #136 as the natural close-out follow-up to the just-shipped trio (PR #135). Issue #136 was filed during PR #135's in-PR polish (`1fc7eca`) when it became clear that the new `tracing::warn!` at the `pending_acquires` threshold fires once per acquirer under sustained queue depth — log-spam at request rate under exactly the scenario the warn is designed to surface. Operators are likely to filter the line out, defeating its purpose. Cooldown design was deferred from the polish pass because the "right cooldown duration" question deserved a separate design micro-pass; the slice picks 30 s and ships.

**Why now:** smallest, best-defined item in HANDOVER's Item 29 operator-picks bucket immediately after PR #135 merged. The trio work was fresh in head context (warm-cache key invariants, `ToolSlot` shape, `PendingAcquireGuard` RAII pattern, the warn site itself), so the debounce slot in well — adding it later would have required re-loading the manager.rs / idle_timeout.rs cluster.

**Key design choices:**

1. **30 s cooldown, not 60 s or 10 s.** 30 s is the rule-of-thumb pick from the issue body. Long enough to dedupe steady-state spam (sustained backpressure produces ~2 warns/min/slot — visible without being noise). Short enough that an operator paging on the metric still sees the next warn within half a minute of the queue refilling after the original episode resolved. 60 s would be too sparse (full minute of silence on a recurring issue); 10 s too noisy (still ~6/min/slot at sustained depth). Pinned by `pending_acquires_warn_cooldown_is_thirty_seconds` so a silent bump trips the test.

2. **`AtomicI64` on `ToolSlot`, peer to `pending_acquires`.** Two-data-points "have I warned recently?" is the simplest possible state; no mutex needed, no allocation, cache-line-adjacent to the field that already gates whether we even check the cooldown. Per the issue body sketch.

3. **`SystemTime` (wall-clock unix nanos), not `Instant` (monotonic).** `Instant` has no portable epoch conversion; converting `Instant::now().elapsed()` against a process-start anchor adds startup overhead and breaks across process restarts (irrelevant for a per-slot atomic that lives only as long as the daemon). `SystemTime` is widely used elsewhere in the workspace (audit rows, `submitted_at`/`completed_at` timestamps); the clock-skew failure mode (NTP correction stepping the clock) is the only concern — handled by the `saturating_sub` in `debounce_warn` returning a non-positive delta on backward skew, which is always less than the positive cooldown so the predicate suppresses. Clock-skew-forward is harmless (warns fire slightly earlier than expected; still bounded).

4. **CAS to claim the warn slot, not just `store`.** Without the CAS, two concurrent acquirers that both see the gate open would both emit the warn (twice per cooldown window per slot is still a regression, just smaller). The CAS makes "at most one warn per cooldown window per slot" a structural invariant — only the winner of the `compare_exchange(last, now, AcqRel, Relaxed)` updates the timestamp + emits the warn. The pure `debounce_warn` predicate is still useful in isolation for testing the gate logic without driving the CAS race.

5. **`AcqRel` on CAS, `Relaxed` on the load.** Per the issue body sketch. The counter has no synchronizes-with relationship with other state — neither the warn emission nor any other observer requires a happens-before ordering. `Relaxed` on the load is enough for the gate check; `AcqRel` on the CAS keeps the CAS-success edge synchronized with any future observer that might Acquire-load this value (e.g. a future `kastellan-cli supervisor status` reader). One-knob simplicity.

6. **Pure predicate is the testable surface, not the production hot path.** The production warn site does syscalls (`SystemTime::now()`) and atomic operations (`load` + `compare_exchange`) that are awkward to unit-test in isolation. Extracting `debounce_warn(last_warn_nanos, now_nanos, cooldown) -> bool` as a free function gives a 5-case unit test (cold / within / boundary / past / skew-backward) that pins the gate logic exactly. The CAS race is structural (Rust's atomic ordering guarantees + tokio's executor); it's not worth driving concurrent test threads to observe — the unit pin + manual smoke is enough.

7. **Realistic timestamp in the "no prior warn" test.** First draft used `now=1` ns since epoch to test the `last=0` branch — and the test correctly failed because `1.saturating_sub(0)=1 < 30_000_000_000`. Production `SystemTime::now()` is decades past epoch (~`1.78e18` ns in 2026), vastly larger than any sensible cooldown, so `now >= cooldown` is trivially true on the cold path. Fixed the test to assert with realistic 2023-era unix nanos. Lesson: the gate doesn't short-circuit on `last=0`; it just observes that `now - 0 = now` is very large in production.

**What shipped (2 commits on branch `fix/issue-136-pending-acquires-warn-debounce`):**

1. **`8204a3c` — `docs(handover,roadmap): sync to main = 7f98ee4 (PR #135 trio merged + polish 1fc7eca)`.** Session-start doc-sync. PR #135 (worker_lifecycle hardening trio) had been merged at `7f98ee4` plus the in-PR polish `1fc7eca` (5-issue review pass + #136 filed) between the previous session's claim-of-work commit `51692a1` and this session's start. HANDOVER's header + ROADMAP's trio entry both still said "PR pending" — stale. Updates: HANDOVER header (flip to "merged + post-review polish"), "Last commit on main" pointer (`8655319` → `7f98ee4`), trio branch-status entry (current working → merged + safe-to-delete), trio section heading (PR pending → merged), Item 29 trio sub-bullet (in progress → ~~SHIPPED~~), new Open follow-up issue entry for #136. ROADMAP: trio entry flipped from PR-pending to merged + polish summary inline.

2. **`9fae2f9` — `feat(worker_lifecycle/idle_timeout): debounce pending-acquires queue-depth warn (closes #136)`.** `core/src/worker_lifecycle/idle_timeout.rs` 576 → ~647 LOC; `core/src/worker_lifecycle/idle_timeout/tests.rs` 337 → ~467 LOC. Production-code additions: (a) imports `AtomicI64, SystemTime, UNIX_EPOCH`; (b) new `last_warn_unix_nanos: AtomicI64` field on `ToolSlot` with explanatory doc cross-referencing `debounce_warn` and `PENDING_ACQUIRES_WARN_COOLDOWN`; (c) new `pub const PENDING_ACQUIRES_WARN_COOLDOWN: Duration = Duration::from_secs(30)` colocated with the threshold; (d) new pure predicate `pub(crate) fn debounce_warn(last_warn_nanos: i64, now_nanos: i64, cooldown: Duration) -> bool`; (e) `slot_for` initializer extended; (f) warn site in `acquire_impl` rewritten as outer depth gate → debounce predicate → CAS-to-claim → emit-on-win, with `cooldown_secs` field added to the structured warn payload. Two existing `ToolSlot` struct-literal sites in `idle_timeout/tests.rs` updated to include the new field. +7 tests in `idle_timeout::tests`: `pending_acquires_warn_cooldown_is_thirty_seconds` (const pin); `debounce_warn_fires_when_no_prior_warn` (cold path with realistic 2023-era unix nanos); `debounce_warn_suppresses_within_cooldown` (1 ns / 29 s / 30 s - 1 ns); `debounce_warn_fires_at_cooldown_boundary_inclusive`; `debounce_warn_fires_after_cooldown_elapsed` (31 s + hours); `debounce_warn_suppresses_on_clock_skew_backward` (extreme skew + 1 ns skew); `slot_for_initializes_last_warn_unix_nanos_to_zero`.

**Test count delta:** **+7**. Workspace `cargo test --workspace` on macOS: **1058 → 1065**, all green. Linux DGX expected at +7 by the same argument (every new test is platform-independent; `SystemTime`/`UNIX_EPOCH`/`AtomicI64` are stdlib). The 6 existing `worker_lifecycle_idle_timeout_e2e` integration tests pass unchanged — they're the regression pin for the load-bearing acquire+release semantics (the warn site is on the same hot path but the debounce only affects log volume, not behaviour).

**File-size watch (post-slice):**

- `core/src/worker_lifecycle/idle_timeout.rs` 576 → ~647 LOC (was 76 over cap, now 147 over — the production file grew 71 LOC for this slice's new field + const + helper + warn-site refactor)
- `core/src/worker_lifecycle/idle_timeout/tests.rs` 337 → ~467 LOC (still under cap)

**The natural sibling-lift target is now a fresh `core/src/worker_lifecycle/idle_timeout/release.rs`** carrying `release_idle_timeout_worker` (~85 LOC) + `abort_idle_teardown_handle` (~5 LOC) + `replace_idle_teardown_handle` (~35 LOC) — a clean ~125-LOC seam between "acquire path" (warm-cache lookup, lock wait, queue-depth observability) and "release path" (cap evaluation, teardown task lifecycle). The split would land both files at ~520 LOC each, both well within cap with breathing room for the next ~50 LOC of growth. Deliberately deferred to a separate refactor slice; this slice ships the surface, the lift is the obvious follow-up.

**Tech-debt deliberately not addressed (separate follow-up slices):**

- **Issue #84 BUSY-rejection half** — still waiting on empirical evidence of queue starvation per the trio's design choice. The observability path (warn + debounce + future supervisor-status accessor) is the cheap one; BUSY rejection is the more invasive remedy.
- **Operator-facing `kastellan-cli supervisor status` accessor** — the atomics on `ToolSlot` are `pub(crate)` and would be reached via a new `pub fn slot_last_warn_unix_nanos(&self, tool_name: &str) -> Option<i64>` (parallel to a future `pub fn slot_pending_acquires`) wrapping the same atomic load. Per the trio's polish #3, this is the right shape for production observability (not repurposing the `_test_slot_*` inspector).
- **`idle_timeout.rs` sibling-lift** — see "File-size watch" above. Deferred; flagged.
- Pre-existing `MutexGuard`-across-await in `manager.rs::_test_slot_*` test inspectors — still flagged in HANDOVER tech-debt.

**Architectural notes worth keeping:**

- **The threshold-and-cooldown pair (`PENDING_ACQUIRES_WARN_THRESHOLD = 5` + `PENDING_ACQUIRES_WARN_COOLDOWN = 30s`) is now the canonical shape for any operator-visible "noisy condition" warn in the workspace.** Any future per-slot or per-resource observability warn should follow the same shape: a depth/severity gate + a per-instance cooldown + the CAS-to-claim pattern. Each constant gets a `*_is_<value>` test that pins the value so a silent bump trips a re-think.
- **`saturating_sub` over `i64` is the right shape for clock-skew-aware debounce.** Negative deltas on backward skew → predicate suppresses (under-warn). Positive deltas during forward skew → predicate may fire earlier than expected (still bounded). The asymmetry is deliberate: under-warning during clock drift is strictly safer than over-warning.
- **CAS-to-claim, not just store.** Under concurrency, multiple acquirers that pass the time gate would all `store` the same value but each could still see the gate open before any of them wins. `compare_exchange` ensures only the first writer wins; subsequent CAS attempts see the now-fresh timestamp and the gate closes for them. Tokio's executor + atomic ordering guarantees + the Drop-runs-on-all-paths property of `PendingAcquireGuard` together make this a structural invariant rather than a probabilistic one.

---

## Recently completed (prior, this session, 2026-05-27 — worker_lifecycle hardening trio (issues [#84](https://github.com/hherb/kastellan/issues/84) + [#85](https://github.com/hherb/kastellan/issues/85) + [#86](https://github.com/hherb/kastellan/issues/86)), branch `feat/worker-lifecycle-hardening-trio`, **merged to `main` via PR [#135](https://github.com/hherb/kastellan/pull/135) at `7f98ee4`** + in-PR polish `1fc7eca`)
...
5. **`_test_slot_pending_acquires` is sync, not async.** The pending-acquires counter is a plain atomic on `ToolSlot`; reading it doesn't need the tokio mutex. Making the inspector sync matches what the future `kastellan-cli supervisor status` surface actually wants (`kastellan-cli supervisor status` is async overall but the read of pending depth is point-in-time). The other two `_test_slot_*` inspectors are async because they touch `ToolState`, which IS behind the tokio mutex; the divergence is structural, not a style choice.
...
## Recently completed (prior, this session 2026-05-27 — Issue [#131](https://github.com/hherb/kastellan/issues/131): `bring_up_pg_cluster_with_timeout` sibling for per-call timeout override, branch `feat/issue-131-bring-up-pg-cluster-timeout-override`, **merged to `main` via PR [#133](https://github.com/hherb/kastellan/pull/133) at `8655319`** + post-merge nit `876fb42`)
...
## Recently completed (prior, this session 2026-05-27 — test-module lift quadruple, branch `refactor/item-29-test-module-lift-quadruple`, **merged to `main` via PR [#132](https://github.com/hherb/kastellan/pull/132) at `162f71f`**)
...
## Recently completed (prior, this session 2026-05-26 → 2026-05-27 — `bring_up_pg_cluster` polling-cap lift + 5-site consolidation, branch `feat/bring-up-pg-cluster-polling-cap-lift`, **merged to `main` via PR [#129](https://github.com/hherb/kastellan/pull/129) at `4e94e42` on 2026-05-27** + same-PR post-review doc polish `f906d18`)
...
4. **Service-name templates preserved verbatim per-site.** `agent_prompts_e2e` keeps `kastellan-sched-test-pg-ap-{suffix}`, `scheduler_step_dispatch_e2e` keeps `kastellan-supervisor-test-pg-stepdisp-{suffix}`, etc. Any developer greping for an old service unit in their launchd/systemctl domain still finds the same name. Data + log labels (`ap-d`/`ap-l`, `step-d`/`step-l`, `lnd`/`lnl`, `ilp-d`/`ilp-l`, `crd`/`crl`) also preserved.
...
**Empirical verification at session-time (KASTELLAN_PG_BIN_DIR override active):** under the env-var override pointing at Postgres.app v18 (`/Applications/Postgres 2.app/Contents/Versions/18/bin/`), each of the 5 previously-failing test files now passes when run individually:
...
## Recently completed (prior, this session, 2026-05-26 — Issues [#114](https://github.com/hherb/kastellan/issues/114) + [#115](https://github.com/hherb/kastellan/issues/115): `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL, branch `fix/issues-114-115-walk-edges-dedupe-and-combine`, **merged to `main` via PR [#128](https://github.com/hherb/kastellan/pull/128) at `bb32cab`** + post-merge polish `978b5be` (named const `WALK_EDGES_AROUND_DIRECTION_COL` for the trailing partition column index + SQL comment on the outer `ORDER BY direction ASC` lex-order incidental + test header pin documenting that `per_direction_limit` shares semantics with each per-direction method's `limit`; no behaviour change))
...
3. **`c12774a` — `refactor(kastellan-cli/relations_show): use walk_edges_around for single round-trip`.** CLI replaces two separate `walk_outbound_edges` + `walk_inbound_edges` calls with one `walk_edges_around` call. Halves DB round-trip count for `relations show <entity-id>` (now 1 seed lookup + 1 walk = 2 trips, down from 3). Pure-functionally byte-equivalent: same input fixture produces the same rendered output. The existing `cli_relations_show_e2e` integration test is the regression pin — its plain-format substring assertions, NDJSON field assertions, and per-depth output checks all pass unchanged. `SHOW_PER_DIRECTION_LIMIT` doc-comment updated to reference the new method.
...
- `core/src/bin/kastellan-cli/relations_show.rs`: 380 → **376 LOC** (-4: two separate match blocks collapsed to one). Under cap.
...
## Recently completed (prior session, 2026-05-26 — Next-TODO Item 29: `KASTELLAN_PG_BIN_DIR` env-var override on test-fixture Postgres discovery, branch `feat/pg-bin-dir-env-override`, **merged to `main` via PR [#126](https://github.com/hherb/kastellan/pull/126) at `7adc582`** + post-merge refactor `463553d`)

Picked up Next-TODO Item 29 immediately after the session-start doc-sync (`c3dd3e4`) that brought HANDOVER + ROADMAP in line with the PR #125 / Item 28 merge state. Item 29's HANDOVER bucket is "operator picks" with seven candidates; operator selected the **`KASTELLAN_PG_BIN_DIR=<path>` env-var override** sub-bullet (originally tagged "test-infra, ~10 LOC + 2 unit tests; closes the Postgres.app session-local-override papercut that PR #118 + #125 both re-encountered").
...
1. **New library function rather than mutating `default_pg_bin_dir_candidates`.** `default_pg_bin_dir_candidates()` stays pure (returns defaults only — unit-testable without env state); the new `pg_bin_dir_candidates_with_env_override()` consults `KASTELLAN_PG_BIN_DIR` and prepends valid values to the defaults. Function name makes the env-source visible at the call site; production code that wants defaults-only continues to use `default_pg_bin_dir_candidates` (the `kastellan-db-init` binary stays on the pure function — production runtime discovery is documented as out of scope for the override).

2. **`PG_BIN_DIR_ENV: &str = "KASTELLAN_PG_BIN_DIR"` const.** Eliminates the literal-drift footgun between production read site and the 4 unit tests that mutate the var.

3. **Trim-then-check semantics, single-path-only.** Unset / empty string / whitespace-only → defaults only; non-blank value → prepended. Matches the strict-trim parse style used elsewhere in the workspace for boolean env knobs (`KASTELLAN_GLINER_RELEX_USE_CONTAINER=1`). Single-path: a `:`-separated multi-path would invite shell-PATH confusion; defer until a real need emerges.
...
2. **`557ca3b` — `feat(db): pg_bin_dir_candidates_with_env_override helper`.** New public surface in `kastellan_db`: `PG_BIN_DIR_ENV` const + `pg_bin_dir_candidates_with_env_override()` function + 4 unit tests under `crate::env_lock()` (`returns_defaults_when_unset` / `prepends_valid_env_path` / `treats_empty_string_as_unset` / `treats_whitespace_as_unset`). `default_pg_bin_dir_candidates` doc-comment updated to point at the override helper for the test-fixture path. Production runtime discovery (`kastellan-db-init`) unchanged. +158 LOC in `db/src/lib.rs` (753 → 911; pre-existing over-cap breach grows — sibling-`tests.rs` lift is a separate slice flagged in tech-debt).
...
**Follow-up issue filed during review:** [#127](https://github.com/hherb/kastellan/issues/127) — env-var save/restore RAII helper. The 4 new unit tests use a repeated save-mutate-restore pattern around `KASTELLAN_PG_BIN_DIR`; if any assertion panics between `set_var` and the manual restore, the var leaks. `env_lock()` poisoning gives some protection but a scope-guard (small RAII helper with `Drop`) is the cleaner pattern. Small test-only refactor; ~20 LOC + 4-test mechanical switch.
...
- Pre-existing `mem_burner` clippy warning + `kastellan-protocol` `io_other_error` warnings — unrelated to this slice.
...
- `KASTELLAN_PG_BIN_DIR=<path>` env-var override for `find_pg_bin_dir` — would close the Postgres.app session-local-override papercut that this slice re-encountered (and PR #118 / Slice 2.5 re-encountered before it). HANDOVER already tracks this as a "medium test-infra slice"; option (b) is ~10 LOC + 2 unit tests. Worth bundling with the next test-infra touch.
...
## Recently completed (prior session, 2026-05-25 — Next-TODO Item 27: Slice 2.5 follow-up triple (#120 + #121 + #122), branch `feat/item-27-slice-2.5-followups`, **merged to `main` via PR [#124](https://github.com/hherb/kastellan/pull/124) at `e93997e`** + same-PR post-review fixup `55bb8f1`)
...
## Recently completed (this session, 2026-05-25 — Next-TODO Item 26: `gliner_relex.rs` test-module sibling lift, branch `refactor/item-26-gliner-relex-test-module-lift`, **merged to `main` via PR [#123](https://github.com/hherb/kastellan/pull/123) at `920e0c9`**)
...
**Merge:** PR [#123](https://github.com/hherb/kastellan/pull/123) merged to `main` at `920e0c9` on 2026-05-25. Workspace re-verified post-merge: macOS **1011 / 0 / 3** (matches the pre-merge branch tip — pure refactor + doc trim, test count unchanged).
...
- Slice 2.5 follow-ups [#120](https://github.com/hherb/kastellan/issues/120) / [#121](https://github.com/hherb/kastellan/issues/121) / [#122](https://github.com/hherb/kastellan/issues/122) — promoted to HANDOVER Next-TODO Item 27 (bundle as one small PR; ~0.5 session).
...
## Recently completed (this session, 2026-05-25 — Next-TODO Item 25: GLiNER-Relex Slice 2.5 — Containerfile + macOS image build, branch `feat/gliner-relex-slice-2.5`, merged via PR [#118](https://github.com/hherb/kastellan/pull/118) at `a9e3385` — closes [#107](https://github.com/hherb/kastellan/issues/107))

Picked up Next-TODO Item 25 immediately after Item 23 (a) merged to `main` via PR #117. Item 25 is the **first real workload routed through Apple `container` micro-VM on macOS** — `gliner-relex` worker now opts into `MacosContainer` (via `KASTELLAN_GLINER_RELEX_USE_CONTAINER=1`), closing the macOS memory-enforcement gap (Seatbelt has no memory primitive; `mem_mb=4096` is finally enforced on darwin). The slice also closes Issue #107 (PID-1 signal handling) inline by adding `--init` unconditionally to `build_container_argv`. Implementation followed a brainstorm → spec → plan → 10-task TDD-ordered execution under subagent-driven-development.
...
2. **Branch inside `gliner_relex_entry` by `GlinerRelexEnv.use_container_backend`** rather than two parallel entry constructors. Single source of truth; shared helpers (`build_runtime_env` for env vars; `build_idle_timeout_lifecycle` for the lifecycle caps) eliminate duplication between host-mode and container-mode entries. `host_mode_entry` is byte-equivalent to the pre-Slice-2.5 shape; `container_mode_entry` emits `binary = /usr/local/bin/kastellan-worker-gliner-relex` (in-container shim from `[project.scripts]`), `fs_read = [weights_dir]` only (venv + src baked into image), `sandbox_backend = Some(Container)`, `container_image = Some(<tag>)`.

3. **Mount weights at the SAME host path inside the container.** `build_container_argv` uses `source=<P>,target=<P>` convention, so the existing `KASTELLAN_GLINER_RELEX_WEIGHTS_DIR` env value (host path like `/Users/hherb/.local/share/kastellan/workers/gliner-relex/weights/multi-v1.0/`) works verbatim inside the container without a path-rewrite. Slightly weird operator UX but zero policy/argv-builder change.
...
6. **Strict-only-"1" trim parsing** on `KASTELLAN_GLINER_RELEX_USE_CONTAINER`, symmetric with existing `KASTELLAN_GLINER_RELEX_ENABLE` convention. Inviting `true` / `yes` / `on` would surface the operator-dialect debate; the README documents `=1` explicitly. Pin: dedicated `resolve_env_strict_about_use_container_value` test loops over `["true", "yes", "on", "0", " 1 \n"]` asserting only `value.trim() == "1"` counts.

7. **Containerfile shape: `python:3.12-slim` + `uv pip install --system .`** (no `.venv` indirection; console-script lands at `/usr/local/bin/kastellan-worker-gliner-relex` via pyproject's `[project.scripts]`). Debian-slim because PyTorch wheels are glibc-only (manylinux2014); Alpine is OUT. `USER nobody` as defense-in-depth complement to the policy-driven `--user nobody` flag. Weights NOT baked in (operator mounts at runtime via `policy.fs_read` host path; image stays ~3 GB instead of ~4.5 GB; weight refreshes don't require rebuild).

8. **Operator-runnable build script, not `cargo` integration.** New `scripts/workers/gliner-relex/build-image.sh` mirrors the existing `install.sh` shell-hygiene shape: `set -euo pipefail`, probes both `container --version` AND `container system status` before building (clear diagnosis instead of a cryptic `container build` failure), final message reminds operator to set both `KASTELLAN_GLINER_RELEX_ENABLE=1` and `KASTELLAN_GLINER_RELEX_USE_CONTAINER=1`. `KASTELLAN_GLINER_RELEX_IMAGE` env overrides the default tag.
...
5. **Task 5 — `GlinerRelexEnv` + `resolve_env` widening** (`44983e6` + doc fix `5a7579b`). New `use_container_backend: bool` + `container_image: Option<String>` fields on `GlinerRelexEnv`; `resolve_env` reads `KASTELLAN_GLINER_RELEX_USE_CONTAINER` (strict-only-"1" after trim) and `KASTELLAN_GLINER_RELEX_IMAGE`; container mode skips the host-venv existence check. +4 unit tests. Implementer caught a real plan bug in `resolve_env_strict_about_use_container_value` (non-"1" values fall through to venv-resolution which needed a HOME anchor; fixed inline). Doc fix restored two inline comments dropped by the wholesale-replace + added the two new env vars to the function-level doc-comment.

6. **Task 6 — `gliner_relex_entry` host/container branching** (`6f5839d` + idiom fix `27f95be`). Top-level `gliner_relex_entry` dispatches on `env.use_container_backend` to `host_mode_entry` (byte-equivalent to pre-Slice-2.5) or `container_mode_entry` (in-container `binary` + weights-only `fs_read` + `Some(Container)` + `Some(<image>)`). Extracted shared helpers `build_runtime_env(env)` + `build_idle_timeout_lifecycle()`. Two new constants pinned at module top: `CONTAINER_IMAGE_DEFAULT = "kastellan/gliner-relex:dev"` + `CONTAINER_BINARY = "/usr/local/bin/kastellan-worker-gliner-relex"`. +3 unit tests. Followup switched the image-tag fallback to idiomatic `.as_deref().unwrap_or(CONST).to_string()` (was `clone().unwrap_or_else(...)`); restored the deleted Path::parent() rationale comment in `host_mode_entry`.
...
9. **Task 9 — Manual operator image build (OPERATOR ACTION)**. Built `kastellan/gliner-relex:dev` via `scripts/workers/gliner-relex/build-image.sh` on this Mac (M3 Max, macOS 26.5+, `container` 0.12.3). Image present in `container image list`; `container run --rm <image> which kastellan-worker-gliner-relex` confirms the entrypoint binary is in place (the "error" output is the worker correctly running with no `KASTELLAN_GLINER_RELEX_WEIGHTS_DIR` env set — proving ENTRYPOINT is wired correctly).
...
- **No CI integration**, no `cargo`-driven image build, no `KASTELLAN_GLINER_RELEX_REQUIRE_E2E=1` env knob to fail-loud on container-e2e skips. Operator-action follow-ups for the future.
...
- **No image-tag namespacing for multiple model versions.** Single tag `kastellan/gliner-relex:dev` for this slice; future multi-model support would mean separate tags + per-tag manifests.
- **No operator CLI for runtime backend swap.** Daemon restart required after env-var flip — same posture as `KASTELLAN_GLINER_RELEX_ENABLE`. The warm-cache key in `IdleTimeoutLifecycle` doesn't include the image tag, so a hypothetical runtime swap would not invalidate stale warm workers; the warm-cache-key comment added in Task 4's followup makes this explicit for future maintainers.
...
- `core/src/bin/kastellan-cli/relations_show.rs`: 576 → **380 LOC** (under cap ✓)
- `core/src/bin/kastellan-cli/relations_show/tests.rs`: new, **203 LOC** (under cap ✓)
...
## Recently completed (this session, 2026-05-23 — Next-TODO Item 22: kinds-CLI shared lift + over-cap CLI splits, **merged to `main` via PR [#116](https://github.com/hherb/kastellan/pull/116) at `1abb061`** — closes [#111](https://github.com/hherb/kastellan/issues/111) + [#112](https://github.com/hherb/kastellan/issues/112))
...
* split `entities.rs` (723 LOC, [#112](https://github.com/hherb/kastellan/issues/112)) into `entities.rs` + `entities_kinds.rs`,
...
* address [#111](https://github.com/hherb/kastellan/issues/111)'s three kinds-CLI tech-debt items in lockstep across both consumers.
...
- `core/src/bin/kastellan-cli/entities.rs`: 723 → **488 LOC** (under cap ✓)
- `core/src/bin/kastellan-cli/entities_kinds.rs`: new, **293 LOC** (under cap ✓)
- `core/src/bin/kastellan-cli/relations.rs`: 851 → **47 LOC** (under cap ✓)
- `core/src/bin/kastellan-cli/relations_kinds.rs`: new, **291 LOC** (under cap ✓)
- `core/src/bin/kastellan-cli/relations_show.rs`: new, **576 LOC** (76 LOC overage from inline test module; production code ~400 LOC well under cap; flagged for sibling-`tests.rs` lift)
- `core/src/bin/kastellan-cli/common.rs`: 149 → 343 LOC (under cap)
...
1. **`SHOW_PER_DIRECTION_LIMIT` doc tightened** ([`relations.rs`](../../../core/src/bin/kastellan-cli/relations.rs)). The previous doc-comment ("the cap prevents a runaway query on a pathologically dense subgraph") overstated what `LIMIT` does. The recursive CTE is enumerated to completion *before* `ORDER BY (depth ASC, edge_id ASC) LIMIT N` clips the output, so the constant bounds the row count rendered, not the row count Postgres traverses. The actual walk-cost bound is `MAX_WALK_DEPTH`. Doc now spells out the distinction.
...
3. **NDJSON `"type": "header" | "edge"` discriminant** ([`relations.rs`](../../../core/src/bin/kastellan-cli/relations.rs)). Previous JSON output required consumers to special-case "first line is the header." Adding a `type` field lets `jq -c 'select(.type == "edge")'` filter cleanly without line-position assumptions. Backward-compatible addition (all existing fields preserved); e2e test extended to pin the new field on both header + edge lines.

4. **`endpoint_str` escapes embedded `"` and `\` in entity names** ([`relations.rs`](../../../core/src/bin/kastellan-cli/relations.rs)). `entities.name` is `TEXT NOT NULL` with no character-set CHECK, so a name like `Dr "Bob" Smith` is legal. Plain output previously emitted it as `(person, "Dr "Bob" Smith")` — breaks naive regex parsers of the plain stream. Two-pass escape (`\\` first, then `\"`) gives `(person, "Dr \"Bob\" Smith")` unambiguously. JSON path was already correct via `serde_json::json!`. +2 unit tests pinning the new behaviour.

**Deferred as follow-up issues:**

5. **[#114](https://github.com/hherb/kastellan/issues/114)** — path-based recursive CTE emits duplicate `edge_id` rows on diamond topologies (one row per traversal path, not per unique edge). Doc-comment said "one [`WalkedEdge`] per traversed edge" which readers will interpret as one per unique edge. Resolution requires a semantic decision (outer `SELECT DISTINCT ON (edge_id)` vs documenting the path-based view explicitly) plus a new diamond-fixture regression test. Not in scope for the review fixup.

6. **[#115](https://github.com/hherb/kastellan/issues/115)** — `relations show` makes 2 separate DB round-trips (`walk_outbound_edges` + `walk_inbound_edges`) that could be collapsed into a single `UNION ALL` query. Perf optimization only; deferred until #114's semantics settle (re-sorting/deduping a UNION ALL has different SQL ergonomics than re-sorting two independent CTE results).

**Test count after fixup: 969 → 971 (+2)** on Linux DGX, all green.

## Recently completed (this session, 2026-05-23 — Next-TODO Item 21: `kastellan-cli relations show <entity-id> [--depth N] [--format plain|json]`, branch `feat/relations-show-cli`, **merged to `main` via PR [#113](https://github.com/hherb/kastellan/pull/113) at `9a46e18`**)
...
4. **Docs** — `core/src/bin/kastellan-cli/main.rs` (207 → 215 LOC, +8): doc-comment subcommands list extended; usage block + inline help-string updated with `relations show <entity-id> [--depth N] [--format plain|json]`; module-map updated with `relations` entry. HANDOVER + ROADMAP updated (this commit).
...
- `core/src/bin/kastellan-cli/relations.rs`: 293 → ~803 LOC (+510). **Now over cap**; symmetric to issue #112's `entities.rs` split. Should be bundled with the #111 / #112 lift since `relations_show.rs` would slot in cleanly next to `relations_kinds.rs` and `entities_kinds.rs` (issue #112's proposal).
...
- `core/src/bin/kastellan-cli/main.rs`: 207 → 215 LOC (+8). Under cap.
...
- `db/src/graph.rs` + `core/src/bin/kastellan-cli/relations.rs` both now over cap. Best addressed alongside the kinds-CLI shared lift (#111) and the `entities.rs` split (#112): all three files want a similar "lift handlers + parsers into per-action sibling files" refactor. Flagged below in Next-TODO.
...
## Recently completed (previous session, 2026-05-22 — Next-TODO Item 20: `kastellan-cli entities kinds {add,remove,list}`, **merged to `main` via PR #110 at `a65bb4a`** — stacked on PR #109)
...
3. **CLI layer** — `core/src/bin/kastellan-cli/entities.rs` (+245 LOC, 478 → 723): added `"kinds"` arm to `run_entities` dispatch; new `run_entities_kinds` per-action sub-dispatcher; new `entities_kinds_{add,remove,list}` async handlers (parallel shape to `relations.rs`); new pure `parse_kinds_add_args` parser. +5 unit tests on `parse_kinds_add_args` in the existing `entities_parser_tests` module. `core/src/bin/kastellan-cli/main.rs` (+10 LOC): doc-comment + module-map + `// kastellan-cli entities kinds ...` usage block + inline help-string updates. `core/tests/cli_dispatcher_bad_args_e2e.rs` (+34 LOC): +1 `cli_entities_kinds_unknown_action_exits_two` test mirroring the relations-kinds bad-args pin. NEW `core/tests/cli_entities_kinds_e2e.rs` (315 LOC, under cap): subprocess-level round-trip mirror of `cli_relations_kinds_e2e.rs` — add (no desc) → idempotent re-add → add `--description` → list shows headers + kinds + descriptions + seeds → remove undefined rejected with "fallback" stderr → remove happy path → idempotent re-remove → oversize-kind validation → audit multiset pin (2 add + 1 remove) → payload spot-check.
...
- `core/src/bin/kastellan-cli/entities.rs`: 478 → 723 LOC (+245). **Now over cap**; deliberate, see "deliberately does NOT do" above.
- `core/src/bin/kastellan-cli/main.rs`: 194 → 204 LOC (+10). Under cap.
...
**Architectural note worth keeping:** the relations-kinds slice flagged that `connect_admin_pool` should never be wired into the daemon. This slice's grep audit confirms compliance: `connect_admin_pool` appears only in `core/src/bin/kastellan-cli/{entities.rs, relations.rs}` and the doc-comment elsewhere. Daemon bring-up + every other runtime code path stays on `connect_runtime_pool`.

**Follow-up tracking issues filed against the PR-#110 review (2026-05-22):**

- **[#111](https://github.com/hherb/kastellan/issues/111)** — consolidated kinds-CLI tech-debt (admin-pool-for-list / fixed-width truncation / no description size cap). Three items deliberately not fixed in #109 or #110 because doing so unilaterally would break the byte-symmetric mirror contract. Natural shared-lift refactor: a `KindsCli<T>` generic that hosts validate / add / remove / list_all / audit-action-string / `parse_kinds_add_args` for both consumers.
- **[#112](https://github.com/hherb/kastellan/issues/112)** — split `entities.rs` (723 LOC, over the 500-LOC soft cap) into `entities.rs` + `entities_kinds.rs`. Worth bundling with #111 so the shared lift lands cleanly.
...
- **No `kastellan-cli entities kinds` parallel for `entity_kinds`.** Same architectural posture (REVOKE on runtime; admin pool needed) but the operator backlog has not surfaced an entity-vocab management need yet. The new `connect_admin_pool` makes a future symmetric `entities kinds {add,remove,list}` slice nearly mechanical. Flagged as low-priority follow-up if vocabulary management becomes frequent.
...
- New `core/src/bin/kastellan-cli/relations.rs`: 293 LOC. Under cap.
- `core/src/bin/kastellan-cli/main.rs`: 178 → 194 LOC (+16). Under cap.
...
**Architectural pin worth noting:** the new `connect_admin_pool` is the only place in the runtime tree that holds a non-runtime-role pool. A grep for `connect_admin_pool` in `core/` should return zero hits outside `kastellan-cli/relations.rs` and the doc-comment elsewhere; the daemon's bring-up path stays runtime-role-only. A future contributor who copy-pastes the admin pool into a daemon code path would re-open the privilege escalation that 0017's REVOKE closes; the doc-comment on `connect_admin_pool` calls that out explicitly. Worth a code-review eye if a second non-CLI consumer appears.
...
1. **`5776fb8`** (`feat(sandbox): add SandboxBackendKind enum (cfg-gated per-OS variants)`) — new operator-facing enum in `kastellan_sandbox`. Variants are cfg-gated so `Container` on Linux or `Bwrap` on darwin is a compile-time error rather than a runtime surprise. `Copy + Eq + Serialize + Deserialize` for cheap threading through async dispatch. +1 test (`sandbox_backend_kind_is_copy_and_eq`, cross-platform).
...
- **No operator CLI for runtime backend swapping.** Backends are declared in `ToolEntry` literals (`shell_exec_entry`, `gliner_relex_entry`), not configured by `kastellan-cli` at runtime. Future slice if it becomes a real operator need.
...
6. **Deferred to [Issue #107](https://github.com/hherb/kastellan/issues/107):** the `--init` / PID-1 signal-handling posture concern. Not a regression vs the existing backends (bwrap explicitly opts into PID-1 via `--as-pid-1`; Seatbelt doesn't use a VM) but worth verifying end-to-end for the first long-lived worker that opts into the container backend (Slice 2 or 2.5).
...
1. **`workers/gliner-relex/src/kastellan_worker_gliner_relex/__main__.py::_resolve_device`** rewritten as a per-platform decision tree:
...
2. **`workers/gliner-relex/src/kastellan_worker_gliner_relex/model.py`** docstring updated. Previously claimed `mps` was "reserved for the macOS follow-up"; now lists the per-platform legal device strings + points at `__main__._resolve_device` for the canonical rules. No code change to `model.py` — `instance.to(device)` is generic and handles `mps` natively.
...
```sh
WEIGHTS_DIR="$HOME/.local/share/kastellan/workers/gliner-relex/weights/multi-v1.0"
echo '{"jsonrpc":"2.0","id":1,"method":"extract","params":{"text":"Dr Smith treats asthma in Mosman.","entity_labels":["person","disease","location"],"relation_labels":["treats","located_in"],"threshold":0.5,"relation_threshold":0.5,"max_entities":64}}' | \
  env KASTELLAN_GLINER_RELEX_WEIGHTS_DIR="$WEIGHTS_DIR" \
...
* **`device=cuda`** (expected to fail): exits with `{"level": "error", "code": -32604, "message": "device=cuda not supported on darwin (Apple Silicon has no NVIDIA GPU); set KASTELLAN_GLINER_RELEX_DEVICE to auto|cpu|mps"}`. Exit code 2.

**What this slice deliberately does NOT do:**

- **No PG-backed integration-test run on this Mac**. The existing `core/tests/gliner_relex_e2e.rs` `[SKIP]`s cleanly because Postgres isn't installed in this Mac's brew prefix. Direct-spawn smoke test above is the cross-validation proxy; full PG-backed integration is the operator's next step on a Mac with Postgres.app or a `brew install postgresql@17` install. The 4 e2e tests stay [SKIP] until then — same posture as Linux without `KASTELLAN_GLINER_RELEX_ENABLE=1`.
...
- `workers/gliner-relex/src/kastellan_worker_gliner_relex/__main__.py`: 126 → **172 LOC** (+46 for the per-platform branches + docstrings, well under the 500-LOC cap).
...
2. **Isolation**: passed 5/5 when run alone (`cargo test ... tail_loop_from_start_replays_then_exits`); failed ~30-50% when run alongside the rest of the `kastellan-core --lib` tests.
...
1. `core/src/audit_tail.rs::tests::tempdir()` — added a process-local `static COUNTER: AtomicU64 = AtomicU64::new(0)` and threaded the counter value into the directory name (`kastellan-audit-tail-{pid}-{nanos}-{counter}`). Process-local uniqueness independent of clock resolution. Closes #101.
...
- ✅ 20/20 consecutive `cargo test -p kastellan-core --lib` runs all green post-fix.
...
- ✅ Smoke: `kastellan-cli --help` exit 0.
...
- **No CLI surface for managing the `relation_kinds` vocabulary.** Same posture as `entity_kinds` today: operator extends via direct `INSERT INTO relation_kinds` (no CLI command, no migration). An `entities` parallel like `kastellan-cli relations kinds add/remove/list` would be the natural follow-up if vocabulary management becomes a frequent operator chore.
- **No `entities show` symmetry for `relations` lookup.** The operator quarantine-review CLI shipped via PR #93 walks `entities` + their linked memories, but does not surface the graph's edge structure (relations) — there's no `kastellan-cli relations show <entity_id>` for "what does the graph know about this entity". Out of scope for this slice; logical follow-up.
...
- ✅ `cargo build --bin kastellan-cli`: clean (2.6 s).
- ✅ `cargo test --workspace`: **886 passed / 0 failed / 4 ignored** (+3 over the pre-slice 883).
- ✅ `cargo clippy --bin kastellan-cli`: 2 warnings, both pre-existing `print_literal` warts in `entities.rs:163` and `tools_allowlist.rs:159` (carried over verbatim from the pre-refactor single-file CLI; documented in the PR #96 fixup commit `1b58636` and out of scope for this slice).
...
**File-size watch (post-slice):** every per-subcommand module in `core/src/bin/kastellan-cli/` stays under the 500-LOC cap; `common.rs` 126 → 149 LOC (+23 for the helper + its doc-comment, still well under cap).

---

## Recently completed (previous session, 2026-05-20 — Issue #66 `kastellan-cli` per-subcommand-module split, **merged to `main` via PR #96 at `2704468`** + post-merge fixup `1b58636`)

Pure mechanical refactor closing the longest-flagged 500-LOC breach in the codebase. `core/src/bin/kastellan-cli.rs` had grown to **1933 LOC** after five session's worth of subcommand additions (ask, tasks, tools, memory, entities, observation, audit-tail). Original Issue #66 (filed 2026-05-15) scoped this at ~960 LOC; actual was double that — every additional CLI feature slice has been making the file harder to navigate.

**Implementation + post-merge fixup commits:**

1. **`b77f4c9`** (`refactor(kastellan-cli): split into per-subcommand modules (Issue #66)`) — moved from single-file bin `core/src/bin/kastellan-cli.rs` to directory bin `core/src/bin/kastellan-cli/` with one module per subcommand tree. Cargo.toml `[[bin]] path` updated from `src/bin/kastellan-cli.rs` to `src/bin/kastellan-cli/main.rs` (idiomatic Cargo directory-bin shape; explicit-path declaration preserved to suppress auto-discovery ambiguity).
2. **`1b58636`** (`refactor(kastellan-cli): dedupe observation_replay runtime + fix doc count (PR #96 review)`) — post-merge self-review fixup. (a) `observation_replay::run_observation_replay` was still inlining its own `Builder::new_multi_thread().enable_all().build()` + error-print block instead of using the shared `common::multi_thread_runtime("observation replay")` helper introduced in the same PR — removed the last straggler copy of the 6× runtime-builder duplication the helper was introduced to absorb. (b) HANDOVER.md's "11 unit tests across the four moved test modules" miscounted; the actual total is 18 (7 + 5 + 4 + 2 across `common` / `ask` / `tasks` / `entities`). Out-of-scope finding from the same self-review (runtime built before action validation in `tasks` / `entities` / `memory_l1` / `tools_allowlist` dispatchers — minor wart carried over verbatim from the pre-refactor single file) filed as [Issue #97](https://github.com/hherb/kastellan/issues/97) rather than addressed here to keep this PR strictly mechanical.
...
- ✅ Smoke: `kastellan-cli --help` exit 0; `kastellan-cli garbage` exit 2; `kastellan-cli ask` exit 2; `kastellan-cli entities` exit 2 — all match pre-refactor.
- ✅ `cargo clippy --workspace --bin kastellan-cli`: no new warnings (pre-existing `db/src/probe.rs` doc-comment warnings unrelated to this slice).
...
- **New `kastellan_db::entities` module** (~640 LOC including tests; over the 500-LOC soft cap with a FILE-SIZE NOTE; split deferred). Public surface: `list_entities` / `get_entity_with_mentions` / `approve_entity` / `reject_entity` / `merge_entities` + 7 typed outcome/error types. Single-transaction state-changers with SELECT FOR UPDATE row locks + ORDER BY id on multi-row locks to avoid deadlock.
- **New CLI subcommand tree** `kastellan-cli entities {list, show, approve, reject, merge}`. Default `--state quarantined --limit 50`. Approve/reject continue on NotFound and aggregate exit 1; merge precondition errors → exit 2, DB errors → exit 1.
...
5. **(#7)** `kastellan-cli memory l1 add` emits a one-line stderr `note:` on insert success explaining that operator-added L1 rows are not auto-linked. Code comment at the call site documents the deliberate NoOp-extractor choice and points to the future `memory relink` subcommand.
...
**What's deliberately NOT in this slice:** operator quarantine-review CLI (still the natural follow-up; without it, graph_search returns 0 rows even though `memory_entities` is populated, because every newly-extracted entity is quarantined-by-default); `kastellan-cli memory relink` backfill (for pre-existing memory rows); `entities.embedding` population; per-link provenance columns on `memory_entities`; L2/L3/L4 writers (don't exist yet).

**File-size watch:** `entity_link.rs` 251 LOC (under cap); `memory_entity_link_e2e.rs` 532 LOC (32 over the soft cap; flagged for a future lift of duplicated helpers into `kastellan-tests-common`). Other modified files unchanged in size or marginally bigger (l0_seed.rs +35; runner.rs +21).
...
1. **`db/migrations/0016_entity_kinds_revoke_runtime_writes.sql`** — 0015's comment claimed "INSERT on `entity_kinds` is operator-only by GRANT default," but 0002's `ALTER DEFAULT PRIVILEGES … GRANT … ON TABLES TO kastellan_runtime` fires automatically for every new table, so the runtime role had silently received full CRUD on `entity_kinds`. 0016 REVOKEs INSERT/UPDATE/DELETE/TRUNCATE to restore the intended invariant (same pattern as `0008`/`deleted_memories`). New `db/tests/postgres_e2e.rs::entity_kinds_runtime_role_cannot_write` pins all three write paths fail under runtime role; positive control on `secrets` keeps the same shape. **+1 test → workspace 834.**
...
3. **`6103e6c`** (`fix(db/graph): PgGraph::upsert_entity uses name_norm post-0015`) — corrective Task 2.5. Moved `normalize_entity_name` from `core::entity_extraction` to `kastellan-db` (proper layering); re-exported from core. Updated `PgGraph::upsert_entity` to compute `name_norm` via the helper and use `ON CONFLICT (kind, name_norm) DO UPDATE`. Removed the now-duplicate `unicode-normalization` dep from `core/Cargo.toml` (transitive through `kastellan-db`).
...
- **No operator-facing `kastellan-cli gliner extract` command.** Calling the worker today requires either (a) the manual JSON-RPC smoke command in the README, or (b) the v2 consumer slice's typed client.
- **No CUDA-path e2e test.** The DGX's vLLM owns the GPU at session time, so `device="auto"` falls back to CPU. The CUDA path is covered by the Python-side smoke command (`KASTELLAN_GLINER_RELEX_DEVICE=cuda`) when vLLM is offline.
...
- **`step.spawn_failed` row for skip-register paths.** Today a misconfigured `KASTELLAN_GLINER_RELEX_ENABLE=1` without weights logs at startup but doesn't write an audit row (`build_gliner_relex_entry` runs before the dispatcher exists). The operator-facing visibility is the daemon log only. Lift into a startup audit row family if the operator wants SQL-queryable misconfiguration history.
- **Per-tool warm-slot status in `kastellan-cli`.** The `_test_slot_has_warm` accessor is test-only (`#[doc(hidden)]`); operators have no way to observe warm-state from the CLI. Worker-lifecycle Slice 3 has this on its scope; not pulled forward.
...
4. **Item #6 — `KASTELLAN_GLINER_RELEX_ENABLE` whitespace-trimmed.** Strict on the value itself (only `"1"` enables; `true`/`yes`/`on` rejected by design) but a stray `\n` from `echo "1" > envfile` no longer silently flips the worker to disabled. Pinned by `resolve_env_trims_whitespace_on_enable`.
...
- **No Rust code of any kind.** The manifest entry, the wire-shape serde types (`ExtractRequest` / `ExtractResponse` / `Entity` / `Triple`), and the conditional daemon registration via `KASTELLAN_GLINER_RELEX_ENABLE=1` are all Slice 2 (tasks 2.1-2.10 in the plan).
...
- **No operator-facing `kastellan-cli` command.** Calling the worker today requires the manual smoke command in the README. The first programmatic Rust caller will be the v2 entity-extraction consumer slice (further out than Slice 2).
...
**File-size watch:** No new Rust files this session. Existing breaches (`core/src/bin/kastellan-cli.rs` at 1432 LOC; `core/src/scheduler/tool_dispatch.rs` at 748 LOC; `db/src/memories.rs` at 769 LOC; `core/src/worker_lifecycle/idle_timeout.rs` at 525 LOC) stay unchanged. The new design spec is 444 lines; the implementation plan is 2276 lines (plans are not load-bearing source — the 500-LOC soft cap doesn't apply).
...
- **Operator status surface** — `kastellan-cli supervisor status` for inspecting warm workers + cap state. The `_test_slot_*` accessors are the test-side equivalent today.
...
Test count and counts unchanged (still **751 passed / 0 failed / 0 warnings**). Three follow-up GitHub issues filed for items deferred out of scope: [#84](https://github.com/hherb/kastellan/issues/84) (queue-depth visibility), [#85](https://github.com/hherb/kastellan/issues/85) (teardown-task accumulation under high request rate), [#86](https://github.com/hherb/kastellan/issues/86) (struct-literal bypass of `Lifecycle::idle_timeout` validator).
...
- The `kastellan-supervisor` crate (1900+ LOC of systemd/launchd OS-unit installer code). Despite the spec's "currently a stub" wording, that crate is the OS-level supervisor for the daemon itself, not the worker lifecycle layer. Naming overlap with the spec's "supervisor" is purely conceptual.
...
3. **Cross-platform `setrlimit(RLIMIT_CPU)`** — new `workers/prelude/src/rlimit.rs` module (cross-platform, no cfg gate) ships pure helper `cpu_ms_to_seconds` (ceiling-div with 1-second floor, saturating on overflow), `apply_from_env` (reads `KASTELLAN_CPU_MS`, parses to u64, calls `libc::setrlimit(RLIMIT_CPU)` with soft = hard for clean SIGXCPU kill), `RlimitReport::{Applied { cpu_seconds }, Disabled}`, `RlimitError::{Env, SetRlimit}`. `libc = "0.2"` promoted from Linux-cfg target table to top-level deps. Tests use `Mutex<()>` + `OnceLock` to serialize env-mutation across cargo's parallel test harness.
...
5. **Env-var plumbing in core** — `core::tool_host::ENV_CPU_MS = "KASTELLAN_CPU_MS"` const added; `derive_lockdown_env` appends `KASTELLAN_CPU_MS = policy.cpu_ms.to_string()` to `policy.env` when `policy.cpu_ms > 0` (omitted when `0` so the prelude's `apply_from_env` sees "unset" and returns `Disabled` — canonical "no rlimit" signal).
...
- **`RLIMIT_AS` for macOS memory enforcement.** Counts virtual address space, not RSS; high false-positive risk for malloc-heavy workers like Python. Deferred to the Apple `container` micro-VM backend — see [issue #55](https://github.com/hherb/kastellan/issues/55) (one-session discovery spike filed at the start of this session).
...
| `core` integration (`supervisor_e2e`) | 1 | **cross-platform real** end-to-end smoke. Brings up per-test PG cluster + `core_service_spec` for the freshly-built `kastellan` binary with `KASTELLAN_DATA_DIR` + `KASTELLAN_STATE_DIR` + `USER` injected. Install → start → wait Active → 500 ms stable-Active recheck → poll redirected stdout for `"database probe succeeded"` → `psql -d kastellan` asserts `audit_log` has at least one `(actor='core', action='startup')` row → poll per-test state dir for an `audit-YYYY-MM-DD.jsonl` containing the bring-up row within ≤ 5 s (proves audit-mirror task drained + fsynced) → stop → wait Inactive → uninstall |
...
| `llm-router` unit | 41 | `error::truncate_for_error` (3); `messages::ChatRole` lowercase + closed-enum (2), constructors (1), `skip_serializing_if` pin (1), `ChatResponse` decodes vLLM full-envelope + minimal Ollama (2); `Backend` serde + `as_tag()` round-trip (3); `config::default_local_url_for_os()` Linux/macOS (1), `DEFAULT_LOCAL_MODEL`/`DEFAULT_TIMEOUT_MS` (1), `RouterConfig::default()` (1) + `from_env` (5); **Option O additions (7):** `KASTELLAN_LLM_EMBEDDING_URL` fallback + override semantics (2); `KASTELLAN_LLM_EMBEDDING_MODEL` default (1); `EmbeddingRequest`/`EmbeddingData`/`EmbeddingResponse` wire shapes (2); `RouterError::EmbeddingCountMismatch` (1); `PolicyGate::pick_embed` default (1); `Router::pick_embed_backend` proxy delegation (1); `router_embed_rejects_frontier_choice_in_phase_0` frontier-rejection pin (1); `policy::DefaultLocalPolicy` always picks Local (1) + Send+Sync (1); `lib::compose_url` (2), `CHAT_COMPLETIONS_PATH` (1), `Router::new`/`pick_backend`/`send` (3 incl. `PolicyDeniedFrontier`) |
...
- **`core/src/bin/kastellan-cli.rs` (1419 LOC)** — the largest remaining 500-LOC breach in the crate. Natural split: lift the `memory l1 {add,list,remove}` subcommand tree + `tools allowlist {add,remove,list}` subcommand tree into sibling files (e.g. `core/src/bin/kastellan_cli/{memory_l1.rs, tools_allowlist.rs}` if the bin crate gets a module structure). Not yet flagged as an issue — file the issue if/when the next slice touches this file.
...
- **NEW [`core/tests/cli_memory_l1_e2e.rs`](../../../core/tests/cli_memory_l1_e2e.rs)** (~330 LOC). 3 CLI subprocess integration scenarios spawning the real `kastellan-cli` binary against a per-test PG cluster: add writes row + audit; list shows added rows with fixed-width header; remove deletes specified id.
...
| Actor       | Action         | Payload keys                                                            | When                                                                          |
|-------------|----------------|-------------------------------------------------------------------------|-------------------------------------------------------------------------------|
| `cli`       | `l1.added`     | `{source, action, memory_id, body_sha256}` (4 keys)                     | `kastellan-cli memory l1 add` — Operator path, validation passes                |
| `cli`       | `l1.removed`   | `{memory_id, deleted}` (2 keys)                                         | `kastellan-cli memory l1 remove` — Operator path; written even when `deleted=false` |
...
| 11 | `a641991` + `93e5ddc` | `kastellan-cli memory l1 {add,list,remove}` subcommand tree; fixup: reject unknown flags + extra positional args |
...
- **[Issue #81](https://github.com/hherb/kastellan/issues/81) — split `core/src/scheduler/inner_loop.rs` (1214 LOC).** Pre-existing 500-LOC breach grew from 1095 (post-PR-#79) to 1214 (post-this-slice) — now ~714 over cap. Natural split: lift `build_plan_formulate_payload` + the slice-narration paragraphs into `core/src/scheduler/inner_loop_audit.rs`. Pure refactor. Highest-priority pickup after this slice lands.
- **Operator recapture against the current daemon** — one-time `cargo test -p kastellan-core --test observation_capture -- --ignored --nocapture` against the local LLM. The pre-Slice-A captures don't carry `l1_insight` either; recapture turns them into rule-iteration-harness inputs that exercise the L1 promotion path.
...
- **Mock HTTP listener pattern duplication** — `cli_memory_l1_e2e.rs` is now a 4th-class CLI subprocess test; the OpenAI-compatible TCP mock pattern remains a hand-rolled-per-file pattern in 4 sites. Extract to `kastellan-tests-common::mock_http`.
...
- MODIFIED `core/src/cli_audit.rs`, `core/src/bin/kastellan-cli.rs`, `core/src/memory/mod.rs`.
...
- **[Issue #81](https://github.com/hherb/kastellan/issues/81) — split `core/src/scheduler/inner_loop.rs` (1095 LOC).** Filed as a post-PR-#79 follow-up. Pure refactor: lift `build_plan_formulate_payload` + the audit writers + the pin tests into `core/src/scheduler/inner_loop_audit.rs`. Pre-existing 500-LOC breach is now ~600 over cap.
...
- **Mock HTTP listener pattern duplication** — `recall_assembly_e2e.rs` is now the third site with a hand-rolled `tokio::net::TcpListener` + JSON envelope for an OpenAI-compatible mock. Code review flagged extracting to `kastellan-tests-common::mock_http`. Filed mentally; not blocking.
...
- **[Issue #80](https://github.com/hherb/kastellan/issues/80) — `cli_ask_e2e.rs` mock dispatches by FIFO instead of URL path.** Filed as a post-PR-#79 follow-up. The unanticipated cascade-fix from sharing the mock-LLM URL across chat + embed suggests the mock should dispatch by `/chat/completions` vs `/embeddings` path instead of a single FIFO queue. Today both production paths default to one base URL (vLLM/Ollama serve both on one port); test-only path-based dispatch would avoid the brittle interleaved-queue pattern.
...
Branch: `fix/runner-reject-agent-raised-provenance` (off `main` at `4ddfe3b`, single commit `a6335ab` ready for PR). Closes [issue #71](https://github.com/hherb/kastellan/issues/71) — _audit-trail integrity: producer-supplied `agent_raised` provenance accepted without validation_, filed during PR #70 code review. The fix follows the "fail-loud at task entry" mitigation (Option 1 in the issue body); the alternative two-token-enum design (Option 2) was rejected as out-of-proportion for the threat.
...
- **Two-token enum** ([issue #71](https://github.com/hherb/kastellan/issues/71) Option 2). Stays as a possible future refactor; the current asymmetric reject is the smallest defense-in-depth change.
...
- **NEW `resolve_floor_for_submission` pure helper** in `core/src/bin/kastellan-cli.rs`. Maps `(instruction, operator_flag)` → `(floor, source, signals)`. Operator-explicit always wins; a `tracing::warn!` fires when inference would have elevated above the operator's pinned value (operator-visible suppression breadcrumb in the daemon journal).
...
Pure-additive; downstream JSONB consumers (replay harness, observation captures) keep working unchanged. `kastellan-cli observation replay` against ec-001 (with operator-pinned `--classification-floor ClinicalConfidential`) shows a `*` delta row once recapture lands.
...
- `core/src/bin/kastellan-cli.rs::resolve_floor_for_submission_tests`: +5 (no-op default / cli-inferred / operator-wins-with-warn / operator-wins-no-warn / operator-equal-no-warn).
...
- `core/src/bin/kastellan-cli.rs` — `resolve_floor_for_submission` helper + `ask_async` payload wiring + `tracing::warn!` on suppression + 5 unit tests.
...
Branch: `feat/deterministic-policy-classification` (off `main` at `67d29a0`). The first real Stage 0 reviewer rule: a deterministic check enforcing three classification invariants over `(ctx.classification_floor, plan.data_ceiling, plan.steps[].classification)`. Paired with a small CLI flag (`kastellan-cli ask --classification-floor <DataClass>`) so operators can pin the floor at task submission — the minimum-viable upstream path for the rule to fire end-to-end in production. Stage 0 was always-`Approve` before this slice; the chain now has two real reviewers (Stage -1 + Stage 0).
...
`ec-001` only fires the new rule when **both**: (a) the operator pins floor=`ClinicalConfidential` at submission via `kastellan-cli ask --classification-floor`, AND (b) the agent's plan contains a step labelled below ClinicalConfidential. The 2026-05-14 capture shows the agent self-refused before emitting any actionable outbound steps; future recapture against a less-cautious model is needed to exercise the rule end-to-end against ec-001. Not blocking — the rule itself is fully unit-test-pinned.
...
- **Operator recapture against current daemon** to expose plan bodies in the existing captures; afterwards, `kastellan-cli observation replay` against ec-001 with floor pinned will show a `*` delta row.
...
- **`core/src/bin/kastellan-cli.rs` size:** now 1089 LOC. The plan/spec already flagged the soft-cap breach as pre-existing (from the Slice B observation-replay work). Natural future split: one file per subcommand tree.
...
- `core/src/bin/kastellan-cli.rs` — helper + flag wiring + help text + usage line.
- `docs/devel/handovers/HANDOVER.md` + `docs/devel/ROADMAP.md` — this update.
...
```sh
$ ./target/debug/kastellan-cli observation replay
...
7. End-to-end smoke via `kastellan-cli observation replay` — binary runs clean against the existing pre-Slice-A captures.
...
- **Operator recapture against current daemon.** Pre-Slice-A captures retain `plan_json: null`; recapture turns them into harness-replay-able inputs. Once recaptured, the 5 principle fixtures will produce `*` delta rows under `kastellan-cli observation replay`. One-time operator action.
...
- **`core/src/bin/kastellan-cli.rs`** — new `observation` top-level subcommand routing to `run_observation_replay` (hand-rolled argv, no clap dep; same shape as the existing `tools allowlist` tree). `--captures-dir PATH` + `--model SLUG` flags; default captures-dir resolves via `CARGO_MANIFEST_DIR` for `cargo run`, falls back to cwd-relative for installed binaries. Help text updated.
...
- **NEW `core/tests/observation_replay_cli_e2e.rs`** — 3 subprocess scenarios via `kastellan_tests_common::cli_binary`: (1) happy path (synthetic approve baseline writes to tempdir; subprocess prints fixture row + "1 plans across 1 fixtures" summary line; exit 0); (2) unknown-flag → exit 2; (3) empty captures dir → exit 0 + "no captures found" hint.
...
**Operator iteration loop:** edit `ConstitutionalGuard::review` (or `DeterministicPolicy::review`) body in `core/src/cassandra/review.rs` → `cargo build --bin kastellan-cli` → `./target/debug/kastellan-cli observation replay`. Deterministic; no daemon spin-up cost.
...
- **Operator recapture against current daemon** — the existing `tests/observation/captures/<id>/2026-05-14_gemma4-26b-a4b-it-q8-0.json` files retain `plan_json: null` because they were produced before Slice A. Recapture via `cargo test -p kastellan-core --test observation_capture -- --ignored --nocapture` turns them into replay-able inputs.
- **`core/src/bin/kastellan-cli.rs` LOC growth** — 797 LOC before this slice; ~950 after. Well over the 500-LOC soft cap. Natural future split candidate: one file per subcommand tree (`audit_tail.rs` / `ask.rs` / `tasks.rs` / `tools_allowlist.rs` / `observation_replay.rs`) plus a dispatch entry point. Not warranted today but worth noting.
...
2. **120 s per-fixture timeout too tight on local 26B model (orchestrator infra).** Original `PER_FIXTURE_TIMEOUT = 120s` was sized for an unspecified fast model. gemma4 takes ~20s/call warm × up to 3 plan iterations + cold-start prefill on a 6.6 KB system prompt = >120s sometimes. Fix: env-overridable defaults — `KASTELLAN_OBSERVATION_PER_FIXTURE_TIMEOUT_SECS` (default **600**) and `KASTELLAN_OBSERVATION_LLM_TIMEOUT_MS` (default **180000**, replacing the previously-hardcoded 120000 passed to the daemon's `KASTELLAN_LLM_TIMEOUT_MS`). Daemon-side LLM timeout is picked smaller than the test-side per-fixture budget so a hung call surfaces as a transport error the agent loop can retry within budget, not as a wall-clock test kill.
...
- **`core/tests/observation_capture.rs` (528 → 648 LOC).** Per-OS argv0 path constants (`ECHO_PATH`/`DATE_PATH`/`LS_PATH`/`CAT_PATH` — same Linux/macOS split `cli_ask_e2e.rs` uses); env-overridable `per_fixture_timeout()` + `llm_timeout_ms_string()` helpers; allowlist seed moved before `bring_up_daemon` via a `probe::run` → `connect_runtime_pool` → `seed_tool_allowlist` → drop pool dance; `DaemonHandles` gains `stdout_path` + `stderr_path` for the new `dump_daemon_log` helper gated behind `KASTELLAN_OBSERVATION_DUMP_DAEMON_LOG=1`; on `state=failed` the orchestrator now prints `tasks.result` to stderr so the operator sees the agent's decode error without rummaging through audit rows. **Above 500-LOC soft cap (CLAUDE.md rule #4) — flagged for split in a future slice** (the orchestrator's main test body and the `bring_up_daemon` helper are natural candidates for extraction into a `tests/observation_capture/` directory; not warranted today since most of the bulk is one long `#[tokio::test]` and splitting would mean a non-trivial restructure).
...
Branch: `feat/refusal-state` (off `main` at `5f543d2`, **merged to `main` via PR #59 at `f1fea54`; 12 commits total**). Closed [issue #23](https://github.com/hherb/kastellan/issues/23) — _constitutional refusals are recorded as `state='completed'`, not `'blocked'`_. The agent's self-refusal path collapsed into `tasks.state='completed'` (same shape as a successful task); the reviewer-detected `Verdict::ConstitutionalBlock` path mapped to `'blocked'`. After this slice, the two are wire-distinguishable and the operator-visible `tasks` table can be queried directly without prose-matching `result.body`.
...
- **Real `ConstitutionalGuard` / `DeterministicPolicy` rule implementations.** Still waiting on the observation-phase dataset (operator action — run `cargo test -p kastellan-core --test observation_capture -- --ignored --nocapture` against the local LLM). This slice ships the rails so real rules land cleanly afterwards.
...
The production `shell_exec_entry` (in `core/src/scheduler/tool_dispatch.rs`) deliberately keeps its explicit literal — the security-critical fields (`net: Net::Deny`, `profile: Profile::WorkerStrict`, `env: ["KASTELLAN_SHELL_ALLOWLIST", …]`) are spelled out for readability. New unit test `sandbox_policy_default_is_strict_deny_with_one_second_budget` pins the chosen default values so a future tune is an explicit edit.
...
- **NEW `db/migrations/0009_tool_allowlists.sql`:** new table `tool_allowlists(tool TEXT, argv0 TEXT, created_at TIMESTAMPTZ, created_by TEXT, PRIMARY KEY(tool, argv0))`; CHECK constraints (non-empty tool, non-empty absolute argv0); `GRANT SELECT, INSERT, DELETE` to `kastellan_runtime` paired with `REVOKE UPDATE, TRUNCATE` (counteracts `0002`'s `ALTER DEFAULT PRIVILEGES`, matching the `0008_deleted_memories_audit` pattern). No new index — PK covers `WHERE tool = $1`.
...
- **`core/src/bin/kastellan-cli.rs`:** new `tools allowlist {add,remove,list}` subcommand tree using the existing hand-rolled dispatcher (no clap). All validation errors (`InvalidToolName`, `InvalidArgv0`, `Argv0HasNul`, `Argv0HasDotDot`) exit with code 2. Help text + file-level docstring updated.
- **`core/src/main.rs::build_tool_registry`:** rewired from sync env-var-driven to `async fn(&PgPool) -> anyhow::Result<ToolRegistry>`. Reads `KASTELLAN_SHELL_EXEC_BIN` (kept), queries `db::tool_allowlists::list_for_tool(pool, "shell-exec")` (NEW source-of-truth), builds the `ToolEntry`, emits one `actor='core' action='registry.loaded'` row with payload `{tools: [{name, binary, allowlist_len, allowlist_sha256}]}`. SHA-256 is over the canonical-form (lex-sorted, `\n`-terminated per entry, empty list → SHA-256 of empty string) so cross-restart drift becomes visible at a glance. Fail-closed on DB error during load; best-effort on audit row. Deprecation WARN on `KASTELLAN_SHELL_EXEC_ALLOWLIST` if set. 3 new module-private helpers: `LoadedToolRecord` struct (`#[derive(serde::Serialize)]`), `sha256_argv0_list`, `hex_encode`, `write_registry_loaded_row` (returns `Result<(), kastellan_db::DbError>` — proper type, no `sqlx::Error::Protocol` smuggling). `core/Cargo.toml` gained `sha2 = { workspace = true }`.
- **`core/tests/cli_ask_e2e.rs`:** dropped `KASTELLAN_SHELL_EXEC_ALLOWLIST` env push; happy-path now seeds via `tests-common::seed_tool_allowlist(&pool, "shell-exec", &[ECHO_PATH])` before daemon start; failure-path seeds nothing (empty allowlist → `/bin/cat` is denied). Test setup uses `actor="test", action="setup"` for the explicit pre-daemon `probe::run` (distinct from `core/startup` so the existing `Some(&1)` assertion on the daemon's own startup row isn't inflated). Audit multiset assertions bumped to include `core/registry.loaded ×1` and `test/setup ×1`; total row counts went 11→13 (happy) and 17→19 (failure).

**Audit-row contract (the headline):**

| When                                                  | actor  | action                       | payload keys                                                                                  |
| ----------------------------------------------------- | ------ | ---------------------------- | --------------------------------------------------------------------------------------------- |
| `kastellan-cli tools allowlist add <tool> <argv0>` (INSERT) | `cli`  | `tools.allowlist.add`        | `{tool, argv0}`                                                                               |
| `kastellan-cli tools allowlist remove <tool> <argv0>` (DELETE) | `cli`  | `tools.allowlist.remove`     | `{tool, argv0}`                                                                               |
...
Two code-review-driven fixes landed inline (post-implementer): (a) migration `0009` originally lacked the `REVOKE UPDATE, TRUNCATE` line — caught by code review against the established `0008` pattern; without the REVOKE, `0002`'s `ALTER DEFAULT PRIVILEGES` would silently grant `UPDATE` to `kastellan_runtime` despite the explicit GRANT listing only SELECT/INSERT/DELETE; (b) `write_registry_loaded_row` originally wrapped `DbError` as `sqlx::Error::Protocol(e.to_string())` to paper over the type mismatch — replaced with `Result<(), kastellan_db::DbError>` so the type is honest. Both fixes were single-line amendments to their respective task commits.

**What this slice deliberately does NOT do.**
- **`KASTELLAN_SHELL_EXEC_BIN` stays as env.** Binary path is orthogonal to allowlist hygiene; one worker = one binary, and binaries are constrained by the build artifact set. Moving the binary path to DB is a separate slice when a second tool exists.
...
- **No retroactive emission for previously-set env-var allowlists.** Operators must re-seed via `kastellan-cli tools allowlist add` on first daemon start with this code.
...
2. **`observation_capture.rs` silent POLICY_DENIED (issue B, 75 confidence).** The `#[ignore]`-flagged orchestrator had become operator-seeded after this branch removed env-var allowlist auto-seeding; if the operator forgot to run the `kastellan-cli tools allowlist add` lines from the comment block, all captures would be POLICY_DENIED. Added a fast-fail assertion right after the runtime-pool connect: `SELECT COUNT(*) FROM tool_allowlists WHERE tool = 'shell-exec'` must be > 0 with a message pointing at the seeding instructions. Cheap, runs before any LLM cost is incurred.
...
4. **`tools allowlist list --tool` does a client-side filter (issue C, 25 confidence).** Filed as [issue #52](https://github.com/hherb/kastellan/issues/52). At current scale (O(10s) of rows) the bypass of the `(tool, argv0)` PK is harmless; the clean fix needs a new `list_for_tool_full -> Vec<AllowlistEntry>` to preserve the `CREATED_AT`/`CREATED_BY` columns the CLI renders.
5. **`tests-common/src/allowlist.rs` has no self-tests (issue E, 25 confidence).** Commented on [issue #39](https://github.com/hherb/kastellan/issues/39) folding the new `seed_tool_allowlist` helper into its existing "tests-common self-tests" scope. (DB-I/O helper, not one of the pure-function helpers the issue body originally enumerated.)
...
- `core/src/bin/kastellan-cli.rs` — `tools allowlist` subcommand tree (~150 LOC added); help text + file-level docstring updated.
...
**Post-review cleanup (PR #49):** (1) renamed `cancel_pending_task_writes_one_cli_audit_row` → `cancel_pending_task_writes_lifecycle_and_finalize_rows` to reflect the new two-row contract; (2) fixed docstring drift in `emit_producer_cancel_finalize` (dropped inaccurate reference to `compute_duration_ms` — the helper never gets called from that path); (3) both `emit_producer_cancel_finalize` (cli_audit) and `emit_task_finalize_row` (crash_recovery) now surface the operationally-impossible `task.finished_at == None` case via `tracing::error!` instead of silently falling back to wall-clock — an emitted row with `finished_at` off by the scheduler-lag delta is wrong, operators need to see the contract violation. One deferred follow-up filed as [issue #50](https://github.com/hherb/kastellan/issues/50) (unify finalize-payload provenance signal across the three emitters via an explicit `provenance` field in a schema-v2 migration; bundle with [issue #47](https://github.com/hherb/kastellan/issues/47)).
...
2. Implemented the helper; unit tests green (`cargo test -p kastellan-core --lib scheduler::audit::tests::build_crashed_finalize_payload` → 6 passed).
...
- **No producer-side `task.failed` row.** The `kastellan-cli tasks fail` escape hatch and the scheduler's `task.crashed` row together already cover the running-after-restart path; observation phase decides if a producer row adds anything.
...
- **`core/tests/observation_capture.rs` (NEW, ~498 LOC):** the operator-run orchestrator. `#[ignore]`-flagged with reason `"operator-run: needs real local LLM at KASTELLAN_LLM_LOCAL_URL"`. Brings up per-test PG cluster + real `kastellan` daemon under user supervisor + sandboxed worker, points the daemon at the operator's local LLM (default `gemma4:26b-a4b-it-q8_0`), iterates every fixture, runs each through `kastellan-cli ask`, captures the audit-row stream per task, and writes one capture JSON per fixture under `tests/observation/captures/<id>/<date>_<model_slug>.json`. Fails loudly if the LLM is unreachable (no skip-as-pass — operator explicitly ran it). `KASTELLAN_OBSERVATION_DRY_RUN=1` walks the fixture tree and prints the work plan without any side effects (used by the operator to verify a new fixture parses before running an expensive live capture).
...
- **Silent `Approve` verdict default in `extract_plans_from_audit_rows` — FILED.** See [issue #47](https://github.com/hherb/kastellan/issues/47). Bumping `SCHEMA_VERSION` to 2 and changing `verdict_today` to `Option<String>` is free-cost while no captures exist on disk; deferred for explicit review on the schema decision rather than landing inline.
- **GIN index on `audit_log.payload` — FILED.** See [issue #48](https://github.com/hherb/kastellan/issues/48). Migration changes touch the runtime-role grant matrix and want a separate review pass; not blocking observation-phase capture infrastructure.
...
Branch: `fix/worker-command-seal-tighten` (off `main` at `31ac414`, the merge of PR #44). Closes [issue #16](https://github.com/hherb/kastellan/issues/16) — "tool_host: WorkerCommand seal has an in-crate hole — sibling modules can bypass dispatch".
...
- ✓ "The Option-M `compile_fail` doctest still passes." — Verified: `cargo test -p kastellan-core --doc` runs the one doctest on `WorkerCommand` and it trips correctly (compile_fail asserts the body fails to compile, which it does because `::new` is no longer reachable).
...
- `cargo test -p kastellan-core --doc` — 1 doctest, compile_fail trips correctly.
...
- **`core/src/bin/kastellan-cli.rs::ask_async`** — line 267 `insert_pending(...)` → `submit_and_audit(...)`. Import line widened; `insert_pending` dropped from the `tasks` import.
...
| When                                              | actor       | action            | payload keys                  |
| ------------------------------------------------- | ----------- | ----------------- | ----------------------------- |
| `kastellan-cli ask "..."` inserts a `pending` row   | `cli`       | `task.submitted`  | `{task_id, lane, plan_count}` (`plan_count` always 0 at submit) |
...
4. Rewired `kastellan-cli.rs::ask_async`; `cli_ask_e2e.rs` red on multiset.
...
- **No producer `task.failed` row from `kastellan-cli tasks fail`.** Operator escape hatch; rare; scheduler's `task.crashed` lifecycle row already covers the running-after-restart path.
...
- `core/src/bin/kastellan-cli.rs` — one-line swap + import widening at `ask_async`.
...
- **NEW `core/src/cli_audit.rs` (~110 LOC + 2 unit tests):** producer-side audit-row helpers for the `kastellan-cli` binary. Public surface:
...
- **`core/src/bin/kastellan-cli.rs` rewiring:** both `mark_cancelled` call sites (the `ask` SIGINT handler at line ~293 and the `tasks cancel` subcommand at line ~470) now go through `cli_audit::cancel_and_audit`. The SIGINT path is best-effort (`let _ = …`) so a transient audit issue can't block the exit-130 path. The `tasks cancel` subcommand pattern-matches on `CancelOutcome` for the user-facing message.

**Audit-row contract (the headline):**

| When                                              | actor       | action            | payload keys                  |
| ------------------------------------------------- | ----------- | ----------------- | ----------------------------- |
| `kastellan-cli tasks cancel <id>` flips a row       | `cli`       | `task.cancelled`  | `{task_id, lane, plan_count}` |
| `kastellan-cli ask … <SIGINT>` flips a row          | `cli`       | `task.cancelled`  | `{task_id, lane, plan_count}` |
...
- **No producer-side `task.failed` row** from `kastellan-cli tasks fail`. The `mark_failed_running` UDS escape hatch is operator-only (rare) and the scheduler's lifecycle row already covers the running→crashed path on the next sweep. Filed implicitly — if observation phase shows it's a gap, add a producer row there too with the same `CLI_AUDIT_ACTOR` constant.
- **No new producer-side migration.** The `audit_log` GRANT shape from migration 0002 already allows `INSERT` from `kastellan_runtime`; the CLI binary uses the same runtime pool via `connect_runtime_pool`, so the audit write inherits the existing append-only contract.
...
- `core/src/bin/kastellan-cli.rs` — both `mark_cancelled` call sites wired to `cancel_and_audit`.
...
- **`core::memory::recall.rs`:**
  ...
  * `use kastellan_db::graph::Graph;` added at top of file (trait import required for `.neighbors()` to resolve on `PgGraph`).
...
- `core/src/memory/recall.rs` — `RecallModes::graph` field, `RecallModes::GRAPH_ONLY`, `RecallParams::seed_entity_ids`, `GRAPH_FANOUT_CAP_PER_SEED`, graph lane execution body, `use kastellan_db::graph::Graph` import. 4 new unit tests; 4 existing unit tests updated.
...
## Recently completed (previous session, 2026-05-12 — issue #15 hoist: shared `kastellan-tests-common` dev-dep crate, merged via PR #38 at `97f2743`)

Branch: `refactor/tests-common-hoist`, merged into `main` at `97f2743`. Closes [issue #15](https://github.com/hherb/kastellan/issues/15). Post-merge review-nits fixup at `066927e` (in the merged history); two further deferred items filed as [issue #39](https://github.com/hherb/kastellan/issues/39).
...
- **NEW `tests-common/` (`kastellan-tests-common`, member #9 in the workspace):** `publish = false`, dev-dep only. `Cargo.toml` depends on `kastellan-db` + `kastellan-supervisor` + `kastellan-sandbox` (the runtime crates whose surfaces it composes) plus `serde_json` + `sha2`. Crate doc-comment enumerates the module layout. **Public surface (re-exported at the crate root for ergonomic single-line `use` blocks):**
  ...
  * `temp.rs`: `unique_suffix() -> String` (pid+nanos), `unique_temp_root(label) -> PathBuf` (`$TMP/kastellan-<label>-<pid>-<nanos>`), `current_username() -> String` (`$USER` → `whoami` → `"kastellan"`).
  ...
  * `embedding.rs`: `text_to_embedding(text) -> Vec<f32>` deterministic SHA-256-seeded L2-normalised vector of length `kastellan_db::memories::EMBEDDING_DIM`.

- **8 test files migrated** (each one's local helpers replaced with `use kastellan_tests_common::{...}`; bring-up bodies compressed to one `bring_up_pg_cluster(...)` call returning a `PgCluster`):
...
- **`db/Cargo.toml`** + **`core/Cargo.toml`** gained `kastellan-tests-common = { path = "../tests-common" }` under `[dev-dependencies]`.
...
- `db/Cargo.toml` — `[dev-dependencies]` += `kastellan-tests-common`.
- `core/Cargo.toml` — `[dev-dependencies]` += `kastellan-tests-common`, removed `sha2`.
...
- **`core/src/main.rs:51`:** the existing `if let Err(e) = kastellan_db::tasks::sweep_crashed(&pool)` direct call replaced by `match kastellan_core::scheduler::crash_recovery::sweep_and_audit(&pool).await { Ok(0) => {}, Ok(n) => info!(crashed_tasks = n, …), Err(e) => warn!(…) }` — successful sweep logs a count, error stays non-fatal.
...
Branch: `refactor/split-core-memory` (off `main` at `d39023b`). Closes [issue #30](https://github.com/hherb/kastellan/issues/30).

**Why this slice now.** The Option O slice (shipped earlier today, merged via PR #29) grew `core/src/memory.rs` from 489 LOC to 602 LOC — 102 over the 500-LOC soft cap in CLAUDE.md. The file had two natural halves: pure retrieval (`recall` + `reciprocal_rank_fusion` + `RecallModes` + `RecallParams` + `RRF_K_CONSTANT` + `LANE_FANOUT`) which has zero dependencies beyond `kastellan-db`, and the LLM-router-touching helper (`embed_query` + `MemoryError` + `build_embed_audit_payload`) which depends on `kastellan-llm-router` + the audit module. Splitting them tightens the dependency surface of each file and keeps both well under the cap, with no behaviour change and no public-API change.
...
**API surface preserved.** The two integration tests that import the module (`core/tests/memory_recall_e2e.rs` and `core/tests/embedding_recall_e2e.rs`) needed zero changes — they use the flat `kastellan_core::memory::{recall, RecallModes, RecallParams, embed_query, MemoryError}` paths, which the `mod.rs` re-exports satisfy.

**Visibility tightening.** `build_embed_audit_payload` went from `pub(crate)` to module-private (no `pub` keyword at all). Pre-split, the rest of the `kastellan_core` crate *could* have called it; post-split, only `embed.rs` and its tests can. The audit + dispatcher chokepoint pattern in HANDOVER and CLAUDE.md says payload builders are internal helpers — the new visibility makes that contract structural rather than conventional.
...
- No public-surface change. `embed_query`, `recall`, `MemoryError`, `RecallModes`, `RecallParams`, `reciprocal_rank_fusion`, `RRF_K_CONSTANT` all reachable at the same `kastellan_core::memory::{...}` paths.
...
- `llm-router/src/config.rs` — `embedding_url`/`embedding_model` fields + `KASTELLAN_LLM_EMBEDDING_URL`/`KASTELLAN_LLM_EMBEDDING_MODEL` env vars
...
- **`ask_subprocess_completes_planned_task_end_to_end` (happy path):**
  ...
  * Bring up the real `kastellan` daemon under `systemd --user` (Linux) / `launchctl` (macOS) with env wiring: `KASTELLAN_DATA_DIR`, `KASTELLAN_STATE_DIR`, `KASTELLAN_PROMPTS_DIR` → workspace `prompts/`, `KASTELLAN_LLM_LOCAL_URL` → mock `/v1`, `KASTELLAN_LLM_LOCAL_MODEL` → `test-local-model`, `KASTELLAN_LLM_TIMEOUT_MS=5000`, `KASTELLAN_SHELL_EXEC_BIN` → workspace `kastellan-worker-shell-exec`, `KASTELLAN_SHELL_EXEC_ALLOWLIST` → `ECHO_PATH` (per-OS).
  * Wait for the daemon's `"scheduler spawned"` log line (signals scheduler ready to claim).
  * Spawn the real `kastellan-cli ask "say <marker>"` subprocess via `std::process::Command::output()`.
...
- **`ask_subprocess_fails_after_plan_iteration_cap` (failure path):**
  ...
  * CLI's `ask_async` (`kastellan-cli.rs:319-322`) sees `state != "completed"`, prints `"ask: task ended in state 'failed'"` to stderr, and exits 1.
...
**Five-runs determinism check.** `for i in 1 2 3 4 5; do cargo test -p kastellan-core --test cli_ask_e2e; done` passed clean: ~5.4 s per run, both tests green every time, zero `[SKIP]` lines.
...
- **New module `core/src/scheduler/tool_dispatch.rs` (~330 lines + 13 unit tests):** ownership of the production dispatcher moved out of `runner.rs` into its own file. Contains:
  ...
  * `pub fn shell_exec_entry(binary, allowlist) -> ToolEntry` — canonical recipe for the shell-exec worker: `Net::Deny`, `Profile::WorkerStrict`, `cpu_ms = 5_000`, `mem_mb = 256`, `wall_clock_ms = Some(30_000)`, `KASTELLAN_SHELL_ALLOWLIST` env carrying the argv allowlist.
...
- **`core/src/main.rs` rewiring:**
  * New helper `build_tool_registry()` reads `KASTELLAN_SHELL_EXEC_BIN` and `KASTELLAN_SHELL_EXEC_ALLOWLIST` (colon-separated) from env. If `KASTELLAN_SHELL_EXEC_BIN` is unset or the binary doesn't exist, shell-exec is simply *not registered* — plans that name it will fall through to `UNKNOWN_TOOL`. **Deny-by-default**: empty/unset `KASTELLAN_SHELL_EXEC_ALLOWLIST` means no programs are allowlisted, every shell-exec step returns `POLICY_DENIED`. The daemon admin opts programs in explicitly. This is the same posture used in the Phase 3 egress proxy plan.
  * Workspace-root computation removed entirely. `Workspace::new` reads `KASTELLAN_WORKSPACE_ROOT` directly, so the env seam still exists; nothing in the scheduler currently uses per-step workspaces. When a tool that needs writable scratch lands, the `Workspace` integration will go *inside* `dispatch_step` (or its trait sig will grow `task_id`).
...
**Why deny-by-default for shell-exec allowlist.** The planner LLM supplies `step.parameters` (the argv); if the host-side allowlist came from the LLM-supplied params, a prompt-injected channel would directly control which programs ran inside the jail — defeating the whole point of the allowlist. The allowlist must come from a source the LLM cannot influence: daemon-admin env vars. Empty allowlist + worker-side `POLICY_DENIED` is the safest starting position; operators opt programs in by setting `KASTELLAN_SHELL_EXEC_ALLOWLIST=/usr/bin/echo:/bin/cat:...` at daemon start.
...
- `build_tool_registry` now filters empty entries out of the colon-split `KASTELLAN_SHELL_EXEC_ALLOWLIST`. An operator typo like `:` or `/usr/bin/echo::/bin/echo` was silently shipping an empty argv[0] to the worker, surfacing as a less-obvious `POLICY_DENIED` at a different layer than the misconfiguration.
...
- `kastellan-cli tasks list`: char-based truncation (was `&instr[..60]` — UTF-8 panic on multi-byte input); rejects unknown flags consistently with `run_ask`; replaced `std::process::exit(2)` with `ExitCode::from(2)` to keep the pool-drop path correct.
- `kastellan-cli tasks tail`: JSON-aware filter (was substring-matching `"task_id":N` which false-positives on `parent_task_id`). Pure `line_matches_task` helper with unit tests.
- `core::scheduler::runner`: `max_plans` payload override uses `try_into::<u32>()` so a producer-supplied 2^33 doesn't roll over.
- `core::scheduler::runner`: `ToolHostStepDispatcher` placeholder logs at `tracing::error!` before returning `NOT_IMPLEMENTED` — operators running `kastellan-cli ask` today get pointed at Task 3.2.bis from the journal.
...
- `supervisor_e2e`: sets `KASTELLAN_PROMPTS_DIR` pointing at the workspace `prompts/` so the daemon under systemd doesn't fail prompt-load on a `prompts/` cwd-relative miss.
- `prompts/agent_planner.md`: documents the JSON input shape the inner loop sends each iteration.

Five follow-up issues filed: [#20](https://github.com/hherb/kastellan/issues/20), [#21](https://github.com/hherb/kastellan/issues/21), [#22](https://github.com/hherb/kastellan/issues/22), [#23](https://github.com/hherb/kastellan/issues/23), [#24](https://github.com/hherb/kastellan/issues/24).

### Mock-HTTP coverage for `RouterAgent::formulate_plan` (PR #26, merged 2026-05-11)

Branch `fix/router-agent-mock-http-tests`. Commits `2e2657c` (initial) + `44d42c3` (review nits). Closes [#22](https://github.com/hherb/kastellan/issues/22).
...
- **`kastellan-cli` subcommands:** `ask` (LISTEN-before-INSERT for completion, ctrl-C cancel), `tasks list`, `tasks status`, `tasks cancel`, `tasks fail`, `tasks tail`.
...
- **Two design discussions filed as GitHub issues:**
  * **Issue #16** — `tool_host`: `WorkerCommand` seal has an in-crate hole. Sibling modules inside `kastellan_core` can construct one and reach `SupervisedWorker::call` directly. Three candidate fixes filed.
...
The last application-layer plumbing required before Phase 1: every future model call goes through `kastellan_llm_router::Router::send(&ChatRequest) -> Result<ChatResponse, RouterError>`.

- **New top-level workspace crate `llm-router` (`kastellan-llm-router`, member #3):** ~960 lines + ~340 lines integration test, 32 tests (28 unit + 4 integration). The user explicitly chose the new-crate boundary (vs `core::llm_router`) because the router is a self-contained subsystem with a stable typed surface and the Phase-5 grow-out adds a real policy gate that will read state from `db::secrets`, emit telemetry, and gain its own integration test surface.
- **Modules:** `messages.rs` (OpenAI-compatible wire shapes; `ChatRole` is closed enum with serde lowercase; `skip_serializing_if = Option::is_none` on optional fields so older llama.cpp builds don't reject `null`); `backend.rs` (`Backend::{Local, Frontier}` closed enum with `as_tag()` for audit-log payloads); `config.rs` (`RouterConfig::from_env` reads `KASTELLAN_LLM_*`; per-OS default URL — Linux vLLM/SGLang :8000, macOS Ollama :11434; **API keys NOT read from env** by design, they belong in `db::secrets`); `policy.rs` (`PolicyGate` trait + `DefaultLocalPolicy`); `error.rs` (truncated body capture at 1 KiB); `lib.rs` (`Router::new` + `Router::with_policy`; `Router::send` calls `policy.pick(&request)` then dispatches or returns `PolicyDeniedFrontier`).
...
Every Phase 0+ tool call now goes through a single `tool_host::dispatch` chokepoint that writes one `audit_log` row per call. A long-lived `audit_mirror` task replicates committed rows to `~/.local/state/kastellan/audit-YYYY-MM-DD.jsonl` with fsync per write and daily UTC rotation; `kastellan-cli audit tail` reads those files with no DB connection.
...
- **`db/src/pool.rs` (~110 lines):** `connect_runtime_pool(spec)` opens a `PgPool` with `PgPoolOptions::after_connect` running `set_role_runtime_statement()` on every dialed connection. Closes [issue #11](https://github.com/hherb/kastellan/issues/11) ahead of schedule. Defaults: `max_connections = 4`, `acquire_timeout = 10 s`, `idle_timeout = 5 min`.
...
- **`core/src/bin/kastellan-cli.rs` (~140 lines):** new operator CLI binary. Today: `kastellan-cli audit tail [--from-start] [--no-follow] [--state-dir PATH]`. Hand-rolled argv (no `clap` dep). State-dir resolution: `--state-dir` → `$KASTELLAN_STATE_DIR` → `$HOME/.local/state/kastellan`.
- **`core/src/main.rs` rewrite:** after `probe::run`, daemon now calls `connect_runtime_pool` (fail-closed) and `spawn_mirror` (best-effort). On SIGTERM/SIGINT, shuts down mirror *before* closing the pool so the mirror's final `sync_all` observes an alive pool. New env-var seam `KASTELLAN_STATE_DIR` (parallel to `KASTELLAN_DATA_DIR`).
...
- **`db/migrations/0002_runtime_role.sql` (~140 lines):** creates `kastellan_runtime` with `NOSUPERUSER NOCREATEROLE NOCREATEDB NOLOGIN NOINHERIT`, grants the OS user membership via `EXECUTE format('GRANT kastellan_runtime TO %I', current_user)`, then carves the GRANT/REVOKE shape: `GRANT SELECT, INSERT ON audit_log` paired with `REVOKE UPDATE, DELETE, TRUNCATE`. Other five tables get bulk `GRANT SELECT, INSERT, UPDATE, DELETE`. Sequences get explicit `GRANT USAGE`. `ALTER DEFAULT PRIVILEGES` covers future migrations' tables. `CREATE ROLE` wrapped in `DO $$ IF NOT EXISTS … END $$` (Postgres has no `CREATE ROLE IF NOT EXISTS`).
- **`db/src/conn.rs` additions:** `pub const RUNTIME_ROLE: &str = "kastellan_runtime"` and `pub fn set_role_runtime_statement() -> String` returning `SET ROLE "kastellan_runtime"` (identifier-quoted via existing `quote_ident`).
...
- **`core::main::bring_up_database`:** wired into `main.rs` before `wait_for_shutdown`. Reads `KASTELLAN_DATA_DIR` env (test override; production uses `default_data_dir()`), constructs `ConnectSpec` from `$USER`, calls `probe::run` with `actor="core" action="startup"`.
...
- **`core/tests/supervisor_e2e.rs` rewrite:** test renamed to `core_starts_runs_db_probe_writes_audit_row_and_shuts_down_cleanly`. Brings up a per-test PG cluster before installing the `kastellan` core service. Forwards `KASTELLAN_DATA_DIR` and `USER` via `spec.env`.
...
**Why the probe lives in `kastellan-db` rather than `kastellan-core`:** the probe's logic (connect → ensure DB → migrate → audit row) is pure database orchestration with zero `core`-specific shape. Future memory worker (Phase 1) can call the same function for its own bring-up without dragging core in.

**Why peer auth, role = OS user, application DB = `kastellan`:** smallest containment story. Peer auth on a UDS → remote auth structurally impossible. Role = OS user → different OS users on the same host literally cannot connect. Application DB = `kastellan` keeps `postgres`/`template0`/`template1` for maintenance.
...
- **2026-05-08 — Linux supervisor scaffold (`kastellan-supervisor::systemd_user`):** pure `build_unit_file(spec)` + `validate_service_name`, `SystemdUser` driver with atomic write (write-to-tmp + fsync + rename), `daemon-reload` only for canonical dir, `probe()` via `systemctl --user show-environment`. 27 unit + 2 smoke tests. Test count 67 → 96.
...
- **2026-05-06 — Phase 0 hardening stage 1:** new `workers/prelude` crate (Linux-only Landlock + seccomp lock_down with `serve_stdio` drop-in around `kastellan_protocol::server::serve_stdio`). `core::tool_host::derive_lockdown_env()` injects `KASTELLAN_LANDLOCK_RW` + `KASTELLAN_SECCOMP_PROFILE`. **bwrap probe bug fix:** `LinuxBwrap::probe()` was launching `bwrap` without the `/lib*` symlinks so `execvp /usr/bin/true` returned ENOENT → probe failed-closed → integration tests `[SKIP]`'d silently → previous handover's "0 skipped" was wrong.
...
- ~~**Rule-iteration harness — Slice B (the harness itself)**~~ **Shipped earlier this session 2026-05-15** on branch `feat/rule-iteration-harness`, merged via PR #65 at `9c01e30`. New pure-Rust library `core::observation::replay` + `kastellan-cli observation replay` subcommand. See "Recently completed (earlier this session)" entry above.
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    4. **Operator recapture against the current daemon** (operator action) — one-time `cargo test -p kastellan-core --test observation_capture -- --ignored --nocapture` against the local LLM. Turns the pre-Slice-A capture JSONs into rule-iteration-harness-replayable inputs.
    ...
    8. ~~**[Issue #90](https://github.com/hherb/kastellan/issues/90) — `upsert_entities_and_relations` per-entity round-trip reduction**~~ **SHIPPED 2026-05-20** on branch `feat/issue-90-upsert-roundtrip-layer-a` — Layer A (`xmax = 0` discriminator + no-op `SET name_norm = entities.name_norm` to force RETURNING on conflict). Workspace 881 → **883** (+2). Audit-row contract turned out to need no update — the public `UpsertOutcome` shape stayed byte-identical, so `build_extract_entities_payload`'s 8-key contract was preserved verbatim. Layer B (full-batch unnest) and triple batching deferred per brainstorming. See "Recently completed (this session)" entry above.
    ...
    25. ~~**First idle-timeout worker — GLiNER-Relex Slice 2.5 — Containerfile + macOS image build**~~ **SHIPPED 2026-05-25, merged to `main` via PR [#118](https://github.com/hherb/kastellan/pull/118) at `a9e3385`** + post-merge `e11e3f3` (macOS test-infra hardening + container e2e empirically verified at 12.58s) + post-merge `1c7eced` (3 review-comment cleanups + 3 deferred follow-ups filed as issues [#120](https://github.com/hherb/kastellan/issues/120) / [#121](https://github.com/hherb/kastellan/issues/121) / [#122](https://github.com/hherb/kastellan/issues/122)). Closes [#107](https://github.com/hherb/kastellan/issues/107). 18 + 2 commits; operator built `kastellan/gliner-relex:dev` image via the new `scripts/workers/gliner-relex/build-image.sh`. New `workers/gliner-relex/Containerfile` (python:3.12-slim + `uv pip install --system .` + `USER nobody` + `ENTRYPOINT`); `gliner_relex_entry` branches on new `GlinerRelexEnv.use_container_backend` field driven by `KASTELLAN_GLINER_RELEX_USE_CONTAINER=1`; `SandboxBackends::resolve` widens to take `image: Option<&str>`; `ToolEntry.container_image: Option<String>` new field; `--init` always-on in `build_container_argv` (closes #107). Workspace **998 → 1011 (+13) macOS**; Python 36 passed. Empirical verification done at session-time: PASSED in 12.58s through Apple `container` on this Mac. See "Recently completed (this session, 2026-05-25)" entry at the top for the full breakdown.
    26. ~~**`core/src/workers/gliner_relex.rs` (1547 LOC) test-module lift**~~ **SHIPPED this session 2026-05-25, merged to `main` via PR [#123](https://github.com/hherb/kastellan/pull/123) at `920e0c9`**. Mirror of Item 23(a). Pure mechanical lift: `core/src/workers/gliner_relex.rs` 1547 → **811 LOC** (−736) via Rust 2018 sibling-directory module resolution; new `core/src/workers/gliner_relex/tests.rs` 745 LOC (9-line module-doc-comment header + dedented body). One post-review doc-trim commit (`c4dd3ee`) dropped historical commentary from the new `tests.rs` module header. Production-code path stays at the existing parent file. Existing tests are the regression pin — workspace test count unchanged at macOS **1011 / 0 / 3** (verified post-merge on main at `920e0c9`). Parent file at 811 LOC still over the 500-LOC cap (production-code-only residual; deferred until a second logical seam appears in the Extractor + Client + wire-type cluster). See "Recently completed (this session, Item 26)" entry at the top for full breakdown.
    27. ~~**Slice 2.5 follow-up triple** (engineering, small bundle)~~ **SHIPPED this session 2026-05-25, merged to `main` via PR [#124](https://github.com/hherb/kastellan/pull/124) at `e93997e`** + same-PR post-review fixup `55bb8f1`. Three commits + 1 fixup closing all three issues filed during PR #118 post-merge review:
        - `07de743` — closes [#121](https://github.com/hherb/kastellan/issues/121) — `slot_for_key_excludes_container_image` invariant test in `core/src/worker_lifecycle/idle_timeout.rs::tests`. Calls `slot_for(&registry, "twice-name")` twice and asserts `Arc::ptr_eq` — proves the warm-cache key carries `tool_name` only. Failure message references the live-reconfigure scenario explicitly.
        - `9cb375e` — closes [#120](https://github.com/hherb/kastellan/issues/120) — `MacosContainer::probe_image(tag)` via `container image inspect`; pure `build_image_inspect_argv` helper; new `core/src/sandbox_health` module with cross-platform `collect_container_image_targets` + macOS-gated `probe_registered_container_images`; daemon `main.rs` walks registered Container-backed `ToolEntry` image tags once at startup, `tracing::warn!` on miss but daemon continues bring-up. +10 tests.
        - `a569885` — closes [#122](https://github.com/hherb/kastellan/issues/122) — `skip_if_image_missing` in `core/tests/gliner_relex_e2e.rs` rewrites from `image list` substring-parsing to `MacosContainer::probe_image` delegation. The chosen mechanism (switch from `list` to `inspect`) supersedes the issue body's proposed tokenization fix because it removes the substring-matching surface entirely.
    ...
    28. ~~**Issue [#95](https://github.com/hherb/kastellan/issues/95) — entity-upsert Layer B (full-batch unnest + per-row attribution fallback)**~~ **SHIPPED 2026-05-26, merged to `main` via PR [#125](https://github.com/hherb/kastellan/pull/125) at `dac0dcd`** + same-PR post-review fix `a1dbef6` (six review follow-ups; +1 unit pinning `is_constraint_violation_code` 5-char SQLSTATE guard, +1 e2e pinning empty-input early-return; `dedup_entity_inputs` returns parallel `Vec<String>` of name_norms; phase-2 partial-commit semantics documented). 12 implementation commits TDD-ordered: 2 docs (spec + plan) + 4 pure-helper slices + 6 integration slices. New module `core/src/entity_extraction/batch_upsert.rs` (~550 LOC) holds both phases + 5 pure helpers; `gliner_relex.rs` 625 → 571 LOC with public function becoming a 1-line delegate; legacy per-row relation loop deleted. **Workspace 1023 → 1042 (+19, +11 unit / +8 integration) on macOS**, all green. Empirically verified against real PostgreSQL (Postgres.app v18) via session-local override; override reverted before each commit. Both fallback tests fire the per-row attribution chain correctly — error messages contain `kind='drug', name_norm='aspirin'` (entity FK) and `src=N, dst=M, kind='eats'` (relation FK), proving the attribution improvement over Layer A's `format!("upsert entity: {e}")` which doesn't identify the failing row. Public API (`UpsertOutcome`, `EntityExtractionError`, 8-key audit payload) byte-frozen. Two review items deferred per the PR description: `batch_upsert.rs` 547 LOC over the 500-LOC cap by inline tests (sibling-lift candidate, Item 29(a) below); single-quote ambiguity in error formatters when an entity name itself contains `'` (not load-bearing, defer until a PII-safe quote shape emerges). See "Recently completed (this session, Item 28)" entry at the top for full breakdown.
    29. **Operator picks (bucket).** Sub-bullet status:
        - ~~**`batch_upsert.rs` 547 LOC sibling-`tests.rs` lift + bundled quadruple** (engineering, ~1.5 hours)~~ **SHIPPED 2026-05-27, merged to `main` via PR [#132](https://github.com/hherb/kastellan/pull/132) at `162f71f`**. Pure mechanical refactor: 4 test-module sibling lifts in one PR. `core/src/entity_extraction/batch_upsert.rs` 589 → **428 LOC** (under cap; closes the Item 28 / Issue #95 Layer B overage); `core/src/worker_lifecycle/idle_timeout.rs` 581 → **415 LOC** (under cap); `core/src/worker_lifecycle/manager.rs` 565 → **339 LOC** (under cap; closes the Slice 2.5 routing-pin overage); `db/src/lib.rs` 912 → **525 LOC** (25 LOC over cap — production-code residual). Net **-940 LOC** off the 4 production parent files; 945 LOC of inline test code moved verbatim to 4 new sibling `tests.rs` modules (58 tests). `crate::env_lock` deliberately stays at crate root (shared with `conn::tests`). Workspace **1048 / 0 / 3 macOS unchanged** from pre-lift baseline. No new clippy warnings. See "Recently completed (this session, 2026-05-27 — test-module lift quadruple)" entry at the top for the full breakdown.
        - ~~**`KASTELLAN_PG_BIN_DIR=<path>` env-var override**~~ **SHIPPED 2026-05-26, merged to `main` via PR [#126](https://github.com/hherb/kastellan/pull/126) at `7adc582`** + post-merge polish `463553d` (compute capacity from `defaults.len()` instead of `Vec::with_capacity(17)` magic number) + filed issue [#127](https://github.com/hherb/kastellan/issues/127) (env-var save/restore RAII helper for the 4 unit tests). New public `kastellan_db::pg_bin_dir_candidates_with_env_override()` + `PG_BIN_DIR_ENV` const; 6 test-fixture sites switched (`tests_common::skip::pg_bin_dir_or_skip` + 5 byte-identical local copies in `core/tests/scheduler_*_e2e.rs` + `core/tests/agent_prompts_e2e.rs`). Production runtime discovery (`kastellan-db-init`) unchanged — helper documented test-infra-only. Workspace **1042 → 1046 (+4) macOS**, all green. See "Recently completed (this session, Item 29)" entry at the top for full breakdown. **Follow-up tech-debt** that this slice exposed but deliberately did not touch: (a) consolidate the 5 local-copy `pg_bin_dir_or_skip` definitions onto the shared `tests_common::pg_bin_dir_or_skip` (~30 LOC removal); (b) lift the 15-second `wait_for_status_active` polling cap in the 6 sites with local `bring_up_pg_cluster` copies that still overshoot on Postgres.app (operator memory `postgres-app-bin-paths.md` documents the 12 expected test-failures the env override unmasks).
        - **`KASTELLAN_GLINER_RELEX_REQUIRE_E2E=1` CI env knob** (test-infra, small) to turn the container e2e's skip-as-pass into a hard fail for any future CI runner with PG + container + image + weights all staged.
        - ~~**`bring_up_pg_cluster` polling-cap lift** (test-infra, ~1 hour)~~ **SHIPPED 2026-05-26, merged to `main` via PR [#129](https://github.com/hherb/kastellan/pull/129) at `4e94e42` on 2026-05-27** + same-PR post-review doc polish `f906d18` (inline `// IMPORTANT: do not reorder fields` comment above `PgCluster` + clarifies `service_name.len() <= 200` cap as launchd label + systemd unit-file basename limit, *not* `sun_path`; no behaviour change). 2 substantive commits TDD-ordered: widen `tests_common::bring_up_pg_cluster` polling cap from 15s → 30s via new `PG_BRING_UP_TIMEOUT_SECS = 30` named const (`02d6691`) + migrate 5 local-copy sites onto the shared helper (`0c7b644` — `core/tests/{agent_prompts,scheduler_lanes,scheduler_step_dispatch,scheduler_inner_loop,scheduler_crash_recovery}_e2e.rs`, net **-1062 LOC** across 5 files). Empirically: each of 13 previously-failing tests on this Mac now passes when run individually with `KASTELLAN_PG_BIN_DIR` pointed at Postgres.app v18 (was: `pg active: timeout 15s; last=Inactive` for 12-13 tests). Workspace **1048 / 0 / 3 macOS** unchanged from pre-slice (no behaviour change on hosts where env var is unset). Two new follow-up issues filed during PR #129 review: **[#130](https://github.com/hherb/kastellan/issues/130)** parallel-launchd bring-up contention under `KASTELLAN_PG_BIN_DIR` override (replaces the (b) item below — now formally tracked); **[#131](https://github.com/hherb/kastellan/issues/131)** optional per-call timeout override for `bring_up_pg_cluster` (heterogeneous launchd vs Homebrew bring-up timing). Review item #4 (two supervisor handles in `PgCluster.sup` + `ServiceGuard`) deferred to existing issue #39. Three follow-up tech-debt items kept out of scope per slice discipline: (a) `skip_if_no_supervisor` / `pg_bin_dir_or_skip` / `unique_suffix` local-copy consolidation in the 5 migrated files (~30 LOC removal, separate code-dedup slice); (b) parallel-launchd-bring-up contention under full `cargo test --workspace` with the override — **now tracked as [#130](https://github.com/hherb/kastellan/issues/130)**; (c) `observation_capture.rs`'s `Duration::from_secs(15)` line waits for the *core daemon's* log, not PG-Active (different concern). See "Recently completed (this session, 2026-05-26 → 2026-05-27 — `bring_up_pg_cluster` polling-cap lift)" entry at the top for full breakdown.
        - ~~**Issues [#114](https://github.com/hherb/kastellan/issues/114) + [#115](https://github.com/hherb/kastellan/issues/115)** — `walk_*_edges` diamond-graph duplicate edges + collapse outbound/inbound walks into single UNION ALL query~~ **SHIPPED this session 2026-05-26** on branch `fix/issues-114-115-walk-edges-dedupe-and-combine`; **merged to `main` via PR [#128](https://github.com/hherb/kastellan/pull/128) at `bb32cab`** + post-merge polish `978b5be` (named const `WALK_EDGES_AROUND_DIRECTION_COL` for the trailing partition column index + SQL comment on the outer `ORDER BY direction ASC` lex-order incidental + test header pin documenting that `per_direction_limit` shares semantics with each per-direction method's `limit`; no behaviour change). 4 commits TDD-ordered: dedupe via `DISTINCT ON (edge_id) ORDER BY edge_id, depth ASC` intermediate CTE in both per-direction walks (`668a36f`, closes #114) + new `walk_edges_around` + `EdgesAround` struct + smoke integration test (`c25bf96`) + CLI migration to single round-trip (`c12774a`) + doc-comment hint on per-direction methods (`0bb65b1`). Per-direction methods retained as stable surface for single-direction callers. Workspace **1046 → 1048 (+2) on macOS**, all green. See "Recently completed (this session, Issues #114 + #115)" entry at the top for full breakdown.
        - ~~**Issue [#89](https://github.com/hherb/kastellan/issues/89)** — sandbox `/tmp is per-spawn ephemeral tmpfs` invariant test.~~ **SHIPPED 2026-05-28, merged to `main` via PR [#139](https://github.com/hherb/kastellan/pull/139) at `504094e`** on branch `fix/issue-89-tmpfs-per-spawn-invariant-test`. One new Linux integration test (`tmp_is_per_spawn_ephemeral_tmpfs_under_worker_strict`) appended to `sandbox/tests/linux_smoke.rs` (222 → 311 LOC). Two real-bwrap spawns under `Profile::WorkerStrict` pin three properties of the `--tmpfs /tmp` mount: ephemerality (spawn A's marker not visible in spawn B), writability (spawn B exits 0 — no EROFS), listing-is-real (spawn B's own marker visible in its own `ls /tmp`). `/bin/sh -c '...'` is used so the test resolves via bwrap's existing `--symlink usr/bin /bin` mapping without depending on host usr-merge. `skip_if_no_userns` discipline preserved. **No production-code changes** — pure test addition. Verification on macOS: workspace 1065/0/3 unchanged (test cfg-gated to Linux); cross-target `cargo check --tests -p kastellan-sandbox --target aarch64-unknown-linux-gnu` clean. **DGX verification deferred to operator** (two steps: pass-on-current-code + catches-drift via temporary `--tmpfs` → `--bind-try` mutation in `linux_bwrap::build_argv`). See "Recently completed (this session, Issue #89)" entry at the top for full breakdown.
        - ~~**Issue [#86](https://github.com/hherb/kastellan/issues/86)** / **[#85](https://github.com/hherb/kastellan/issues/85)** / **[#84](https://github.com/hherb/kastellan/issues/84)** — worker_lifecycle hardening trio.~~ **SHIPPED this session 2026-05-27, merged to `main` via PR [#135](https://github.com/hherb/kastellan/pull/135) at `7f98ee4`** + in-PR polish `1fc7eca` (5-issue review pass; filed [#136](https://github.com/hherb/kastellan/issues/136) for the deferred warn-debounce cooldown). 4 commits + polish: 1 doc-sync to main = `8655319` + 1 feature for #86 (`#[non_exhaustive]` on `Lifecycle::IdleTimeout` variant blocks out-of-crate struct-literal construction; pinned by `compile_fail,E0639` doctest) + 1 feature for #85 (`ToolState.idle_teardown_handle: Option<JoinHandle<()>>` + `abort_idle_teardown_handle` + `replace_idle_teardown_handle` helpers — abort-and-respawn pattern keeps exactly one teardown task per slot at steady state instead of accumulating ~`idle_seconds` stale sleepers per tool under high request rate) + 1 feature for #84 (`ToolSlot.pending_acquires: AtomicU32` + `PendingAcquireGuard` RAII bracket on lock-wait + threshold `PENDING_ACQUIRES_WARN_THRESHOLD = 5` + `tracing::warn!` when queue depth crosses the boundary + sync `_test_slot_pending_acquires` inspector) + 1 in-PR polish (`1fc7eca`: success-path `slot: None` abort symmetry + `PendingAcquireGuard::depth()` doc tightening for the concurrent case + `_test_slot_pending_acquires` doc reframing + test-isolation fix on the two `replace_idle_teardown_handle_*` tests + #136 filed for warn-debounce). Workspace **1050 → 1058 (+8) on macOS**, all green; 1058 → 1058 after polish (pure correctness/clarity). Linux DGX expected at +8 by the same argument. No new clippy warnings. **Deliberately NOT done:** (a) issue #84's third "Consider..." remedy — soft timeout on lock acquisition that surfaces `StepOutcome::Err { code: 'BUSY' }` — would inflate from 0.5 to 1.5 sessions (new `ToolHostError::Busy` variant + per-tool timeout config on `ToolEntry` + plumbing through the dispatcher + a distinct audit-row code). Observability ships now; BUSY rejection waits for empirical evidence of queue starvation. (b) Warn-debounce cooldown — tracked as new issue [#136](https://github.com/hherb/kastellan/issues/136) (~1-LOC field + 1 helper + 1 test, blocked only on picking the right cooldown duration). See "Recently completed (this session, worker_lifecycle hardening trio)" entry at the top for the full breakdown.
        - **Issue [#130](https://github.com/hherb/kastellan/issues/130)** (filed 2026-05-27 from PR #129 review) — parallel-launchd bring-up contention under `KASTELLAN_PG_BIN_DIR` override. Under the env-var override pointing at Postgres.app, healthy clusters bring up in ~1 s individually but **5+ tests racing for macOS launchd registration in parallel under full `cargo test --workspace`** can exceed even the new 30 s cap. Observed during PR #129 verification: 4 tests in `scheduler_inner_loop_e2e` + 2 in `embedding_recall_e2e` failed under parallel run with override, every one passes alone. Orthogonal to the per-test polling cap (already widened in PR #129). Two candidate shapes: (a) a global serial-mutex around PG bring-up in `tests_common::serial` (acquired inside `bring_up_pg_cluster` before the launchd-register call, released after `wait_for_status_active`); (b) opt-in `--test-threads=1` for the override path only. Operator picks the shape.
        - ~~**Issue [#131](https://github.com/hherb/kastellan/issues/131)** — optional per-call timeout override for `bring_up_pg_cluster` (small, ~30 LOC + 2 unit tests)~~ **SHIPPED 2026-05-27, merged to `main` via PR [#133](https://github.com/hherb/kastellan/pull/133) at `8655319`** + post-merge nit `876fb42`. Chose the sibling-function shape over the `Option<Duration>` parameter on the existing function: new `pub fn bring_up_pg_cluster_with_timeout(bin_dir, data_label, log_label, service_name, timeout: Duration) -> PgCluster` next to the existing 4-arg `bring_up_pg_cluster` which becomes a 1-line wrapper forwarding `Duration::from_secs(PG_BRING_UP_TIMEOUT_SECS)`. Matches the `pg_bin_dir_candidates_with_env_override` precedent from PR #126; avoids touching 50+ existing call sites. +2 signature-pin unit tests catch any future drop/rename/reorder of either parameter list. Workspace **1048 → 1050 (+2) on macOS**, all green. See "Recently completed (this session, 2026-05-27 — Issue #131)" entry at the top for full breakdown. **Follow-up tech-debt deliberately deferred:** the issue's framing mentioned "a test that knows it's running against Homebrew" could keep the tighter 15 s bound — this slice ships the surface; the demonstrative caller (Homebrew-detection helper, or env-var caller hint) is a separate slice.
        - ~~**`idle_timeout/release.rs` sibling-lift** (refactor, ~1.5h, the natural follow-up flagged at end of Issue #136)~~ **SHIPPED this session 2026-05-28** on branch `refactor/idle-timeout-release-sibling-lift`, **merged to `main` via PR [#138](https://github.com/hherb/kastellan/pull/138) at `5fc5fee`** + in-PR polish `78305cf` (banner-rot cleanup in `idle_timeout.rs` + `replace_idle_teardown_handle` visibility narrowed `pub(crate)` → `pub(super)`; defensive `None`-slot branch flagged but deferred as pre-existing). Pure mechanical refactor lifting `release_idle_timeout_worker` + `abort_idle_teardown_handle` + `replace_idle_teardown_handle` into new `core/src/worker_lifecycle/idle_timeout/release.rs`. File-size delta: `idle_timeout.rs` 647 → **490 LOC** (now 10 under the 500-LOC cap, closes the 147-LOC overage filed by Issue #136); new `release.rs` **195 LOC** (well under cap); `idle_timeout/tests.rs` 467 → 472 LOC (+5 — explicit `use super::release::replace_idle_teardown_handle;` import banner). `release_idle_timeout_worker` stays `pub(crate)` in the new file and is re-exported from `idle_timeout.rs` via `pub(crate) use release::release_idle_timeout_worker;` so the external call site in `manager.rs` (`super::idle_timeout::release_idle_timeout_worker(...)`) resolves byte-equivalently. Workspace **1065 → 1065 (+0) on macOS**, all green: pure refactor, existing tests are the regression pin — the two issue #85 pin tests (`replace_idle_teardown_handle_aborts_prior_and_stores_new` + `_steady_state_holds_at_most_one_alive_per_slot`) pass byte-equivalently through the new import path; the 6 `worker_lifecycle_idle_timeout_e2e` integration tests pin the end-to-end release-path semantics. Linux DGX expected at +0 by the same argument. No new clippy warnings introduced; the 2 transient unused-import warnings (`OwnedMutexGuard` + `IdleTimeoutCaps`, both moved to `release.rs` with their sole callers) were cleaned up inline. Closes the Item 29 operator-picks sub-bullet flagged at end of Issue #136. See "Recently completed (this session, 2026-05-28 — refactor: `idle_timeout/release.rs` sibling-lift)" entry at the top for the full breakdown.
        - ~~**Issue [#136](https://github.com/hherb/kastellan/issues/136)** — `worker_lifecycle/idle_timeout`: debounce pending-acquires queue-depth warn (filed 2026-05-27 from PR #135 in-PR polish review)~~ **SHIPPED 2026-05-27, merged to `main` via PR [#137](https://github.com/hherb/kastellan/pull/137) at `fed0f21`** on branch `fix/issue-136-pending-acquires-warn-debounce`. 2 commits: 1 doc-sync to main = `7f98ee4` (`8204a3c`) + 1 feature `9fae2f9` (new `last_warn_unix_nanos: AtomicI64` field on `ToolSlot` peer to `pending_acquires`; new `PENDING_ACQUIRES_WARN_COOLDOWN: Duration = Duration::from_secs(30)` const colocated with `PENDING_ACQUIRES_WARN_THRESHOLD`; new pure predicate `debounce_warn(last_warn_nanos: i64, now_nanos: i64, cooldown: Duration) -> bool` using `saturating_sub` over i64 so clock-skew-backward suppresses; `acquire_impl` warn site rewritten as outer depth gate → debounce predicate → `compare_exchange(last, now, AcqRel, Relaxed)` to claim the warn slot so only the CAS winner emits the warn under concurrent acquirers; structured warn payload gains `cooldown_secs` field). **Workspace 1058 → 1065 (+7) on macOS**, all green: 1 cooldown const pin + 5 `debounce_warn` predicate tests (cold-path with realistic 2023-era unix nanos + suppress-within + boundary inclusive + past-cooldown + clock-skew-backward) + 1 `slot_for` zero-init pin. Linux DGX expected at +7 by the same argument (every new test is platform-independent; `SystemTime`/`UNIX_EPOCH`/`AtomicI64` are stdlib). No new clippy warnings. **File-size watch:** `core/src/worker_lifecycle/idle_timeout.rs` 576 → 647 LOC (+71); was 76 over the 500-LOC cap, now 147 over. The natural sibling-lift target is now a fresh `idle_timeout/release.rs` carrying `release_idle_timeout_worker` (~85 LOC) + `abort_idle_teardown_handle` (~5 LOC) + `replace_idle_teardown_handle` (~35 LOC) — clean ~125-LOC seam between "acquire path" and "release path", lands both files at ~520 LOC. Deferred to a separate refactor slice. **Deliberately NOT done:** (a) Issue #84 BUSY-rejection half — still waiting on empirical evidence of queue starvation; (b) `kastellan-cli supervisor status` accessor surfacing `pending_acquires` + `last_warn_unix_nanos` — atomics are pub(crate)-accessible to a future `pub fn slot_*` wrapper without further plumbing; (c) the `idle_timeout/release.rs` sibling-lift mentioned above. See "Recently completed (this session, Issue #136)" entry at the top for full breakdown.
    30. ~~**★ Worker-output prompt-injection guard slice 1**~~ **SHIPPED this session 2026-05-28** on branch `feat/injection-guard-slice-1`; PR pending. New module `core::cassandra::injection_guard` with pure-function 22-entry English-substring catalogue across 4 attack classes (instruction_override, role_hijack, secret_exfiltration, unsafe_tool_coercion); per-rule weights summed (cap 1.0); ≥ `BLOCK_THRESHOLD` (0.70) → Block. Wired into `tool_host::dispatch` chokepoint: on Block, replace worker result with redacted placeholder JSON + write `actor='policy' action='injection.blocked'` audit row carrying SHA-256 + byte length + score + class codes; raw scanned text never persisted (pinned end-to-end by `policy_audit_row_contains_no_substring_of_blocked_body`). `InjectionDecision` is `#[non_exhaustive]` so a future Review tier (0.45–0.70 band) slots in without breaking callers. Errors are not screened (not text-channel). Workspace 1065 → 1093 (+28) on macOS, all green; no new clippy warnings. See "Recently completed (this session, 2026-05-28 — ★ Worker-output prompt-injection guard slice 1)" entry at the top for the full breakdown. **Deferred to future slices:** Review tier + heuristic scoring + multilingual + per-tool policy + `kastellan-cli policy review` operator surface + `tool_host.rs` sibling-lift (now 763 LOC, 263 over cap). **Original spec text (kept for reference):** ~~sandboxing contains code, not text. Today `core::tool_host::dispatch` returns worker stdout verbatim into the next planner turn (`scheduler::inner_loop`), so a poisoned tool output (a file the worker dutifully `cat`'d, a web page once `web-fetch` lands, a malicious MCP response once that lands) can rewrite the planner's instructions on the spot. **The chokepoint already exists** (Option M's sealed `dispatch`) so the wiring is one file. Adopt openhuman's `docs/PROMPT_INJECTION_GUARD.md` shape: new `core::policy::injection_guard::screen(text) -> Verdict { score: f32, decision: Allow|Review|Block, reason_codes: Vec<&'static str> }` called from `dispatch` on every successful worker result before it's appended to history. Pipeline: normalize (lowercase, zero-width strip, leetspeak fold) → regex catalogue (instruction override / role hijack / secret exfiltration / unsafe tool coercion) → optional heuristic for combinatorial cues. Thresholds: ≥0.70 → block (return a redacted placeholder + write one `actor='policy' action='injection.blocked'` row), 0.45–0.70 → surface to operator via a new `verdict='review'` audit row. **Privacy-conscious logging**: persist SHA-256 + byte length + score + decision + reason codes; **never the raw scanned text**. Backend authoritative (only enforcement point); any future UI advisory only. License posture: openhuman is GPL-3.0 (one-way compatible into AGPL-3.0); re-implement to avoid ambiguity. Pre-req for Phase 2 channels (untrusted inbound text) but the open-loop risk exists *today* via shell-exec reading any FS-allowed file — file this as priority because it closes a real-not-theoretical attack surface that the production `cli_ask_e2e` chain already exercises every test run.~~
    31. ~~**Opaque secret references — `secret://<8-hex>`**~~ **SHIPPED this session 2026-05-29, merged to `main` via PR [#146](https://github.com/hherb/kastellan/pull/146) at `bc36e4c`** + post-review polish `5885e26` + `9348e9c`. New `core::secrets` module (`Vault` TTL'd `RwLock<HashMap>` default 1h + lazy GC, `SecretRef` opaque newtype with manual `Debug` that never leaks the ref string, `RedeemResult` Hit/Expired/NotFound, `substitute_refs_in_params` exact-match recursive walker). Chokepoint wiring in `tool_host::dispatch`: substitute BEFORE `worker.call`, fail-closed on miss (`ToolHostError::SecretRedemptionFailed`, worker not called, no tool row, one `policy / secret.redemption_failed` row); on Hit, one `policy / secret.redeemed` row per ref (best-effort); `materialize` hard-fails on audit error. `KASTELLAN_BOOTSTRAP_SECRETS` env var drives daemon-startup materialization via `OsKeyringProvider`. `ToolHostError` widened to `#[non_exhaustive]`; classifier maps the new variant to `POLICY_DENIED`. Workspace 1096 → 1137 (+41) on macOS without PG; 8 integration tests verified live against Postgres.app v18. **Deferred to Slice 2:** CLI surface (`kastellan-cli secrets materialize`) + CLI↔daemon IPC; per-task vault scoping; embedded substitution (`"Bearer secret://…"`); `tool_host.rs` sibling-lift (now 867 LOC, 367 over cap, bundle Items 30 + 31). Filed follow-ups: [#147](https://github.com/hherb/kastellan/issues/147) (tool audit row carries substituted plaintext) — **✅ SHIPPED this session 2026-05-29 via snapshot-before-substitution, branch `fix/issue-147-redact-tool-req-plaintext`; see the top "Recently completed" entry**; [#148](https://github.com/hherb/kastellan/issues/148) (audit-insert failure paths untested), [#149](https://github.com/hherb/kastellan/issues/149) (Vault ref-collision branch untested). See "Recently completed (this session, 2026-05-29 — ★ Opaque secret references slice 1)" entry at the top for the full breakdown. **Original spec text (kept for reference):** ~~`db::secrets` ships AES-256-GCM at rest (PR #12 era, migration 0001+0004) but the LLM-context boundary is undefended: the planner currently has no way to pass a credential to a worker without putting the raw value in `tool_host::dispatch` `params`, where it lands in the audit log payload and in the chat transcript that gets re-fed to the LLM on next turn. The leak surface is the audit log, the LLM transcript, and any future operator UI replaying transcripts. Adopt openhuman's `docs/MCP_SETUP_AGENT.md` pattern: planner sees only opaque refs (`secret://7c9f2e`); core keeps a TTL'd `HashMap<SecretRef, Zeroizing<Vec<u8>>>` materialised from `db::secrets::get` at runtime. `tool_host::dispatch` walks `params` recursively and substitutes any `secret://…` string via `Vault::redeem` before forwarding to the worker; the substitution writes one `actor='policy' action='secret.redeemed'` row carrying the ref hash + tool name + actor — never the value. Concrete surface: new `core::secrets::Vault::{materialize(name) -> SecretRef, redeem(&SecretRef) -> Option<Zeroizing<Vec<u8>>>, expire_after(Duration)}`. Pre-req for the Phase-5 frontier-router policy gate (needs an API key it cannot leak into transcripts) and for any credential-carrying tool (`web-fetch` first, MCP servers second). Pair with the Phase-3 egress proxy's future leak scanner so even if a worker exfiltrates the resolved value, the byte signature is caught at the boundary. Pinning: chokepoint pattern is already proven by Option M; this slice extends it with one new helper module + one substitution pass + one audit row, no schema change (`db::secrets` table already exists). The redaction & substitution logic is unit-testable without a live PG (build a fake `Vault`).~~
...
- ~~**`task.submitted` producer row from `kastellan-cli ask`**~~ **Shipped this session 2026-05-13** as `actor='cli' action='task.submitted'` via the new `core::cli_audit::submit_and_audit` helper. Branch: `feat/cli-task-submitted-audit` (`ACTION_TASK_SUBMITTED` const, not a builder, slotted next to `ACTION_TASK_RUNNING` / `ACTION_TASK_FINALIZE`). See the "Recently completed (this session)" entry at the top.
- ~~**Per-tool argv allowlist hygiene**~~ **Shipped this session 2026-05-14** on branch `feat/tool-allowlist-db` — see "Recently completed (this session)" entry at the top. Migration `0009_tool_allowlists.sql` + new `db::tool_allowlists` module + `core::cli_audit::tools_allowlist_{add,remove}_and_audit` helpers + `kastellan-cli tools allowlist {add,remove,list}` subcommands + async DB-backed `build_tool_registry` + `actor='core' action='registry.loaded'` audit row with SHA-256 of canonical-form allowlist for cross-restart drift detection.
...
### ~~Option G — make `cpu_quota_pct`/`tasks_max` policy-driven + setrlimit-based `cpu_ms` enforcement ([#6](https://github.com/hherb/kastellan/issues/6))~~ SHIPPED 2026-05-14

Branch `feat/sandbox-cpu-rlimit-quota` (15 commits, not yet merged). See the "Recently completed (this session)" entry at the top of this file for the full slice breakdown. Headline: cross-platform CPU-budget parity is closed; macOS memory parity still waits on the Apple `container` micro-VM backend ([issue #55](https://github.com/hherb/kastellan/issues/55)).

---

## Open follow-up issues (filed but not picked)

- [#1](https://github.com/hherb/kastellan/issues/1) — narrow macOS `(allow mach-lookup)` to a `global-name` allowlist  *(closed in code 2026-05-09; rule removed entirely from `build_profile`)*
- [#2](https://github.com/hherb/kastellan/issues/2) — evaluate `setpgid` → `setsid` for stronger session isolation on macOS  *(closed in code 2026-05-09; `pre_exec` hook calls `libc::setsid()`)*
...
- ~~[#5](https://github.com/hherb/kastellan/issues/5) — audit `BASE_ALLOW` against a fixture of common worker binaries~~ **closed 2026-05-14 by this session** (`chore/issues-batch-2026-05-14`). New `workers/prelude/tests/coreutils_smoke.rs` runs 19 coreutils under strict lockdown; added the 6 syscall gaps it found (`mkdirat`, `unlinkat`, `renameat2`, `utimensat`, `fchown`, `fchmodat`, `fchmod` + legacy x86_64 variants + adjacent set) to `BASE_ALLOW`. `tar` skips NSS via `--numeric-owner` to dodge the strict-vs-NetClient `socket()` boundary (NSS is not a BASE_ALLOW gap)
- ~~[#6](https://github.com/hherb/kastellan/issues/6) — tunable `cpu_quota_pct`/`tasks_max` policy fields + `setrlimit`-based `cpu_ms` enforcement (Option G above)~~ **closed 2026-05-14 by this session** (`feat/sandbox-cpu-rlimit-quota`). Two new `SandboxPolicy` fields drive Linux cgroup ceilings (`CPUQuota`, `TasksMax`); cross-platform `setrlimit(RLIMIT_CPU)` via new `workers/prelude/src/rlimit.rs` enforces `policy.cpu_ms` on both Linux and macOS via `KASTELLAN_CPU_MS` env plumbed by `derive_lockdown_env`. Test count 429 → 446 (+17). Macros memory enforcement remains the open gap → [issue #55](https://github.com/hherb/kastellan/issues/55) micro-VM spike.
...
- ~~[#11](https://github.com/hherb/kastellan/issues/11) — daemon-scoped `PgPool`~~ **closed 2026-05-10** by Option I's `pool::connect_runtime_pool`
- ~~[#12](https://github.com/hherb/kastellan/issues/12) — reject empty `secrets.aad`~~ **closed 2026-05-10** — `db::secrets::put` always populates AAD via `compute_aad(name, _)`; migration `0004_secrets_aad_nonempty.sql` adds `CHECK (octet_length(aad) > 0)`
- [#13](https://github.com/hherb/kastellan/issues/13) — write a migration numbering / rename hygiene checklist; sqlx fingerprints version+slug, so a rename or edit on a shipped migration silently breaks startup on existing clusters
- [#14](https://github.com/hherb/kastellan/issues/14) — replace the brittle `wait_for_log_match("database probe succeeded")` in `core/tests/supervisor_e2e.rs` with a constant in `kastellan-core`'s public API or a real readiness signal
- ~~[#15](https://github.com/hherb/kastellan/issues/15) — hoist the duplicated PG bring-up boilerplate into a workspace-level `tests-common` dev-dep crate~~ **closed 2026-05-12 by this session** (`refactor/tests-common-hoist`). New crate `kastellan-tests-common` ships `PgCluster` + `bring_up_pg_cluster` + RAII guards + skip helpers + sandbox factory + binary discovery + macOS launchd serial lock + deterministic SHA-256-seeded embedding seed; 8 byte-duplicated copies eliminated; workspace count unchanged at 342
- ~~[#16](https://github.com/hherb/kastellan/issues/16) — close the in-crate hole in the `WorkerCommand` seal (filed 2026-05-10)~~ **closed 2026-05-13 by this session** (`fix/worker-command-seal-tighten`) — minimal-diff variant of issue fix #1: narrowed `WorkerCommand::{method, params, new}` + `SupervisedWorker::call` from `pub(crate)`/`pub` to module-private. The workspace build is the structural regression pin for sibling-module exclusion; the `compile_fail` doctest on `WorkerCommand` remains the out-of-crate pin.
- ~~[#17](https://github.com/hherb/kastellan/issues/17) — tighten `memory::recall` behaviour when input is missing (filed 2026-05-10)~~ **closed 2026-05-14 by this session** (`chore/issues-batch-2026-05-14`). Hybrid policy: single-lane missing-input degrades with a `tracing::warn`; all-lanes-missing-input returns `DbError::Query("recall: no lanes ran…")`. Pinned by new tests in `core/src/memory/recall.rs` + Assertion 4 in `memory_recall_e2e.rs`. Paired with the #40 closure (graph-off `RecallParams::new` default + new `with_seeds` constructor).
- ~~[#20](https://github.com/hherb/kastellan/issues/20) — `agent_prompts` schema: PK on sha256 means renamed prompt files lose their original name (filed 2026-05-10 from PR #25 review)~~ **closed 2026-05-14 by this session** (`chore/issues-batch-2026-05-14`). New migration `0011_agent_prompts_composite_pk.sql` changes PK to `(sha256, name)`; `upsert_prompt` now `ON CONFLICT (sha256, name) DO NOTHING`. Non-destructive — pre-migration rows are already unique on the composite key.
- [#21](https://github.com/hherb/kastellan/issues/21) — `core::scheduler::runner` per-iteration cancellation poll could be a `watch::Receiver` instead of a DB round-trip (filed 2026-05-10 from PR #25 review)
- ~~[#22](https://github.com/hherb/kastellan/issues/22) — `RouterAgent::formulate_plan` has no mock-HTTP test coverage~~ **addressed by PR #26 (open)**
- ~~[#23](https://github.com/hherb/kastellan/issues/23) — scheduler: constitutional refusals are recorded as `state='completed'`, not `'blocked'`~~ **closed 2026-05-14** (`feat/refusal-state`, 12 commits, merged via PR #59 at `f1fea54`). New optional `Plan.refused` field + new `Outcome::Refused` variant + new terminal `tasks.state='refused'` distinct from `'blocked'` (reviewer-detected). Migration `0012` widens CHECK + trigger. Audit row gains `refused: {…}` + `decision_kind="refused"`. Inner-loop short-circuit after reviewer always runs (defense in depth — `Verdict::ConstitutionalBlock` still wins, provenance preserved; `Verdict::Block` arm honours the same refusal short-circuit, pinned by post-merge review fixup commit `91a792d`). Planner prompt updated. Test count 446 → 455 (+9). See "Recently completed (previous session)" for the full breakdown.
- [#24](https://github.com/hherb/kastellan/issues/24) — deployment: `KASTELLAN_PROMPTS_DIR` has a cwd-relative fallback; production unit files must set it explicitly (filed 2026-05-10 from PR #25 review)
- ~~[#30](https://github.com/hherb/kastellan/issues/30) — split `core/src/memory.rs` into `recall.rs` + `embed.rs` submodules~~ **closed 2026-05-12 by this slice** (`core/src/memory/{mod.rs, recall.rs, embed.rs}`, all under the 500-LOC soft cap)
- [#42](https://github.com/hherb/kastellan/issues/42) — `deleted_memories` AFTER DELETE trigger uses `SECURITY INVOKER`; a future role with DELETE on `memories` but no INSERT on `deleted_memories` will silently break DELETE. Deferred until a second DELETE-capable role is proposed; current single-role state is internally consistent and integration-test-pinned (filed 2026-05-13 from PR #41 review).
- ~~[#40](https://github.com/hherb/kastellan/issues/40) — design: `RecallParams::new()` graph-off default (filed 2026-05-12 from PR #41 final review)~~ **closed 2026-05-14 by this session** (`chore/issues-batch-2026-05-14`). Option B picked: new `RecallParams::with_seeds` constructor for the seed-bearing case; `RecallParams::new()` defaults to new `RecallModes::SEMANTIC_AND_LEXICAL` const. Paired with #17 hybrid policy so the no-seeds-graph-on combination is rejected as a caller bug instead of silently warning forever.
- ~~[#47](https://github.com/hherb/kastellan/issues/47) — observation/capture: distinguish 'no verdict row' from a real Approve verdict (filed 2026-05-13 from PR #46 review)~~ **closed 2026-05-14 by this session** (`chore/issues-batch-2026-05-14`). `SCHEMA_VERSION` bumped to 2; `CapturedPlan.verdict_today` is now `Option<String>`. Missing verdict row → `None`; real Approve verdict → `Some("Approve")`. Zero captures on disk made this a free-cost migration.
- ~~[#50](https://github.com/hherb/kastellan/issues/50) — unify finalize-payload provenance signal across crashed / producer-cancelled / runtime emitters (filed 2026-05-13 from PR #49 review)~~ **closed 2026-05-14 by this session** (`chore/issues-batch-2026-05-14`). New `provenance` field on `task.finalize` payloads, closed set `"runtime"` / `"crash_recovery"` / `"producer_cancel_pending"`. New `build_producer_cancel_finalize_payload` helper replaces `emit_producer_cancel_finalize`'s previous reuse of `build_finalize_payload`. 9-key shape pin in `cli_cancel_audit_e2e` + `scheduler_crash_recovery_e2e` is now a 10-key pin.
- ~~[#71](https://github.com/hherb/kastellan/issues/71) — audit-trail integrity: producer-supplied `agent_raised` provenance accepted without validation (filed 2026-05-16 from PR #70 review)~~ **closed this session 2026-05-16** (`fix/runner-reject-agent-raised-provenance`, commits `a6335ab` + post-review fixup). New pure helper `parse_classification_floor_source_from_payload` in `core/src/scheduler/runner.rs` parses the payload via serde first, then rejects the `ClassificationFloorSource::AgentRaised` variant on a structural match — the reject is bound to the enum variant (via `as_snake_str()` in the diagnostic) so a future rename propagates automatically. The "unknown value" generic-reject diagnostic no longer lists `agent_raised` as expected. +9 unit tests (598 → 607). Follow-up: [issue #73](https://github.com/hherb/kastellan/issues/73) tracks the deferred e2e integration test + `TaskContext` constructor doc note.
- ~~[#136](https://github.com/hherb/kastellan/issues/136) — `worker_lifecycle/idle_timeout`: debounce pending-acquires queue-depth warn (filed 2026-05-27 from PR #135 in-PR polish review)~~ **SHIPPED 2026-05-27, merged to `main` via PR [#137](https://github.com/hherb/kastellan/pull/137) at `fed0f21`** on branch `fix/issue-136-pending-acquires-warn-debounce`. 2 commits: 1 doc-sync to main = `7f98ee4` (`8204a3c`) + 1 feature `9fae2f9` (`AtomicI64 last_warn_unix_nanos` field on `ToolSlot`; `PENDING_ACQUIRES_WARN_COOLDOWN = 30s` const; pure `debounce_warn(last_warn_nanos, now_nanos, cooldown) -> bool` predicate; CAS-to-claim warn-emit pattern). +7 tests. **Workspace 1058 → 1065 (+7) on macOS**, all green. See "Recently completed (this session, Issue #136)" entry at the top for full breakdown.
- [#107](https://github.com/hherb/kastellan/issues/107) — `MacosContainer` PID-1 signal-handling posture (filed 2026-05-21 from PR #106 review). Surfaced during code review of Slice 1: the new backend does not pass `--init`, so the worker process inherits PID 1 inside the micro-VM with whatever default Apple `container run` provides. Not a regression vs the existing backends (bwrap explicitly opts into PID-1 via `--as-pid-1`; Seatbelt doesn't use a VM at all). For Slice 1's smoke tests this is invisible — `echo` / `cat` / short-lived `sh -c` never need zombie reaping or SIGTERM forwarding — but for long-lived workers under `IdleTimeoutLifecycle` (`gliner-relex`, future `python-exec`) both properties matter. Verify end-to-end in Slice 2 or 2.5 before `gliner-relex` migrates onto the container backend.
...
(Closed won't-fix: [#9](https://github.com/hherb/kastellan/issues/9) Apache AGE, [#10](https://github.com/hherb/kastellan/issues/10) ParadeDB pg_search — both 2026-05-09 after review. Closed in earlier 2026-05-09: [#7](https://github.com/hherb/kastellan/issues/7) — daemon log-line substring is now precise after `(skeleton)` was dropped from the startup line.)
