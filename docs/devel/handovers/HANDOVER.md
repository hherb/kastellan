...
**Last updated:** 2026-06-26 (**Linux Firecracker micro-VM backend — SLICE 1 Tasks 1–6 IMPLEMENTED + DGX-VERIFIED on branch `feat/firecracker-microvm-slice1-impl` (off `main`@`1cf8597`, PR pending). Task 7 (live KVM-boot e2e) PARKED — needs operator privileged DGX setup.**
Executed the 7-task slice-1 plan via subagent-driven TDD (fresh implementer + task-reviewer per task, opus final whole-branch review). **Shipped (9 commits):**
(T1) `SandboxBackendKind::FirecrackerVm` variant + `SandboxBackends.firecracker` registry slot + `resolve` arm + stub backend (all `#[cfg(target_os="linux")]`).
(T2) pure `build_launch_plan` + `FirecrackerLaunchPlan`/`FirecrackerImage` + `render_firecracker_config` in `sandbox/src/linux_firecracker/plan.rs` (mem_mb→mem_size_mib, cpu_quota_pct→vcpu ceil-clamp[1,8], Net::Deny→no net device, vsock always present, relative-path reject).
(T3) fail-closed `probe()` + pure `probe_report(&ProbeInputs)` naming each operator fix (`probe.rs`).
(T4) new pure-std crate **`kastellan-microvm-run`** (the Child launcher: firecracker_argv + stdin↔vsock↔stdout `pump`; host connects `<uds>_<port>`, the `CONNECT` handshake helper retained `#[allow(dead_code)]` for Task-7 live transport resolution).
(T5) new **`kastellan-microvm-init`** guest PID1 (mount pseudo-fs → AF_VSOCK listen/accept → dup2 onto fd 0/1 → exec unchanged worker; Linux-only libc cfg-gated, macOS stub main, pure helper runs on macOS) + `scripts/workers/microvm/{build-rootfs.sh,install-firecracker.sh}` + privileged `scripts/linux/install-firecracker-vsock.sh` (narrow `setfacl u:<user>:rw` on /dev/vhost-vsock + udev persist; mirrors install-bwrap-apparmor-profile.sh) + CLAUDE.md note.
(T6) real `LinuxFirecracker::spawn_under_policy` (build plan→write fc.json to per-spawn temp dir→spawn launcher as Child; std-only temp dir, NO new dep) + `firecracker_mode_entry` + `KASTELLAN_PYTHON_EXEC_USE_MICROVM=1` resolver (mirrors `container_mode_entry` field-for-field: Net::Deny + WorkerStrict + 512 MiB/10s/30s caps); `parse_idle_caps`/`container_lifecycle` cfg widened macos→any(macos,linux).
**VERIFICATION — both platforms GREEN:** macOS `cargo clippy --workspace --all-targets -D warnings` clean; **DGX (aarch64 Linux, via format-patch|ssh dgx git am relay onto origin/main):** workspace `--all-targets` clippy clean + unit tests — sandbox **44/0** (firecracker plan/probe/spawn/registry), microvm-run **2/0**, microvm-init **1/0**, core python_exec **26/0** (incl. 4 firecracker tests). **The DGX gate caught two false-greens the Mac couldn't** (fixed): (a) plan.rs test compared `Value` vs `Cow<str>` — only fails under `--all-targets`/real `cargo test`, which earlier per-task cross-clippy omitted (commit 723a776); (b) core test-literals (`composite.rs`×3, `worker_lifecycle_idle_timeout_e2e.rs`) missing T1's new linux `firecracker` field — `core` can't cross-clippy (ring C dep) so Linux-only (commit 674a240). **Lesson: the per-task Mac gate MUST be `--all-targets`; core linux-cfg code is DGX-verified-only.** Final opus review = READY WITH FOLLOW-UPS, no Critical/Important; 3 must-fixes applied (commit ae4e559: microvm-run teardown-before-connect to stop orphan-firecracker leak on boot-timeout; install-firecracker.sh TMPDIR→WORKDIR; Slice-1 env-gap docs). **Follow-up issues filed: [#360](https://github.com/hherb/kastellan/issues/360)** (policy.env not forwarded into guest — `params_file_max` inert in-VM, safe-by-default), **[#361](https://github.com/hherb/kastellan/issues/361)** (guest-boot dup2/fd hygiene — Task-7 wave), **[#362](https://github.com/hherb/kastellan/issues/362)** (per-spawn temp dir never cleaned), **[#363](https://github.com/hherb/kastellan/issues/363)** (python_exec.rs 587 LOC split). **Task 7 (PARKED):** operator runs `sudo scripts/linux/install-firecracker-vsock.sh` + `install-firecracker.sh` + `build-rootfs.sh` on the DGX, resolves the hybrid-vsock connect direction live (plan T7 Step 2), then the `#[ignore]` `python_exec_firecracker_e2e` (42 round-trip + KVM mem-cap + net-deny + >64 KiB params). DGX spike artifacts still at `/tmp/fc-spike`. Plan: `docs/superpowers/plans/2026-06-26-linux-firecracker-microvm-slice1.md`.)
...
**★ LEADING PICK — Linux Firecracker micro-VM slice 1, Task 7 (live KVM-boot e2e + runbook).** Tasks 1–6 are DONE + DGX-unit-verified on
`feat/firecracker-microvm-slice1-impl` (see the "Last updated" header; PR pending). Task 7 is the only remaining slice-1 piece and needs the
DGX with a **one-time privileged operator setup the agent can't do**: `sudo scripts/linux/install-firecracker-vsock.sh` (vsock module persist +
ACL grant on /dev/vhost-vsock) + per-user `scripts/workers/microvm/install-firecracker.sh` (firecracker v1.16.0) + `build-rootfs.sh` (kernel +
python-exec.ext4); `/dev/kvm` already works (pass `--kvm` if not). Then: (a) **resolve the one genuine unknown** — the hybrid-vsock connect
direction (host `CONNECT <port>\n` on the base UDS vs the guest-listen `_<port>` suffix; `kastellan-microvm-run/src/main.rs` currently ships the
`_<port>` path, with `bridge::firecracker_vsock_connect_line` retained `#[allow(dead_code)]` as the alternative) — plan T7 Step 2, live on the DGX;
delete the losing branch. (b) write `core/tests/python_exec_firecracker_e2e.rs` (`#[ignore]`: `print(6*7)→42` + KVM-enforced mem-cap→MemoryError +
net-deny + >64 KiB params file-channel) + the operator runbook + threat-model note. **Fold in [#361](https://github.com/hherb/kastellan/issues/361)**
(guest-boot dup2/fd hygiene — same `microvm-init` syscall path). **Note [#360](https://github.com/hherb/kastellan/issues/360):** the >64 KiB
params test passes only at the DEFAULT ceiling until policy.env is forwarded into the guest. Spike artifacts at `/tmp/fc-spike` on the DGX.
Slices 2–5 (warm/idle, fs-sharing, net workers, jailer) are sketched in the spec for later sessions.

**Firecracker slice-1 follow-up issues (small, on demand):** [#360](https://github.com/hherb/kastellan/issues/360) forward policy.env into the
guest (params_file_max inert in-VM, safe-by-default); [#362](https://github.com/hherb/kastellan/issues/362) per-spawn temp-dir RAII cleanup;
[#363](https://github.com/hherb/kastellan/issues/363) split `core/src/workers/python_exec.rs` (587 LOC).
