...
**Last commit on `main`:** `e93997e` (merge commit `Merge pull request #124 from hherb/feat/item-27-slice-2.5-followups`). Branch carried 4 commits: `07de743` (warm-cache invariant test, closes #121) + `9cb375e` (probe_image + daemon-startup walk, closes #120) + `a569885` (skip helper delegation, closes #122) + `55bb8f1` (post-review fixup). Workspace at macOS **1023 / 0 / 3** on `main` at `e93997e`. Earlier merge `185ec2f` (doc-sync for PR #123); substantive predecessor `920e0c9` (PR [#123](https://github.com/hherb/hhagent/pull/123), Item 26 — `gliner_relex.rs` test-module sibling lift). PR [#118](https://github.com/hherb/hhagent/pull/118) (Item 25 GLiNER-Relex Slice 2.5) merged earlier at `a9e3385` with post-merge `e11e3f3` + `1c7eced`. Doc-only PR [#119](https://github.com/hherb/hhagent/pull/119) (developer onboarding manual at `docs/devel/manual/`) merged at `99bbfab`.
...
## Recently completed (this session, 2026-05-25 — Next-TODO Item 27: Slice 2.5 follow-up triple (#120 + #121 + #122), branch `feat/item-27-slice-2.5-followups`, **merged to `main` via PR [#124](https://github.com/hherb/hhagent/pull/124) at `e93997e`** + same-PR post-review fixup `55bb8f1`)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    28. **★ Next concrete engineering pickup — Issue [#95](https://github.com/hherb/hhagent/issues/95) — entity-upsert Layer B (full-batch unnest + per-row attribution fallback)** (engineering, ~1 session). Issue body originally framed Layer B as a defer-until-trigger because full-batch unnest collapses N row-level failures into one batch failure with row indices the caller has to map back, losing per-row error attribution. Picked up this session anyway with a **batch-first + per-row fallback** design that preserves attribution: the happy path (no constraint violations, which is the steady-state expectation now that quarantine is the recoverable-constraint discriminator) issues 1 RT for N entities via `unnest(...)`; on constraint failure (`sqlx::Error::Database` with a unique/check/foreign-key code), the call transparently falls back to the existing per-row loop so the caller still gets per-row attribution. `UpsertOutcome` shape stays byte-identical (Layer A invariant). `build_extract_entities_payload` 8-key contract preserved. Same `quarantine`-preserving invariant pinned by `upsert_preserves_operator_unquarantine_decision`. Triple-loop in `upsert_entities_and_relations` gets the same treatment for consistency. Brainstorm → spec → plan → TDD execution; see Items 21–26 for the canonical session shape. **Trigger flag this addresses:** none of the original three (no observation data, no production tracing showing it as a hotspot, attribution value not down-revised). **Justification for proceeding anyway:** the batch-first + per-row fallback design lets us bank the RT win on the steady-state happy path without paying the attribution cost, so the original cost/benefit calculus that justified deferring shifts.
