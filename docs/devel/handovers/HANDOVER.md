...
**Last updated:** 2026-05-21 (continuation — Issue #55 macOS Apple `container` backend Slice 1, branch `feat/macos-container-backend-slice-1`, PR pending. Engineering follow-through on the spike merged via PR #105 (`56456da`) earlier today. New `sandbox/src/macos_container.rs` ships `MacosContainer: SandboxBackend` + pure `build_container_argv` + `clamp_memory_to_minimum` 200 MiB-floor helper + `MacosContainer::probe()` two-step check (`container --version` + `container system status`); doc-comments on `SandboxPolicy::{mem_mb, cpu_quota_pct, tasks_max}` widened to call out the new macOS support + the `tasks_max` semantic gap (cgroup `pids.max` per-cgroup on Linux vs `RLIMIT_NPROC` per-UID on macOS). Sibling — NOT a replacement — for `MacosSeatbelt`; `default_backend()` on darwin still returns Seatbelt (Slice 2 introduces per-worker selection). New `sandbox/tests/macos_container_smoke.rs` runs 7 real-`container` smoke tests against `alpine:3.20`: echo round-trip, host `/Users` invisible, `fs_read` bind-mount round-trip, `fs_read` readonly write-rejection, strict-profile `--read-only` root, strict-profile `/tmp` tmpfs scratch. Skip-as-pass when `container --version` / `container system status` / `alpine:3.20` image are missing.

**Last commit on `main`:** `56456da` (`Merge pull request #105 from hherb/spike/issue-55-macos-container-discovery`).
**Last commit on `feat/macos-container-backend-slice-1`:** _<set at commit time>_. Branch carries 1 commit on top of `main`.

**Session-end verification:** **Rust workspace on macOS: 899 passed / 0 failed / 3 ignored** (+33 over the 866 baseline — 26 unit in `macos_container::tests` + 7 real-container smoke in `macos_container_smoke`). All 7 smoke tests passed in ~4 s against real Apple `container` 0.12.3 (warm cache; `alpine:3.20` pre-pulled). **Linux DGX (expected, last verified pre-merge of #100): 896 passed / 0 failed / 4 ignored** unchanged by this slice (`sandbox/src/macos_container.rs` is `#[cfg(target_os = "macos")]`; the doc-comment widenings on `SandboxPolicy::{mem_mb, cpu_quota_pct, tasks_max}` are pure-doc changes and don't add tests; pin remains +1 from PR #103's Rust test → expected ~897 on next DGX run). **Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed** unchanged. 0 new `[SKIP]` lines on macOS (`container` + image are present on this Mac); the smoke tests will skip cleanly on hosts without `container` installed. Pre-existing `mem_burner.rs` clippy warning (`uninit_vec`) is from `3cea642` (cgroup feature), unrelated to this slice. `cargo clippy --workspace --lib --tests --no-deps` is clean for the new `macos_container` module.

## Recently completed (this session, 2026-05-21 — Issue #55 macOS Apple `container` backend Slice 1, branch `feat/macos-container-backend-slice-1`, PR pending)

Picked up Next-TODO item 17 (`MacosContainerBackend` — Slice 1, 1-session scope). Engineering follow-through on the spike merged via PR #105 (`56456da`) earlier today. The spike notes at [`docs/superpowers/specs/2026-05-21-macos-container-spike-notes.md`](../../superpowers/specs/2026-05-21-macos-container-spike-notes.md) locked the design: pure `build_container_argv` mirroring `linux_bwrap::build_argv`, `clamp_memory_to_minimum` with `tracing::warn!` on 200 MiB floor hits, two-step `probe()`, and a smoke test exercising bind-mount + read-only + tmpfs + FS isolation against real `container`.
...
**Test count delta this session:** Rust workspace macOS **866 → 899 (+33)**: 26 unit (`macos_container::tests` — 5 clamp + 16 build_container_argv + 5 constructor/spawn) + 7 smoke (`macos_container_smoke`). Linux DGX expected unchanged (~897, same as the prior session-continuation's projection; the new `macos_container` module is `#[cfg(target_os = "macos")]` so Linux doesn't compile it). Python suite unchanged at 35.
...
**macOS (this slice, `feat/macos-container-backend-slice-1`):** **899 passed / 0 failed / 3 ignored** on this Mac (M3 Max, macOS 26.5) — +33 over the 866 pre-slice baseline from the new `macos_container` module + smoke test against real Apple `container` 0.12.3. Earlier checkpoint: 866 on `main` at `56456da` post-PR-#105-merge. **Pre-Phase-1 macOS (main):** 299 all pass on macOS (skip-as-pass for PG-dependent tests); Option O additions not yet verified on macOS (embedding TCP mock tests are cross-platform clean; the `embedding_recall_e2e` skip-as-pass path is expected).
