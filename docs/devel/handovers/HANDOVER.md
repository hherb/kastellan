...
**Last updated:** 2026-05-18 (Issue #81 — `inner_loop.rs` split into `inner_loop.rs` + `inner_loop_audit.rs`. Pure refactor: lifted `build_plan_formulate_payload` + the three `write_audit_*` async writers + their 9 payload-shape pin tests + 2 test fixtures into a new sibling module. `inner_loop.rs` 1214 → **655 LOC** (under the issue's 700-LOC target; still over the 500-LOC soft cap but now within reach of a future second pass); `inner_loop_audit.rs` ships at **484 LOC** (under the 500-LOC cap). Zero behavioural change; zero workspace test count delta (still **721**). Earlier this session: HANDOVER sync to reflect the L1 promotion writer PR #82 merge + 3 post-merge docs/specs commits.).
**Last commit on `main`:** `742378e` (`docs(handover): sync to current main — PR #82 merged + post-merge specs`). The issue #81 refactor commit lands next. Recent main history: `742378e` (handover sync, this session) → `9893f3a` (ROADMAP sync) → `99e97cf` (worker-lifecycle + GLiNER-Relex specs) → `eb6b8a8` (PR #82 merge of `feat/l1-promotion-writer`) → `8a5e6f0` (entity-extraction graph-lane v1 design — committed pre-merge but ordered after by topology) → `a062896` (pre-PR review fixes: doc-accuracy on `Plan::l1_insight`, `memory l1 remove` arity check, `build_l1_metadata` tightened to `pub(crate)`, clippy `print_literal` fix).
**Session-end verification:** `cargo test --workspace` post-refactor: **721 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux (byte-identical to the pre-refactor baseline — the refactor is pure-mechanical). `core/src/scheduler/inner_loop.rs` is now **655 LOC** (was 1214; ↓ 559); `core/src/scheduler/inner_loop_audit.rs` is **484 LOC** (new file, under the 500-LOC cap). `core/src/memory/l1_promote.rs` at **453 LOC** (well under cap). `core/src/bin/hhagent-cli.rs` at **1419 LOC** (pre-existing breach; the next 500-LOC cap pickup).
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    3. **Entity extraction + graph-lane wiring v1 — implementation** (engineering). Spec at `8a5e6f0`. Pre-req: write an implementation plan; consider whether to skip v1 in favour of v2 once worker-lifecycle ships and GLiNER-Relex POC validates feasibility.
    ...
    7. **`hhagent-cli.rs` (1419 LOC) split** (engineering, pure refactor) — the largest remaining 500-LOC breach in the crate now that `inner_loop.rs` is fixed. Subcommand trees are the natural unit; sibling files under `core/src/bin/hhagent_cli/` if the bin crate gets a module structure. File a tracking issue when picked up.
