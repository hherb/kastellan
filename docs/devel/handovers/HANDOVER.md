...
- ~~[#148](https://github.com/hherb/hhagent/issues/148) — secrets: cover audit-insert failure paths with fault injection~~ **done on branch `fix/issue-148-149-secret-vault-test-gaps` (`AuditSink` seam + `dispatch_with_sink`); closes on merge.**
- ~~[#149](https://github.com/hherb/hhagent/issues/149) — secrets: `Vault::materialize` ref-collision branch is untested~~ **done on the same branch (`insert_fresh` extraction + unit test); closes on merge.**
