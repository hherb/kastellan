...
**Last updated:** 2026-09-08 (review round) · **`main` HEAD:** `ec9a2e94` —
...
- **The C0 negative control ran on the real host, against the real image.** `matrix.ext4` bakes
  `d7e6aee6…` and `build-release.sh` reproduces exactly that; a bare `cargo build --release
  --workspace` then flipped `target/release/kastellan-worker-matrix` to `f606683b…` — a digest **no
  image bakes** — in **0.45 s with no compilation**, while `kastellan-microvm-init` stayed at
  `8a21877a…`. So the blast radius is exactly one binary, and it is the one the gate would then call
  stale. A second `build-release.sh` restored `d7e6aee6…`.
...
"image not present" and accepts *any* tag containing `python-exec`, i.e. #667's staleness class with
no freshness check at all. Cross-platform parity is a `CLAUDE.md` hard constraint, so this is a real
gap — a much smaller one than the Linux side, since the backend is opt-in. Then
