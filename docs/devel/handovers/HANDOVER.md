...
**Done since last session:** PR [#155](https://github.com/hherb/hhagent/pull/155) (#143 `walk()` depth guard) merged to `main` at `6e82252`; #143 closed; the merge also carried commit `40e2911` (sibling-skip-on-truncation doc note → filed [#156](https://github.com/hherb/hhagent/issues/156), still OPEN). Earlier: PR [#152](https://github.com/hherb/hhagent/pull/152) (#144 + #150 fixes) merged at `560d845`; `linux-check` CI green; both issues closed. Still optional/worthwhile (operator action, no code): a full `cargo test --workspace` on the DGX — CI is compile-only, so the Linux *test* baseline is still the stale `990`-at-`1abb061`.
