...
**Last updated:** 2026-06-27 (**Linux Firecracker micro-VM backend — SLICE 1 COMPLETE (Tasks 1–7), DGX-VERIFIED LIVE on branch `feat/firecracker-microvm-slice1-impl` (off `main`@`1cf8597`, PR [#364](https://github.com/hherb/kastellan/pull/364)).**
A `Net::Deny` python-exec worker now boots inside a real Firecracker KVM guest over vsock, with `mem_mb` enforced by the hypervisor — **e2e 4/4 GREEN on the DGX in 0.21s, 0 orphaned VMs.** Tasks 1–6 (enum/registry, pure `build_launch_plan`+config, fail-closed `probe`, `kastellan-microvm-run` launcher, `kastellan-microvm-init` guest PID1, `LinuxFirecracker::spawn`+`firecracker_mode_entry`) were built via subagent-driven TDD; **Task 7 was driven live on the DGX this session** and resolved the genuine unknowns:
**(transport)** the host dials the **base** vsock UDS and sends `CONNECT <port>\n` (firecracker host-initiated direction), NOT the `<uds>_<port>` suffix — `bridge::connect_hybrid_vsock`; the suffix path is deleted and the connect-line helper is load-bearing now.
**(teardown — the subtle one)** `bridge::pump` relayed guest→host through `io::copy` into the `Stdout` LineWriter, which **buffered the JSON-RPC response until the 30 s wall-clock watchdog** (measured **30 s/call**); fixed by read+write+**flush** of each chunk → **0.2 s/call**. The relay thread is detached and `pump` returns on host-stdin EOF, so the launcher kills firecracker promptly (**no orphaned VMs**).
**(concurrency)** `spawn_under_policy` now assigns a per-spawn unique vsock UDS (in the per-spawn run dir) **and** a host-unique guest CID (`next_guest_cid`) — parallel VMs were all-but-one `EarlyExit`ing on the shared image-dir UDS / fixed CID 3.
**(bootable rootfs)** `build-rootfs.sh` now copies the real lib closure (loader + ldd deps for init/worker/python + `lib-dynload`) **and the full python stdlib at its native `/usr` prefix** — the closure was a TODO comment before, so the rootfs never booted; `microvm-init` bakes `/usr/bin/python3` (native prefix → CPython getpath finds the stdlib under `-I`) + **#361 fd hygiene** (checked `dup2`, closed listen+conn fds — [#361](https://github.com/hherb/kastellan/issues/361) CLOSED).
**(operator setup hardening, from first live use)** `install-firecracker-vsock.sh` also provisions `/var/lib/kastellan/microvm` (owner = worker user); all three scripts reject `sh`/`sudo` mis-invocation; `install-firecracker.sh` refuses root. Runbook `docs/devel/runbooks/2026-06-26-linux-microvm-setup.md` + threat-model note added.
**The 4 e2e scenarios** (`core/tests/python_exec_firecracker_e2e.rs`, `#[ignore]`, auto-discovers `kastellan-microvm-run`, `[SKIP]`s via `probe()`): `print(6*7)→42`; KVM mem-cap → `MemoryError`; `Net::Deny` socket containment; >64 KiB params via the in-VM `/tmp` file channel.
**VERIFICATION — both platforms GREEN:** macOS `cargo clippy --workspace --all-targets -D warnings` clean; **DGX (aarch64, KVM):** workspace `--all-targets` clippy clean + e2e **4/4** + unit tests (sandbox 44, microvm-run 2, microvm-init 1, core python_exec 26). **Two false-greens the Mac structurally can't catch were caught on the DGX + fixed** (Tasks 1–6): plan.rs `Value` vs `Cow<str>` test (only fails under `--all-targets`; commit 723a776) and core test-literals missing T1's linux `firecracker` field (core can't cross-clippy — ring C dep; commit 674a240). **Lesson: per-task Mac gate MUST be `--all-targets`; core linux-cfg code is DGX-verified-only.**
**Remaining slice-1 follow-ups (open, non-blocking):** [#360](https://github.com/hherb/kastellan/issues/360) forward `policy.env` into the guest (the >64 KiB file channel works at the DEFAULT ceiling; a custom `params_file_max` is still inert in-VM, safe-by-default); [#362](https://github.com/hherb/kastellan/issues/362) per-spawn run-dir (`/tmp/kastellan-microvm-*` fc.json/log) never cleaned; [#363](https://github.com/hherb/kastellan/issues/363) split `python_exec.rs` (587 LOC). Plan: `docs/superpowers/plans/2026-06-26-linux-firecracker-microvm-slice1.md`. **Slices 2–5 (warm/idle, fs-sharing via per-spawn block devices, net workers over a 2nd vsock, jailer) are the next micro-VM arc.**

**Code-review fixes on PR #364 (2026-06-27, DGX-reverified e2e 4/4):** review caught that the shared `python-exec.ext4` was attached **read-write** while every spawn shares that one backing file (only the vsock UDS + CID are uniquified) — two concurrent VMs mounting the same ext4 RW corrupt it. Fixed by attaching the rootfs **read-only** (`is_read_only: true`, `plan.rs`) **and** building it **journal-less** (`mkfs.ext4 -O ^has_journal`, `build-rootfs.sh`). The DGX caught a real false-green here the Mac can't: read-only alone panicked the guest (`EXT4-fs (vda): recovery required on readonly filesystem … cannot proceed`) because prior RW boots had left the journal dirty; the journal-less image mounts read-only cleanly. Firecracker auto-appends `root=/dev/vda ro` for a read-only drive, so no `ro` boot_arg was needed. **⚠️ Any host with an existing `/var/lib/kastellan/microvm/python-exec.ext4` must rerun `build-rootfs.sh` to get the journal-less image.** Also: `install-firecracker.sh` + `build-rootfs.sh` now pick the firecracker binary / guest kernel by `uname -m` (x86_64|aarch64) instead of hardcoding aarch64 (cross-platform per CLAUDE.md); threat-model micro-VM row reworded — slice 1 is VM-*instead-of*-bwrap for that worker (worker still self-applies Landlock+seccomp inside the guest), not VM-on-top-of-bwrap.)
...
**★ Firecracker micro-VM SLICE 1 is COMPLETE + DGX-verified live (PR [#364](https://github.com/hherb/kastellan/pull/364)) — see the "Last updated" header.** The leading micro-VM pick is now **slice 2+** (a fresh spec/plan, on demand): warm/idle VM reuse (mirror the macOS `IdleTimeout` lifecycle, amortise the ~0.2 s boot), host-dir fs-sharing (per-spawn block devices — Firecracker has no virtio-fs/9p), net workers (egress-proxy UDS over a 2nd vsock), and the jailer. Each is a hard sub-problem the spec staging table anticipates. **Small slice-1 follow-ups (open, on demand):** [#360](https://github.com/hherb/kastellan/issues/360) forward `policy.env` into the guest (the >64 KiB file channel works at the DEFAULT ceiling; a custom `params_file_max` stays inert in-VM until then, safe-by-default); [#362](https://github.com/hherb/kastellan/issues/362) per-spawn run-dir RAII cleanup (`/tmp/kastellan-microvm-*` fc.json/log accumulate); [#363](https://github.com/hherb/kastellan/issues/363) split `core/src/workers/python_exec.rs` (587 LOC). **DGX spike artifacts at `/tmp/fc-spike` (reusable).**
