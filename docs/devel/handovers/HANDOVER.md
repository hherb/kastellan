...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    20. ~~**`hhagent-cli entities kinds {add,remove,list}` subcommand tree**~~ **SHIPPED this session 2026-05-22** on branch `feat/entities-kinds-cli` (stacked on `feat/relations-kinds-cli`; PR pending). Mechanical mirror of item 16: new `db::entity_kinds::{validate_entity_kind, add, remove, list_all}` + `ENTITY_KIND_UNDEFINED` sentinel + `EntityKindError`; new `ACTION_ENTITY_KINDS_{ADD,REMOVE}` audit constants + `entity_kinds_{add,remove}_and_audit` helpers; existing `entities.rs` CLI extended with a `kinds` action delegating to `run_entities_kinds` (chose nested rather than new top-level namespace — operator UX consistency: everything entity-related lives under `entities`). Reuses `connect_admin_pool` from item 16. Workspace **921 → 939 (+18)** on Linux DGX. See "Recently completed (this session)" entry at the top.
    21. ~~**`relations show <entity-id>` graph-edge introspection**~~ **SHIPPED this session 2026-05-23** on branch `feat/relations-show-cli` (PR pending). New operator-facing graph-edge introspection: walks outbound + inbound edges from the seed up to `--depth N` hops (default 1, hard-capped at new `MAX_WALK_DEPTH = 5`), prints one row per edge in canonical `(src_kind, "src_name") --[edge_kind]--> (dst_kind, "dst_name")` shape regardless of which walk surfaced it, tags quarantined endpoints `[Q]`. Plain text default (dynamic column widths — avoids the #111 fixed-width footgun) + `--format json` opt-in (NDJSON for `jq`). Two new `Graph` trait methods (`walk_outbound_edges` / `walk_inbound_edges`) carry the recursive-CTE machinery with visited-set cycle protection. **Workspace 939 → 969 (+30)** on Linux DGX. See "Recently completed (this session)" entry at the top.
    22. **Split over-cap CLI files into per-action siblings** (engineering, batch refactor — bundle the natural shared lift across all three) — three over-cap files all wanting the same shape: `core/src/bin/hhagent-cli/entities.rs` (723 LOC, [#112](https://github.com/hherb/hhagent/issues/112)) → `entities.rs` + `entities_kinds.rs`; `core/src/bin/hhagent-cli/relations.rs` (803 LOC, this session) → `relations.rs` + `relations_kinds.rs` + `relations_show.rs`; the kinds-CLI tech-debt consolidation ([#111](https://github.com/hherb/hhagent/issues/111)) — `list` uses runtime pool, dynamic column widths, description size cap, ideally a shared `KindsCli<T>` generic. Doing all three in one PR keeps the symmetry visible and avoids the byte-symmetric mirror contract breaking under partial fixes. Estimated effort: 1 session.
    23. **`db/src/graph.rs` test-module lift / walk-impl split** (engineering, pure refactor) — `graph.rs` now 753 LOC. Two natural splits: (a) lift `#[cfg(test)] mod tests` into a sibling `graph/tests.rs`; (b) lift the recursive-CTE `walk_*_edges` impls + `decode_walked_edge` + `clamp_walk_depth` into `graph/walk.rs`. (b) is preferred if a second `WalkedEdge` consumer materialises (e.g. a future `recall(GRAPH_ALL_HOPS)` mode); until then, (a) is the cheaper one. Defer until either bundles with item 22 or a real call site forces the issue.
