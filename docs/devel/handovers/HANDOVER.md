...
**Last updated:** 2026-05-18 (Worker lifecycle slice 1 — branch `feat/worker-lifecycle-slice-1`, 5 commits. New `core::worker_lifecycle` module ships `Lifecycle` enum (`SingleUse` default + `IdleTimeout { caps, contract }` declarable), `IdleTimeoutCaps`, `Contract`, `Lifecycle::idle_timeout(caps, contract)` validated constructor rejecting `stateless=false`, `LifecycleValidationError`. Runtime layer: `WorkerLifecycleManager` async trait + `WorkerHandle::worker_mut()` exposing `&mut SupervisedWorker` to dispatch through the existing `tool_host::dispatch` chokepoint + `SingleUseLifecycle` (production, byte-equivalent to today's per-request spawn) + `IdleTimeoutLifecycle` (stub — `acquire()` calls `unimplemented!()` so accidental wiring trips loudly). `ToolEntry` gains a `lifecycle: Lifecycle` field; `shell_exec_entry` declares `Lifecycle::SingleUse`. `ToolHostStepDispatcher`'s `sandbox: Arc<dyn SandboxBackend>` field is replaced by `lifecycle: Arc<dyn WorkerLifecycleManager>`; `dispatch_step` asks the manager for a handle instead of calling `spawn_worker` directly. `main.rs` instantiates `SingleUseLifecycle::new(sandbox.clone())` and passes it to `ToolHostStepDispatcher::new`. The `hhagent-supervisor` crate (OS-unit installer for systemd/launchd) is untouched — naming collision with the spec's "supervisor" wording is purely conceptual. Earlier this session (now merged to `main` at `6b73893`): Issue #81 `inner_loop.rs` split into `inner_loop.rs` (655 LOC) + `inner_loop_audit.rs` (484 LOC).
**Last commit on `main`:** `6b73893` (`refactor(core,scheduler,inner_loop): split into inner_loop + inner_loop_audit (#81)`). The worker-lifecycle-slice-1 commits live on `feat/worker-lifecycle-slice-1`, not yet merged. Branch tip: `780c9a8`. Recent main history: `6b73893` (issue #81 split, merged via PR) → `742378e` (handover sync) → `9893f3a` (ROADMAP sync) → `99e97cf` (worker-lifecycle + GLiNER-Relex specs) → `eb6b8a8` (PR #82 merge of `feat/l1-promotion-writer`).
**Session-end verification:** `cargo test --workspace` on `feat/worker-lifecycle-slice-1`: **731 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux (baseline 721 + 10 new tests: 6 in `worker_lifecycle::types`, 3 in `worker_lifecycle::manager`, 1 new pin `shell_exec_entry_declares_single_use_lifecycle` in `scheduler::tool_dispatch`). `core/src/worker_lifecycle/types.rs` at **200 LOC**; `core/src/worker_lifecycle/manager.rs` at **180 LOC**; `core/src/scheduler/tool_dispatch.rs` at **751 LOC** (pre-existing 500-LOC soft-cap breach; slice 1 added ~25 LOC for the new field, the rewire, and the single-use pin test). `core/src/bin/hhagent-cli.rs` at **1432 LOC** still the headline 500-LOC breach pickup.

## Recently completed (this session, 2026-05-18 — worker lifecycle slice 1, branch `feat/worker-lifecycle-slice-1`, not yet merged)
...
- ~~**Worker lifecycle policy — implementation slice 1**~~ **Shipped this session 2026-05-18** on branch `feat/worker-lifecycle-slice-1` (not yet merged). New `core::worker_lifecycle` module with `Lifecycle` enum + `WorkerLifecycleManager` trait + `SingleUseLifecycle` (production, byte-equivalent to today's per-request spawn) + `IdleTimeoutLifecycle` (stub — `acquire()` panics with `unimplemented!()`). `ToolEntry` gains `lifecycle: Lifecycle` field; `ToolHostStepDispatcher` routes through the manager. The `hhagent-supervisor` crate (OS-unit installer) is untouched — the spec's "supervisor is a stub" wording was a misread; slice 1 grew the worker-lifecycle role in `hhagent-core` where it naturally belongs. Test count 721 → 731 (+10). See "Recently completed (this session)" entry at the top. Spec at `docs/superpowers/specs/2026-05-18-worker-lifecycle-policy-design.md`; plan at `docs/superpowers/plans/2026-05-18-worker-lifecycle-slice-1.md`.
- **★ Next concrete engineering pickups (in priority order):**
    1. **Worker lifecycle policy — implementation slice 2** (engineering, this slice unblocks everything inference-worker-shaped). Pre-req: write the slice-2 implementation plan against the same spec. Slice 2 fills in `IdleTimeoutLifecycle::acquire` with spawn-on-demand, post-completion cap evaluation (`idle_seconds` / `max_requests` / `max_age_seconds`), idle teardown, restart-on-crash with exponential backoff, request serialisation queue. Slice 1 already shipped the type-level invariants (`stateless = true` required) and the manager trait; slice 2 swaps the `unimplemented!()` body for the real runtime. Verification: a synthetic test worker declares `IdleTimeout { caps: { idle_seconds: 2, ... } }` and a parameterised test counts spawn invocations (warm-cache reuse vs cold-start).
