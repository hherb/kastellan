...
**Last updated:** 2026-05-15 (storage primitive for the GenericAgent-inspired 5-layer memory hierarchy shipped: `memories.layer SMALLINT NOT NULL CHECK BETWEEN 0 AND 4` + `MemoryLayer` enum + `insert_memory_at_layer` + `load_layer` + new `core::memory::layers::load_l1` wrapper with `cap_rows`/`cap_bytes` hard caps; unblocks L0/L3/L4 writers and the future prompt assembler).
**Last commit (main):** `b1c63e2` (Merge pull request #68 — first real `DeterministicPolicy` rule). Branch `feat/memory-layer-l1-index` ready for PR — 3 commits off `main`: db slice (`b63fe00`), core slice (`326950b`), this docs update.
**Session-end working state (this session, 2026-05-15 fresh):** branch `feat/memory-layer-l1-index` ready for PR. Workspace test count **546 → 556** (+10 exactly as the spec promised — 3 DB integration in `postgres_e2e` + 3 core unit in `memory::layers::tests` + 4 core integration in `memory_layers_e2e.rs`). Zero failures, zero warnings, zero `[SKIP]` lines on Linux. New files under the 500-LOC soft cap (`core/src/memory/layers.rs` 138 LOC incl. tests; `core/tests/memory_layers_e2e.rs` 274 LOC; `db/migrations/0013_memories_layer.sql` 35 LOC; `db/migrations/0014_deleted_memories_layer.sql` 32 LOC). `db/src/memories.rs` 580 → **769** LOC (+189, 269 over the 500-LOC soft cap — pre-existing breach extended; the natural future split is to lift `MemoryLayer`/`insert_memory_at_layer`/`load_layer` into a sibling `memories/layers.rs` once a second consumer outside the test suite materialises). Baseline 544 in the prior HANDOVER was an off-by-2 undercount; real baseline on `b1c63e2` is **546** (verified by `cargo test --workspace`).
