...
**`cargo test --workspace` on Linux: 342 tests passed, 0 failed, 0 `[SKIP]` lines** on branch `feat/scheduler-task-crashed-audit` (off `main` at `2054a16`). Baseline on `main` (post-PR #34 + fixup `e6856d7`) was 341; this session adds **+1** integration test (`sweep_and_audit_emits_one_task_crashed_row_per_recovered_task` in `scheduler_crash_recovery_e2e`). One pre-existing dead-code warning in `core/tests/embedding_recall_e2e.rs::ServedRequest` (introduced in PR #29) is unchanged. Two pre-existing doctests in `hhagent-sandbox` and `hhagent-worker-prelude` are `ignored` (explicit markers).
...
## Recently completed (this session, 2026-05-12 — `task.crashed` audit row from startup crash-recovery sweep)
...
- **Issue #15 — hoist tests-common dev-dep:** still **eight** duplication sites for the per-test PG cluster bring-up. Pure mechanical refactor; not blocking but increasingly cheap to do.
