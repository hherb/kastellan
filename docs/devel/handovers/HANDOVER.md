...
**Last commit on `main`:** `919882d` (merge of PR #117, Item 23(a) test-module sibling lifts) — main hasn't moved since the start of this Slice 2.5 session; the spec/plan/code commits all live on the `feat/gliner-relex-slice-2.5` branch awaiting PR.
**Last commit on `feat/gliner-relex-slice-2.5`:** `f41fa8b` (`fix(test): align gliner_relex container skip helpers with project convention (Task 10 followup)`). Branch carries 18 commits on top of `main` (per `git log --oneline 919882d..HEAD`): the spec doc, the implementation plan, the 10-task TDD-ordered slice, the manual operator image build (Task 9), and 8 review-fixup commits.
...
## Recently completed (this session, 2026-05-25 — Next-TODO Item 25: GLiNER-Relex Slice 2.5 — Containerfile + macOS image build, branch `feat/gliner-relex-slice-2.5`, PR pending — closes [#107](https://github.com/hherb/hhagent/issues/107))
...
- `core/tests/gliner_relex_e2e.rs` empirical-verification gap — the new container-mode e2e exists and is correct structurally but skip-passed on this macOS because Postgres isn't installed; first host with PG + container + image + weights will run it and produce the "real" green.
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    25. ~~**First idle-timeout worker — GLiNER-Relex Slice 2.5 — Containerfile + macOS image build**~~ **SHIPPED 2026-05-25** on branch `feat/gliner-relex-slice-2.5`; PR pending. Closes [#107](https://github.com/hherb/hhagent/issues/107). 18 commits (2 docs + 10 TDD-ordered code/test + 8 review-fixups); operator built `hhagent/gliner-relex:dev` image via the new `scripts/workers/gliner-relex/build-image.sh`. New `workers/gliner-relex/Containerfile` (python:3.12-slim + `uv pip install --system .` + `USER nobody` + `ENTRYPOINT`); `gliner_relex_entry` branches on new `GlinerRelexEnv.use_container_backend` field driven by `HHAGENT_GLINER_RELEX_USE_CONTAINER=1`; `SandboxBackends::resolve` widens to take `image: Option<&str>`; `ToolEntry.container_image: Option<String>` new field; `--init` always-on in `build_container_argv` (closes #107). Workspace **998 → 1011 (+13) macOS**; Python 36 passed. Empirical-verification deferred: the e2e exists structurally but skip-passed at `bring_up_pg` on this Mac (no Postgres); fires green on any host with PG + container + image + weights all present. See "Recently completed (this session, 2026-05-25)" entry at the top for the full breakdown.
