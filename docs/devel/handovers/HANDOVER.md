...
**Last updated:** 2026-05-16 (issue #71 — runner rejects producer-supplied `agent_raised` provenance — shipped on `fix/runner-reject-agent-raised-provenance`; +8 unit tests; workspace 598 → **606**).
...
**Session-end working state (this session, 2026-05-16):** branch `fix/runner-reject-agent-raised-provenance` carries one commit `a6335ab` ready for PR. Workspace test count **598 → 606** (+8 unit tests in a new `scheduler::runner::tests` module). Zero failures, zero warnings, zero `[SKIP]` lines on Linux. `core/src/scheduler/runner.rs` 380 → 538 LOC (just over the 500-LOC soft cap — the new pure helper + its `#[cfg(test)] mod tests` block is +158 LOC. Natural future split candidate: lift the parsing helpers into a sibling `scheduler/payload_validation.rs` once a second helper materialises; not warranted today).
...
**Shape (1 modified file, 1 new pure helper, 8 new unit tests):**

- **`core/src/scheduler/runner.rs`** — extracted the inline `match` over `task.payload["classification_floor_source"]` into a pure helper `parse_classification_floor_source_from_payload(value: Option<&serde_json::Value>) -> Result<ClassificationFloorSource, String>`. The helper rejects the string `"agent_raised"` BEFORE the generic serde parse with a distinct diagnostic that names the contract violation ("reserved for the inner loop's apply_floor_raise"). Producers may only supply `operator` / `cli_inferred` / `default` or omit the field. The "unknown value" generic-reject diagnostic no longer lists `agent_raised` as an expected value (defense-in-depth pin asserts this).
- **New `#[cfg(test)] mod tests` block in `runner.rs`** — 8 unit tests covering: absent field → `Ok(Default)`; each of `operator` / `cli_inferred` / `default` → `Ok(<matching variant>)`; shape error (non-string) → `Err("...not a string...")`; reserved `"agent_raised"` → `Err` containing "agent_raised" AND ("reserved" OR "apply_floor_raise"); unknown string → `Err` containing the bad value + "unknown" or "expected one of"; defense-in-depth pin that the dedicated `agent_raised` reject message does NOT contain "expected one of" and that the generic "unknown" message does NOT advertise `agent_raised` as legal.
...
**Test count delta:** **598 → 606** (+8 unit tests in the new `scheduler::runner::tests` module). Zero failures, zero warnings, zero `[SKIP]` lines on Linux.
...
- ~~[#71](https://github.com/hherb/hhagent/issues/71) — audit-trail integrity: producer-supplied `agent_raised` provenance accepted without validation (filed 2026-05-16 from PR #70 review)~~ **closed this session 2026-05-16** (`fix/runner-reject-agent-raised-provenance`, commit `a6335ab`). New pure helper `parse_classification_floor_source_from_payload` in `core/src/scheduler/runner.rs` rejects `"agent_raised"` BEFORE the generic serde parse with a dedicated diagnostic ("reserved for the inner loop's apply_floor_raise"). The "unknown value" generic-reject diagnostic no longer lists `agent_raised` as expected. +8 unit tests (598 → 606).
