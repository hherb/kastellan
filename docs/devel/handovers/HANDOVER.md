...
**Last commit:** `48dfeee` (`feat(memory): Option N — memory::recall skeleton (semantic + lexical lanes fused via RRF)`) + post-review cleanup (this commit, see "Code-review follow-up" below)
**Branch:** `main`
...
### Code-review follow-up to Options M + N (post-`/review`)

After Options M and N landed, a `/review` pass surfaced four
actionable nits and two design discussions worth tracking. The
nits were fixed in a single follow-up commit; the design
discussions were filed as GitHub issues for the next session.

- **`db::memories::check_embedding_dim(label, v)` extracted** as a
  shared helper used by both `insert_memory` (write path, label
  `"insert"`) and `semantic_search` (read path, label `"query"`).
  Two payoffs: (1) the error-message shape is identical at both
  ends, and the call-site label lets an operator tell INSERT from
  query failures apart in a single grep; (2) the unit test
  exercises the *real* production helper instead of a mirrored
  copy that could drift from the call sites silently. Same change
  for `db::memories::limit_as_i64(k)`: the previous `k as i64`
  cast at two `bind` sites would wrap to a negative for
  `k > i64::MAX`, which Postgres would reject far from the call
  site. The helper saturates at `i64::MAX` instead.

- **`db::memories::fetch_by_ids` doc clarifies dedupe behaviour.**
  Its internal `HashMap::remove` returns the row on first
  occurrence and `None` on later occurrences, so a duplicate id
  in the input list is silently dropped on the second mention.
  RRF (the only production caller today) is keyed by id and
  cannot produce duplicates, but a future caller passing
  arbitrary id lists must not rely on `fetch_by_ids` to expand
  them. One extra paragraph in the doc-comment, no behaviour
  change.

- **`vector_literal` doc-comment correction.** The original
  comment claimed `f32::Display` emits "standard decimal form"
  unconditionally. Rust actually emits the shortest
  round-trippable representation, which is decimal for
  human-scale values (`0.5`, `-1.25`) but scientific for very
  small or very large values (`1e-10`, `3.4e38`). pgvector's
  text parser accepts both forms losslessly, so correctness is
  invisible — but the doc was overstating. Updated to reflect
  reality.

- **Two design discussions filed as GitHub issues** rather than
  fixed in this commit:

  * **Issue #16** — *`tool_host`: `WorkerCommand` seal has an
    ...
    `dispatch`; (b) CI grep guardrail; (c) clippy lint via
    a private marker trait. Defer to the next session — fix is
    "any one of these is enough," not blocking.

  * **Issue #17** — *`memory::recall`: warn-and-degrade on
    missing input may mask caller bugs.* Today, semantic
    enabled + `query_embedding=None` returns `Ok(vec![])` after
    a `tracing::warn`. The doc justifies this as "lets a
    scheduler flip a mode on optimistically without owning the
    input," but a forgotten embedding looks like "no matches"
    to the caller. Three options on the issue (status quo,
    `RecallError::MissingInput`, hybrid). Should land before
    Phase 1's scheduler is the production caller — easier to
    relax than tighten.

- **No production code outside `db/src/memories.rs`** is
  affected. The change is internal: callers (`core::memory::recall`
  and the integration tests) don't observe any signature change.
  The helper functions are private (no `pub`), so the public
  surface is identical.

- **CLAUDE.md compliance.** `db/src/memories.rs` is now 490
  lines (was 467) — under the 500-line bound. No new dependencies.
  No new modules. Zero changes to `core/` or any test file: the
  refactor is a strict win on `db/src/memories.rs` alone.

**Test count:** 247 → **249** on Linux (-1 inline-mirror test
`insert_memory_rejects_wrong_dim`, +3 real-helper tests
`check_embedding_dim_rejects_too_short` + `check_embedding_dim_accepts_correct_length`
+ `limit_as_i64_saturates_at_i64_max`; net +2). 0 failed, 0 `[SKIP]`
lines, 0 warnings. macOS projects to ~196 (was ~194) — every new
test is platform-neutral pure unit.
