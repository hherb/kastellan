...
where possible): new pure `secrets::collect_refs_in_params` (recovers which refs a call carries — `RedemptionEvent.ref_hash`
is one-way, can't be reversed, so we re-scan the **pre-substitution** `req_for_audit` snapshot); `Vault::value_fingerprint`
(reused, fingerprints under the lock, never exposes plaintext); `egress::leak_provision::merge_secret_hashes` (read-merge-
atomic-write, **union** across worker reuse, dedup by sha256, returns newly-added) + `provision_failed_audit_row`;
`EgressSidecar::provision_dispatch_secrets` (resolves the scratch dir = sidecar UDS parent); and a `tool_host/egress_provision`
helper (`compute_provision` runs **synchronously** so the `&EgressSidecar` borrow is released before `worker.call`;
`emit_provision` writes audit rows). **Decisions:** D1 **fail-closed** (write fails ⇒ dispatch refused + `egress.secret_hash.provision_failed`
row, worker never egresses unscanned), D2 **union** across reused IdleTimeout workers, D3 **audit only newly-added** fps
(one `egress.secret_hash.provisioned` row each, `name` = the secret's `ref_hash` so it correlates to the `secret.redeemed`
...
Landlock active on Linux (RO from `fs_read`, RW=`["/tmp"]` for torch's inductor cache), closing the last deferred FS half —
**#281 FULLY CLOSED, both pure-Python workers now have seccomp + Landlock.** Condensed into "Recently merged" below.)_
...
macOS (2026-06-17, #268 — **live PG up**, sandbox + PG e2e actually ran): full workspace **1855 / 0 / 13, no `[SKIP]`** +
clippy `--workspace --all-targets -D warnings` clean. (Prior macOS skip-as-pass posture without PG was **1841 / 0 / 13**,
2026-06-16.) **DGX native-Linux not re-run for #268** — the dispatch-provisioning path is hermetic + its no-op for non-net
workers is proven by the live-PG `shell_exec_e2e`; the 1839/0/15 Linux baseline is carried forward as the standing gate.
...
**Egress follow-ups now that the proxy is feature-complete (each small, on demand):** (1) **slice #4 frontier wiring** — read the operator's real frontier pin config on the daemon + route frontier LLM egress through a **pinned** sidecar (lands with the first frontier worker / the Phase-5 escalation path; today's callers pass `cert_pins_json: None`). ~~(2) slice #3b dispatch-time live-append ([#268])~~ — **DONE 2026-06-17** (this session, branch `feat/268-egress-dispatch-time-provisioning`): `tool_host::dispatch` now provisions each materialized secret's fingerprint into the force-routed worker's sidecar `secret_hashes.json` before egress (fail-closed, union, `ref_hash`-keyed audit). Activates with the first secret-bearing egress worker. (The spawn-time `secret_fingerprints` field stays `&[]`; the live path is the dispatch hook.)
...
- **(a) Clean test-lifts** (lifting the inline `mod tests` block alone lands the parent under cap): **none meaningfully remaining.** The substantial ones are done — `cassandra/types.rs`, `inner_loop_audit.rs`, `entity_extraction/gliner_relex.rs` (2026-06-07 batch); `macos_seatbelt.rs` (PR #192); `recall.rs`/`l0_seed.rs`/`capture.rs`/`inner_loop.rs`/`replay.rs` (Earlier history). A fresh census shows only files sitting **1–27 LOC over cap** still carry a liftable block (`core/src/main.rs` 527, `db/src/lib.rs` 525, `core/src/bin/kastellan-cli/memory_l3/run.rs` 519, `core/src/cassandra/constitutional.rs` 502, `core/src/memory/l1_promote.rs` 501) — a lift would save little; defer unless one grows. **`core/src/tool_host.rs` is now 609** (was already 584 over cap on `main` before #268; the #268 dispatch hook added ~25, with the bulk kept out in `tool_host/egress_provision.rs`). A real prod-split of `tool_host.rs` (its tests already live under `tool_host/`) is the leading over-cap candidate now — needs a seam (e.g. lift `dispatch_with_sink`'s audit-emission arms into a sibling).
