...
**Last updated:** 2026-05-29 (Issue [#156](https://github.com/hherb/hhagent/issues/156) — `injection_guard::walk()` depth-skip now continues scanning siblings instead of aborting the whole walk. Branch `fix/issue-156-walk-sibling-continue`, code commit `43598b4`, PR **pending** (open after this docs commit). Also reconciled the stale "PR #157 not yet merged" header in the prior commit `908fd6d`.)

**Last commit on `main`:** `53e68ed` (Merge pull request #157 from hherb/fix/issue-148-149-secret-vault-test-gaps). Confirm with `git log --oneline -1`. The #156 work below lives on branch `fix/issue-156-walk-sibling-continue` (docs-reconcile `908fd6d` + fix `43598b4` + this docs commit), PR pending.
...
**Done this session (on branch `fix/issue-156-walk-sibling-continue`, commit `43598b4`, PR pending):** Issue [#156](https://github.com/hherb/hhagent/issues/156) — `walk()` depth-skip now continues scanning siblings (see Recently-completed). **Done last session (merged to `main` via PR [#157](https://github.com/hherb/hhagent/pull/157) at `53e68ed`):** Issues [#148](https://github.com/hherb/hhagent/issues/148) + [#149](https://github.com/hherb/hhagent/issues/149) — secret-vault test gaps closed; both CLOSED. **Done earlier:** PR [#155](https://github.com/hherb/hhagent/pull/155) (#143 `walk()` depth guard) merged to `main` at `6e82252`; #143 closed; that merge filed [#156](https://github.com/hherb/hhagent/issues/156) (still OPEN). PR [#152](https://github.com/hherb/hhagent/pull/152) (#144 + #150) merged at `560d845`; `linux-check` CI green. Still optional (operator action, no code): a full `cargo test --workspace` on the DGX — CI is compile-only, so the Linux *test* baseline is still the stale `990`-at-`1abb061`.
