...
```
kastellan (Rust workspace, 17 crates [core, db, leak-scan, llm-router, sandbox, supervisor, protocol, tests-common, prelude, shell-exec, web-common, web-fetch, web-search, browser-driver, python-exec, egress-proxy, matrix, matrix-wire + mail .gitkeep stub], AGPL-3.0)
...
3. **Wire `disable_mitm`-by-worker-name** for the matrix worker in `core/src/egress/spawn.rs` (find the existing call for browser-driver; add an analogous arm for the matrix worker name).
