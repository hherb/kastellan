...
**Last updated:** 2026-05-18 (L1 promotion writer **MERGED via PR #82 at `eb6b8a8`**; subsequent pre-PR fixes at `a062896` then two forward-looking design specs landed directly on main: entity-extraction + graph-lane v1 design (`8a5e6f0`) and worker-lifecycle-policy + GLiNER-Relex feasibility study (`99e97cf`); ROADMAP synced at `9893f3a`. No new code or tests after the PR-#82 merge — only docs/specs. Workspace test count remains **721**.).
**Last commit on `main`:** `9893f3a` (`docs(roadmap): mark L1 promotion writer merged + add worker lifecycle + entity extraction entries`). Recent main history: `9893f3a` (ROADMAP sync) → `99e97cf` (worker-lifecycle + GLiNER-Relex specs) → `eb6b8a8` (PR #82 merge of `feat/l1-promotion-writer`) → `8a5e6f0` (entity-extraction graph-lane v1 design — committed pre-merge but ordered after by topology) → `a062896` (pre-PR review fixes: doc-accuracy correction on `Plan::l1_insight`, `memory l1 remove` arity check matching Task 11 hardening, `build_l1_metadata` tightened to `pub(crate)`, clippy `print_literal` fix) → `aa2957c` (handover bump on the branch before merge).
**Session-end verification:** `cargo test --workspace` on `main`: **721 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux (pin from the merged branch — no code changes since). `core/src/scheduler/inner_loop.rs` is still **1214 LOC** (~714 over the 500-LOC soft cap); **[Issue #81](https://github.com/hherb/hhagent/issues/81)** for the `inner_loop_audit.rs` split is the highest-priority engineering pickup. `core/src/memory/l1_promote.rs` at **453 LOC** (well under cap). `core/src/bin/hhagent-cli.rs` at **1419 LOC** (pre-existing breach).
...
- **★ Next concrete engineering pickups (in priority order):**
    1. **[Issue #81](https://github.com/hherb/hhagent/issues/81) — split `core/src/scheduler/inner_loop.rs` (1214 LOC)** (engineering, pure refactor). Pre-existing 500-LOC breach is ~714 over cap. Lift `build_plan_formulate_payload` + the three `write_audit_*` async writers + their slice-narration doc paragraphs + their unit tests into `core/src/scheduler/inner_loop_audit.rs`. One-session pickup; zero behavioural change; no public API surface bump (the only export crossing the module boundary is `pub(crate) fn build_plan_formulate_payload`, which stays `pub(crate)` and is re-exported via `pub(crate) use` if needed, or callers update their `use` paths).
    2. **Worker lifecycle policy — implementation slice 1** (engineering). Spec landed 2026-05-18 at `99e97cf`. First implementation slice would bump `hhagent-supervisor` from stub to lifecycle manager with the `single_use` policy implemented (shell-exec keeps current behaviour, no semantic change), `idle_timeout` defined but not yet consumed. Unblocks v2 entity extraction + every future inference worker. Pre-req: write an implementation plan against the spec.
    ...
    4. **Entity extraction + graph-lane wiring v1 — implementation** (engineering). Spec at `8a5e6f0`. Pre-req: write an implementation plan; consider whether to skip v1 in favour of v2 once worker-lifecycle ships and GLiNER-Relex POC validates feasibility.
