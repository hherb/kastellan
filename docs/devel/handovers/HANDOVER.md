...
**Last updated:** 2026-05-29 (Next-TODO **item 5**: `tool_host.rs` sibling-lift — lifted the watchdog subsystem, the lockdown-env derivation, and the seal tests into three child modules; production LOC 911 → 519. Branch `refactor/item-5-tool-host-sibling-lift`, code commit `379f77a`. This branch also carries the post-#159 docs-reconcile commit as a ride-along. **This session also reconciled the stale header** — PR [#159](https://github.com/hherb/hhagent/pull/159) (item 4b) merged to `main` at `1106145`.)

**Last commit on `main`:** `1106145` (Merge pull request #159 from hherb/refactor/injection-guard-test-module-lift). Confirm with `git log --oneline -1`. The item-5 lift below lives on branch `refactor/item-5-tool-host-sibling-lift` (reconcile commit + lift `379f77a` + this docs commit); not yet a PR.
...
## Recently completed (this session, 2026-05-29 — Next-TODO **item 5**: `tool_host.rs` sibling-lift, branch `refactor/item-5-tool-host-sibling-lift`, code commit `379f77a` — not yet a PR)
...
5. ~~**`tool_host.rs` sibling-lift.**~~ **DONE this session** (branch `refactor/item-5-tool-host-sibling-lift`, code commit `379f77a`): lifted the watchdog subsystem → `tool_host/watchdog.rs`, lockdown-env derivation → `tool_host/lockdown_env.rs`, and the seal tests → `tool_host/tests.rs`; production LOC **911 → 519**. The dispatch chokepoint, `WorkerCommand` seal, and `SupervisedWorker` stay in the parent (trust boundary). 519 is 19 over cap — the irreducible trust-boundary residual. See Recently-completed. **Not yet a PR** — operator to push + open.
