...
**Last updated:** 2026-05-19 (Memory-write-time entity auto-linker — **merged to `main` via PR #92 at `d58ecc9`** + post-review fixup `8c6cded`, **workspace 834 → 848 (+14)** with 0 failures / 0 warnings / 0 [SKIP]).

**Last commit on `main`:** `d58ecc9` (`Merge pull request #92 from hherb/feat/memory-entity-link`) with post-review fixup `8c6cded` immediately preceding it on the branch.

**Session-end verification:** **Rust workspace: 848 passed / 0 failed / 4 ignored / 0 warnings on Linux, 0 [SKIP] lines** (`cargo test --workspace` on the DGX, branch `main`).

## Recently completed (this session, 2026-05-19 — Memory-write-time entity auto-linker, merged to `main` via PR #92 at `d58ecc9` + post-review fixup `8c6cded`)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    2. **★ Operator quarantine-review UI / CLI** (engineering, unblocked by PR #91 merge; **now the #1 unblocking pickup**) — `hhagent-cli entities review` lists quarantined entities (filterable by kind / age / mention count), supports `unquarantine` / `delete` / `merge` actions. Until this lands, the graph lane stays observably empty in production even with v2 + the auto-linker running — every newly-extracted entity is quarantined and invisible to recall. The auto-linker (item 3) now populates `memory_entities` rows, but `graph_search` filters quarantined entities by default, so the lane still returns 0 rows until the operator approves entities. Single-session slice, design pattern is the L0 seed loader. **Note**: post-cleanup `2cf2a0a` migration `0016` REVOKEs runtime writes on `entity_kinds` (not on `entities` — `quarantine` flips remain runtime-writable). The quarantine-review CLI uses the existing runtime role for `UPDATE entities SET quarantine = FALSE` / `DELETE FROM entities` / merge actions; only kind-vocabulary edits would need elevation.
