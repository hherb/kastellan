...
**Last updated:** 2026-05-23 (Next-TODO Item 23 option (a) — test-module sibling lifts shipped on branch `refactor/item-23-test-module-lifts`, PR pending. Two pure-refactor commits: `db/src/graph.rs` 761 → 673 LOC + new `graph/tests.rs` 96 LOC; `core/src/bin/hhagent-cli/relations_show.rs` 576 → 380 LOC (now well under cap, closing the 76-LOC inline-tests overage flagged at the end of Item 22) + new `relations_show/tests.rs` 203 LOC. Both use Rust 2018 sibling-directory module resolution (`mod tests;` in parent resolves to `<parent>/tests.rs`); `use super::*` continues to import from the parent module so test names + behaviour are byte-equivalent. macOS workspace **998 / 0 / 3 unchanged**; existing tests are the regression pin. Option (b) — walk-impl split into `graph/walk.rs` — deliberately deferred per the design note ("preferred if a second `WalkedEdge` consumer materialises… until then, (a) is the cheaper one"). The `graph.rs` 673 LOC residual stays over cap; called out for follow-up. Earlier this session: docs sync on `main` (`9a2ea91`) backfilling PR #116 (Item 22) merge.
...
**Last commit on `main`:** `9a2ea91` (`docs(handover): backfill Item 22 merge — PR #116 at 1abb061`). Earlier on `main` this session: PR #116 (Item 22) merge at `1abb061`, PR #113 (relations show CLI) at `9a46e18`. Previous session: PR #109 (relations kinds CLI) at `f234d0c` and PR #110 (entities kinds CLI, stacked) at `a65bb4a`. Two follow-up issues filed against the PR-#110 review: [#111](https://github.com/hherb/hhagent/issues/111) and [#112](https://github.com/hherb/hhagent/issues/112) — **both closed by PR #116**.
**Last commit on `refactor/item-23-test-module-lifts`:** `72cd3e7` (`refactor(db): lift graph.rs inline tests into sibling graph/tests.rs`). Branch carries 2 commits on top of `9a2ea91`: relations_show test-module lift (`40d9be2`) + graph.rs test-module lift (`72cd3e7`).

**Session-end verification (Item 23 option (a) on `refactor/item-23-test-module-lifts`, 2026-05-23):** **Rust workspace on macOS (M3 Max): 998 passed / 0 failed / 3 ignored** at branch tip — unchanged from the Item 22 baseline (pure refactor; existing tests are the regression pin). Linux DGX expected to remain at 990 / 0 / 4 by the same argument. Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed unchanged. The pre-existing 3 doc-list-indent warnings in `db/src/probe.rs` and 2 `io_other_error` warnings in `hhagent-protocol` are unchanged; no new clippy warnings introduced. Item 23 test counts: 6 `graph::tests::*` + 17 `relations_show::tests::*` — same names, same behaviour, only the storage location changed (parent module path is unchanged via Rust 2018 sibling-directory resolution).
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    23. ~~**`db/src/graph.rs` test-module lift / walk-impl split** — option (a) test-module lifts~~ **SHIPPED this session 2026-05-23** on branch `refactor/item-23-test-module-lifts` (PR pending). Two test-module sibling lifts bundled (HANDOVER Item 22 flagged them as natural-to-bundle): `db/src/graph.rs` 761 → 673 LOC (still over cap; option-(b) walk-impl split deliberately deferred per the design note) and `core/src/bin/hhagent-cli/relations_show.rs` 576 → 380 LOC (now well under cap, closing the 76-LOC inline-tests overage flagged at the end of Item 22). Both use Rust 2018 sibling-directory module resolution — `mod tests;` in the parent file resolves to `<parent>/tests.rs`. Test count unchanged at macOS 998 / 0 / 3, Linux DGX 990 / 0 / 4 (existing tests are the regression pin for the move; same names, same behaviour, same paths-up-to-storage). See "Recently completed (this session)" entry above for full breakdown.
