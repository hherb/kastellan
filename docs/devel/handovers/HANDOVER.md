...
**Last updated:** 2026-06-27 (**Firecracker micro-VM slice-1 follow-ups #363 + #360 — DONE, both-platform verified, PRs [#365](https://github.com/hherb/kastellan/pull/365) + [#366](https://github.com/hherb/kastellan/pull/366) open (off `main`@`2818708`; #366 stacked on #365).**
Slice 1 itself is MERGED to `main` (`2818708`, PR #364).
**#363 — split `core/src/workers/python_exec.rs`** (594 → 283 LOC, back under the rule-4 cap): the three `*_mode_entry` builders + warm/idle lifecycle helpers (`parse_idle_caps`/`container_lifecycle`) + interpreter dep helpers lifted into a new sibling `python_exec/entries.rs` (359 LOC); the parent keeps the manifest+resolver and re-exports the builders so every `python_exec::<name>` call site (resolver + e2e) is unchanged. Also fixed the fused doc comment (m6b — `container_mode_entry`'s rustdoc had bled onto `parse_idle_caps`). Test-import gotcha codified: `pub(crate) use` re-exports consumed only by `#[cfg(test)]` trip unused-import under `--all-targets` on the *non-test* lib build → the entry-internal consts/`ToolEntry`/`Net`/`Profile` are imported in `tests.rs` directly (the macOS-only warm-idle ones `#[cfg(target_os="macos")]`-gated). No behavior change. macOS python_exec **30/0** + core lib **1079/0** + clippy `--all-targets -D warnings` clean; **DGX** python_exec **22/0** + workspace `--all-targets` clippy clean.
**#360 — forward `policy.env` into the guest** (slice-1 left it inert — threaded to `FirecrackerLaunchPlan.env` but never rendered; the `params_file_max` operator override fell back to the 1 MiB default in-VM). **Transport = kernel cmdline** (the one host→guest channel a `Net::Deny` guest has — no NIC for MMDS, rootfs read-only/shared): `sandbox/plan.rs` pure `encode_env_cmdline` hex-encodes the `K=V\n…` block into a ` kastellan.env=<hex>` token appended to `boot_args`; `build_launch_plan` enforces a **fail-closed 1 KiB cmdline cap** (truncating a cmdline corrupts the boot). `microvm-init` pure `parse_env_cmdline`+`hex_decode` read `/proc/cmdline` and apply the env **over** the baked defaults before `exec` (**fail-safe**: a missing/garbled token keeps the baked defaults → still boots). The hex codec is a manually-kept-in-sync pair across the crate boundary (like `WORKER_VSOCK_PORT`), pinned by an **identical roundtrip fixture in both crates**. **Latent bug fixed:** `firecracker_mode_entry` set PYTHON `/usr/local/bin/python3` but the rootfs/init reality is `/usr/bin/python3` — only harmless while env was never forwarded; corrected. Verification — macOS: microvm-init **6/0**, sandbox Linux cross-clippy `--all-targets -D warnings` clean; **DGX (aarch64, real KVM):** workspace+clippy `--all-targets -D warnings` clean, sandbox `linux_firecracker` **18/0**, microvm-init **6/0**, python_exec **22/0**, **rootfs rebuilt** with the new init, firecracker **e2e 5/5** incl. the new differential `microvm_forwarded_params_file_max_is_enforced_in_guest` (a forwarded 80 000 ceiling rejects a 100 KiB param the default 1 MiB accepts — `microvm_large_params_ride_file_channel` is the negative control), **0 orphaned VMs**. Spec `docs/superpowers/specs/2026-06-27-firecracker-guest-env-forwarding-design.md`. **Remaining slice-1 follow-up: [#362](https://github.com/hherb/kastellan/issues/362)** per-spawn run-dir (`/tmp/kastellan-microvm-*` fc.json/log) RAII cleanup (open). **Slices 2–5 (warm/idle reuse, fs-sharing via per-spawn block devices, net workers over a 2nd vsock, jailer) are the next micro-VM arc.**
...
**★ Firecracker micro-VM SLICE 1 is COMPLETE + MERGED (`2818708`, PR [#364](https://github.com/hherb/kastellan/pull/364)); follow-ups #363 + #360 DONE this session (PRs #365 + #366, see header).** The leading micro-VM pick is now **slice 2+** (a fresh spec/plan, on demand): warm/idle VM reuse (mirror the macOS `IdleTimeout` lifecycle, amortise the ~0.2 s boot), host-dir fs-sharing (per-spawn block devices — Firecracker has no virtio-fs/9p), net workers (egress-proxy UDS over a 2nd vsock), and the jailer. Each is a hard sub-problem the spec staging table anticipates. **Remaining slice-1 follow-up (open, on demand):** [#362](https://github.com/hherb/kastellan/issues/362) per-spawn run-dir RAII cleanup (`/tmp/kastellan-microvm-*` fc.json/log accumulate — the issue flags it's "not a quick fix": the dir must outlive the `Child`, and `spawn_under_policy` returns a bare `Child` with no teardown hook → needs a lifecycle-owned RAII run-dir or a dead-pid reaper like the egress stale-scratch sweep #251). **DGX spike artifacts at `/tmp/fc-spike` (reusable); the DGX microvm rootfs at `/var/lib/kastellan/microvm/` is current as of #360.**
