...
**Last updated:** 2026-06-28 (**Firecracker micro-VM SLICE 4b — first real net worker in a VM (web-fetch) — DONE, DGX-verified, branch `feat/firecracker-microvm-slice4b-web-fetch` (off `main`@`bed1326`), PR pending.**
...
**★ Firecracker micro-VM SLICES 1 + 2 + 3 + 4a + 4b are COMPLETE.** Slice 1 merged (`2818708`, PR [#364](https://github.com/hherb/kastellan/pull/364)) + follow-ups (#363/#360/#362). Slice 2 merged (`555f611`, PR [#369](https://github.com/hherb/kastellan/pull/369)). Slice 3 merged (`b12f0dc`, PR [#371](https://github.com/hherb/kastellan/pull/371)). Slice 4a merged (`bed1326`, PR [#373](https://github.com/hherb/kastellan/pull/373)). **Slice 4b (web-fetch in a VM) — DONE, DGX-verified, branch `feat/firecracker-microvm-slice4b-web-fetch`, PR pending** (see header): the web-fetch net worker runs in a VM reaching the egress proxy over the 4a vsock channel; per-worker rootfs filename (`KASTELLAN_MICROVM_ROOTFS`), file-aware CA delivery into the guest, `kastellan.worker=<hex>` exec-path forwarding (root-cause fix for the baked python path), `KASTELLAN_WEB_FETCH_USE_MICROVM=1` opt-in.
