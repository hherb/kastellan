...
**Last updated:** 2026-05-21 (continuation — Issue #55 macOS `container` discovery spike, branch `spike/issue-55-macos-container-discovery`, PR pending. Verdict: **COMMIT** — Apple `container` 0.12.3 is a viable macOS micro-VM backend for the existing `SandboxBackend` trait. Spike notes shipped at [`docs/superpowers/specs/2026-05-21-macos-container-spike-notes.md`](../../superpowers/specs/2026-05-21-macos-container-spike-notes.md). Spike installed `container` via Homebrew + the kata-containers static kernel + pulled `alpine:3.20`; exercised the full primitive matrix (stdio JSON-RPC round-trip, memory cap via SIGKILL, FS bind-mount, `--network none`, `--cap-drop ALL`, `--read-only` + `--tmpfs`); measured cold + warm spawn latency. Header sync earlier in this session: PRs #102 + #103 from the prior session-continuation are now both merged to `main`.

**Last commit on `main`:** `9220f40` (`Merge pull request #103 from hherb/feat/gliner-relex-macos-slice`).
**Last commit on `spike/issue-55-macos-container-discovery`:** `85f82d1` (`docs(spike): Issue #55 — macOS Apple `container` discovery spike notes`). Branch carries 2 commits on top of `main`: `5481bd8` (HANDOVER header sync) + `85f82d1` (spike notes + HANDOVER/ROADMAP updates).

**Session-end verification:** **No code changes this session** — discovery spike (docs-only). Test counts unchanged from the prior session-continuation: **Rust workspace on macOS post-merge of #102 + #103: 866 passed / 0 failed / 3 ignored** (last full `cargo test --workspace --no-fail-fast` was on the `feat/gliner-relex-macos-slice` tip pre-merge; commit-graph is byte-equivalent post-merge). **Linux DGX (expected, last verified pre-merge of #100): 896 passed / 0 failed / 4 ignored** (+1 from PR #103's `entry_forwards_device_verbatim_regardless_of_value` Rust test → expected ~897 on next DGX run; verify when the next operator pickup runs `cargo test --workspace` on the DGX and update this line with the actual number). **Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed** (matches prior session-continuation count). 4 `[SKIP]` lines on Linux `--nocapture` are GLiNER-Relex real-model tests gated on `HHAGENT_GLINER_RELEX_ENABLE=1`. **Spike verification (Apple `container` 0.12.3 on M3 Max / macOS 26.5):** stdio JSON-RPC round-trip clean (5 sequential requests through one `container run -i` in 0.76 s total); memory cap @ `-m 256M` enforced via SIGKILL exit 137 at ~192 MiB allocated; FS isolation by default (`/Users`, `/home`, host `/etc/passwd` invisible); `--mount type=bind,...,readonly` rejects writes with `Read-only file system`; `--cap-drop ALL` makes `ping` fail `permission denied`; cold-start 14.5 s first run, **0.76–0.81 s warm spawn** (80× bwrap, 15× Seatbelt), per-call cost inside a long-lived container effectively zero (matches `IdleTimeoutLifecycle` shape).

## Recently completed (this session, 2026-05-21 — Issue #55 macOS `container` discovery spike, branch `spike/issue-55-macos-container-discovery`, PR pending)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    6. ~~**[Issue #55](https://github.com/hherb/hhagent/issues/55) — macOS `container` micro-VM discovery spike**~~ **SHIPPED this session 2026-05-21** on branch `spike/issue-55-macos-container-discovery` (PR pending). Verdict: **COMMIT** — Apple `container` 0.12.3 is a viable macOS micro-VM SandboxBackend. Spike notes at [`docs/superpowers/specs/2026-05-21-macos-container-spike-notes.md`](../../superpowers/specs/2026-05-21-macos-container-spike-notes.md). Recommended Slice 1 (1 session) adds `MacosContainerBackend` as a sibling to `MacosSeatbelt` selected per-worker; primitive-mapping table + 3-slice rollout plan in the spike notes. See "Recently completed (this session)" entry at the top.
    ...
    16. **`hhagent-cli relations kinds` subcommand tree** (engineering, follow-up to slice 14) — parallel to operator CLI vocabulary management. Same posture as `entity_kinds`: today operators extend via direct `INSERT INTO relation_kinds`. If vocabulary management becomes frequent, ship `relations kinds {add, remove, list}` mirroring how `entities` was built on top of `entity_kinds`. Low priority until at least one operator-driven vocabulary extension lands.
    17. **`MacosContainerBackend` — Slice 1 (1 session, unblocked by spike Item 6 above)** — new `sandbox/src/macos_container.rs` implementing `SandboxBackend`. Mirror `linux_bwrap.rs`'s pure-function `build_argv` pattern: `pub fn build_container_argv(policy: &SandboxPolicy, image: &str, cmd: &[OsString]) -> Vec<OsString>` is the testable core. `probe()` checks `container --version` exit 0 + `container system status` returns `running`; fails closed if either fails (operator sees the failure, no degraded green). New pure helper `clamp_memory_to_minimum(mem_mib: u64) -> ClampedMemory` (or `(u64, bool)`) encodes the 200 MiB floor *and* returns a flag so the callsite in `build_container_argv` can emit `tracing::warn!(requested, clamped_to = 200, "container backend raised mem_mb below 200 MiB floor")` exactly when clamping fires — operators see silent policy widening. Unit-test battery: 1 MiB → (200, true); 100 MiB → (200, true); 256 MiB → (256, false); 1 GiB → (1024, false). Smoke test `macos_container_smoke.rs` exercises bind-mount + memory-cap-via-SIGKILL + stdio JSON-RPC round-trip + `--cap-drop ALL`-blocks-ping + `--network none`-blocks-curl + `--read-only`-rejects-writes (parallel to `macos_smoke.rs` for Seatbelt). **Defense-in-depth stderr-mux test:** pin `tool_host` resilience against unexpected stderr lines (so future `container` releases that remove `--progress none` don't break tool dispatch — JSON-RPC stdout response still parses, noise is logged but not fatal). **Field doc-comment updates** (small but load-bearing for cross-platform correctness): widen the `Linux only` notes on `SandboxPolicy::{mem_mb, cpu_quota_pct, tasks_max}` to reflect macOS support via the container backend; call out the `tasks_max` semantic difference (cgroup `pids.max` per-cgroup on Linux vs `RLIMIT_NPROC` per-UID on macOS) in that field's doc-comment. **Worker image:** use plain `alpine:3.20` for Slice 1's smoke (no Containerfile build pipeline yet — that's Slice 2.5). **Out of scope for Slice 1:** per-worker backend selection (`WorkerSpec.sandbox_backend`, that's Slice 2); `gliner-relex` Containerfile (Slice 2.5); migrating any production worker to use the new backend (Slice 3). Full primitive-mapping table in [`docs/superpowers/specs/2026-05-21-macos-container-spike-notes.md`](../../superpowers/specs/2026-05-21-macos-container-spike-notes.md). Estimated effort: 1 session.
    18. **`MacosContainerBackend` — Slice 2 (0.5–1 session, depends on Slice 1)** — `WorkerSpec.sandbox_backend: Option<SandboxBackendKind>` enum (`Seatbelt | Container | Bwrap`); `default_sandbox()` on darwin returns `Seatbelt` unless `spec.sandbox_backend == Some(Container)`. **Validation target is a plain `alpine` smoke worker, not `gliner-relex`** — proves the backend-selection plumbing works without depending on the image-build pipeline. Real-model validation against `gliner-relex` moves to Slice 2.5. Estimated effort: 0.5–1 session.
    19. **`MacosContainerBackend` — Slice 2.5 (1 session, depends on Slice 2)** — `workers/gliner-relex/Containerfile` (Python 3.12 + `uv sync` + weights mounted at `/weights` via `--mount`) + operator-runnable `container build -t hhagent/gliner-relex:dev workers/gliner-relex/` step (no `cargo build` automation — that's a future slice). Update `gliner-relex`'s `WorkerSpec` to `sandbox_backend: Some(Container)` + the image tag, then re-run the e2e on macOS and confirm canonical `Dr Smith --[treats]--> asthma (0.994)` output through the container. This slice is where the end-to-end story on a real workload that needs memory enforcement actually lands. Estimated effort: 1 session.
