...
1. **`docs/superpowers/specs/2026-05-21-macos-container-spike-notes.md`** — 138-line write-up answering the 4 discovery questions from issue #55 (CLI stability, JSON-RPC stdio over the container boundary, `SandboxPolicy` mapping shape, cold-start latency), with verbatim mappings from each `SandboxPolicy` field to its `container run` flag, the recommended 3-slice shape, and the back-out procedure. Follows the existing spike-notes filename convention (`docs/superpowers/specs/<date>-<topic>-spike-notes.md`, parallel to `2026-05-18-gliner-relex-spike-notes.md`).
...
- **`SandboxPolicy` mapping:** clean 1-to-1. `fs_read/fs_write` → `--mount type=bind,source=...,target=...[,readonly]`; `mem_max` → `-m <N>` with a **200 MiB minimum floor** (clamped); `cpu_ms` → existing `workers/prelude::rlimit::apply_from_env` via `HHAGENT_CPU_MS` env (POSIX `RLIMIT_CPU` inside the Linux VM, unchanged code path); `cpu_quota_pct` → `-c <fractional>`; `tasks_max` → `--ulimit nproc=<N>:<N>`; `env` → `-e <key>=<value>`; `net::Deny` → `--network none` (verified loopback-only); `net::Allowlist` → `--network default` + future egress proxy (same architectural posture as Linux); `Profile::WorkerStrict` → `--read-only --cap-drop ALL --network none --user nobody --tmpfs <scratch>`.
...
- **No Linux Firecracker counterpart.** Linux already has cgroup memory enforcement; Firecracker would be defense-in-depth strengthening, not a parity fix. Out of scope per issue #55.
...
**Test count delta:** 0 (docs-only). Workspace stays at 866/0/3 on macOS, projected 897/0/4 on Linux DGX (next run).
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    17. **`MacosContainerBackend` — Slice 1 (1 session, unblocked by spike Item 6 above)** — new `sandbox/src/macos_container.rs` implementing `SandboxBackend`. Mirror `linux_bwrap.rs`'s pure-function `build_argv` pattern: `pub fn build_container_argv(policy: &SandboxPolicy, image: &str, cmd: &[OsString]) -> Vec<OsString>` is the testable core. `probe()` checks `container --version` exit 0 + `container system status` returns `running`; fails closed if either fails (operator sees the failure, no degraded green). New pure helper `clamp_memory_to_minimum(u64) -> u64` encodes the 200 MiB floor with a unit-test battery (1 MiB → 200 MiB; 100 MiB → 200 MiB; 256 MiB → 256 MiB; 1 GiB → 1 GiB). Smoke test `macos_container_smoke.rs` exercises bind-mount + memory-cap-via-SIGKILL + stdio JSON-RPC round-trip + `--cap-drop ALL`-blocks-ping + `--network none`-blocks-curl + `--read-only`-rejects-writes (parallel to `macos_smoke.rs` for Seatbelt). **Worker image:** use plain `alpine:3.20` for Slice 1's smoke (no Containerfile build pipeline yet — that's a separate slice). **Out of scope for Slice 1:** per-worker backend selection (`WorkerSpec.sandbox_backend`, that's Slice 2); image-build automation (deferred); migrating any production worker to use the new backend (Slice 3). Full primitive-mapping table in [`docs/superpowers/specs/2026-05-21-macos-container-spike-notes.md`](../../superpowers/specs/2026-05-21-macos-container-spike-notes.md). Estimated effort: 1 session.
    18. **`MacosContainerBackend` — Slice 2 (0.5–1 session, depends on Slice 1)** — `WorkerSpec.sandbox_backend: Option<SandboxBackendKind>` enum (`Seatbelt | Container | Bwrap`); `default_sandbox()` on darwin returns `Seatbelt` unless `spec.sandbox_backend == Some(Container)`. One-line operator opt-in for `gliner-relex` on macOS to validate end-to-end with the real model (cross-validates the container backend's stdio + memory shape against a workload that genuinely benefits from memory enforcement). Estimated effort: 0.5–1 session.
