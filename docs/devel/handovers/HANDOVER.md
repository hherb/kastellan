...
**Last commit on `main`:** `920e0c9` (merge of PR [#123](https://github.com/hherb/hhagent/pull/123), Item 26 — `gliner_relex.rs` test-module sibling lift). Branch carried 3 commits on top of `main = a9e3385`: `4e6bbe5` (docs sync) + `cbbdf2d` (the mechanical lift, 1547 → 811 LOC) + `c4dd3ee` (post-review trim of historical commentary in the new tests.rs module-doc header). Pure refactor; workspace stays at macOS **1011 / 0 / 3**.

Earlier this multi-day session, PR [#118](https://github.com/hherb/hhagent/pull/118) (Item 25 GLiNER-Relex Slice 2.5) merged at `a9e3385` with post-merge `e11e3f3` (test-infra hardening) + `1c7eced` (review-comment cleanups). Doc-only PR [#119](https://github.com/hherb/hhagent/pull/119) (developer onboarding manual at `docs/devel/manual/`) merged earlier as `99bbfab`.
