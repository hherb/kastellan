...
--all-targets -D warnings` clean. **DGX live round-trip NOT yet run** — needs operator homeserver setup (run
`scripts/matrix/setup-conduwuit.sh`, create 2 accounts + a shared **encrypted** room both joined, then the `#[ignore]` e2e);
the live worker has also not yet been compiled on aarch64 Linux (first DGX `--features live-matrix` build is the cross-platform
gate to run). Default-feature Linux baseline (1839/0/15) is carried forward (matrix live path is `live-matrix`-cfg-gated; the
`force_route` change is a platform-agnostic pure-fn refactor, clippy+unit covered). **Remaining Phase D:**
...
**Matrix Phase D live `LiveSdk` is DONE this session** (see the header up top) — `sdk_live.rs` + worker `main.rs`
live serving + core `disable_mitm_for` + the `#[ignore]` `matrix_live_e2e.rs`, all hermetically green on macOS.
The two Matrix follow-ups below are the natural continuation.

**★ TOP PICK — DGX live verification of the LiveSdk (the acceptance the spike deferred).** The code is written +
hermetically verified; what's left is the real round-trip. Steps (DGX, native aarch64): (1) **first** build the live
worker on the DGX — `cargo build -p kastellan-worker-matrix --features live-matrix` — this is also the cross-platform
compile gate (matrix-sdk has never been built on aarch64 Linux; the spike was macOS-only). Run
`cargo test -p kastellan-worker-matrix --features live-matrix` there too. (2) `scripts/matrix/setup-conduwuit.sh`
(federation-off, loopback) → create **two** accounts with the printed registration token → create one **encrypted** room →
join both. (3) Run the e2e: `KASTELLAN_MATRIX_LIVE_E2E=1 KASTELLAN_MATRIX_HOMESERVER_URL=… _USER/_PASSWORD …
_PEER_USER/_PEER_PASSWORD … _ROOM=… cargo test -p kastellan-core --test matrix_live_e2e -- --ignored --nocapture`
(see the test's module docs for the exact env block). If the locked-down (`net_client`/Landlock) path needs tuning, the
e2e currently runs lockdown as `none`/`none` (SDK-correctness focus) — promoting it to a real lockdown is a small follow-up.

**★ NEXT — channel-worker egress-coupled production spawn (plan Task 5) + daemon wiring.** This is what makes the live
