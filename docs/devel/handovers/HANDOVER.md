...
**Last updated:** 2026-06-15 (**[#281] RESOLVED for browser-driver — pure-Python workers now get worker-side Linux seccomp
via a new exec-shim.** Branch `feat/281-python-worker-seccomp-shim`, PR open, HEAD `eece9c0`. Pure-Python venv workers
(browser-driver) are spawned by bwrap directly and never ran the Rust prelude, so on Linux they had **no seccomp/Landlock**
— the built-and-tested `browser_client` seccomp profile was never actually applied. Fix: a new **`kastellan-worker-lockdown-exec`**
binary in the `prelude` crate runs `rlimit::apply_from_env()` → `lock_down()` → `execve(target)`; the venv script inherits the
seccomp filter under `NO_NEW_PRIVS` (the production form of the proven `exec-after-lockdown` fixture). browser-driver's
manifest discovers the shim (fail-closed on Linux — missing shim ⇒ `Misconfigured`, never an unfiltered browser), sets
`ToolEntry.lockdown_shim` + binds the shim into bwrap `fs_read`. **seccomp-only for browser-driver** (Landlock deferred behind
a new additive `KASTELLAN_LANDLOCK_PROFILE=none` signal — bwrap's mount namespace remains the FS boundary; residual-risk
analysis in the spec). New pure `tool_host::build_program_and_args` wraps the spawn at both cold-spawn sites (default `None`
= byte-identical for every Rust worker). **DGX acceptance GREEN:** prelude `lockdown_exec_smoke` proves seccomp crosses
`execve` (raw-unshare→SIGSYS); `browser_driver_e2e --ignored` renders real Chromium under the `browser_client` filter **4/4**
(forced + non-forced). Two syscalls were empirically required on aarch64 and added to `browser_client`: **`capget`**
(Playwright's Node driver at startup) + **`capset`** (Chromium zygote during page spawn — both confirmed load-bearing on the
DGX; both touch only the same-process capability bitmask, no privilege uplift under `NO_NEW_PRIVS`). Spec/plan:
`docs/superpowers/{specs,plans}/2026-06-15-python-worker-linux-seccomp*`.
**Process lesson:** a re-verify run rebuilt with `cargo build -p kastellan-core --tests`, which does NOT rebuild the separate
`kastellan-worker-lockdown-exec` bin — the e2e then ran a stale shim and failed misleadingly. Always `cargo build --workspace`
before the browser-driver e2e so the shim bin is fresh.)

_(Prior session — **#287 RESOLVED** (macOS forced-egress "no decisions" was a STALE browser-driver venv, not a code bug;
`install.sh` now `--force-reinstall`s + has a staleness tripwire), PR [#290](https://github.com/hherb/kastellan/pull/290) /
`5c228be` — condensed into "Recently merged" below.)_

---

**Recently merged to `main` (condensed, newest first).** Full reasoning in the PRs / `docs/superpowers/specs` / archive snapshots:
- **[#281] pure-Python Linux seccomp via `kastellan-worker-lockdown-exec`** (PR open this session, HEAD `eece9c0`): browser-driver now spawns through a prelude exec-shim that applies `lock_down()` then `execve`s the venv script (inherits the `browser_client` seccomp filter under `NO_NEW_PRIVS`); `ToolEntry.lockdown_shim` + pure `build_program_and_args` + `KASTELLAN_LANDLOCK_PROFILE=none` (seccomp-only; Landlock deferred). Fail-closed on Linux. DGX `browser_driver_e2e` 4/4 + `lockdown_exec_smoke`; `capget`/`capset` added to `browser_client` (empirically required by Playwright-Node / Chromium-zygote). See top block.
...
The current native-Linux test baseline is **1829 / 0**
(`feat/281-python-worker-seccomp-shim`, 2026-06-15 — full `cargo test --workspace` with live PG 18 + worker binaries built
[`cargo build --workspace`, so the `kastellan-worker-lockdown-exec` shim bin is fresh — see the #281 process lesson]; clippy
`-D warnings` clean. Was 1790 pre-#281).
...
├── workers/prelude      kastellan-worker-prelude: Linux-only Landlock + seccomp lock_down (no-op on macOS) + cross-platform setrlimit(RLIMIT_CPU). Landlock derives BOTH RW (from fs_write) and RO (from fs_read, env KASTELLAN_LANDLOCK_RO) rules so net workers can read /etc/resolv.conf in-jail; **`KASTELLAN_LANDLOCK_PROFILE=none` skips the Landlock layer** (additive, `LandlockReport::Disabled`; only browser-driver sets it — #281). 2 bins: `kastellan-lockdown-probe` (test fixture; + `raw-getpid`/`raw-unshare` pre-lockdown subcommands) and **`kastellan-worker-lockdown-exec`** (#281 — production exec-shim: `rlimit::apply_from_env()` → `lock_down()` → `execve(target)`; the target inherits seccomp under `NO_NEW_PRIVS`; used to give pure-Python venv workers worker-side Linux seccomp since bwrap spawns them directly, bypassing the Rust prelude). `browser_client` seccomp ADDITIONS include `capget`+`capset` (Playwright-Node + Chromium-zygote; DGX-confirmed load-bearing)
...
**Test baselines.** Native-Linux (DGX, PG 18 live, rustc 1.96.0, worker bins built via `cargo build --workspace`): **1829 / 0**
on `feat/281-python-worker-seccomp-shim` (2026-06-15 #281 acceptance; the real-sandbox e2e suites actually run here; + the 4
`#[ignore]` `browser_driver_e2e` render tests pass **under the `browser_client` seccomp filter applied via the lockdown-exec
shim** — both forced and non-forced; + `lockdown_exec_smoke` proves seccomp crosses `execve`). macOS skip-as-pass posture (no
`KASTELLAN_PG_BIN_DIR`): full workspace **1837 / 0 / 13** + clippy `-D warnings` clean (2026-06-15). 8–13 ignored =
...
**`browser-driver` is egress-proxy-routed (slice #2, PR #285), renders under Seatbelt on macOS (#284), the macOS forced path is green (#287), and on Linux it now gets the `browser_client` seccomp filter via the lockdown-exec shim (#281 this session — browser-driver portion DONE).** Leading remaining picks: the **two #281 follow-ups** (below); **MITM-of-browser** (in-Chromium CA trust via NSS — deferred slice #2 follow-up, once leak-scanning #3b is wired); or Phase-2 channels (IMAP/Telegram inbound) as the next phase boundary.

**★ #281 follow-ups (the shim infra now exists — both small, reuse `kastellan-worker-lockdown-exec`):**
1. **Landlock for browser-driver** — flip off `KASTELLAN_LANDLOCK_PROFILE=none` and validate the Chromium-compatible read-only path set on the DGX (the deferred half; needs `/sys` probing, the force-routed `proxy_uds` path under Landlock, fonts — see the spec's residual-risk section). Iterate the `fs_read`/RO set until a real render passes with Landlock on.
2. **gliner-relex Linux seccomp** — route gliner-relex (idle-timeout lifecycle) through the same shim by setting `lockdown_shim` in its manifest. NB: gliner-relex is a heavy torch/ML worker that has run unfiltered — needs its own profile choice (likely `net_client`/`strict`) + DGX validation that the filter doesn't break the model load. Use the **log-mode seccomp diagnostic** (temporarily set `build_bpf`'s mismatch action to `SeccompAction::Log`, run, read `dmesg`/`ausearch -m SECCOMP` for denied syscalls, add only the necessary ones, revert to KillProcess) — this is the reliable enumeration method (auditd may be off; the kill is otherwise silent). **Process gotcha:** always `cargo build --workspace` before the worker e2e so the shim bin is fresh (a `-p <crate> --tests` build leaves a stale shim and the e2e silently runs the old binary).
...
- **[#281](https://github.com/hherb/kastellan/issues/281) — pure-Python Linux seccomp:** **browser-driver DONE this session**
  (the `kastellan-worker-lockdown-exec` shim now applies `browser_client` seccomp; see top block). **Still open for
  `gliner-relex`** (reuse the shim — see the two #281 follow-ups under "Next TODO"), and **Landlock for browser-driver** is
  the deferred half (flip `KASTELLAN_LANDLOCK_PROFILE=none` off + validate the Chromium RO set).
