...
**Last commit on `feat/relations-show-cli`:** post-review fixup (hash backfilled in a follow-up commit). Branch carries 6 commits on top of `a65bb4a`: DB layer (`7787e48`) / CLI layer (`eef991d`) / e2e + bad-args + help text (`70e056c`) / docs (`d02d82b`) / branch-tip hash backfill (`94d3719`) / **post-review fixup** (this commit; addresses PR #113 review items #2/#3/#4/#5 inline + lodges #114 + #115 for deferred items #1/#6). PR [#113](https://github.com/hherb/hhagent/pull/113) open.
