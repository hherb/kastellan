...
**Last updated:** 2026-05-18 (L1 promotion writer — shipped on branch `feat/l1-promotion-writer`, **NOT YET MERGED** — needs PR + merge; new `core::memory::l1_promote` module + `L1Source`/`L1Error`/`L1WriteOutcome` types + `promote_l1`/`list_l1`/`remove_l1` + `Plan::l1_insight` field + `Plan::completion_insight()` accessor + `InnerLoopResult::terminal_l1_insight` + `build_l1_write_payload` audit helper + 3 new audit actions (`l1.added`/`l1.removed`/`l1.promoted`) + new `agent/plan.formulate` payload key (`l1_insight`) + `hhagent-cli memory l1 {add,list,remove}` operator CLI + `db::memories::delete_memory_at_layer`; +47 tests; workspace 674 → **721**).
**Last commit (branch HEAD):** `667933c`. Branch lineage: 20 commits off `main` at `a2e97a0`: Task 1 (`141c777`) + fixup (`348cc44`) → Task 2 (`2a82bd6`) + fixup (`8f0dd57` — renamed `is_completion_with_insight` → `completion_insight` for Rust convention; +2 tests) → Task 3 scaffold (`43d6aa6`) + fixup (`7717de6` — doc-order correction + bare-`\r` test + L1Source/build_l1_metadata cross-pin) → Task 4 `promote_l1` (`b3c4b69`) + fixup (`af55cf4` — top-of-file imports + drop redundant ORDER BY) → Task 5 `list_l1`/`remove_l1` (`1222760`) → Task 6 audit (`07de76c`) → Task 7 prompt (`849e85b`) → Task 8 `InnerLoopResult.terminal_l1_insight` + payload key (`607a3b0`) + doc-comment fix (`e09784c`) → Task 9 `drain_lane` hook (`a6ea35f`) → Task 10 `cli_audit` helpers (`8f72883`) → Task 11 CLI subcommands (`a641991`) + fixup (`93e5ddc` — reject unknown flags + extra positional args) → Task 12 `memory_l1_promote_e2e` (`460b073`) → Task 13 `cli_memory_l1_e2e` (`6cd0f1f`) → Task 14 `scheduler_inner_loop_e2e` pin (`667933c`).
**Session-end verification:** `cargo test --workspace` on branch HEAD: **721 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux. `core/src/scheduler/inner_loop.rs` now **1214 LOC** (was 1095 pre-slice; +119 from the new field + payload key + 3 new pin tests + the Slice-E doc-comment paragraph); the pre-existing 500-LOC breach is now ~714 over cap. **[Issue #81](https://github.com/hherb/hhagent/issues/81)** for the `inner_loop_audit.rs` split is even more urgent. `core/src/memory/l1_promote.rs` ships at **453 LOC** (new file, well under cap). `core/src/bin/hhagent-cli.rs` grew to **1419 LOC** (pre-existing breach, the slice's contribution is `+163` for the three new subcommands).
...
## Recently completed (this session, 2026-05-18 — L1 promotion writer, branch `feat/l1-promotion-writer`)

Branch: `feat/l1-promotion-writer` (off `main` at `a2e97a0`, 20 commits, **NOT YET MERGED**). Spec: [`docs/superpowers/specs/2026-05-17-l1-promotion-writer-design.md`](../../superpowers/specs/2026-05-17-l1-promotion-writer-design.md). Plan: [`docs/superpowers/plans/2026-05-17-l1-promotion-writer.md`](../../superpowers/plans/2026-05-17-l1-promotion-writer.md). First writer for `MemoryLayer::Index` rows. Until this slice lands, `<l1_insights>` is empty in every production prompt and `l1_count` is always 0 in audit rows. Hybrid design: operator-explicit CLI + agent-raised channel via `Plan.l1_insight` consumed by the inner loop on `Outcome::Completed`.
...
- ~~**L1 promotion writer**~~ **Shipped this session 2026-05-18** on branch `feat/l1-promotion-writer` (NOT YET MERGED; awaiting PR + merge). +47 tests (674 → 721). See "Recently completed (this session)" entry at the top.
- **★ Next concrete engineering pickups:**
    1. **Merge `feat/l1-promotion-writer` to main** (operator action) — `gh pr create` + review + merge. This slice is the immediate next thing to land.
    2. **[Issue #81](https://github.com/hherb/hhagent/issues/81) — split `core/src/scheduler/inner_loop.rs` (1214 LOC)** (engineering, pure refactor). Pre-existing 500-LOC breach grew significantly post-this-slice (1095 → 1214). Lift `build_plan_formulate_payload` + the slice-narration paragraphs into `core/src/scheduler/inner_loop_audit.rs`. One-session pickup; zero behavioural change.
    ...
    4. **L3 skill crystallisation** (engineering, depends on L1 promotion writer landing first — now done). The highest-leverage GenericAgent import: on observed task success, distil the successful trajectory into a parameterised JSON-RPC tool-call template stored as an L3 `memories` row. The L1 distillation pattern (`Plan.l1_insight` consumed in `drain_lane`) sets the precedent; L3 follows the same shape but distils multi-step trajectories. Spec yet to be written.
    5. **Entity extraction + graph-lane wiring** (engineering) — closes the only remaining no-op `RecallModes` variant. Needs NER over `ctx.instruction` (deterministic + LLM fallback) → resolve to `entities.id` → plumb into `RecallParams::with_seeds`. Spec yet to be written.
