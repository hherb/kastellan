...
- **Test budget delta: 848 → 880 (+32).** Spec called +26; actuals +32 due to the Task 4 fix-up's regression pin + the explicit reject-NotFound test (split out for clarity).
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    2. ~~**Operator quarantine-review UI / CLI**~~ **SHIPPED 2026-05-20** — branch `feat/entities-quarantine-review`, 10 implementation commits + 2 docs commits, **awaiting PR review**; workspace 848 → **880** (+32). New `hhagent-cli entities {list, show, approve, reject, merge}` subcommand tree + new `hhagent_db::entities` module + 3 new wire-stable audit-row action constants (`entities.approved` / `entities.rejected` / `entities.merged`) + 3 new `core::cli_audit` helpers. End-to-end recall pin demonstrates `recall(GRAPH_ONLY)` returns 0 while entities are quarantined, then surfaces approved entities' memories. Spec at `docs/superpowers/specs/2026-05-20-operator-quarantine-review-cli-design.md`; plan at `docs/superpowers/plans/2026-05-20-operator-quarantine-review-cli.md`. **No new migrations** — runtime role already had full CRUD on `entities` + `memory_entities`-cascade per migrations 0002 + 0007; `entity_kinds` (REVOKE per 0016) deliberately untouched.
