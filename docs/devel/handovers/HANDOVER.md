...
**Session-end working state (this session, 2026-05-15 continuation):** branch `feat/deterministic-policy-classification` ready for PR. Workspace test count **544** (+25 across this slice: 14 in `cassandra::deterministic::tests` net of the deleted scaffold test, 4 in `cassandra::review::tests` net of the deleted stub test, 7 in `parse_classification_floor_tests`; the +1 over the "+24 in plan" came from an off-by-one I had in plan-step-7 prose, real net is +25). Zero failures, zero warnings, zero [SKIP] lines on Linux. `core/src/cassandra/deterministic.rs` is **454 LOC** (~162 production + ~292 tests) — well under the 500-LOC soft cap; test-heavy split is appropriate for a pure invariant-check module. `core/src/bin/hhagent-cli.rs` grew ~95 LOC across the helper + flag wiring + help text and is now **1089 LOC** — over the soft cap, but that pre-existed (flagged in plan + spec; natural future split is one file per subcommand tree).
