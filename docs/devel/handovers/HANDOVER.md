...
**Last updated:** 2026-05-18 (PR #83 merged — worker lifecycle slices 1 + 2 landed on `main` at `b7dba3a` + post-review fixup `2fece27` for items raised on the PR: `WorkerLifecycleManager::acquire` now takes `tool_name: &str` so the warm-cache key is the logical registry key instead of binary basename; `dispatch_indicates_worker_dead` is exhaustive on `ClientError` so a new variant breaks the build; `WorkerHandle` doc states the Drop-runtime contract; `concurrent_acquires_for_same_tool_serialize` uses a deterministic `tokio::sync::oneshot` signal; e2e tests own the logical tool name; process-narrative comments in `dispatch_step` trimmed. Three follow-ups filed: [#84](https://github.com/hherb/hhagent/issues/84) queue-depth visibility, [#85](https://github.com/hherb/hhagent/issues/85) teardown-task accumulation, [#86](https://github.com/hherb/hhagent/issues/86) struct-literal bypass of the `idle_timeout` validator. **Slice 1** shipped the type/manager scaffolding (`Lifecycle` enum, `IdleTimeoutCaps`, `Contract`, `WorkerLifecycleManager` trait, `SingleUseLifecycle` production impl, `IdleTimeoutLifecycle` stub). **Slice 2** filled in the idle-timeout runtime: per-tool warm cache (`Arc<TokioMutex<ToolState>>` for owned-guard semantics from acquire through Drop), spawn-on-demand, post-completion cap evaluation (`max_requests` / `max_age_seconds` / `idle_seconds`), one-shot idle-teardown tasks (stale ones no-op via `last_completion` comparison), passive crash detection via post-dispatch error-variant classifier (`Io`/`Decode`/`EarlyExit`/`IdMismatch` → dead; `Rpc` → alive), exponential restart backoff (1s/2s/4s/.../60s default; resets on successful dispatch). `WorkerHandle` widens to `WorkerHandleKind::SingleUse | IdleTimeout` enum so Drop semantics diverge cleanly. `ToolHostStepDispatcher::dispatch_step` calls `handle.report_crash()` between `dispatch` and `map_dispatch_result` gated on the classifier. The `hhagent-supervisor` crate (OS-unit installer for systemd/launchd) stays untouched — naming collision with the spec's "supervisor" wording is purely conceptual; slices 1+2 grew the worker-lifecycle role in `hhagent-core`. Earlier in the upstream session (already merged to `main` at `6b73893`): Issue #81 `inner_loop.rs` split.
**Last commit on `main`:** `b7dba3a` (merge commit for PR #83, `feat(core,worker_lifecycle): slices 1 + 2 — single_use runtime + idle_timeout runtime`). Recent main history: `b7dba3a` (PR #83 merge) → `2fece27` (post-review fixups on the branch, pre-merge) → `dcf8438` (slice-2 handover/ROADMAP sync) → `3cd3bb4` (slice-2 e2e tests) → `f5f0944` (slice-2 runtime + crash detection) → `f281428` (slice-2 pure helpers) → `2c938ee` (slice-2 plan) → `334f4e2` (slice-1 handover/ROADMAP sync) → `780c9a8` (slice-1 dispatcher wiring) → `4b4fd4e` (slice-1 ToolEntry.lifecycle) → `fa96d71` (slice-1 manager trait) → `b5e519e` (slice-1 types) → `781acba` (slice-1 plan) → `6b73893` (issue #81 split) → `742378e` (handover sync) → `9893f3a` (ROADMAP sync) → `99e97cf` (worker-lifecycle + GLiNER-Relex specs) → `eb6b8a8` (PR #82 merge of `feat/l1-promotion-writer`).
**Session-end verification:** `cargo test --workspace` on `main` at `b7dba3a`: **751 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux (baseline 721 + 30 new tests: 6 in `worker_lifecycle::types`, 2 in `worker_lifecycle::manager`, 14 in `worker_lifecycle::idle_timeout`, 1 new pin `shell_exec_entry_declares_single_use_lifecycle`, 6 e2e in new `worker_lifecycle_idle_timeout_e2e.rs`, 1 wiring-error replacing the slice-1 panic-pin). `core/src/worker_lifecycle/types.rs` at **200 LOC**; `core/src/worker_lifecycle/manager.rs` at **342 LOC** post-review (was 312 before the `tool_name: &str` signature widening); `core/src/worker_lifecycle/idle_timeout.rs` at **525 LOC** post-review (was 521 before fixup-2's exhaustive classifier; still ~25 LOC over the 500 soft cap — half is the embedded test module; split candidate is to lift the inline `#[cfg(test)] mod tests` out into a sibling `idle_timeout_tests.rs`, deferred); `core/src/scheduler/tool_dispatch.rs` at **748 LOC** post-review (was 758 before fixup-6 trimmed process-narrative comments; still a pre-existing 500-LOC soft-cap breach). `core/src/bin/hhagent-cli.rs` at **1432 LOC** still the headline 500-LOC breach pickup.

## Recently completed (this session, 2026-05-18 — worker lifecycle slice 2, branch `feat/worker-lifecycle-slice-1`, bundled with slice 1 in one PR)
...
- **★ Next concrete engineering pickups (in priority order):**
    1. **First idle-timeout worker — GLiNER-Relex implementation** (engineering). Now that slice 2 ships the runtime, the next natural slice is the first `Lifecycle::IdleTimeout` consumer: GLiNER-Relex (Knowledgator's joint NER+RE model, ~1.3 GB resident, Apache 2.0 weights). Spec exists at `docs/superpowers/specs/2026-05-18-gliner-relex-feasibility-study.md`; entity extraction v1 spec at `docs/superpowers/specs/2026-05-18-entity-extraction-graph-lane-design.md`. Pre-req: write an implementation plan and a half-day macOS MPS smoke test.
    2. **Operator recapture against the current daemon** (operator action) — one-time `cargo test -p hhagent-core --test observation_capture -- --ignored --nocapture` against the local LLM. Turns the pre-Slice-A capture JSONs into rule-iteration-harness-replayable inputs.
    3. **Entity extraction + graph-lane wiring v1 — implementation** (engineering). Spec at `8a5e6f0`. Pre-req: write an implementation plan; consider whether to skip v1 in favour of v2 once worker-lifecycle slice 2 ships and GLiNER-Relex POC validates feasibility.
    ...
    6. **Pattern catalogue lifecycle for `classification_inference`** (engineering, depends on observation-phase recapture) — once recapture shows under-detection cases (clinical work that didn't elevate), add the missing pattern. Tracking in a future `pattern_misses.md` if the catalogue grows enough to warrant.
    7. **`hhagent-cli.rs` (1432 LOC) split** (engineering, pure refactor) — the largest remaining 500-LOC breach in the crate now that `inner_loop.rs` is fixed. Subcommand trees are the natural unit; sibling files under `core/src/bin/hhagent_cli/` if the bin crate gets a module structure. File a tracking issue when picked up.
    8. **Worker manifest plumbing — design slice** (engineering) — slice 1 ships `Lifecycle` directly on `ToolEntry`. The spec's open question 1 (TOML files vs Rust consts) is unresolved. A design slice picks the shape; an implementation slice plumbs it through `build_tool_registry`. Low priority until a second worker exists and the Rust-const seam starts to feel cramped.
