...
**Last updated:** 2026-05-29 (Next-TODO **item 5**: `tool_host.rs` sibling-lift — lifted the watchdog subsystem, the lockdown-env derivation, and the seal tests into three child modules; production LOC 911 → 519. Branch `refactor/item-5-tool-host-sibling-lift`, code commit `379f77a`, PR [#160](https://github.com/hherb/hhagent/pull/160). This branch also carries the post-#159 docs-reconcile commit as a ride-along. **This session also reconciled the stale header** — PR [#159](https://github.com/hherb/hhagent/pull/159) (item 4b) merged to `main` at `1106145`.)

**Last commit on `main`:** `1106145` (Merge pull request #159 from hherb/refactor/injection-guard-test-module-lift). Confirm with `git log --oneline -1`. The item-5 lift below lives on branch `refactor/item-5-tool-host-sibling-lift` (reconcile commit + lift `379f77a` + this docs commit), PR [#160](https://github.com/hherb/hhagent/pull/160).

**Currently on:** `refactor/item-5-tool-host-sibling-lift` (off `main` at `1106145`). `main` itself is clean at `1106145` (one untracked, unrelated `docs/essay-medium-draft.md`). All recent feature/fix branches are merged — see "Recently merged branches" below.

**Session-end verification:** `cargo test --workspace` on macOS (M3 Max, on branch `refactor/item-5-tool-host-sibling-lift`): **1152 passed / 0 failed / 3 ignored** — identical to the `main` baseline at `1106145` (the item-5 lift is a pure-mechanical move with zero behavior change; skip-as-pass posture without `HHAGENT_PG_BIN_DIR`). The `WorkerCommand` `compile_fail` seal doctest passes (one of the 3 ignored/doctest markers). `cargo clippy -p hhagent-core --all-targets`: **no new lints** — the 2 `doc list item` warnings now reported against `tool_host/watchdog.rs` relocated verbatim with the moved `is_valid_target_pid`/`send_sigkill` doc comments; the pre-existing lib warnings (~10) are unchanged. `RUSTDOCFLAGS=-D rustdoc::broken_intra_doc_links cargo doc -p hhagent-core --document-private-items`: **no new broken-link errors** (the existing `WorkerCommand → SupervisedWorker::call` private-link warning at `tool_host.rs:51` predates this change and was not touched). **`main` baseline (item 4b):** `cargo test --workspace` **1152 / 0 / 3** (confirmed live this session at `1106145`). **PG-gated note:** `injection_guard_e2e` was **not** re-verified live this session — every attempt (parallel, serialized, and single-test) failed in `tests-common`'s PG **cluster bring-up** (`postgres should reach Active within 30s`), the known launchd-registration contention flake (#130), never reaching any assertion. The first parallel run got 2/6 clusters up before launchd throttled. The #156 fix is a pure synchronous function (no PG/dispatch path), fully covered by the new unit test + the 29-test module; the e2e exercises only the unchanged dispatch→screen wiring. Live `secret_vault_e2e` 11/11 was confirmed last session (PR #157). `cargo clippy -p hhagent-core --all-targets`: the 10 pre-existing lib warnings unchanged, no new lints from the change. **PR #152 context (now merged):** `cargo test --workspace` on `main` post-merge: 1145/0/3 (unchanged from `main` — skip-as-pass posture without `HHAGENT_PG_BIN_DIR`; with PG live ~640 passed + 2 pre-existing `embedding_recall_e2e`/`gliner_relex_e2e` PG-race flakes identical on `main`). `cargo clippy --workspace --all-targets`: **0 errors** (pre-existing warnings remain; #150's deny-level `uninit_vec` is fixed). **Linux:** **CONFIRMED GREEN via CI** — the new `.github/workflows/linux-check.yml` job on PR [#152](https://github.com/hherb/hhagent/pull/152) ran `cargo check --workspace --all-targets` + `cargo clippy --workspace --all-targets` on `ubuntu-latest` and passed (1m17s). This is the real Linux verification for #144 and confirms there was no *other* Linux-only breakage beyond the `Container` variant. (Full `cargo test --workspace` on the DGX still pending — CI is compile-only; the last DGX test run was `990` at `1abb061`.) **Build gaps now fixed:** [#144](https://github.com/hherb/hhagent/issues/144) (Linux build — `container_mode_entry` macOS-gated) and [#150](https://github.com/hherb/hhagent/issues/150) (`clippy::uninit_vec` in `mem_burner`). Note: #144 was **NOT verifiable on the dev Mac** (cross `cargo check` dies in a C dep needing `aarch64-linux-gnu-gcc`) — hence the CI job.

**Recently merged branches — safe to delete locally** (`git branch -d <name>`): `refactor/injection-guard-test-module-lift` (PR #159, just merged at `1106145`), `fix/issue-156-walk-sibling-continue` (PR #158, merged at `f3c380f`), `fix/issue-148-149-secret-vault-test-gaps` (PR #157), `fix/issue-143-walk-recursion-depth-guard` (PR #155), `fix/issue-144-150-linux-build-clippy-gate` (PR #152), `fix/issue-147-redact-tool-req-plaintext` (PR #151), `feat/opaque-secret-refs-slice-1` (PR #146), `feat/injection-guard-slice-1` (PR #141), `fix/issue-89-tmpfs-per-spawn-invariant-test` (PR #139), `refactor/idle-timeout-release-sibling-lift` (PR #138). Older merged branches are listed in the archive snapshots.
...
## Recently completed (this session, 2026-05-29 — Next-TODO **item 5**: `tool_host.rs` sibling-lift, branch `refactor/item-5-tool-host-sibling-lift`, code commit `379f77a`, PR [#160](https://github.com/hherb/hhagent/pull/160))
...
8b. **[#153](https://github.com/hherb/hhagent/issues/153) — clear the pre-existing `hhagent-core` lib clippy warnings, then flip `linux-check` to `-D warnings`.** The 10 long-standing lib warnings (unchanged across recent sessions) currently keep CI from enforcing a clean clippy gate. Clear them, then add `-D warnings` to the `cargo clippy` step in [`.github/workflows/linux-check.yml`](../../../.github/workflows/linux-check.yml) so any new lint hard-fails. Verification: `cargo clippy -p hhagent-core --all-targets` → 0 warnings; CI green.
...
- [#153](https://github.com/hherb/hhagent/issues/153) — CI: clear the pre-existing `hhagent-core` lib clippy warnings, then flip the `linux-check` workflow to `-D warnings` so new lints hard-fail CI. Filed from the PR #152 CI work; bug-labelled.
