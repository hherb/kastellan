...
**Last commit (main):** `305941a` (Merge pull request #72 — fix/runner-reject-agent-raised-provenance). Branch history on main from the merge: `a6335ab` (initial mitigation) → `e9d0162` (docs) → `5fabac5` (post-review fixup binding the reject to the parsed `ClassificationFloorSource::AgentRaised` variant rather than the wire literal `"agent_raised"`) → `305941a` (merge commit).
**Session-start verification:** `cargo test --workspace` clean on `main` at `305941a`: **607 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux. `core/src/scheduler/runner.rs` ~540 LOC (just over the 500-LOC soft cap — natural future split candidate: lift the parsing helpers into a sibling `scheduler/payload_validation.rs` once a second helper materialises; not warranted today). Follow-up [issue #73](https://github.com/hherb/hhagent/issues/73) filed for the deferred e2e integration test + `TaskContext` constructor doc note.
