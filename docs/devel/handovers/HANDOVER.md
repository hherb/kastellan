...
**`cargo test --workspace` on Linux: 342 tests passed, 0 failed, 0 `[SKIP]` lines, 0 warnings** on branch `refactor/tests-common-hoist` (off `main` at `2efd074`). Baseline on `main` (post-PR #36 at `2efd074`) was 342; this session's hoist is a pure refactor — same 342 tests pass, every assertion byte-identical, but ~2500 LOC of duplicated bring-up boilerplate eliminated. The previously-noted dead-code warning in `core/tests/embedding_recall_e2e.rs::ServedRequest` is silenced by the hoist (the migrated file `#[allow(dead_code)]`s the field; pre-existing PR #29 warning closed). Two pre-existing doctests in `hhagent-sandbox` and `hhagent-worker-prelude` are `ignored` (explicit markers).
...
## Recently completed (this session, 2026-05-12 — issue #15 hoist: shared `hhagent-tests-common` dev-dep crate)

Branch: `refactor/tests-common-hoist` (off `main` at `2efd074`). Closes [issue #15](https://github.com/hherb/hhagent/issues/15).
