...
**Last commit on `main`:** `bb32cab` (PR #128 merge commit `Merge pull request #128 from hherb/fix/issues-114-115-walk-edges-dedupe-and-combine`) carrying 7 branch commits + post-merge polish `978b5be` (3 small in-scope follow-ups from self-review: named const `WALK_EDGES_AROUND_DIRECTION_COL` for the trailing partition column index + SQL comment on the outer `ORDER BY direction ASC` lex-order incidental + test header pin on `per_direction_limit` semantics; no behaviour change). Substantive merges in order: `bb32cab` (PR [#128](https://github.com/hherb/hhagent/pull/128), Issues #114 + #115 — `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL), `7adc582` (PR [#126](https://github.com/hherb/hhagent/pull/126), Item 29 — `HHAGENT_PG_BIN_DIR` env-override), `dac0dcd` (PR [#125](https://github.com/hherb/hhagent/pull/125), Item 28 — Issue #95 entity-upsert Layer B), `e93997e` (PR [#124](https://github.com/hherb/hhagent/pull/124), Item 27 — Slice 2.5 follow-up triple).
...
**Last commit on `feat/bring-up-pg-cluster-polling-cap-lift`:** `0c7b644` (`refactor(core/tests): migrate 5 local-copy bring_up_pg_cluster sites to shared helper`). Branch tip carries 2 commits over main; PR pending. Substantive commits: `02d6691` (cap-widening), `0c7b644` (5-file migration).
...
## Recently completed (this session, 2026-05-26 — `bring_up_pg_cluster` polling-cap lift + 5-site consolidation, branch `feat/bring-up-pg-cluster-polling-cap-lift`, PR pending)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    29. **Operator picks (bucket).** Sub-bullet status:
        - **`batch_upsert.rs` 547 LOC sibling-`tests.rs` lift** (engineering, ~30 minutes). Mirror of Items 23(a) + 26. Closes the 47-LOC overage left by Item 28. Bundle with the `worker_lifecycle/idle_timeout.rs` (583 LOC) + `worker_lifecycle/manager.rs` (559 LOC) + (now also) `db/src/lib.rs` (908 LOC, post-Item-29) test-module lifts for a same-pattern quadruple — ~1.5 hours total.
