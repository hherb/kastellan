...
**Last updated:** 2026-05-17 (recall-lane wiring — shipped on branch `feat/recall-lane-wiring`, **merged to `main` via PR #79 at `7553404`**; new `core::recall_assembly` module + `RecallBuilder` trait + `PgRecallBuilder`/`StaticRecallBuilder` impls + widened `assemble_system_prompt` 3-arg → 4-arg + `SystemPromptBuilder::build_with_recalled` + 3 new `plan.formulate` audit-row keys; +22 tests including post-review fixups; workspace 652 → **674**).
**Last commit (`main` HEAD):** `7553404` (merge of PR #79). Branch lineage: 18 commits off `main` at `2f339c3`: spec (`76a342b`) + plan (`45b2121`) → Task 1 + fixup (`2b7e773`, `9d4432a`) → Task 2 + fixup (`3127031`, `67c84e5`) → Task 3 (`5f072ea`) → Task 4 + fixup (`690da86`, `57632a1`) → Task 5 + fixup (`3270b8c`, `84d62cd`) → Tasks 6+7+8 atomic (`abffe51`) → Task 9 + fixup (`1055f7f`, `8d2e580`) → Task 10 (`ae8af69`) → Task 11 (`0c68328`) → docs(handover) (`f9b5c33`) → **final-review pin in `cli_ask_e2e` (`81d9e9b`)** → **post-PR-review cleanups (`1a18e93`: invariant constructor, cap_and_split guard simplification, doc clarifications, threat-model scenario 6)** → merge (`7553404`).
**Session-end verification:** `cargo test --workspace` on `main` at `7553404`: **674 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux. `core/src/scheduler/inner_loop.rs` now **1095 LOC** (was 991 pre-slice; +104 from the 3 new payload keys + 4 new pin tests + `make_text_plan` fixture); this widens the pre-existing 500-LOC breach to ~600 over cap. **[Issue #81](https://github.com/hherb/hhagent/issues/81) filed for the `inner_loop_audit.rs` split** — substantially more urgent post-slice. `core/src/recall_assembly/{mod.rs, pg_builder.rs}` ship at ~250 LOC + 297 LOC (both well under the cap). `core/src/scheduler/agent.rs` grew from 176 → **212 LOC** (still well under cap). **[Issue #80](https://github.com/hherb/hhagent/issues/80) filed** for the `cli_ask_e2e.rs` cascade-fix code-smell (mock dispatches by FIFO instead of URL path). **`docs/threat-model.md` scenario 6 added** (memory-write injection — recall-lane verbatim rendering means INSERT-on-memories is a prompt-injection vector; Phase 1 trust posture matches L0/L1).
