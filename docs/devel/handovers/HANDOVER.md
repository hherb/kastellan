...
**Last commit on `main`:** `4e94e42` (PR #129 merge commit `Merge pull request #129 from hherb/feat/bring-up-pg-cluster-polling-cap-lift`) carrying 4 branch commits (`02d6691` cap-widening + `0c7b644` 5-file migration + `7b71003` docs claim + `f906d18` post-review doc polish). Substantive merges in order: `4e94e42` (PR [#129](https://github.com/hherb/hhagent/pull/129), `bring_up_pg_cluster` polling-cap lift + 5-site consolidation), `bb32cab` (PR [#128](https://github.com/hherb/hhagent/pull/128), Issues #114 + #115 — `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL), `7adc582` (PR [#126](https://github.com/hherb/hhagent/pull/126), Item 29 — `HHAGENT_PG_BIN_DIR` env-override), `dac0dcd` (PR [#125](https://github.com/hherb/hhagent/pull/125), Item 28 — Issue #95 entity-upsert Layer B), `e93997e` (PR [#124](https://github.com/hherb/hhagent/pull/124), Item 27 — Slice 2.5 follow-up triple).

**Last commit on `refactor/item-29-test-module-lift-quadruple`:** `91e2e6e` (`refactor(db): lift lib.rs inline tests to sibling tests.rs`). Branch tip carries 5 commits over main; **PR [#132](https://github.com/hherb/hhagent/pull/132) open**. Substantive commits: `91b9dcc` (doc-sync to main = 4e94e42), `4e39cf9` (batch_upsert.rs lift), `17e3540` (idle_timeout.rs lift), `d891d41` (manager.rs lift), `91e2e6e` (db/src/lib.rs lift).
...
## Recently completed (this session, 2026-05-27 — test-module lift quadruple, branch `refactor/item-29-test-module-lift-quadruple`, **PR [#132](https://github.com/hherb/hhagent/pull/132) open**)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    29. **Operator picks (bucket).** Sub-bullet status:
        ...
        - **Issue [#131](https://github.com/hherb/hhagent/issues/131)** (filed 2026-05-27 from PR #129 review) — optional per-call timeout override for `bring_up_pg_cluster`. Today's `PG_BRING_UP_TIMEOUT_SECS = 30` const is one value for all callers; heterogeneous launchd vs Homebrew bring-up timing means a Linux CI runner could safely use a tighter cap while macOS Postgres.app callers need the 30 s headroom. Candidate: add an optional `timeout_override: Option<Duration>` parameter to `bring_up_pg_cluster`, defaulting to `Duration::from_secs(PG_BRING_UP_TIMEOUT_SECS)` when None. Per-test sites that want a tighter cap pass `Some(Duration::from_secs(10))`; the default stays at 30 s for the launchd common case. Small (~30 LOC + 1 unit test pinning the override path).
