...
- **Privacy invariant is end-to-end enforced.** The raw scanned body never appears in any audit-row column — pinned at run-time by `policy_audit_row_contains_no_substring_of_blocked_body` which uses a unique `"AUDIT_LEAK_MARKER_xyz123"` substring and asserts it does NOT appear in the serialized payload of any audit_log row. This catches accidental leaks via the tool row's `result` field, the policy row's payload, or any future code path that might serialize the body.
