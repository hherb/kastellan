...
**Last updated:** 2026-05-21 (continuation — session-start sync + audit_tail flake fix (PR #102) + GLiNER-Relex macOS slice. PR #100 (relation-label vocabulary) was merged into `main` at `5bcd060` between sessions; post-merge code-review fixup `4cd3411`. **This session shipped two stacked branches on top of `main@5bcd060`**: (a) `fix/issue-101-audit-tail-tempdir-collision` → PR #102 — fixes a tokio-multi-thread test-parallelism flake in `audit_tail::tail_loop_from_start_replays_then_exits` caused by macOS's coarser `SystemTime` clock resolution colliding the test-harness `tempdir()` between sibling tests. (b) `feat/gliner-relex-macos-slice` → PR pending — closes Next-TODO item 7 (GLiNER-Relex macOS slice): adds `mps`-accept + darwin-`auto`→`cpu` branch in Python `_resolve_device` (11 new pytest cases + 1 new Rust manifest forwarding test). Smoke-tested end-to-end on this M3 Max Mac with both `device=auto` (resolves to cpu) and `device=mps` (explicit opt-in) returning the canonical `Dr Smith --[treats]--> asthma (0.994)` triple.).

**Last commit on `main`:** `5bcd060` (`Merge pull request #100 from hherb/feat/relation-label-vocabulary`).
**Last commit on `feat/gliner-relex-macos-slice`:** pending (about to commit + push).

**Session-end verification:** **Rust workspace on Linux (DGX, prior session): 896 passed / 0 failed / 4 ignored** (`cargo test --workspace` on `feat/relation-label-vocabulary` pre-merge). **On macOS (this session, on `feat/gliner-relex-macos-slice` tip): 866 passed / 0 failed / 3 ignored** (`cargo test --workspace --no-fail-fast`; +1 over the 865-baseline-post-#101 from this session's `entry_forwards_device_verbatim_regardless_of_value` Rust test). **Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed** (+11 over the 24 baseline from this session's `test_resolve_device.py`). The macOS run's lower count vs Linux is from per-platform sandbox / supervisor / GLiNER-Relex e2e tests skip-as-pass on darwin (no real PG cluster bring-up in some, no real-model weights in others). 4 `[SKIP]` lines on Linux `--nocapture` are GLiNER-Relex real-model tests gated on `HHAGENT_GLINER_RELEX_ENABLE=1`. Smoke: `hhagent-cli --help` exit 0. End-to-end smoke (direct Python worker spawn against the on-disk weights at `$HOME/.local/share/hhagent/workers/gliner-relex/weights/multi-v1.0`): `device=auto` → cpu → returns 3 entities + 3 triples for the canonical `"Dr Smith treats asthma in Mosman."` fixture (`Dr Smith --[treats]--> asthma 0.994`, plus two `located_in` pairs); `device=mps` → mps → same shape, sub-1e-6 numeric differences on the encoder forward; `device=cuda` → fails loud with `UNSUPPORTED_DEVICE` (-32604). **Flakes found + fixed this session:** audit_tail tempdir collision (PR #102, see below).

## Recently completed (this session, 2026-05-21 continuation — GLiNER-Relex macOS slice, branch `feat/gliner-relex-macos-slice`, PR pending)
...
## Recently completed (this session, 2026-05-21 continuation — audit_tail flake fix on macOS, branch `fix/issue-101-audit-tail-tempdir-collision`, **PR #102 pending review**)
