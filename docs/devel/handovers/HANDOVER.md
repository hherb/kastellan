...
**Last updated:** 2026-06-19 (**Matrix Phase D egress-transport spike — DONE on branch
`feat/matrix-phase-d-egress-spike`.** matrix-sdk 0.8.0 landed behind `live-matrix` feature; AGPL license pass (225 new crates, all PASS);
...
├── workers/matrix       kastellan-worker-matrix: Matrix inbound worker (Phase D spike COMPLETE). `MatrixSdk` seam + `MatrixHandler` for `matrix.init/poll/send`; fake-SDK unit tests (5 handler tests). **Phase D spike additions (branch `feat/matrix-phase-d-egress-spike`):** `matrix-sdk = 0.8.0` added as an OPTIONAL dep behind `live-matrix = ["dep:matrix-sdk"]` (features: `e2e-encryption, sqlite, bundled-sqlite, rustls-tls`; default-features=false; default build unaffected). `ProxyBridge` (`src/bridge.rs`): loopback-TCP↔UDS relay, `bind(uds_path) → ProxyBridge`, `proxy_addr() → SocketAddr`, owns the accept loop, Drop-aborts; 2 unit tests. `egress_spike.rs` (gated `#[cfg(all(test, feature="live-matrix"))]`): hermetic spike confirming `matrix-sdk` routes its first HTTPS request as `CONNECT fake-homeserver.invalid:443` through the bridge. `#![allow(dead_code)]` crate-wide (documented; narrowed in the live-wiring slice). Crate-wide `main()` arms `bail!` (live serving deferred). Tests: 7/0/0 (default), 8/0/0 (`live-matrix`, +1 spike).
...
**★ TOP PICK — live `LiveSdk` integration (Matrix Phase D, next slice).** The egress-transport spike is CONFIRMED on branch `feat/matrix-phase-d-egress-spike` (2026-06-19): matrix-sdk 0.8.0 routes its first HTTPS request through `ProxyBridge` as `CONNECT fake-homeserver.invalid:443`; transport decision is transparent tunnel + `disable_mitm`. The following slice implements the actual live sync loop. All work is in `workers/matrix/`; plan reference: `docs/superpowers/plans/2026-06-12-matrix-inbound-sandboxed-worker.md` Task 8 Steps 2–5. Spec for the SDK API names: `docs/superpowers/specs/2026-06-19-matrix-phase-d-egress-transport-spike-design.md#spike-outcome-2026-06-19`.

1. **`workers/matrix/src/sdk_live.rs`** — `LiveSdk` implementing the `MatrixSdk` seam. Use the confirmed builder sequence: `Client::builder().homeserver_url(url).sqlite_store(path, None).proxy(bridge_addr).build().await?`. Login via `client.matrix_auth().login_username(user, pass).send().await?`. Persistent encrypted SQLite state store at `<scratch>/matrix.db`. Sync task: `client.sync(SyncSettings::default())` in a `tokio::spawn`, push events to a bounded `VecDeque`; implement `poll()` + `send()` against it.
2. **Restore `main.rs` live serving wiring** — `if cfg.live_sdk_enabled { build LiveSdk → prelude::lock_down → serve_stdio }` — and narrow the crate-wide `#![allow(dead_code)]` back to only the items that remain in-progress.
3. **Wire `disable_mitm`-by-worker-name** for the matrix worker in `core/src/egress/spawn.rs` (find the existing call for browser-driver; add an analogous arm for the matrix worker name). **Carry the #286 macOS-loopback caveat:** the `ProxyBridge` binds `127.0.0.1:0` inside the worker — the identical pattern to browser-driver's `shim.py`, which `docs/threat-model.md` documents as having a macOS-only containment divergence (no netns ⇒ worker loopback IS host loopback). It is latent in the spike slice (no Seatbelt loopback grant exists yet), but the moment this slice grants the matrix worker a loopback-widening Seatbelt profile, matrix inherits #286 — scope the grant to the bridge's bound port (or prefer a UDS-only transport / the `MacosContainer` VM-netns backend), and pair the wiring with the #286 mitigation. Also **narrow the crate-wide `#![allow(dead_code)]`** (Task 2 of step 2 above) and the redundant `#[allow(dead_code)]` on `bridge.rs`'s impl block once `LiveSdk` consumes `ProxyBridge`.
4. **`core/tests/matrix_live_e2e.rs`** — `#[ignore]` + `#[cfg(feature="live-matrix")]` live round-trip test: start conduwuit (see `scripts/matrix/setup-conduwuit.sh` from slice #6), spin up the real worker, do init/poll/send, assert the round-trip. DGX-verified.
   - Verification: `cargo test -p kastellan-worker-matrix --features live-matrix` + `cargo test -p kastellan-core --test matrix_live_e2e --ignored -- --nocapture` (on DGX with conduwuit running).
