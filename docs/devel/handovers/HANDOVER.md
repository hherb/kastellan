...
**Last commit:** `f261d2c` (`feat(core/scheduler): task-lifecycle audit rows (spec §7)`; docs follow in the next commit on this branch). Pre-session main was at `2367d94` (merge of PR #33). **This session's working branch:** `feat/scheduler-task-lifecycle-audit` (off `main` at `2367d94`). Ships spec §7 task-lifecycle audit rows: every claim writes a `scheduler/task.running` row, every finalize writes a `scheduler/task.<terminal_state>` row plus a `scheduler/task.finalize` summary row carrying the aggregate counters (`plan_count`, `total_llm_calls`, `total_dispatch_calls`, `total_duration_ms`, `started_at`, `finished_at`) the observation-phase SQL queries need. New module `core/src/scheduler/audit.rs` (~280 LOC incl. tests; pure helpers + constants); `run_to_terminal`'s return type widened from `Outcome` to a new `InnerLoopResult { outcome, plan_count, dispatch_count }` so the lane runner can build the finalize payload without re-querying audit_log. Workspace count 329 → **341** (+12 unit tests in the new audit module). `cli_ask_e2e` happy-path multiset bumped 7 → 10 rows; failure-path bumped 13 → 16. Three consecutive focused runs of `cli_ask_e2e` deterministic at ~3.25 s each. Closes the audit-trail gap HANDOVER flagged after Task 3.2.bis: until this slice, `ToolHostStepDispatcher::dispatch_step` short-circuited on two pre-RPC failures (registry miss → `UNKNOWN_TOOL`, spawn failure → `SPAWN_FAILED`) **without writing an audit row**, because both paths return before reaching the `tool_host::dispatch` chokepoint. The dispatcher now writes a `scheduler/step.unknown_tool` or `scheduler/step.spawn_failed` row itself, best-effort, with the same `{tool, method, req, ms}` skeleton as the chokepoint plus an optional `err` field for the spawn-failure path. Pinned by two new unit tests (`build_payload_unknown_tool_shape_has_no_err_field`, `build_payload_spawn_failed_shape_includes_err_string` — pure helper, no I/O) and an extended `scheduler_step_dispatch_e2e` with a 4th dispatch scenario: a `broken-tool` `ToolEntry` whose `policy.fs_read` carries a relative path (rejected up-front by both `LinuxBwrap` and `MacosSeatbelt` with `SandboxError::Backend`) gives a deterministic, cross-platform SPAWN_FAILED trigger. Final audit_log assertion bumped from 3 rows (bring-up + ok + denied) to 5 rows (+ unknown_tool + spawn_failed). All 329 tests pass; 0 failures; 0 SKIP lines; one pre-existing dead-code warning in `embedding_recall_e2e.rs::ServedRequest` is unchanged.

Branch lineage: PR #29 (Option O — embedding router) merged 2026-05-11 at `d39023b`; PR #31 (memory split — closes #30) merged 2026-05-11 at `a7a0c12`. This branch is off `main` at `a7a0c12`.
...
**`cargo test --workspace` on Linux: 341 tests passed, 0 failed, 0 `[SKIP]` lines** on branch `feat/scheduler-task-lifecycle-audit` (off `main` at `2367d94`). Baseline on `main` (post-PR #33) was 329; this session adds **+12** pure-helper unit tests for the new `core::scheduler::audit` module (action-builder + lifecycle-payload + finalize-payload + duration-ms helpers). The integration test count is unchanged — `scheduler_inner_loop_e2e` and `cli_ask_e2e` gained assertions in-place. One pre-existing dead-code warning in `core/tests/embedding_recall_e2e.rs::ServedRequest` (introduced in PR #29) is unchanged. Two pre-existing doctests in `hhagent-sandbox` and `hhagent-worker-prelude` are `ignored` (explicit markers).
...
## Recently completed (this session, 2026-05-12 — spec §7 task-lifecycle audit rows)
...
- **Observation phase** (spec §9) — now that real CLI-driven plans execute, scheduler short-circuit failure modes (UNKNOWN_TOOL + SPAWN_FAILED) leave audit rows, **and per-task lifecycle + finalize rows now record claim/terminalisation with full aggregate counters**, the audit log is rich enough to drive observation-phase SQL queries without joining `tasks`. Practical step: build a small fixture set of "real-ish" instructions (5–10 prompts spanning safe + edge-case + clearly-blockable), run them through the CLI, dump the audit log, look at which plans CASSANDRA *would* have wanted to block under each candidate rule, iterate the rule set against the dump rather than against speculation.
- **`task.crashed` audit row from `sweep_crashed`** — small follow-up to this session's spec §7 slice. The startup sweep currently marks crashed tasks via `UPDATE tasks SET state='crashed' WHERE state='running' AND lease_expires_at < now()` returning only `rows_affected: u64`, so per-row audit emission needs `sweep_crashed` refactored to return the rows it crashed. Then main.rs can emit one `scheduler/task.crashed` row per recovered task. Wholly independent of the work that just shipped.
- **e2e coverage for `task.finalize` with `started_at: null`** — flagged in PR #34 review (post-/fixall). The `started_at: None` JSON path in `build_finalize_payload` is unit-tested (`build_finalize_payload_started_at_null_when_absent`) but has no integration coverage. The race window in the wired code is essentially zero on a UDS-local PG (claim_one's `started_at = now()` lands before the row returns), so the missing coverage is theoretical — but worth adding when the producer-side `task.cancelled` row from CLI direct-cancel of a `pending` task lands, since both touch the same "never claimed, but still finalised" semantics. Open design question: should the scheduler write a `task.finalize` row for a task it never claimed? Today the answer is "yes, with null fields" but that may not be the right contract.
