...
**Known flake fixed this session:** `tasks_lifecycle_e2e` (in `db/tests/postgres_e2e.rs`) had a structural deadlock — `pool.close().await` blocks until all `max_connections` permits are released, but two `PgListener`s were still in scope when close() was called. The multi-thread tokio runtime exposed it reliably (90 s+ hang) while the single-thread runtime variant in `audit_helpers_pool_and_notify_round_trip` (same pattern, one listener) appears to be lucky-enough timing-wise to always pass. Fix: explicitly `drop(listener)` before `pool.close().await`. If you ever see the audit_helpers test hang too, apply the same fix there — it has the same structural issue.
...
Free-text query → embedding vector → semantic recall.

- **Shape:** new `workers/embedding-worker/` crate + binary, same prelude (`hhagent-worker-prelude::serve_stdio`) as shell-exec. JSON-RPC method `embed` takes `{texts: [string]}`, returns `{embeddings: [[f32; 1024]]}`. Inside the worker: HTTP POST to `<HHAGENT_LLM_LOCAL_URL>/embeddings` with the OpenAI-compat shape; decode the response. Worker side is a thin shim — the `Router::send` path lives in `core` because the policy gate is `core`'s concern.
- **`hhagent-llm-router::messages` extension:** add `EmbeddingRequest` / `EmbeddingResponse` types parallel to `ChatRequest` / `ChatResponse`. New `Router::embed(&request)` method that calls `policy.pick_embed(&request)` (Phase 5 seam — `PolicyGate` may grow a separate method or reuse `pick`), dispatches to `<base>/embeddings` against the local backend.
- **`core::memory::recall` enhancement:** when `query_embedding` is `None` but `query_text` is `Some` and the semantic lane is enabled, the function calls the embedding worker via `tool_host::dispatch(pool, &mut embed_worker, "llm:router", "embed", req)`. The dispatch writes the first `actor='llm:router'` audit row.
- **Verification:** integration test that adds a hand-rolled TCP mock for the `/embeddings` endpoint (cf. the `local_backend_e2e.rs` shape), spawns the embedding worker pointed at the mock, calls `recall` with `query_text` only, asserts the matched memory is rank-1 AND the audit log has a fresh `actor='llm:router'` row with `action='embed'`.
- **Gotcha:** the embedding model dim must match `EMBEDDING_DIM = 1024`. Mismatched output should surface as a typed router-level error before the worker tries to insert shorter vectors. Validate dim in `Router::embed` against the constant.
- **Files:** `workers/embedding-worker/Cargo.toml` + `src/` (~150 LOC), `llm-router/src/embeddings.rs` (~150 LOC), `core/src/memory.rs` (~30-LOC additions), integration test (~300 LOC).
