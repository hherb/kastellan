...
**Session-end verification:** `cargo test --workspace` on macOS (M3 Max, on the fix branch): **1145 passed / 0 failed / 3 ignored** (unchanged from `main` — skip-as-pass posture without `HHAGENT_PG_BIN_DIR`; with PG live ~640 passed + 2 pre-existing `embedding_recall_e2e`/`gliner_relex_e2e` PG-race flakes identical on `main`). `cargo clippy --workspace --all-targets`: **0 errors** (pre-existing warnings remain; #150's deny-level `uninit_vec` is fixed). **Linux DGX:** not re-verified since `main` at `1abb061` (990); the #144 fix should now let `hhagent-core` compile there — operator-driven re-run pending. **Build gaps now fixed (pending Linux/CI confirmation for #144):** [#144](https://github.com/hherb/hhagent/issues/144) (Linux build — `container_mode_entry` now macOS-gated) and [#150](https://github.com/hherb/hhagent/issues/150) (`clippy::uninit_vec` in `mem_burner`). #144's true Linux green is the new `.github/workflows/linux-check.yml` CI job or a DGX `cargo check` — **NOT verifiable on the dev Mac** (cross `cargo check` dies in a C dep needing `aarch64-linux-gnu-gcc`).
...
**Verification (macOS M3 Max).** `cargo test --workspace`: **1145 / 0 / 3** (unchanged — the gated tests still run on macOS; #150 only touched a fixture binary). `cargo clippy --workspace --all-targets`: 0 errors. **Linux: not verified locally — needs the new CI job or a DGX `cargo check`.**
...
**First — confirm the Linux build fix landed (this session's #144, not yet verified on Linux):**

0. **Verify [#144](https://github.com/hherb/hhagent/issues/144) on a real Linux box.** The fix is committed on branch `fix/issue-144-150-linux-build-clippy-gate` but was only verified on macOS. Run the new `.github/workflows/linux-check.yml` (push the branch / open the PR to trigger it) **or** `cargo check --workspace --all-targets` on the DGX. If green, merge the branch. If the CI surfaces *other* Linux-only breakage beyond the `Container` variant (possible — Linux hasn't compiled `hhagent-core` for a while), fix those too before closing #144.
