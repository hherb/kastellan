...
**Last commit on `refactor/item-26-gliner-relex-test-module-lift`:** `cbbdf2d` (`refactor(workers/gliner_relex): lift #[cfg(test)] mod tests into sibling tests.rs`). Branch carries 2 commits on top of `main`: `4e6bbe5` (docs sync to main = a9e3385) + `cbbdf2d` (the actual Item 26 mechanical lift). **PR pending** — pure refactor, byte-equivalent tests as regression pin, workspace stays at macOS 1011 / 0 / 3.
...
## Recently completed (this session, 2026-05-25 — Next-TODO Item 26: `gliner_relex.rs` test-module sibling lift, branch `refactor/item-26-gliner-relex-test-module-lift`, PR pending)
...
**What shipped (2 commits, TDD ordering not applicable — pure mechanical refactor):**
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    26. ~~**`core/src/workers/gliner_relex.rs` (1547 LOC) test-module lift**~~ **SHIPPED this session 2026-05-25** on branch `refactor/item-26-gliner-relex-test-module-lift`; PR pending. Mirror of Item 23(a). Pure mechanical lift: `core/src/workers/gliner_relex.rs` 1547 → **811 LOC** (−736) via Rust 2018 sibling-directory module resolution; new `core/src/workers/gliner_relex/tests.rs` 745 LOC (9-line module-doc-comment header + dedented body). Production-code path stays at the existing parent file. Existing tests are the regression pin — workspace test count unchanged at macOS **1011 / 0 / 3**. Parent file at 811 LOC still over the 500-LOC cap (production-code-only residual; deferred until a second logical seam appears in the Extractor + Client + wire-type cluster). See "Recently completed (this session, Item 26)" entry at the top for full breakdown.
    27. **★ Next concrete engineering pickup — Slice 2.5 follow-up triple** (engineering, small bundle) — three small follow-ups filed during PR #118 post-merge review and not yet picked up:
