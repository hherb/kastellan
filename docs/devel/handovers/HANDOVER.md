...
## Recently completed (this session, 2026-05-28 — ★ Worker-output prompt-injection guard slice 1, branch `feat/injection-guard-slice-1`, PR pending — closes HANDOVER Item 30)
...
- `cargo test --workspace`: **1093 / 0 / 3** (+28 over the 1065 baseline). Breakdown: +2 const + +7 extract_scannable_text + +13 screen/normalize + +6 integration. The 6 integration tests skip-as-pass on this Mac (Postgres not configured by default; see `postgres-app-bin-paths.md` for the session-local override workflow).
