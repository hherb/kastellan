...
**Branch:** `feat/scheduler-spawn-failure-audit` (off `main` at `a7a0c12`). Working tree clean.
...
**`cargo test --workspace` on Linux: 329 tests passed, 0 failed, 0 `[SKIP]` lines** on branch `feat/scheduler-spawn-failure-audit` (this commit, off `main` at `a7a0c12`). Baseline on `main` after PR #31 (memory split) was 327; this slice adds **+2** pure-helper unit tests (`scheduler_step_dispatch_e2e` is still one integration test function, just with two more dispatch scenarios + bumped audit-row assertions). One pre-existing dead-code warning in `core/tests/embedding_recall_e2e.rs::ServedRequest` (introduced in PR #29) is unchanged — could be cleaned up in any future touch of that file. Two pre-existing doctests in `hhagent-sandbox` and `hhagent-worker-prelude` are `ignored` (explicit markers).
...
## Recently completed (this session, 2026-05-12 — audit rows for scheduler short-circuit paths: `UNKNOWN_TOOL` + `SPAWN_FAILED`)
...
- **Observation phase** (spec §9) — now that real CLI-driven plans execute *and the scheduler short-circuit failure modes (UNKNOWN_TOOL + SPAWN_FAILED) now leave audit rows*, the scheduler can be driven with real tasks to collect failure modes before designing the real `ConstitutionalGuard` + `DeterministicPolicy` rules. Do not skip this phase or the real stage rules will be guesses. Practical step: build a small fixture set of "real-ish" instructions (5–10 prompts spanning safe + edge-case + clearly-blockable), run them through the CLI, dump the audit log, look at which plans CASSANDRA *would* have wanted to block under each candidate rule, iterate the rule set against the dump rather than against speculation.
- **Spec §7 task-lifecycle audit rows** — `actor='scheduler', action='task.<state>'` per lifecycle transition (with `{task_id, lane, plan_count}` payload), and `actor='scheduler', action='task.finalize'` per terminal task (with `{task_id, lane, state, plan_count, total_llm_calls, total_dispatch_calls, total_duration_ms, started_at, finished_at}` payload). Currently `tasks::finalize` updates the DB but emits no audit row; observation-phase SQL queries will need per-task / per-lane latency distributions. Spec at `docs/superpowers/specs/2026-05-10-scheduler-design.md` §7. Distinct from the *step-level* short-circuit rows shipped this slice.
