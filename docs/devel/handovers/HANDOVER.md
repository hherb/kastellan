...
**Last commit on `main`:** `185ec2f` (doc-sync `docs(handover,roadmap): sync to main = 920e0c9 (PR #123 / Item 26 merged)`). The substantive previous merge was `920e0c9` (PR [#123](https://github.com/hherb/hhagent/pull/123), Item 26 — `gliner_relex.rs` test-module sibling lift; 3 branch commits + merge); workspace at macOS **1011 / 0 / 3** there. PR [#118](https://github.com/hherb/hhagent/pull/118) (Item 25 GLiNER-Relex Slice 2.5) merged earlier at `a9e3385` with post-merge `e11e3f3` + `1c7eced`. Doc-only PR [#119](https://github.com/hherb/hhagent/pull/119) (developer onboarding manual at `docs/devel/manual/`) merged at `99bbfab`.

**Last commit on `feat/item-27-slice-2.5-followups`:** `a569885` (`test(gliner_relex_e2e): skip_if_image_missing delegates to probe_image (closes #122)`). Branch carries 3 commits on top of `main = 185ec2f`: `07de743` (warm-cache invariant test, closes #121), `9cb375e` (probe_image + daemon-startup walk, closes #120), `a569885` (skip helper delegation, closes #122). **PR pending** — workspace **1011 → 1022 (+11)** on macOS, all green.
...
## Recently completed (this session, 2026-05-25 — Next-TODO Item 27: Slice 2.5 follow-up triple (#120 + #121 + #122), branch `feat/item-27-slice-2.5-followups` in worktree at `../hhagent-item-27`, **PR pending**)
...
**Test count delta this session:** macOS workspace **1011 → 1022 (+11)** at branch tip `a569885`. Linux DGX expected to gain +7 (every cross-platform new test: 1 #121 + 3 #120-argv + 6 #120-sandbox-health; the #120 smoke test and the daemon-walk are macOS-only). Ignored unchanged at 3. No new clippy warnings. 69 `test result: ok` lines, 0 FAILED, 0 SKIP markers in the full workspace run.
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    27. ~~**Slice 2.5 follow-up triple** (engineering, small bundle)~~ **SHIPPED this session 2026-05-25** on branch `feat/item-27-slice-2.5-followups`; PR pending. Three commits closing all three issues filed during PR #118 post-merge review:
        ...
        Workspace **1011 → 1022 (+11)** on macOS. See "Recently completed (this session, Item 27)" entry at the top for full breakdown.
