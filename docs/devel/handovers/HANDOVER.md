...
**Last updated:** 2026-05-21 (session-start sync — Issue #55 macOS Apple `container` backend Slice 1, **merged to `main` via PR #106 at `cc0b0de`** + pre-merge post-review fixup `18cb5e2` (5 in-scope review issues; Issue #107 filed for the `--init` / PID-1 signal-handling posture as a Slice 2/2.5 follow-up). Engineering follow-through on the spike merged via PR #105 (`56456da`) earlier on 2026-05-21. New `sandbox/src/macos_container.rs` ships `MacosContainer: SandboxBackend` + pure `build_container_argv` + `clamp_memory_to_minimum` 200 MiB-floor helper + `MacosContainer::probe()` two-step check (`container --version` + `container system status`); doc-comments on `SandboxPolicy::{mem_mb, cpu_quota_pct, tasks_max}` widened to call out the new macOS support + the `tasks_max` semantic gap (cgroup `pids.max` per-cgroup on Linux vs `RLIMIT_NPROC` per-UID on macOS). Sibling — NOT a replacement — for `MacosSeatbelt`; `default_backend()` on darwin still returns Seatbelt (Slice 2 introduces per-worker selection). New `sandbox/tests/macos_container_smoke.rs` runs 7 real-`container` smoke tests against `alpine:3.20`: echo round-trip, host `/Users` invisible, `fs_read` bind-mount round-trip, `fs_read` readonly write-rejection (now also asserts `read-only` appears in combined output to pin EROFS-vs-EACCES), strict-profile `--read-only` root (now also asserts EROFS to distinguish from `--user nobody` permission-denied), strict-profile `/tmp` tmpfs scratch. Skip-as-pass when `container --version` / `container system status` / `alpine:3.20` image are missing. Pre-merge fixup also guarded `cpu_quota_pct=Some(0)` (now drops `-c` rather than emitting `-c 0` which Apple `container` rejects opaquely) and pinned `clamp_memory_to_minimum(0) → (200, true)` for any future direct caller.

**Last commit on `main`:** `cc0b0de` (`Merge pull request #106 from hherb/feat/macos-container-backend-slice-1`).

**Session-end verification:** **Rust workspace on macOS: 901 passed / 0 failed / 3 ignored** (+35 over the 866 baseline — 26 unit in `macos_container::tests` + 7 real-container smoke in `macos_container_smoke` + 2 in `18cb5e2`'s fixup: `cpu_quota_pct_zero_drops_c_flag_entirely` + `clamp_zero_raises_to_floor_and_flags_clamping`). All 7 smoke tests passed in ~4 s against real Apple `container` 0.12.3 (warm cache; `alpine:3.20` pre-pulled). **Linux DGX (expected, last verified pre-merge of #100): 896 passed / 0 failed / 4 ignored** unchanged by this slice (`sandbox/src/macos_container.rs` is `#[cfg(target_os = "macos")]`; the doc-comment widenings on `SandboxPolicy::{mem_mb, cpu_quota_pct, tasks_max}` are pure-doc changes and don't add tests; pin remains +1 from PR #103's Rust test → expected ~897 on next DGX run). **Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed** unchanged. 0 new `[SKIP]` lines on macOS (`container` + image are present on this Mac); the smoke tests will skip cleanly on hosts without `container` installed. Pre-existing `mem_burner.rs` clippy warning (`uninit_vec`) is from `3cea642` (cgroup feature), unrelated to this slice. `cargo clippy --workspace --lib --tests --no-deps` is clean for the new `macos_container` module.

## Recently completed (this session, 2026-05-21 — Issue #55 macOS Apple `container` backend Slice 1, **merged to `main` via PR #106 at `cc0b0de`** + pre-merge post-review fixup `18cb5e2`)
...
**macOS (`main` post-PR-#106-merge at `cc0b0de`):** **901 passed / 0 failed / 3 ignored** on this Mac (M3 Max, macOS 26.5) — +35 over the 866 baseline (33 from Slice 1's new `macos_container` module + smoke; 2 from pre-merge fixup `18cb5e2`). Earlier checkpoint: 866 on `main` at `56456da` post-PR-#105-merge. **Pre-Phase-1 macOS (main):** 299 all pass on macOS (skip-as-pass for PG-dependent tests); Option O additions not yet verified on macOS (embedding TCP mock tests are cross-platform clean; the `embedding_recall_e2e` skip-as-pass path is expected).
