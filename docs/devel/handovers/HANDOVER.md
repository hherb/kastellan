...
**Last updated:** 2026-05-22 (Next-TODO Item 16 — `hhagent-cli relations kinds {add, remove, list}` shipped on branch `feat/relations-kinds-cli`, PR pending. Mirror of the `tools_allowlist` CLI surface for the operator-curated `relation_kinds` vocabulary table (migration 0017). New `connect_admin_pool` in `hhagent_db::pool` — non-runtime-role pool (peer auth as OS user = cluster bootstrap superuser, no `SET ROLE`) — needed because 0017 deliberately REVOKEs INSERT/UPDATE/DELETE/TRUNCATE on `relation_kinds` from the runtime role (the daemon must not widen vocab on its own). New `RelationKindError` enum + `validate_relation_kind` + `add`/`remove`/`list_all` + `RELATION_KIND_UNDEFINED` sentinel in `hhagent_db::relation_kinds`; `remove` refuses the `'undefined'` FK fallback up front with a typed error. New `ACTION_RELATION_KINDS_{ADD,REMOVE}` audit constants + `relation_kinds_{add,remove}_and_audit` helpers in `core::cli_audit` (best-effort audit-row posture, mirror of `tools_allowlist_*_and_audit`). New `core/src/bin/hhagent-cli/relations.rs` top-level subcommand with `kinds {add,remove,list}` actions; `add` accepts optional `--description "<text>"` (persists as TEXT, serialised as JSON `null` when omitted). `relations` wired into `main.rs` as a new top-level namespace (alongside `entities`, `tools`, `memory`, `tasks`, `observation`, `audit`); the substree shape leaves room for a future `relations show <id>` graph-edge introspection command. Workspace **901 → 921 (+20)** on Linux DGX, all green at `1b86f84` baseline. Earlier today (same session): Slice 2 **merged to `main` via PR #108 at `1b86f84`** + post-merge fixup `2b271e8` — per-worker sandbox-backend selection. New `SandboxBackendKind` enum + `SandboxBackends` resolver in `hhagent_sandbox` (both cfg-gated per-OS so cross-OS mis-config is a compile-time error). New `sandbox_backend: Option<SandboxBackendKind>` field on `ToolEntry` defaulting to `None` everywhere (byte-equivalent to slice 1; only `gliner-relex` will flip to `Some(Container)` in Slice 2.5 to opt into macOS memory enforcement). All three lifecycle managers (`SingleUseLifecycle`, `IdleTimeoutLifecycle`, `CompositeLifecycle`) switch from holding a single `Arc<dyn SandboxBackend>` to holding an `Arc<SandboxBackends>` bundle; `acquire` calls `self.sandboxes.resolve(entry.sandbox_backend)` per call. Daemon `main.rs` swaps the now-dead local cfg-gated `sandbox_backend()` helper for `SandboxBackends::default_for_current_os()`. New `core/tests/lifecycle_container_routing_e2e.rs` integration smoke spawns alpine's `/sbin/apk` through the production `SingleUseLifecycle::acquire` path, proving end-to-end routing reaches the container backend (skip-as-pass when `container` / `alpine:3.20` missing). Post-merge fixup `2b271e8` addressed two PR-#108 review items: (1) symmetric `idle_timeout_lifecycle_acquire_routes_via_entry_sandbox_backend_kind` counter-backend pin (the resolve call is the same as `SingleUseLifecycle` but it was not independently asserted; a future refactor dropping the resolve from one manager but not the other would have slipped through); (2) `SandboxBackendKind` doc-comment expansion explaining why `Serialize + Deserialize` derives exist (forward-looking for operator-config plumbing) and pinning `Container` as deliberately bound to Apple `container` (a future Linux micro-VM backend would add its own variant rather than overloading). Slice 1 (PR #106 at `cc0b0de`) shipped earlier on 2026-05-21.

**Last commit on `main`:** `1b86f84` (`Merge pull request #108 from hherb/feat/macos-container-backend-slice-2`).
**Last commit on `feat/relations-kinds-cli`:** `43d0ace` (`docs(handover): relations kinds CLI shipped + Slice 2 PR #108 sync`). Branch carries 4 commits on top of `1b86f84` (DB layer / core audit / CLI subcommand / docs).

**Session-end verification:** **Rust workspace on Linux DGX (aarch64): 921 passed / 0 failed / 4 ignored** at branch tip (+20 over the 901 baseline at `1b86f84`): 1 pool-reachability unit test + 5 `relation_kinds` validate/constant unit tests + 4 DB integration tests on `postgres_e2e` (admin-pool privilege bypass / idempotent add / undefined sentinel reject / list_all ordering, all against real PG) + 2 audit-action-string pins + 5 `relations::tests::parse_add_args` unit tests + 2 dispatcher bad-args tests + 1 CLI subprocess round-trip integration test. **macOS (M3 Max, expected): 929 passed / 0 failed / 3 ignored** (+20 over the 909 baseline, all cross-platform Linux/macOS — `connect_admin_pool` + `relation_kinds` CRUD + the CLI subcommand + the integration tests are all `#[cfg(any(target_os = "linux", target_os = "macos"))]`). **Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed** unchanged. 4 `[SKIP]` lines unchanged (pre-existing GLiNER-Relex real-model gates on `HHAGENT_GLINER_RELEX_ENABLE=1`). Pre-existing `mem_burner.rs` clippy warning (`uninit_vec`) and `core/src/worker_lifecycle/manager.rs` MutexGuard-across-await warnings unchanged — both pre-date this slice. **No new clippy warnings introduced.**
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    20. **`hhagent-cli entities kinds {add,remove,list}` subcommand tree** (engineering, unblocked by item 16's `connect_admin_pool`) — symmetric to today's relations-kinds CLI. The `entity_kinds` table (migration 0015) has the same `SELECT`-only-for-runtime GRANT shape as `relation_kinds` post-0016, so the `connect_admin_pool` introduced in item 16 is the right plumbing. Add `db::entity_kinds::{validate_entity_kind, add, remove, list_all}` mirroring the relation_kinds shape (the existing `entity_kinds::KindsCache` is read-only, parallel to `RelationKindsCache`). New audit constants `ACTION_ENTITY_KINDS_{ADD,REMOVE}`; new `cli_audit::entity_kinds_{add,remove}_and_audit` helpers; nest under existing `entities` namespace as `entities kinds {add,remove,list}` rather than a new `entity` namespace. Estimated effort: 0.5–1 session (nearly mechanical mirror; the architectural questions are all resolved by item 16).
    21. **`relations show <entity-id>` graph-edge introspection** (engineering, follow-up to item 16's `relations` namespace) — operator-facing complement to `entities show`. Walks `relations` outbound/inbound for the given entity; prints `(src_kind, src_name) --[kind]--> (dst_kind, dst_name)` rows; supports `--depth N` for n-hop expansion via the existing `Graph::path` recursive-CTE helper. Closes the gap HANDOVER's "Recently completed (PR #100)" entry flagged ("`entities show` walks entities + memories but does NOT surface the graph's edge structure"). Estimated effort: 1 session.
