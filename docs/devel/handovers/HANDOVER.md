...
**Last updated:** 2026-06-27 (**Firecracker micro-VM SLICE 2 — warm/idle reuse + re-armable watchdog — DONE, both-platform verified, branch `feat/firecracker-microvm-slice2-warm-idle` (off `main`@`b25159c`), PR pending.**
The slice-1 entry/resolver/in-guest-wipe wiring for warm reuse already existed (`firecracker_mode_entry` took a `lifecycle` param; the python-exec resolver built it via `container_lifecycle`; the worker's per-call `wipe_scratch_contents` #358 runs in-guest regardless of backend; the `IdleTimeout` warm-cache is backend-agnostic). So slice 2 was **(A) fix a latent watchdog bug + (B) DGX-verify**. **The bug:** the wall-clock watchdog was **one-shot, armed at spawn** (`tool_host/watchdog.rs::spawn_watchdog`), so a warm `IdleTimeout` worker was SIGKILLed `wall_clock_ms` after **boot** regardless of the idle window — a kill could land mid-call on a healthy warm VM; the warm-reuse path handed the worker back without resetting it. Latent on the **macOS container warm path too** — never caught because all those tests run <30 s. **The fix:** a **re-armable `Watchdog`** owned by `SupervisedWorker` (one parked `Mutex`+`Condvar` thread, generation-counter, disarmed by default at `spawn_worker`), armed for the span of each **synchronous** `SupervisedWorker::call` via an RAII `ArmGuard` that disarms on return (success or error). So a warm worker is **never under a kill timer while idle**, but a single overrunning call is still killed; disarm is synchronous at call-return (no Drop-ordering race vs the slot handoff), one parked thread per worker (no per-call churn), single enforcement site uniform across SingleUse/IdleTimeout/plain-dispatch (all funnel through `call`). **The 2026-05-08 host-blackout guards are intact** (`is_valid_target_pid` rejects pid 0/1/`>i32::MAX`; injected-kill test seam *strengthened* to a per-test closure → no shared static → parallel-safe units). Residual (documented): the watchdog now bounds the **call** window, not spawn/boot (boot is bounded by the spawn path — arguably more correct). **Verification — Mac:** watchdog units **6/0**, hermetic re-arm regression GREEN (real shell-exec under Seatbelt: warm worker survives an idle gap > `wall_clock_ms`), tool_host lib **45/0**, idle-timeout **7/0**, shell_exec_e2e **4/0**, workspace clippy `--all-targets -D warnings` clean. **DGX (aarch64, real KVM):** watchdog **6/0**; regression + idle-timeout **7/0** real bwrap; new **Firecracker warm/idle e2e 4/4 REAL** (`python_exec_firecracker_warm_idle_e2e.rs` — warm reuse boots the VM **once**, `/tmp` wiped between warm calls [proves the rootfs ships the #358 wipe], idle teardown clears the slot, warm survives an idle gap past `wall_clock_ms` in-VM); slice-1 firecracker e2e **6/6** no-regression, **0** orphan run-dirs; workspace clippy `--all-targets -D warnings` clean. **opus final review: concurrency primitive correct** (no missed wakeup / lost-disarm / double-fire / deadlock / thread-leak). **Gotcha reconfirmed:** firecracker lives at `~/.local/bin/firecracker`, OFF the non-interactive ssh `$PATH` → the e2e SKIP-as-passes (false green) unless you `export PATH=$HOME/.local/bin:$PATH`. Spec/plan `docs/superpowers/{specs,plans}/2026-06-27-firecracker-microvm-slice2-warm-idle*`. **Post-review cleanup (PR #369):** the byte-duplicated `NoopAuditSink` test fixture (5 copies across `core/tests/*`) was hoisted into `kastellan-tests-common::audit::NoopAuditSink` — this adds a **dev-only** `tests-common → core` dependency (the `core → tests-common` edge is `[dev-dependencies]`, so the cycle is dev-only and Cargo permits it; the normal build graph stays acyclic). Net −73 LOC; workspace clippy `--all-targets -D warnings` clean, watchdog units 6/0 + re-arm regression GREEN re-verified. **Slice 2 done; slices 3–5 (host-dir sharing, net workers, jailer) are the next micro-VM arc.**)
...
**★ Firecracker micro-VM SLICES 1 + 2 are COMPLETE.** Slice 1 merged (`2818708`, PR [#364](https://github.com/hherb/kastellan/pull/364)) + all follow-ups (#363/#360/#362, PRs #365/#366/#367). **Slice 2 (warm/idle reuse + re-armable watchdog) — DONE this session** (branch `feat/firecracker-microvm-slice2-warm-idle`, PR pending; see header). The leading micro-VM pick is now **slice 3 — host-dir sharing** (a fresh spec/plan): Firecracker has **no virtio-fs/9p**, so host `fs_read` becomes a **per-spawn read-only ext4 block device** built from those paths (attached as a 2nd drive) and `fs_write`/`ephemeral_scratch` a **per-spawn writable overlay drive** — unblocking a host-venv worker (e.g. gliner-relex) running in-VM. Then **slice 4** (net workers: egress-proxy UDS over a 2nd vsock) and **slice 5** (jailer + long-lived/channel workers). The slice-1 design spec's staging table sketches each. **DGX microvm rootfs at `/var/lib/kastellan/microvm/` rebuilt 2026-06-27 12:16 (carries the #358 /tmp wipe).** Firecracker e2e gotchas: rebuild the **release** launcher (`cargo build --release -p kastellan-microvm-run`) AND `export PATH=$HOME/.local/bin:$PATH` (firecracker is off the non-interactive ssh PATH → e2e SKIP-as-passes silently otherwise).
