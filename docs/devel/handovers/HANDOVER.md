...
## Recently completed (this session, 2026-05-23 — Next-TODO Item 22: kinds-CLI shared lift + over-cap CLI splits, branch `refactor/issue-111-112-kinds-cli-split`, PR pending — closes [#111](https://github.com/hherb/hhagent/issues/111) + [#112](https://github.com/hherb/hhagent/issues/112))
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    25. **First idle-timeout worker — GLiNER-Relex Slice 2.5 — Containerfile + macOS image build** (engineering, depends on macOS hardware) — Slice 2 shipped per-worker sandbox-backend selection on `main` via PR #108. Slice 2.5 needs `workers/gliner-relex/Containerfile` (Python 3.12 + `uv sync` + weights mounted at `/weights` via `--mount`) + operator-runnable `container build -t hhagent/gliner-relex:dev workers/gliner-relex/` on macOS, then flip `gliner-relex`'s `WorkerSpec` to `sandbox_backend: Some(Container)` and re-run the e2e on macOS to confirm canonical `Dr Smith --[treats]--> asthma (0.994)` output through Apple `container`. Pure macOS work; not actionable on the current DGX session. Estimated effort: 1 session.
    26. **`core/src/workers/gliner_relex.rs` (~1184 LOC post-v2) test-module lift** (engineering, pure refactor) — second worst 500-LOC breach now that the `hhagent-cli` files are addressed (Item 22). Lift `#[cfg(test)] mod tests` into a sibling `workers/gliner_relex/tests.rs`. Mechanical move; existing tests are the regression pin. Estimated effort: 0.5 session.
