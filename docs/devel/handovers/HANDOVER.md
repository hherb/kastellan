...
Linux seccomp filter via the same `kastellan-worker-lockdown-exec` shim.** Branch `feat/281-gliner-relex-seccomp`, PR open,
HEAD `1087cb6`. gliner-relex runs in host mode — a pure-Python venv bwrap spawns directly — so like browser-driver it never
...
- **[#281] gliner-relex Linux seccomp via `ml_client` + the lockdown-exec shim** (PR open this session, HEAD `1087cb6`): the heavy torch worker's host-mode spawn now routes through `kastellan-worker-lockdown-exec` so a real seccomp filter applies on Linux (was unfiltered — bwrap spawns the venv directly). New sandbox `Profile::WorkerMlClient` (strict off Linux) + prelude `ml_client` profile = `net_client` + `{mbind, get_mempolicy, mlock, munlock, mknodat}` (DGX-enumerated via the kill-mode/`journalctl -k` loop). Fail-closed shim discovery; seccomp-only (`LANDLOCK_PROFILE=none`). All 3 real-model e2e suites pass under the kill-mode filter on the DGX; workspace 1839/0/15. See top block.
