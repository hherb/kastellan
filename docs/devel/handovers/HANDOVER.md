...
**Last commit on `feat/item-27-slice-2.5-followups`:** post-review fixup commit (incoming on push). Branch carries 4 commits on top of `main = 185ec2f`: `07de743` (warm-cache invariant test, closes #121), `9cb375e` (probe_image + daemon-startup walk, closes #120), `a569885` (skip helper delegation, closes #122), and the fixup addressing PR #124 self-review feedback — empty-tag guard on `MacosContainer::probe_image` + short-circuit on `MacosContainer::probe()` failure in `probe_registered_container_images` + `tool_names` tracing field switched from Debug to comma-joined Display + `_probe_results` discard-rationale comment in `main.rs`. **PR #124 open** — workspace **1011 → 1023 (+12, post-review fixup)** on macOS, all green.
