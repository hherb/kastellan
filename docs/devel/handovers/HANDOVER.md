...
**Last updated:** 2026-05-21 (continuation — session-start sync. Both PRs from the prior session-continuation are now merged to `main`: **PR #102** (audit_tail tempdir-collision flake fix on macOS) merged at `88218ef` + post-review fixup `2e171af`; **PR #103** (GLiNER-Relex macOS slice — `mps`-accept + darwin-`auto`→`cpu` branch in Python `_resolve_device`, 11 new pytest cases + 1 new Rust manifest forwarding test) merged at `9220f40` + post-review fixup `123f91b`. End-to-end smoke against the on-disk weights on this M3 Max Mac was the verification proxy (no PG-backed e2e on this macOS install). No new code work in this session yet — header sync only.
