...
**Last updated:** 2026-06-16 (**[#281] gliner-relex Landlock DONE — #281 is now FULLY CLOSED.** The torch worker now runs
with Landlock *active* on Linux, closing the last deferred FS half. Branch `feat/281-gliner-relex-landlock`. gliner-relex's
seccomp half (`ml_client`) shipped via the lockdown-exec shim (PR #293, `feffa77`) but Landlock was deferred behind
`KASTELLAN_LANDLOCK_PROFILE=none`. Fix mirrors browser-driver exactly: `host_mode_entry` **no longer emits** the `none`
profile, so the shim's `lock_down()` installs the ruleset alongside the `ml_client` seccomp filter. The Landlock RO set is
derived from `fs_read` (weights, venv, src, interpreter root + lib dirs, the shim) by `derive_lockdown_env`; RW =
`KASTELLAN_LANDLOCK_RW=["/tmp"]` (torch writes its inductor cache to `/tmp/torchinductor` on bwrap's per-spawn `/tmp` tmpfs)
with `fs_write` empty (a `/tmp` entry there would bind the host `/tmp` over the tmpfs). **No `fs_read` iteration was needed**
— the 3 real-model e2e suites passed first try, because the Landlock RO set is `DEFAULT_RO_EXEC_ROOTS ∪ fs_read` = exactly
what bwrap binds (Landlock can only further-restrict the same set). **DGX acceptance GREEN:** the 3 host-mode
`gliner_relex_e2e` real-model suites (`happy_path_extract` / `warm_reuse` / `invalid_input`) load `multi-v1.0` + run
`extract` with Landlock on (~29.5s, real inference — not skipped), and a direct shim probe confirms
`landlock: FullyEnforced` — `/tmp` RW write+readback OK, RO grant (`/etc/resolv.conf`) OK, a **world-readable** out-of-RO
file (`/etc/hostname`, mode 644) **denied** (so the denial is unambiguously Landlock, not DAC — real containment, **not** a
silent `Disabled`/false green). Full native-Linux `cargo test --workspace` **1839/0/15** (unchanged — a test was renamed,
not added) + clippy `-D warnings` clean; Mac unit (52 gliner) + clippy green. Spec/plan: the #281 design
`docs/superpowers/specs/2026-06-16-gliner-relex-linux-seccomp-design.md` (the Landlock follow-up it scoped).
**#281 fully closed: both workers now have seccomp + Landlock.**)

_(Prior session — **[#281] browser-driver Landlock** merged to `main` as `545975e` (PR #294): `browser_driver_entry` no
longer emits `LANDLOCK_PROFILE=none`, so the shim installs the ruleset (RO from `fs_read`, RW = `/tmp` for Chromium's
`--user-data-dir`). Condensed into "Recently merged" below. That work confirmed the method this session reused verbatim.)_
...
macOS skip-as-pass posture (no `KASTELLAN_PG_BIN_DIR`): full workspace **1841 / 0 / 13** + clippy `-D warnings` clean
(2026-06-16). 8–15 ignored = explicit doctest/real-net markers;
