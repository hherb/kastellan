...
## Recently completed (this session, 2026-05-26 — Next-TODO Item 28: Issue #95 entity-upsert Layer B, branch `feat/issue-95-upsert-layer-b` in worktree `/Users/hherb/src/hhagent-issue-95`, **PR [#125](https://github.com/hherb/hhagent/pull/125) open**)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    28. ~~**Issue [#95](https://github.com/hherb/hhagent/issues/95) — entity-upsert Layer B (full-batch unnest + per-row attribution fallback)**~~ **SHIPPED this session 2026-05-26** on branch `feat/issue-95-upsert-layer-b` in worktree `/Users/hherb/src/hhagent-issue-95`; **PR [#125](https://github.com/hherb/hhagent/pull/125) open**. 12 commits TDD-ordered: 2 docs (spec + plan) + 4 pure-helper slices + 6 integration slices. New module `core/src/entity_extraction/batch_upsert.rs` (547 LOC) holds both phases + 5 pure helpers; `gliner_relex.rs` 625 → 571 LOC with public function becoming a 1-line delegate; legacy per-row relation loop deleted. Workspace **1023 → 1040 (+17, +10 unit / +7 integration)** on macOS, all green. Empirically verified against real PostgreSQL (Postgres.app v18) via session-local override; override reverted before final commit per PR #118 / Slice 2.5 pattern. Both fallback tests fire the per-row attribution chain correctly — error messages contain `kind='drug', name_norm='aspirin'` (entity FK) and `src=N, dst=M, kind='eats'` (relation FK), proving the attribution improvement over Layer A's `format!("upsert entity: {e}")` which doesn't identify the failing row. Public API (`UpsertOutcome`, `EntityExtractionError`, 8-key audit payload) byte-frozen. See "Recently completed (this session, Item 28)" entry at the top for full breakdown.
    29. **★ Next concrete engineering pickup — operator picks.** Candidates flagged by prior sessions / tech-debt:
        - **`batch_upsert.rs` 547 LOC sibling-`tests.rs` lift** (engineering, ~30 minutes). Mirror of Items 23(a) + 26. Closes the 47-LOC overage left by Item 28. Bundle with the `worker_lifecycle/idle_timeout.rs` (583 LOC) + `worker_lifecycle/manager.rs` (559 LOC) test-module lifts for a same-pattern triple — ~1 hour total.
        - **`HHAGENT_PG_BIN_DIR=<path>` env-var override** for `find_pg_bin_dir` (test-infra, ~10 LOC + 2 unit tests). Closes the Postgres.app session-local-override papercut that PR #118 + #125 both re-encountered. Filed as HANDOVER tech-debt option (b).
        ...
        - **Issue [#114](https://github.com/hherb/hhagent/issues/114)** — `walk_*_edges` diamond-graph duplicate edges in `relations show`.
        - **Issue [#115](https://github.com/hherb/hhagent/issues/115)** — collapse `walk_outbound_edges` + `walk_inbound_edges` into a single UNION ALL query.
        - **Issue [#89](https://github.com/hherb/hhagent/issues/89)** — sandbox `/tmp is per-spawn ephemeral tmpfs` invariant test.
        - **Issue [#86](https://github.com/hherb/hhagent/issues/86)** / **[#85](https://github.com/hherb/hhagent/issues/85)** / **[#84](https://github.com/hherb/hhagent/issues/84)** — worker_lifecycle hardening trio.
