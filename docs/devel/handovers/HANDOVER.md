...
**Branch `fix/issue-136-pending-acquires-warn-debounce`:** current working branch; closes [#136](https://github.com/hherb/hhagent/issues/136); PR pending.
...
## Recently completed (this session, 2026-05-27 — Issue [#136](https://github.com/hherb/hhagent/issues/136): debounce pending-acquires queue-depth warn, branch `fix/issue-136-pending-acquires-warn-debounce`, PR pending)
...
- ~~[#136](https://github.com/hherb/hhagent/issues/136) — `worker_lifecycle/idle_timeout`: debounce pending-acquires queue-depth warn (filed 2026-05-27 from PR #135 in-PR polish review)~~ **SHIPPED this session 2026-05-27** on branch `fix/issue-136-pending-acquires-warn-debounce`; PR pending. 2 commits: 1 doc-sync to main = `7f98ee4` (`8204a3c`) + 1 feature `9fae2f9` (`AtomicI64 last_warn_unix_nanos` field on `ToolSlot`; `PENDING_ACQUIRES_WARN_COOLDOWN = 30s` const; pure `debounce_warn(last_warn_nanos, now_nanos, cooldown) -> bool` predicate; CAS-to-claim warn-emit pattern). +7 tests. **Workspace 1058 → 1065 (+7) on macOS**, all green. See "Recently completed (this session, Issue #136)" entry at the top for full breakdown.
