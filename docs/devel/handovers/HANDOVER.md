...
**Last commit on `main`:** `a9e3385` (merge of PR [#118](https://github.com/hherb/hhagent/pull/118), Item 25 GLiNER-Relex Slice 2.5) — followed by post-merge `e11e3f3` test-infra hardening + `1c7eced` review-comment cleanups, both merged via the same PR. Doc-only PR [#119](https://github.com/hherb/hhagent/pull/119) (developer onboarding manual at `docs/devel/manual/`) merged earlier as `99bbfab`.
**Working branch this session (Item 26):** `refactor/item-26-gliner-relex-test-module-lift` off `main` at `a9e3385`. Pure mechanical test-module sibling lift in flight (mirror of Item 23(a)) — see "Recently completed (this session, Item 26)" entry once shipped.
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    26. **★ Next concrete engineering pickup — `core/src/workers/gliner_relex.rs` (1547 LOC) test-module lift** (engineering, pure refactor) — second worst 500-LOC breach now that the `hhagent-cli` files are addressed (Item 22) and Item 23(a) lifted the smaller `relations_show.rs` and `graph.rs` test modules. Mirror of Item 23(a): change inline `#[cfg(test)] mod tests { ... }` (starts at line 810) to `#[cfg(test)] mod tests;` and move the test block verbatim to a new sibling `core/src/workers/gliner_relex/tests.rs`. Rust 2018 sibling-directory module resolution; production-code path stays at the existing parent file. Existing tests are the regression pin (no behaviour change; workspace test count unchanged at 1011 macOS). Expected file-size after lift: ~810 LOC production code (still over the 500-LOC cap, but the dominant LOC contributor — the test module — moves out). Estimated effort: 0.5 session.
    27. **Slice 2.5 follow-up triple** (engineering, small bundle) — three small follow-ups filed during PR #118 post-merge review and not yet picked up:
        - [#120](https://github.com/hherb/hhagent/issues/120) — `MacosContainer::with_image` should surface image-not-found earlier via per-tag `probe_image` (currently the missing image only surfaces at first spawn; a probe-time check would make the operator failure mode visible at registry build).
        - [#121](https://github.com/hherb/hhagent/issues/121) — `IdleTimeoutLifecycle` warm-cache key must omit image-tag (currently tool-name-only; the warm-cache-note added in Task 4's followup flagged this; needs an invariant test pinning the key shape so a future live-reconfigure path can't accidentally serve a stale warm worker after an image-tag flip).
        - [#122](https://github.com/hherb/hhagent/issues/122) — `skip_if_image_missing` substring match should tokenize on whitespace boundaries (currently `contains(tag)` matches `dev-old` for `tag=dev`; fix is per-line `starts_with(repo) && contains(":<tag>")` or per-line `split_whitespace` + exact-tag match). Bundle as one PR; ~0.5 session.
