...
**Last commit on `main`:** `5fc5fee` (PR #138 merge commit `Merge pull request #138 from hherb/refactor/idle-timeout-release-sibling-lift`) carrying 2 branch commits (`66d21cf` refactor + `78305cf` review-nit polish) plus `c2e01ef` claim-of-work landed on `main` pre-merge. Doc-sync commit `ad74916` landed on `main` directly afterward (PR #138 merge claim-of-work). **Current working branch: `fix/issue-89-tmpfs-per-spawn-invariant-test`** carries 1 commit ahead of `main` (`784ab51` — the Issue #89 test pin); PR pending. Substantive merges in order: `fed0f21` (PR [#137](https://github.com/hherb/hhagent/pull/137), Issue #136 — debounce pending-acquires queue-depth warn), `7f98ee4` (PR [#135](https://github.com/hherb/hhagent/pull/135), worker_lifecycle hardening trio — closes #84 + #85 + #86), `8655319` (PR [#133](https://github.com/hherb/hhagent/pull/133), Issue #131 — `bring_up_pg_cluster_with_timeout` sibling), `162f71f` (PR [#132](https://github.com/hherb/hhagent/pull/132), test-module lift quadruple), `4e94e42` (PR [#129](https://github.com/hherb/hhagent/pull/129), `bring_up_pg_cluster` polling-cap lift + 5-site consolidation), `bb32cab` (PR [#128](https://github.com/hherb/hhagent/pull/128), Issues #114 + #115 — `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL), `7adc582` (PR [#126](https://github.com/hherb/hhagent/pull/126), Item 29 — `HHAGENT_PG_BIN_DIR` env-override), `dac0dcd` (PR [#125](https://github.com/hherb/hhagent/pull/125), Item 28 — Issue #95 entity-upsert Layer B).
...
**Branch `fix/issue-89-tmpfs-per-spawn-invariant-test`:** current working branch; closes Issue [#89](https://github.com/hherb/hhagent/issues/89) (sandbox `/tmp` per-spawn ephemeral tmpfs invariant test); PR pending. Pure test addition; no production-code changes.
