...
`fs_write` subpath grant + `KASTELLAN_WORKER_SCRATCH`, RAII-cleaned); Linux unchanged (bwrap per-spawn `/tmp` tmpfs — the
flag is a no-op there, `prepare_ephemeral_scratch` returns `None`). The Python worker
(`workers/browser-driver/.../__main__.py::_apply_worker_scratch`) redirects `TMPDIR` (Chromium `--user-data-dir`) and `HOME`
...
15 total); 3 new Python `test_main_scratch.py` cases (43 worker tests); core lib 976/0/1; `cargo clippy --workspace
--all-targets -D warnings` clean. **DGX Linux render e2e CONFIRMED** (native aarch64, real bwrap + Landlock + seccomp +
live PG, venv re-staged + `cargo build --workspace` for a fresh lockdown-exec shim): `browser_driver_e2e --ignored` **4/4**
green — confirming the Linux path is inert (flag no-op + the `if scratch:` Python guard never fires when the env is unset).
The shared `pyexec-` scratch prefix is deliberately reused (generic, not python-exec-specific).
Spec/plan: none — a small, well-scoped follow-up to the #307 mechanism; git-history-documented per ROADMAP convention.)
...
3. **python-exec worker slice-#2 candidates (on demand):** ~~macOS writable scratch~~ — **DONE 2026-06-18** (branch
   ...
   scratch); curated-wheels RO dir if skills demand packages. ~~browser-driver adopting `ephemeral_scratch`~~ — **DONE
