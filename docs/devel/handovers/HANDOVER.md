...
**Last updated:** 2026-05-29 (HANDOVER prune — archived the 886 KB pre-prune snapshot, compressed all sessions before 2026-05-28 into "Earlier history", and rewrote the bloated header. No code change.)

**Last commit on `main`:** `54e8885` (PR [#151](https://github.com/hherb/hhagent/pull/151) merge — Issue [#147](https://github.com/hherb/hhagent/issues/147), redact secret plaintext in the tool audit row). Confirm with `git log --oneline -1`.

**Currently on:** `main` at `54e8885`; no working branch. Next item TBD — the "Next TODO" section below is an operator-picks list.

**Session-end verification:** `cargo test --workspace` on macOS (M3 Max, on `main` at `54e8885`): **1145 passed / 0 failed / 3 ignored** (skip-as-pass posture without `HHAGENT_PG_BIN_DIR`; with PG live ~640 passed + 2 pre-existing `embedding_recall_e2e`/`gliner_relex_e2e` PG-race flakes identical on `main`). `secret_vault_e2e` 9/9 live against Postgres.app v18. **Linux DGX:** not re-verified since `main` at `1abb061` (990); expected higher — operator-driven re-run pending. **Known build gap:** `hhagent-core` does **not** compile on Linux today — [#144](https://github.com/hherb/hhagent/issues/144) (`container_mode_entry` references the macOS-only `SandboxBackendKind::Container` unconditionally). `cargo clippy --workspace --all-targets` also hard-errors on [#150](https://github.com/hherb/hhagent/issues/150) (`clippy::uninit_vec` now deny-level in `mem_burner`). `cargo build`/`cargo test` on macOS are unaffected by both.
...
**Highest priority — real errors, not enhancements (rule 5):**

1. **[#144](https://github.com/hherb/hhagent/issues/144) (`bug`) — `hhagent-core` does not compile on Linux.** `core/src/workers/gliner_relex.rs::container_mode_entry` (line ~296) references the macOS-only `SandboxBackendKind::Container` variant unconditionally, and `gliner_relex_entry` dispatches to it on every target. Linux is the first-class primary host (DGX), so the whole core crate is currently broken there. Recommended fix (issue option 1): `#[cfg(target_os = "macos")]`-gate `container_mode_entry` + the `if env.use_container_backend` branch; on Linux drive the flag to a compile-time `false`. **Verify:** `cargo check -p hhagent-core --target aarch64-unknown-linux-gnu` (needs a Linux linker — `aarch64-linux-gnu-gcc` — which is absent on the dev Mac; the operator's DGX or CI is the real check). Add a Linux `cargo check` smoke so this can't regress.
2. **[#150](https://github.com/hherb/hhagent/issues/150) — `cargo clippy --all-targets` is broken.** `clippy::uninit_vec` is now deny-level; `sandbox/tests/fixtures/mem_burner.rs:46` does `set_len()` after `with_capacity`. One-line fix: `let mut buf = vec![0u8; bytes];` (drop the `set_len`). `cargo build`/`cargo test` are unaffected; this only unblocks the clippy gate. **Verify:** `cargo clippy --workspace --all-targets`.
...
4. **[#142](https://github.com/hherb/hhagent/issues/142) + [#143](https://github.com/hherb/hhagent/issues/143) — injection-guard limits.** #142: chat-template tokens (`<|im_start|>`, `<|system|>`) will false-positive on technical docs once `web-fetch`/MCP workers land (per-tool exemption hook or catalogue tightening). #143: add an explicit `MAX_WALK_DEPTH` const to `extract_scannable_text`'s `walk()` (currently relies on serde_json's ~128-level default).
...
- [#143](https://github.com/hherb/hhagent/issues/143) — injection_guard `walk()` has no recursion depth limit (see Next-TODO item 4).
