...
## Recently completed (this session, 2026-05-12 — graph lane in `memory::recall`, branch `feat/memory-graph-lane`)

Branch: `feat/memory-graph-lane` (off `main` at `97f2743`). 9 implementation commits (`5e68600`–`911215d`) + this docs commit. Closes the Phase 1 ROADMAP item "Graph lane in `memory::recall` — entity↔memory linkage (recommended: `memory_entities` join table) + plumb `Graph::neighbors` as a third lane fused alongside semantic + lexical (Phase 1 cont. — Option P)".
