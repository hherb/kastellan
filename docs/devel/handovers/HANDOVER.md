...
**Last commit on `main`:** `7adc582` (PR #126 merge commit `Merge pull request #126 from hherb/feat/pg-bin-dir-env-override`) with post-merge polish `463553d` on top of it (refactor: compute capacity from `defaults.len()` to eliminate the `with_capacity(17)` magic number; surfaced during post-implementation review; companion follow-up filed as issue [#127](https://github.com/hherb/hhagent/issues/127) for an env-var save/restore RAII helper).

**Last commit on `fix/issues-114-115-walk-edges-dedupe-and-combine`:** `0bb65b1` (`docs(db/graph): recommend walk_edges_around when both directions are needed`). Branch tip carries 4 commits over main; PR pending. Substantive merges in order: `7adc582` (PR [#126](https://github.com/hherb/hhagent/pull/126), Item 29 — `HHAGENT_PG_BIN_DIR` env-override), `dac0dcd` (PR [#125](https://github.com/hherb/hhagent/pull/125), Item 28 — Issue #95 entity-upsert Layer B), `e93997e` (PR [#124](https://github.com/hherb/hhagent/pull/124), Item 27 — Slice 2.5 follow-up triple).
...
## Recently completed (this session, 2026-05-26 — Issues [#114](https://github.com/hherb/hhagent/issues/114) + [#115](https://github.com/hherb/hhagent/issues/115): `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL, branch `fix/issues-114-115-walk-edges-dedupe-and-combine`, **PR [#128](https://github.com/hherb/hhagent/pull/128) open**)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    29. **Operator picks (bucket).** Sub-bullet status:
        ...
        - **`bring_up_pg_cluster` polling-cap lift** (test-infra, ~1 hour) — surfaced by Item 29 above. Either widen the 15s `wait_for_status_active` cap to ~30s in the 6 local-copy sites OR consolidate them onto `tests_common::bring_up_pg_cluster` (which already has appropriate timing). Closes the residual "even with env override, Postgres.app still trips 12 tests" gap.
        - ~~**Issues [#114](https://github.com/hherb/hhagent/issues/114) + [#115](https://github.com/hherb/hhagent/issues/115)** — `walk_*_edges` diamond-graph duplicate edges + collapse outbound/inbound walks into single UNION ALL query~~ **SHIPPED this session 2026-05-26** on branch `fix/issues-114-115-walk-edges-dedupe-and-combine`; **PR [#128](https://github.com/hherb/hhagent/pull/128) open**. 4 commits TDD-ordered: dedupe via `DISTINCT ON (edge_id) ORDER BY edge_id, depth ASC` intermediate CTE in both per-direction walks (`668a36f`, closes #114) + new `walk_edges_around` + `EdgesAround` struct + smoke integration test (`c25bf96`) + CLI migration to single round-trip (`c12774a`) + doc-comment hint on per-direction methods (`0bb65b1`). Per-direction methods retained as stable surface for single-direction callers. Workspace **1046 → 1048 (+2) on macOS**, all green. See "Recently completed (this session, Issues #114 + #115)" entry at the top for full breakdown.
