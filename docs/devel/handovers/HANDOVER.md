...
## Recently completed (this session, 2026-05-29 — Issue [#156](https://github.com/hherb/hhagent/issues/156): `walk()` depth-skip continues siblings, branch `fix/issue-156-walk-sibling-continue`, code commit `43598b4`, PR **pending**)
...
**Done this session (on branch `fix/issue-156-walk-sibling-continue`, commit `43598b4`, PR [#158](https://github.com/hherb/hhagent/pull/158)):** Issue [#156](https://github.com/hherb/hhagent/issues/156) — `walk()` depth-skip now continues scanning siblings (see Recently-completed). **Done last session (merged to `main` via PR [#157](https://github.com/hherb/hhagent/pull/157) at `53e68ed`):** Issues [#148](https://github.com/hherb/hhagent/issues/148) + [#149](https://github.com/hherb/hhagent/issues/149) — secret-vault test gaps closed; both CLOSED. **Done earlier:** PR [#155](https://github.com/hherb/hhagent/pull/155) (#143 `walk()` depth guard) merged to `main` at `6e82252`; #143 closed; that merge filed [#156](https://github.com/hherb/hhagent/issues/156) (still OPEN). PR [#152](https://github.com/hherb/hhagent/pull/152) (#144 + #150) merged at `560d845`; `linux-check` CI green. Still optional (operator action, no code): a full `cargo test --workspace` on the DGX — CI is compile-only, so the Linux *test* baseline is still the stale `990`-at-`1abb061`.
...
4b. **`injection_guard.rs` test-module sibling-lift.** Now **619 LOC**, 119 over the 500-LOC cap (the #143 tests + the #156 note pushed it further over). Lift the inline `#[cfg(test)] mod tests` into a sibling `core/src/cassandra/injection_guard/tests.rs` — the established repo pattern (see the 2026-05-18/05-27 lifts) drops production LOC to ~285. Pure-mechanical; existing tests are the regression pin.
...
- ~~[#156](https://github.com/hherb/hhagent/issues/156) — injection_guard: `walk()` truncation aborts the whole walk, skipping later siblings~~ **done on branch `fix/issue-156-walk-sibling-continue` (`43598b4`): depth-skip now continues siblings; byte-cap residual documented as inherent to accumulate-then-scan. Closes on merge.**
