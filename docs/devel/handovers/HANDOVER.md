...
**Last updated:** 2026-05-18 (Worker lifecycle slices 1 + 2 — branch `feat/worker-lifecycle-slice-1`, 9 commits, bundled into one PR per operator request. **Slice 1** ships the type/manager scaffolding (`Lifecycle` enum, `IdleTimeoutCaps`, `Contract`, `WorkerLifecycleManager` trait, `SingleUseLifecycle` production impl, `IdleTimeoutLifecycle` stub). **Slice 2** fills in the idle-timeout runtime: per-tool warm cache (`Arc<TokioMutex<ToolState>>` for owned-guard semantics from acquire through Drop), spawn-on-demand, post-completion cap evaluation (`max_requests` / `max_age_seconds` / `idle_seconds`), one-shot idle-teardown tasks (stale ones no-op via `last_completion` comparison), passive crash detection via post-dispatch error-variant classifier (`Io`/`Decode`/`EarlyExit`/`IdMismatch` → dead; `Rpc` → alive), exponential restart backoff (1s/2s/4s/.../60s default; resets on successful dispatch). `WorkerHandle` widens to `WorkerHandleKind::SingleUse | IdleTimeout` enum so Drop semantics diverge cleanly. `ToolHostStepDispatcher::dispatch_step` calls `handle.report_crash()` between `dispatch` and `map_dispatch_result` gated on the classifier. The `hhagent-supervisor` crate (OS-unit installer for systemd/launchd) stays untouched — naming collision with the spec's "supervisor" wording is purely conceptual; slices 1+2 grew the worker-lifecycle role in `hhagent-core`. Earlier this session (already merged to `main` at `6b73893`): Issue #81 `inner_loop.rs` split.
**Last commit on `main`:** `6b73893` (`refactor(core,scheduler,inner_loop): split into inner_loop + inner_loop_audit (#81)`). The worker-lifecycle commits live on `feat/worker-lifecycle-slice-1`, not yet merged. Branch tip: `3cd3bb4`. Recent main history: `6b73893` (issue #81 split) → `742378e` (handover sync) → `9893f3a` (ROADMAP sync) → `99e97cf` (worker-lifecycle + GLiNER-Relex specs) → `eb6b8a8` (PR #82 merge of `feat/l1-promotion-writer`).
**Session-end verification:** `cargo test --workspace` on `feat/worker-lifecycle-slice-1`: **751 passed, 0 failed, 4 ignored, 0 [SKIP] lines, 0 warnings** on Linux (baseline 721 + 30 new tests: 6 in `worker_lifecycle::types`, 2 in `worker_lifecycle::manager`, 14 in `worker_lifecycle::idle_timeout`, 1 new pin `shell_exec_entry_declares_single_use_lifecycle`, 6 e2e in new `worker_lifecycle_idle_timeout_e2e.rs`, 1 wiring-error replacing the slice-1 panic-pin). `core/src/worker_lifecycle/types.rs` at **200 LOC**; `core/src/worker_lifecycle/manager.rs` at **312 LOC**; `core/src/worker_lifecycle/idle_timeout.rs` at **521 LOC** (21 LOC over the 500 soft cap — half is the embedded test module; split candidate is to lift the inline `#[cfg(test)] mod tests` out into a sibling `idle_timeout_tests.rs`, deferred); `core/src/scheduler/tool_dispatch.rs` at **758 LOC** (pre-existing 500-LOC soft-cap breach). `core/src/bin/hhagent-cli.rs` at **1432 LOC** still the headline 500-LOC breach pickup.
