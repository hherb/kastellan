...
**Branch `feat/injection-guard-slice-1`:** current working branch; closes HANDOVER Item 30 (worker-output prompt-injection guard slice 1). 4 substantive TDD-ordered commits (`536d23a` Task 1 module skeleton, `11a0a8e` Task 2 `extract_scannable_text`, `eae2a6f` Task 3 22-entry catalogue + `screen` + `normalize`, `4e32988` Task 4 `tool_host::dispatch` wiring + 6 integration tests) + 3 docs commits (`fbc46dc` doc-sync to main, `7f301e2` design spec, `465e03d` implementation plan). Workspace 1065 → 1093 (+28) on macOS; PR pending.
