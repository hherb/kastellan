...
**Last updated:** 2026-05-21 (Relation-label vocabulary slice — branch `feat/relation-label-vocabulary`, 2 implementation commits awaiting PR review. New `relation_kinds` operator-managed vocabulary table (migration 0017, mirror of `entity_kinds`) + `db::relation_kinds::RelationKindsCache` + `GlinerRelexExtractor` wired through the cache so the GLiNER worker's relation-extraction pass is no longer permanently in entity-only mode. **Workspace 886 → 896 (+10)** — 3 unit (cache), 4 unit (`strip_undefined_label` pure helper), 3 integration (postgres_e2e migration 0017 contract). Plus session-start sync that landed Issue #97 PR #98 merge into the HANDOVER body).

**Last commit on `feat/relation-label-vocabulary`:** `4e22ce6` (`feat(entity_extraction): plumb RelationKindsCache through GlinerRelexExtractor`). Branch is based on `main` at `dbee0ac` (PR #98 merge).

**Session-end verification:** **Rust workspace: 896 passed / 0 failed / 4 ignored on Linux** (`cargo test --workspace` on the DGX, branch `feat/relation-label-vocabulary`). 4 `[SKIP]` lines on `--nocapture` are all GLiNER-Relex real-model tests gated on `HHAGENT_GLINER_RELEX_ENABLE=1` (operator-driven skip-as-pass; export the env var to exercise them). No silent sandbox-capability skips. Smoke: `hhagent-cli --help` exit 0.

## Recently completed (this session, 2026-05-21 — Relation-label vocabulary, branch `feat/relation-label-vocabulary`, 2 commits awaiting PR review)
...
- **★ Next concrete engineering pickups (in priority order):**
    ...
    14. ~~**Relation-label vocabulary slice**~~ **SHIPPED this session 2026-05-21** on branch `feat/relation-label-vocabulary` (2 commits awaiting PR review). v2 shipped `relation_labels = vec![]` — GLiNER worker was permanently in entity-only mode. New migration `0017` adds `relation_kinds` lookup table (19 seeds + FK + GRANT/REVOKE) symmetric to `entity_kinds`; new `db::relation_kinds::RelationKindsCache`; `GlinerRelexExtractor` reads relation labels live via the cache. **Workspace 886 → 896 (+10)**. See "Recently completed (this session)" entry at the top.
