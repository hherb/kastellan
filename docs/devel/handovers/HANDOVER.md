...
- ~~**First idle-timeout worker — GLiNER-Relex Slice 1 (Python worker)**~~ **Shipped this session continuation 2026-05-18, merged to `main` at `36a2f4f`** (8 commits, ff-merged after Task 1.8's green workspace verification). See "Recently completed (this session continuation)" entry at the top. The Python worker package + operator install + smoke test is green end-to-end on real weights; Slice 2 (Rust manifest + e2e) is the next session's pickup.
