...
**Last updated:** 2026-05-22 (Next-TODO Item 20 — `hhagent-cli entities kinds {add, remove, list}` shipped on branch `feat/entities-kinds-cli` (stacked on `feat/relations-kinds-cli`), PR pending. Mechanical mirror of Item 16: new `db::entity_kinds::{validate_entity_kind, add, remove, list_all}` + `ENTITY_KIND_UNDEFINED` sentinel + `EntityKindError` (parallel to `relation_kinds` shape); new `ACTION_ENTITY_KINDS_{ADD,REMOVE}` audit constants + `entity_kinds_{add,remove}_and_audit` helpers in `core::cli_audit`; existing `entities.rs` CLI extended with a new `kinds` action that delegates to a `run_entities_kinds` substree (rather than a separate top-level namespace — preserves operator UX "everything about entities is under `entities`"). All vocab-management writes ride on `connect_admin_pool` introduced by Item 16 (migration 0016 REVOKEs writes on `entity_kinds` from the runtime role symmetric to 0017's `relation_kinds`). Workspace **921 → 939 (+18)** on Linux DGX, all green; cumulative session delta from `main` at `1b86f84`: **901 → 939 (+38)**. Earlier this session: Item 16 (`relations kinds` CLI, branch `feat/relations-kinds-cli`, PR pending, +20) and Slice 2 (per-worker sandbox-backend selection, merged via PR #108 + post-merge fixup `2b271e8`). New `SandboxBackendKind` enum + `SandboxBackends` resolver in `hhagent_sandbox` (both cfg-gated per-OS so cross-OS mis-config is a compile-time error). New `sandbox_backend: Option<SandboxBackendKind>` field on `ToolEntry` defaulting to `None` everywhere (byte-equivalent to slice 1; only `gliner-relex` will flip to `Some(Container)` in Slice 2.5 to opt into macOS memory enforcement). All three lifecycle managers (`SingleUseLifecycle`, `IdleTimeoutLifecycle`, `CompositeLifecycle`) switch from holding a single `Arc<dyn SandboxBackend>` to holding an `Arc<SandboxBackends>` bundle; `acquire` calls `self.sandboxes.resolve(entry.sandbox_backend)` per call. Daemon `main.rs` swaps the now-dead local cfg-gated `sandbox_backend()` helper for `SandboxBackends::default_for_current_os()`. New `core/tests/lifecycle_container_routing_e2e.rs` integration smoke spawns alpine's `/sbin/apk` through the production `SingleUseLifecycle::acquire` path, proving end-to-end routing reaches the container backend (skip-as-pass when `container` / `alpine:3.20` missing). Post-merge fixup `2b271e8` addressed two PR-#108 review items: (1) symmetric `idle_timeout_lifecycle_acquire_routes_via_entry_sandbox_backend_kind` counter-backend pin (the resolve call is the same as `SingleUseLifecycle` but it was not independently asserted; a future refactor dropping the resolve from one manager but not the other would have slipped through); (2) `SandboxBackendKind` doc-comment expansion explaining why `Serialize + Deserialize` derives exist (forward-looking for operator-config plumbing) and pinning `Container` as deliberately bound to Apple `container` (a future Linux micro-VM backend would add its own variant rather than overloading). Slice 1 (PR #106 at `cc0b0de`) shipped earlier on 2026-05-21.

**Last commit on `main`:** `1b86f84` (`Merge pull request #108 from hherb/feat/macos-container-backend-slice-2`).
**Last commit on `feat/relations-kinds-cli`:** `5f158e7` (`docs(handover): backfill feat/relations-kinds-cli branch-tip hash`). Branch carries 5 commits on top of `1b86f84` (DB layer / core audit / CLI subcommand / docs / backfill).
**Last commit on `feat/entities-kinds-cli`:** `ecbd8df` (`docs(handover): entities kinds CLI shipped + cumulative session test counts`). Branch stacks on `feat/relations-kinds-cli` (uses `connect_admin_pool` introduced there); carries 4 commits on top of that branch (DB layer / core audit / CLI subcommand / docs). Rebases trivially onto main after the item-16 PR merges.

**Session-end verification:** **Rust workspace on Linux DGX (aarch64): 939 passed / 0 failed / 4 ignored** at `feat/entities-kinds-cli` tip (+38 cumulative over the 901 baseline at `main` `1b86f84`: +20 from Item 16 already on `feat/relations-kinds-cli`, +18 from Item 20 on top). Item 20 breakdown: 5 entity_kinds validate/constant unit + 4 DB integration tests (admin-pool privilege bypass / idempotent add / undefined sentinel reject / list_all ordering, all against real PG) + 2 audit-action-string pins + 5 entities `parse_kinds_add_args` unit + 1 dispatcher bad-args + 1 CLI subprocess round-trip integration test. **macOS (M3 Max, expected): 947 passed / 0 failed / 3 ignored** (+38 cumulative over 909 baseline; every new test is `#[cfg(any(target_os = "linux", target_os = "macos"))]`). **Python (`workers/gliner-relex` via `uv run pytest`): 35 passed / 0 failed** unchanged. 4 `[SKIP]` lines unchanged (pre-existing GLiNER-Relex real-model gates). **No new clippy warnings introduced.**

## Recently completed (this session, 2026-05-22 — Next-TODO Item 20: `hhagent-cli entities kinds {add,remove,list}`, branch `feat/entities-kinds-cli`, PR pending)
