...
**Last commit on `main`:** `bc36e4c` (PR [#146](https://github.com/hherb/hhagent/pull/146) merge — Item 31 opaque secret references slice 1 + post-review polish `5885e26` + `9348e9c`). **Currently on `main`; no working branch — next item TBD (operator picks, see "Next TODO" below).** Substantive merges in order: `bc36e4c` (PR [#146](https://github.com/hherb/hhagent/pull/146), Item 31 — opaque secret references slice 1; the 4 TDD commits `b58cd55` / `3bc66a3` / `7b1e3c5` / `19eebd6` plus post-review polish), `62905ae` (PR [#141](https://github.com/hherb/hhagent/pull/141), Item 30 — injection guard slice 1), `504094e` (PR [#139](https://github.com/hherb/hhagent/pull/139), Issue #89 — `/tmp` per-spawn ephemeral tmpfs invariant test), `6fd82bf` (PR [#140](https://github.com/hherb/hhagent/pull/140), docs-only — 7 openhuman patterns imported), `5fc5fee` (PR [#138](https://github.com/hherb/hhagent/pull/138), refactor — `idle_timeout/release.rs` sibling-lift), `fed0f21` (PR [#137](https://github.com/hherb/hhagent/pull/137), Issue #136 — debounce pending-acquires queue-depth warn), `7f98ee4` (PR [#135](https://github.com/hherb/hhagent/pull/135), worker_lifecycle hardening trio — closes #84 + #85 + #86), `8655319` (PR [#133](https://github.com/hherb/hhagent/pull/133), Issue #131 — `bring_up_pg_cluster_with_timeout` sibling), `162f71f` (PR [#132](https://github.com/hherb/hhagent/pull/132), test-module lift quadruple), `4e94e42` (PR [#129](https://github.com/hherb/hhagent/pull/129), `bring_up_pg_cluster` polling-cap lift + 5-site consolidation), `bb32cab` (PR [#128](https://github.com/hherb/hhagent/pull/128), Issues #114 + #115 — `walk_*_edges` diamond-dedupe + `walk_edges_around` combined UNION ALL), `7adc582` (PR [#126](https://github.com/hherb/hhagent/pull/126), Item 29 — `HHAGENT_PG_BIN_DIR` env-override), `dac0dcd` (PR [#125](https://github.com/hherb/hhagent/pull/125), Item 28 — Issue #95 entity-upsert Layer B).
...
**Branch `fix/issue-136-pending-acquires-warn-debounce`:** merged into `main` via PR #137 at `fed0f21` on 2026-05-27; closes [#136](https://github.com/hherb/hhagent/issues/136). Safe to delete locally (`git branch -d fix/issue-136-pending-acquires-warn-debounce`).
...
**Branch `fix/issue-89-tmpfs-per-spawn-invariant-test`:** merged into `main` via PR #139 at `504094e` on 2026-05-28; closes Issue [#89](https://github.com/hherb/hhagent/issues/89). Pure test addition; no production-code changes. Safe to delete locally (`git branch -d fix/issue-89-tmpfs-per-spawn-invariant-test`).

**Branch `feat/injection-guard-slice-1`:** merged into `main` via PR [#141](https://github.com/hherb/hhagent/pull/141) at `62905ae` on 2026-05-28 + post-claim fixes `45627fd` + `e81b079`; closes HANDOVER Item 30. Safe to delete locally (`git branch -d feat/injection-guard-slice-1`).

**Branch `feat/opaque-secret-refs-slice-1`:** merged into `main` via PR [#146](https://github.com/hherb/hhagent/pull/146) at `bc36e4c` on 2026-05-28 + post-review polish `5885e26` + `9348e9c`; closes HANDOVER Item 31 (opaque secret references slice 1). 4 substantive TDD-ordered commits (`b58cd55` Task 1 module skeleton, `3bc66a3` Task 2 Vault impl, `7b1e3c5` Task 3 walker + FakeVault, `19eebd6` Task 4 dispatch wiring + 8 integration tests + classifier test) + 2 polish commits. Workspace 1096 → 1137 (+41) on macOS without PG. Safe to delete locally (`git branch -d feat/opaque-secret-refs-slice-1`).
...
## Recently completed (this session, 2026-05-29 — Issue [#147](https://github.com/hherb/hhagent/issues/147): redact secret plaintext in the tool audit row, branch `fix/issue-147-redact-tool-req-plaintext`, PR pending)
...
**Verification (macOS M3 Max).** `cargo test --workspace` without PG: **1145 / 0 / 3** (1144 → 1145, +1 from new test 9; test 7's widening adds assertions, no new `#[test]`). `secret_vault_e2e` live against Postgres.app v18 (`HHAGENT_PG_BIN_DIR` override): **9 / 9 passed**. `cargo clippy -p hhagent-core --all-targets`: **10 warnings, all pre-existing** (no new warnings; the 2 `tool_host.rs` doc-list warnings at 581/609 predate this change and are away from the edited regions). Updated the `dispatch` docstring's privacy note (was "tool row's `payload.req` carries plaintext after substitution… scoped to policy rows only" → now documents the pre-substitution snapshot + the result-vs-request scoping).
...
4. **`19eebd6` — Task 4: dispatch wiring + scheduler plumbing + main.rs bootstrap + 8 integration tests + classifier test fix.** Wires `substitute_refs_in_params` into `tool_host::dispatch` before `worker.call`: on `Ok(refs)`, write one `policy / secret.redeemed` audit row per ref (best-effort — redeem-time audit is best-effort because plaintext is already in process memory; killing the tool call at that point would discard the substituted value with no benefit). On `Err(SecretRedemptionFailed)`, write `policy / secret.redemption_failed` row fail-closed, return `Err(ToolHostError::SecretRedemptionFailed)` — **worker not called, no tool row written**. `ToolHostError` widened to `#[non_exhaustive]` (first variant addition since Option M); `scheduler::idle_timeout::classify` maps the new variant to `POLICY_DENIED`. `main.rs` reads `HHAGENT_BOOTSTRAP_SECRETS` env var (comma-separated names) and materializes each via `OsKeyringProvider` at daemon startup. New `core/tests/secret_vault_e2e.rs` with 8 PG-required integration tests pinning the end-to-end audit shape. +1 classifier unit test (TaskStopReason maps `SecretRedemptionFailed` to `POLICY_DENIED`).

**Verification on macOS (M3 Max):**

- `cargo test --workspace` with `HHAGENT_PG_BIN_DIR` set: **636 passed / 2 failed** (the 2 failures are pre-existing `embedding_recall_e2e` + `gliner_relex_e2e` PG initdb/pg_notify race flakes, confirmed identical on `main`; not regressions). Baseline on `main` at `c505b36`: 603 passed / 2 pre-existing flakes.
- `cargo test --workspace` without `HHAGENT_PG_BIN_DIR` (skip-as-pass posture): **1137 passed / 0 failed / 3 ignored**. Baseline 1096 / 0 / 3 on `main` at `62905ae`; delta = +41 (7 unit + 9 vault + 16 walker + 1 classifier + 8 integration).
- `cargo clippy --workspace --all-targets`: **no new warnings** after fixing 4 `clippy::explicit_auto_deref` instances in `core/tests/secret_vault_e2e.rs` (`&*vault` → `&vault`). Pre-existing set unchanged: 10 `hhagent-core` (constant-assertions, `manual_char_comparison`, `err_expect`, 4 `MutexGuard`-across-await), 3 `db::probe` doc-list-indent, 2 `hhagent-protocol` io_other_error, 1 `mem_burner` set_len-after-reserve.
...
- **No CLI surface yet (Slice 2).** Operators can only stage secrets via `HHAGENT_BOOTSTRAP_SECRETS`. Slice 2 builds `hhagent-cli secrets materialize` + CLI ↔ daemon IPC.
...
- `cargo clippy --workspace --all-targets`: **no new warnings**. The 10 pre-existing `hhagent-core` warnings (constant-assertions in `db::memories`/`memory::recall`, `manual_char_comparison` in `observation::capture`, `err_expect` in `scheduler::plan_parser`, 4 `MutexGuard`-across-await in `worker_lifecycle::manager::_test_slot_*` test inspectors), 3 doc-list-indent in `db::probe`, 2 `io_other_error` in `hhagent-protocol`, 1 `mem_burner` `set_len()`-after-`reserve` — all unchanged.

**Verification deferred to operator (PG-required integration suite):**

The 6 integration tests in `core/tests/injection_guard_e2e.rs` require a live Postgres + sandbox + supervisor + shell-exec worker binary. On this Mac without `HHAGENT_PG_BIN_DIR` set they skip-as-pass cleanly via the standard `pg_bin_dir_or_skip` pattern. To force-run: set `HHAGENT_PG_BIN_DIR=/Applications/Postgres\ 2.app/Contents/Versions/18/bin/` (per `postgres-app-bin-paths.md`) and run `cargo test --test injection_guard_e2e`; all 6 should pass. Linux DGX is expected to run the integration suite by default once PG is staged.
...
- **`hhagent-cli policy review` operator surface** for blocked rows (the audit row is the only artifact today).
- **`tool_host.rs` sibling-lift**: 263 LOC over the 500-LOC cap; production-code surgery candidate, separate refactor slice.
- **Issue [#142](https://github.com/hherb/hhagent/issues/142)** (filed 2026-05-28 from `e81b079`) — chat-template tokens (`<|im_start|>`, `<|system|>`) may false-positive on technical docs once `web-fetch` or MCP workers land that legitimately surface model-template content. Catalogue tightening + per-tool exemption hook are the candidate shapes.
- **Issue [#143](https://github.com/hherb/hhagent/issues/143)** (filed 2026-05-28 from `e81b079`) — `extract_scannable_text` recursive `walk()` has no explicit depth guard. Reachable only if upstream protocol or `serde_json` recursion limits are removed; currently defence-in-depth via `serde_json`'s ~128-level default. Add an explicit `MAX_WALK_DEPTH` const if the upstream guard is ever loosened.

---

## Recently completed (this session, 2026-05-28 — test pin: Issue [#89](https://github.com/hherb/hhagent/issues/89) — `/tmp` per-spawn ephemeral tmpfs invariant, branch `fix/issue-89-tmpfs-per-spawn-invariant-test`, **merged to `main` via PR [#139](https://github.com/hherb/hhagent/pull/139) at `504094e`**)
...
- Cross-target syntax check: `cargo check --tests -p hhagent-sandbox --target aarch64-unknown-linux-gnu` clean (after `rustup target add aarch64-unknown-linux-gnu`). Confirms the test code compiles against the Linux target — no `cfg` ladder errors, no missing imports, no lifetime issues with the format-temp argv slice.

**Verification deferred to DGX (operator-driven, two steps):**

1. **Pass-on-current-code (regression pin works):** `cargo test -p hhagent-sandbox --test linux_smoke tmp_is_per_spawn_ephemeral` against unmodified `linux_bwrap.rs` should PASS in ~1 s. Workspace test count Linux DGX: 1065 → 1066 (+1) expected.
...
## Recently completed (prior, this session 2026-05-28 — refactor: `idle_timeout/release.rs` sibling-lift, branch `refactor/idle-timeout-release-sibling-lift`, **merged to `main` via PR [#138](https://github.com/hherb/hhagent/pull/138) at `5fc5fee`** + in-PR polish `78305cf`)
...
- The `hhagent-cli supervisor status` operator-facing accessor for `pending_acquires` + `last_warn_unix_nanos` — atomics are `pub(crate)` on `ToolSlot` and would be reached via a future `pub fn slot_*` wrapper; flagged in both the trio's polish and Issue #136's shipped notes.
...
## Recently completed (prior session, 2026-05-27 — Issue [#136](https://github.com/hherb/hhagent/issues/136): debounce pending-acquires queue-depth warn, branch `fix/issue-136-pending-acquires-warn-debounce`, **merged to `main` via PR [#137](https://github.com/hherb/hhagent/pull/137) at `fed0f21`**)
...
- **Operator-facing `hhagent-cli supervisor status` accessor** — the atomics on `ToolSlot` are `pub(crate)` and would be reached via a new `pub fn slot_last_warn_unix_nanos(&self, tool_name: &str) -> Option<i64>` (parallel to a future `pub fn slot_pending_acquires`) wrapping the same atomic load. Per the trio's polish #3, this is the right shape for production observability (not repurposing the `_test_slot_*` inspector).
...
- ~~[#136](https://github.com/hherb/hhagent/issues/136) — `worker_lifecycle/idle_timeout`: debounce pending-acquires queue-depth warn (filed 2026-05-27 from PR #135 in-PR polish review)~~ **SHIPPED 2026-05-27, merged to `main` via PR [#137](https://github.com/hherb/hhagent/pull/137) at `fed0f21`** on branch `fix/issue-136-pending-acquires-warn-debounce`. 2 commits: 1 doc-sync to main = `7f98ee4` (`8204a3c`) + 1 feature `9fae2f9` (`AtomicI64 last_warn_unix_nanos` field on `ToolSlot`; `PENDING_ACQUIRES_WARN_COOLDOWN = 30s` const; pure `debounce_warn(last_warn_nanos, now_nanos, cooldown) -> bool` predicate; CAS-to-claim warn-emit pattern). +7 tests. **Workspace 1058 → 1065 (+7) on macOS**, all green. See "Recently completed (this session, Issue #136)" entry at the top for full breakdown.
