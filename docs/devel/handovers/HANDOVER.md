...
## Recently completed (this session, 2026-05-29 — Next-TODO **item 4b**: `injection_guard.rs` test-module sibling-lift, branch `refactor/injection-guard-test-module-lift`, commit `c0bb178`, PR [#159](https://github.com/hherb/hhagent/pull/159))
...
4b. ~~**`injection_guard.rs` test-module sibling-lift.**~~ **DONE this session** (commit `c0bb178`, PR [#159](https://github.com/hherb/hhagent/pull/159)): lifted the inline `#[cfg(test)] mod tests` into a sibling `core/src/cassandra/injection_guard/tests.rs`; production LOC 667 → 338, under the cap. See Recently-completed.
5. **`tool_host.rs` sibling-lift (Items 30 + 31 bundle).** Now **911 LOC**, 411 over the 500-LOC cap — the largest remaining over-cap production file, and the natural next pick now that 4b is done. Lift the injection-guard wiring and/or secret-substitution wiring (and the new `audit_sink` wiring from PR #157) into sibling submodules. Pure-mechanical; existing tests are the regression pin. NB: `tool_host.rs` is the dispatch chokepoint / trust boundary, so keep `dispatch` / `dispatch_with_sink` / the `WorkerCommand` seal in the parent and lift only the helpers — more care than the 4b lift needed.
