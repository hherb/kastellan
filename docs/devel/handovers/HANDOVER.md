...
**Last updated:** 2026-05-14 (issue #23 design spec committed on `feat/refusal-state` — distinguishing agent self-refusal from successful completion in `tasks.state`).
...
**This session's working branch:** `feat/refusal-state` (off `main` at `5f543d2`). Ships a **design spec only** (`docs/superpowers/specs/2026-05-14-constitutional-refusal-state-design.md`) closing the design half of [issue #23](https://github.com/hherb/hhagent/issues/23) — _constitutional refusals recorded as `state='completed'` instead of being distinguishable_. The matching implementation plan is the next artefact for this session (via `writing-plans` skill); the code change is a focused single-PR slice across `core::cassandra::types`, `core::scheduler::inner_loop`, `prompts/agent_planner.md`, `db/migrations/0012_tasks_state_refused.sql`, and the matching tests.

**Design summary (full detail in the spec):** new optional `Plan.refused: { principle: u8, reason: String }` field; new `Outcome::Refused { principle, reason, body }` variant; new terminal `tasks.state='refused'` value distinct from existing `'blocked'` (reviewer-detected); inner-loop short-circuit on `plan.refused.is_some()` after reviewer always runs (defense in depth, `Verdict::ConstitutionalBlock` still wins over agent refusal — provenance preserved); `agent/plan.formulate` audit-row payload gains `refused: {…}` + `decision_kind = "refused"`; planner prompt adds one sentence + a `"refused": null` default in the JSON example. Migration `0012` widens both the `tasks_state_check` CHECK constraint and the `finished_at`-setting trigger to include `'refused'`.
