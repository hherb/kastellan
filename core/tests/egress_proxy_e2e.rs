...
fn allowed_literal_origin_round_trips_and_blocks_off_allowlist() {
    ...
    let backend = backend();
    let mut handle = spawn_sidecar(backend.as_ref(), &binary, &allowlist, &scratch, "web-fetch", None)
...
fn real_host_round_trips_through_sidecar() {
    ...
    let backend = backend();
    let handle = spawn_sidecar(backend.as_ref(), &binary, &allowlist, &scratch, "web-fetch", None)
