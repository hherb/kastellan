...
/// Log line emitted by [`hhagent`'s `main`] after the database bring-up
/// probe completes. Tests and supervisors poll the daemon's redirected
/// stdout for this string to decide that the daemon is ready.
///
...
/// consumer, instead of a silent timeout in `supervisor_e2e`. Referenced
/// by [`core::tests::supervisor_e2e`].
///
