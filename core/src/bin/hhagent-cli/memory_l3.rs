...
pub(crate) fn run_memory_l3(args: &[String]) -> ExitCode {
    ...
    match args[0].as_str() {
        ...
        other     => {
            eprintln!("memory l3: unknown action '{other}'; expected: list | approve | pin | revoke | remove | run");
            ExitCode::from(2)
        }
    }
}
...
async fn memory_l3_pin(args: &[String]) -> ExitCode {
    ...
    use hhagent_core::memory::l3_approval::{
        evaluate_approval, ApprovalDecision, RejectReason, SkillTrust,
    };
    use hhagent_db::memories::{fetch_by_ids, MemoryLayer};
    use hhagent_db::pool::connect_runtime_pool;
    ...
    let id: i64 = match id_str.parse() {
        Ok(n) => n,
        Err(e) => {
            eprintln!("memory l3 pin: invalid id '{id_str}': {e}");
            return ExitCode::from(2);
        }
    };
    ...
    let row = match row {
        Some(r) if r.layer == MemoryLayer::Skill => r,
        _ => {
            eprintln!("memory l3 pin: no layer-3 skill with id={id}");
            return ExitCode::from(1);
        }
    };
    let body_sha256 = row.metadata.get("body_sha256").and_then(|v| v.as_str());
