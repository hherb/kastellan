...
fn install(args: &[String]) -> ExitCode {
    let parsed = match parse_install_args(args) {
        Ok(a) => a,
        Err(e) => {
            eprintln!("{e}\nusage: kastellan-cli install [--llm-model <name>] [--llm-url <url>] [--embedding-model <name>] [--pg-bin-dir <dir>] [--from <dir>] [--matrix-homeserver-url <url> --matrix-user <@u:server>] [--no-start]");
