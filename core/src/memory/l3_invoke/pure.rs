...
//! trust gates, and the [`prepare_invocation`] decision (reusing the approval
//! gate against the *live* tool set). No I/O, no async — every function here
//! is deterministic and unit-testable in isolation. Shared verbatim by both
//! the operator path ([`super::operator`]) and the agent path ([`super::agent`]).
