...
and (per profile) a cap on `[SKIP]`. Three evidence markers are greppable in any run: `[SKIP]` (a
test did not run), `[WARN]` (it ran but something about it was not what you think), and `[E2E]` (a
